package com.pinde.core.model;

public class OscaSkillsAssessment extends TeachingActivitySpeakerExample.MybatisObject {
    private String clinicalFlow;

    private String clinicalName;

    private String clinicalYear;

    private String speId;

    private String speName;

    private String actionTypeId;

    private String actionTypeName;

    private String subjectFlow;

    private String subjectName;

    private String appointStartTime;

    private String appointEndTime;

    private Integer appointNum;

    private String examAddress;

    private String isLocal;

    private String orgFlow;

    private String orgName;

    private String isShow;

    private String isReleased;

    private String codeInfo;

    private String recordStatus;

    private String createTime;

    private String createUserFlow;

    private String modifyTime;

    private String modifyUserFlow;

    private String isGradeReleased;

    private String isShowFroom;

    private String skillOrder;

    private String remarks;

    public String getClinicalFlow() {
        return clinicalFlow;
    }

    public void setClinicalFlow(String clinicalFlow) {
        this.clinicalFlow = clinicalFlow;
    }

    public String getClinicalName() {
        return clinicalName;
    }

    public void setClinicalName(String clinicalName) {
        this.clinicalName = clinicalName;
    }

    public String getClinicalYear() {
        return clinicalYear;
    }

    public void setClinicalYear(String clinicalYear) {
        this.clinicalYear = clinicalYear;
    }

    public String getSpeId() {
        return speId;
    }

    public void setSpeId(String speId) {
        this.speId = speId;
    }

    public String getSpeName() {
        return speName;
    }

    public void setSpeName(String speName) {
        this.speName = speName;
    }

    public String getActionTypeId() {
        return actionTypeId;
    }

    public void setActionTypeId(String actionTypeId) {
        this.actionTypeId = actionTypeId;
    }

    public String getActionTypeName() {
        return actionTypeName;
    }

    public void setActionTypeName(String actionTypeName) {
        this.actionTypeName = actionTypeName;
    }

    public String getSubjectFlow() {
        return subjectFlow;
    }

    public void setSubjectFlow(String subjectFlow) {
        this.subjectFlow = subjectFlow;
    }

    public String getSubjectName() {
        return subjectName;
    }

    public void setSubjectName(String subjectName) {
        this.subjectName = subjectName;
    }

    public String getAppointStartTime() {
        return appointStartTime;
    }

    public void setAppointStartTime(String appointStartTime) {
        this.appointStartTime = appointStartTime;
    }

    public String getAppointEndTime() {
        return appointEndTime;
    }

    public void setAppointEndTime(String appointEndTime) {
        this.appointEndTime = appointEndTime;
    }

    public Integer getAppointNum() {
        return appointNum;
    }

    public void setAppointNum(Integer appointNum) {
        this.appointNum = appointNum;
    }

    public String getExamAddress() {
        return examAddress;
    }

    public void setExamAddress(String examAddress) {
        this.examAddress = examAddress;
    }

    public String getIsLocal() {
        return isLocal;
    }

    public void setIsLocal(String isLocal) {
        this.isLocal = isLocal;
    }

    public String getOrgFlow() {
        return orgFlow;
    }

    public void setOrgFlow(String orgFlow) {
        this.orgFlow = orgFlow;
    }

    public String getOrgName() {
        return orgName;
    }

    public void setOrgName(String orgName) {
        this.orgName = orgName;
    }

    public String getIsShow() {
        return isShow;
    }

    public void setIsShow(String isShow) {
        this.isShow = isShow;
    }

    public String getIsReleased() {
        return isReleased;
    }

    public void setIsReleased(String isReleased) {
        this.isReleased = isReleased;
    }

    public String getCodeInfo() {
        return codeInfo;
    }

    public void setCodeInfo(String codeInfo) {
        this.codeInfo = codeInfo;
    }

    public String getRecordStatus() {
        return recordStatus;
    }

    public void setRecordStatus(String recordStatus) {
        this.recordStatus = recordStatus;
    }

    public String getCreateTime() {
        return createTime;
    }

    public void setCreateTime(String createTime) {
        this.createTime = createTime;
    }

    public String getCreateUserFlow() {
        return createUserFlow;
    }

    public void setCreateUserFlow(String createUserFlow) {
        this.createUserFlow = createUserFlow;
    }

    public String getModifyTime() {
        return modifyTime;
    }

    public void setModifyTime(String modifyTime) {
        this.modifyTime = modifyTime;
    }

    public String getModifyUserFlow() {
        return modifyUserFlow;
    }

    public void setModifyUserFlow(String modifyUserFlow) {
        this.modifyUserFlow = modifyUserFlow;
    }

    public String getIsGradeReleased() {
        return isGradeReleased;
    }

    public void setIsGradeReleased(String isGradeReleased) {
        this.isGradeReleased = isGradeReleased;
    }

    public String getIsShowFroom() {
        return isShowFroom;
    }

    public void setIsShowFroom(String isShowFroom) {
        this.isShowFroom = isShowFroom;
    }

    public String getSkillOrder() {
        return skillOrder;
    }

    public void setSkillOrder(String skillOrder) {
        this.skillOrder = skillOrder;
    }

    public String getRemarks() {
        return remarks;
    }

    public void setRemarks(String remarks) {
        this.remarks = remarks;
    }
}