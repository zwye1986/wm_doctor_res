...
import com.pinde.res.dao.qpres.ext.QpresAppMapper;
...
public class QpresStudentBizImpl implements IQpresStudentBiz {
	...
	@Resource
	private IQpresAppBiz hbresAppBiz;
	@Resource
	...
	@Autowired
	private QpresAppMapper qpresAppMapper;
	//读取学员的排班数据
	@Override
...
//					setVal4MapStrFloat(recMap,"all",1f);
	...
	public Map<String,Object> getRegPer(
		...
		SysUser user = null;
		if(StringUtil.isNotBlank(userFlow)){
			user = hbresAppBiz.readSysUser(userFlow);
			if(user==null){
				return null;
			}
		}
...
//			result.setSchMonth(rotationDept.getSchMonth());
		...
		SchDept schDept = hbresAppBiz.readSchDept(schDeptFlow);
		if(schDept!=null){
		...
		SysUser teacher = hbresAppBiz.readSysUser(teacherUserFlow);
		if(teacher!=null){
			process.setTeacherUserFlow(teacherUserFlow);
			process.setTeacherUserName(teacher.getUserName());
		}

		SysUser head = hbresAppBiz.readSysUser(headUserFlow);
		if(head!=null){
	...
	public List<Map<String, Object>> searchStatisticList(Map<String, Object> param) {
		return qpresAppMapper.searchStatisticList(param);
	}
}
