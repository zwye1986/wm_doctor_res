package com.pinde.res.biz.sctcm120.impl;
...
import com.pinde.res.dao.sctcm120.ext.Sctcm120LectureInfoExtMapper;
import com.pinde.res.dao.sctcm120.ext.Sctcm120ResDoctorSchProcessExtMapper;
import com.pinde.res.dao.sctcm120.ext.Sctcm120SchDeptExtMapper;
...
public class Sctcm120AppBizImpl implements ISctcm120AppBiz {
	...
	@Resource
	private Sctcm120ResDoctorSchProcessExtMapper stdpProcessExtMapper;
	@Resource
	...
	@Autowired
	private Sctcm120SchDeptExtMapper sctcm120SchDeptExtMapper;
	@Resource
	...
	@Override
	public List<ResRec> getRecByRecsType(String processFlow,String recTypeId){
		if(StringUtil.isNotBlank(processFlow) && StringUtil.isNotBlank(recTypeId)){
			ResRecExample example = new ResRecExample();
			example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y)
			.andProcessFlowEqualTo(processFlow).andRecTypeIdEqualTo(recTypeId);
...
//						List<Element> elements = root.elements();
	...
	public String editRec(String recFlow,
		...
		if(isNew){
			...
			rec.setRecForm(GlobalConstant.RES_SCTCM123_DEFAULT_FORM);
			...
			if(paramsMap!=null && !paramsMap.isEmpty()){
				...
				if(StringUtil.isNotBlank(oldContent)){
					try {
						...
						if(oldRootEle!=null){
							Element element = oldRootEle.element("fileUrl");
							Element newElement=DocumentHelper.createElement("fileUrl");
							if(element!=null) {
								...
								{
									newElement.addAttribute("id", element.attributeValue("id"));
								}
							}
							root.add(newElement);
						}
...
//			example.setOrderByClause("SIGN_DATE");
			...
			if(signs==null || signs.isEmpty()){
				...
				//获取轮转信息
				SchArrangeResult result = sctcm120studentBiz.searcheDocResult(null,deptFlow);
				if(result!=null){
	...
	public List<SysUser> getUserListBySchDept(String schDeptFlow, String roleFlow,String userName){
		...
		return sctcm120SchDeptExtMapper.getUserListBySchDept(paramMap);
	}
	...
	@Resource
	private ResDoctorSigninMapper ressigninMapper;
	@Resource
	...
	@Autowired
	private Sctcm120LectureInfoExtMapper lectureInfoExtMapper;
	@Autowired
	...
	@Override
	public int editLectureScanRegist(String lectureFlow, SysUser currUser, ResLectureScanRegist regist, ResDoctor doctor){
	...
	@Override
	public List<SchDoctorAbsence> searchSchDoctorAbsenceByDoctorDept(String schDeptFlow, String userFlow) {
			SchDoctorAbsenceExample example = new SchDoctorAbsenceExample();
			example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y).andRepealAbsenceEqualTo(GlobalConstant.RECORD_STATUS_Y).andDoctorFlowEqualTo(userFlow).andSchDeptFlowEqualTo(schDeptFlow);
	...
	@Override
	public List<ResDoctorSignin> searchDoctorSignin(ResDoctorSignin doctorSignin) {
		...
		return ressigninMapper.selectByExample(example);
	}

	@Override
	public int addDoctorSignin(ResDoctorSignin doctorSignin) {
		if(doctorSignin!=null) {
			...
			return this.ressigninMapper.insertSelective(doctorSignin);
		}
		return 0;
	}
	...
	@Override
	public ResOrgAddress readOrgAddress(String orgFlow) {
	...
	public List<SysDict> getDictListByDictId(String id) {
			if(StringUtil.isNotBlank(id))
			{
				...
				example.createCriteria().andDictTypeIdEqualTo(id).andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y);
	...
	public List<SysUser> searchUserWithRole(Map<String, Object> paramMap) {
		...
		return sctcm120SchDeptExtMapper.searchUserWithRole(paramMap);
	}

	@Override
	public ResDoctorSignin readSiginInfoByCode(String codeInfo) {
		ResDoctorSigninExample example = new ResDoctorSigninExample();
		ResDoctorSigninExample.Criteria criteria = example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y);
		if(StringUtil.isNotBlank(codeInfo))
		{
			criteria.andCodeInfoEqualTo(codeInfo);
		}
		List<ResDoctorSignin> list= ressigninMapper.selectByExample(example);
		if(list!=null&&list.size()>0)
		{
			return list.get(0);
		}
