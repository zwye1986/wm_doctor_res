...
public class Sctcm120StudentBizImpl implements ISctcm120StudentBiz {
	
	@Resource
	private Sctcm120SchArrangeResultExtMapper resultExtMapper;
	@Resource
	...
	@Resource
	private Sctcm120SchRotationDeptExtMapper rotationDeptExtMapper;
	@Resource
...
//					setVal4MapStrFloat(recMap,"all",1f);
	...
	public Map<String,Object> getRegPer(
		...
		if(StringUtil.isNotBlank(userFlow)){
			doctor = sctcm120AppBiz.readResDoctor(userFlow);
			if(doctor==null){
				return null;
			}
		}
		...
		SysUser user = null;
		if(StringUtil.isNotBlank(userFlow)){
			user = sctcm120AppBiz.readSysUser(userFlow);
			if(user==null){
				return null;
			}
		}
...
//			result.setSchMonth(rotationDept.getSchMonth());
		...
		SchDept schDept = sctcm120AppBiz.readSchDept(schDeptFlow);
		if(schDept!=null){
		...
		SysUser teacher = sctcm120AppBiz.readSysUser(teacherUserFlow);
		if(teacher!=null){
			process.setTeacherUserFlow(teacherUserFlow);
			process.setTeacherUserName(teacher.getUserName());
		}

		SysUser head = sctcm120AppBiz.readSysUser(headUserFlow);
		if(head!=null){
