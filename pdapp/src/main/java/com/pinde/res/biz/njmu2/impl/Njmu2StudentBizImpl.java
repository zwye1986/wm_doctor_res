...
public class Njmu2StudentBizImpl implements INjmu2StudentBiz{
	
	@Resource
	private StdpSchArrangeResultExtMapper resultExtMapper;
	@Resource
	...
	@Resource
	private INjmu2AppBiz njmu2AppMapper;
...
//					setVal4MapStrFloat(recMap,"all",1f);
	...
	public <T> T searcheDocResult(String doctorFlow,String resultFlow){
		...
		if(StringUtil.isNotBlank(doctorFlow)){
			...
			com.pinde.sci.model.mo.SchArrangeResultExample.Criteria criteria = example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y)
	...
	private <T> T searchDocProcess(String doctorFlow,String processFlow){
		...
		if(StringUtil.isNotBlank(doctorFlow)){
			ResDoctorSchProcessExample example = new ResDoctorSchProcessExample();
			com.pinde.sci.model.mo.ResDoctorSchProcessExample.Criteria criteria = example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y)
	...
	private List<SchRotationDeptReq> searchDocReq(String rotationFlow,String recordFlow,List<String> recTypeId,String itemId){
		...
		com.pinde.sci.model.mo.SchRotationDeptReqExample.Criteria criteria = reqExample.createCriteria()
				.andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y);
	...
	private List<ResRec> searchDocRec(String operUserFlow,String processFlow,List<String> recTypeId,String itemId){
		...
		com.pinde.sci.model.mo.ResRecExample.Criteria criteria = example
				.createCriteria()
				.andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y)
	...
	public Map<String,Object> getRegPer(
		...
		if(StringUtil.isNotBlank(userFlow)){
			doctor = njmu2AppMapper.readResDoctor(userFlow);
			if(doctor==null){
				return null;
			}
		}
		SysUser user = null;
		if(StringUtil.isNotBlank(userFlow)){
			user = njmu2AppMapper.getUserByFlow(userFlow);
			if(user==null){
				return null;
			}
		}
			...
			if(sar!=null){
				...
				if(rdsp!=null){
					...
					for(String typeId : resTypes){
						RegistryTypeEnum rectype=RegistryTypeEnum.valueOf(typeId);
