...
import com.pinde.res.biz.shfx.IShfxJxmsBiz;
...
public class ShfxJxmsBizImpl implements IShfxJxmsBiz {
	...
	public int auditRecDate(String userFlow,String dataFlow,String statusId){
		if(StringUtil.isNotBlank(userFlow) && StringUtil.isNotBlank(dataFlow)){
			...
			if(statusId.equals("Y")) {
				rec.setHeadAuditStatusId(RecStatusEnum.SecretaryAuditY.getId());
				rec.setHeadAuditStatusName(RecStatusEnum.SecretaryAuditY.getName());
			}else{
				rec.setHeadAuditStatusId(RecStatusEnum.SecretaryAuditN.getId());
				rec.setHeadAuditStatusName(RecStatusEnum.SecretaryAuditN.getName());
			}
			rec.setHeadAuditUserFlow(userFlow);
			SysUser user = sysUserMapper.selectByPrimaryKey(userFlow);
			if(user!=null){
	...
	@Override
	public List<ResRec> searchRecByProcessAndRecTypeBatch(String processFlow,String doctorFlow,String recType,String biaoJi) {
		ResRecExample example = new ResRecExample();
		ResRecExample.Criteria criteria=	example.createCriteria();
		criteria.andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y)
				.andProcessFlowEqualTo(processFlow)
				.andOperUserFlowEqualTo(doctorFlow)
				.andRecTypeIdEqualTo(recType)
				.andAuditStatusIdEqualTo(RecStatusEnum.TeacherAuditY.getId());
		if(StringUtil.isNotBlank(biaoJi))
			criteria.andHeadAuditStatusIdIsNull();
		example.setOrderByClause("OPER_TIME");
		return resRecMapper.selectByExampleWithBLOBs(example);
