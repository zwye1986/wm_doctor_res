...
import com.pinde.res.dao.hzyy.ext.HzyySecretaryMapper;
...
public class HzyySecretarieBizImpl implements IHzyySecretarieBiz {
	@Autowired
	private IHzyyAppBiz nfyyAppBiz;
	@Autowired
	private IHzyyStudentBiz nfyyBiz;
	@Autowired
	private HzyySecretaryMapper secretaryMapper;
	@Autowired
	...
	@Override
	public int updateAfterEvaluation(String id, String doctorFlow, String userFlow, HttpServletRequest request) {
		Map<String, String> params= getParam(request.getParameterMap());
		params.put("ID",id);
		params.put("UCSID",doctorFlow);
		params.put("ProfesserID",userFlow);
		...
		return teacherMapper.updateAfterEvaluation(params)+
				secretaryMapper.updateOutSecBriefInfo(doctorFlow,userFlow);
	}
