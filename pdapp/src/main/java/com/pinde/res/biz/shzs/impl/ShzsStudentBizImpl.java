package com.pinde.res.biz.shzs.impl;
...
public class ShzsStudentBizImpl implements IShzsStudentBiz {
	...
	@Resource
	private IShzsAppBiz shzsAppBiz;
	@Resource
...
//					setVal4MapStrFloat(recMap,"all",1f);
	...
	public Map<String,Object> getRegPer(
		...
		if(StringUtil.isNotBlank(userFlow)){
			doctor = shzsAppBiz.readResDoctor(userFlow);
			if(doctor==null){
				return null;
			}
		}
		SysUser user = null;
		if(StringUtil.isNotBlank(userFlow)){
			user = shzsAppBiz.readSysUser(userFlow);
			if(user==null){
				return null;
			}
		}
	...
	public int editDoctorResult(
		...
		ResDoctor doctor = shzsAppBiz.readResDoctor(userFlow);
		if(isNew){
...
//			result.setSchMonth(rotationDept.getSchMonth());
		...
		SchDept schDept = shzsAppBiz.readSchDept(schDeptFlow);
		if(schDept!=null){
		...
		SysUser teacher = shzsAppBiz.readSysUser(teacherUserFlow);
		if(teacher!=null){
			process.setTeacherUserFlow(teacherUserFlow);
			process.setTeacherUserName(teacher.getUserName());
		}

		SysUser head = shzsAppBiz.readSysUser(headUserFlow);
		if(head!=null){
		...
		String isManualFlag = shzsAppBiz.getCfgByCode("res_custom_result_flag");
		//读取是否允许学员自己入科开关 res_custom_result_flag
		String isInBySelfFlag = shzsAppBiz.getCfgByCode("res_doc_in_by_self");
		...
		//轮转计划单位
		String unit = shzsAppBiz.getCfgByCode("res_rotation_unit");
		//默认按月计算
		int step = 30;
		...
		if(!GlobalConstant.FLAG_Y.equals(doctor.getSchFlag())||!GlobalConstant.FLAG_Y.equals(doctor.getSelDeptFlag()))
		{
			...
			shzsAppBiz.editResDoctor(doctor);
		}
		return 1;
	}
