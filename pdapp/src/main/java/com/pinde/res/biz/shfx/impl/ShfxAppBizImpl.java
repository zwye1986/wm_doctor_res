package com.pinde.res.biz.shfx.impl;
...
import com.pinde.res.dao.shfx.ext.ShfxLectureInfoExtMapper;
...
public class ShfxAppBizImpl implements IShfxAppBiz {
	...
	@Autowired
	private ShfxLectureInfoExtMapper lectureInfoExtMapper;
	@Autowired
	...
	@Resource
	private IShfxStudentBiz hbresstudentBiz;
	@Resource
...
//						List<Element> elements = root.elements();
						...
						if(afterEvaluation==null){
							afterEvaluation = root.element(GlobalConstant.RES_ROLE_SCOPE_PROFESSIONALBASE+ResRecTypeEnum.AfterEvaluation.getId());
						}
						if(afterEvaluation==null){
...
//					}
	...
	public String editRec(String recFlow,
		...
		if(isNew){
			...
			rec.setRecForm(GlobalConstant.RES_SHFX_DEFAULT_FORM);
...
//			example.setOrderByClause("SIGN_DATE");
	...
	public int editResLectureEvaDetail(ResLectureEvaDetail resLectureEvaDetail,String userFlow) {
		if(resLectureEvaDetail!=null){
			if(StringUtil.isNotBlank(resLectureEvaDetail.getRecordFlow())){
				...
				return lectureEvaDetailMapper.updateByPrimaryKeyWithBLOBs(resLectureEvaDetail);
			}else{
	...
	public List<ResDoctorSchProcess> getCurrentProcesses(String userFlow) {
		ResDoctorSchProcessExample example = new ResDoctorSchProcessExample();
		example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y).
				andIsCurrentFlagEqualTo(GlobalConstant.RECORD_STATUS_Y).andUserFlowEqualTo(userFlow);
	...
	public List<QingpuDoctorSignin> searchQingpuDoctorSignin(QingpuDoctorSignin doctorSignin) {
		...
		if(StringUtil.isNotBlank(doctorSignin.getDoctorFlow())){
			criteria.andDoctorFlowEqualTo(doctorSignin.getDoctorFlow());
		}
	...
	@Override
	public ResLectureScanRegist readUserRegistInfo(String registFlow) {
		return lectureScanRegistMapper.selectByPrimaryKey(registFlow);
	}

	@Override
	public void addLectureImage(LectureImageFileForm form, SysUser user) {
		String cfg = getCfgCode("upload_base_dir");
		String dateString = DateUtil.getCurrDate2();
		String newDir = cfg+ File.separator+"LectureFlie"+File.separator +dateString ;
		...
		try {
			...
			String url = getCfgCode("upload_base_url")+"/LectureFlie/"+dateString+"/"+fileName;
			if(StringUtil.isNotBlank(form.getRegistFlow())){
				ResLectureScanRegist regist=readUserRegistInfo(form.getRegistFlow());
	...
	@Override
	public int editNewResLectureEvaDetail(ResLectureEvaDetail resLectureEvaDetail, String userFlow, HttpServletRequest request, String evalForm) throws DocumentException {
		String content="";
		if(StringUtil.isNotBlank(evalForm)){
			List<Map<String,Object>> assessMaps = parseAssessCfg(evalForm);
			if(assessMaps!=null && !assessMaps.isEmpty()){
				content = getGradeXml(assessMaps,request);
			}
		}
		if(StringUtil.isNotBlank(content))
		{
			...
			resLectureEvaDetail.setEvaScore(totalScore);
		}
		resLectureEvaDetail.setEvalFormContent(content);
		return editResLectureEvaDetail(resLectureEvaDetail,userFlow);
	}

	@Override
	public void deleteLectureImage(SysUser user, String registFlow, String imageFlow) throws DocumentException {
		ResLectureScanRegist regist=readUserRegistInfo(registFlow);
	...
	@Override
	public String saveGradeInfo(String userFlow,String recFlow, String operUserFlow, String resultFlow, String recTypeId, HttpServletRequest request,String formId) {
		DeptTeacherGradeInfo rec = new DeptTeacherGradeInfo();
		ResDoctorSchProcess writeBackProcess = new ResDoctorSchProcess();
		//大字段内容
		String content = getXmlByRequest(request,recTypeId,",",null,writeBackProcess);
		boolean isNew = StringUtil.isBlank(recFlow);
		if(isNew){
			//生成rec流水号
			recFlow = PkUtil.getUUID();
			//读取医师数据
			...
			//读取科室数据
			if(StringUtil.isNotBlank(resultFlow)){
				SchArrangeResult result = resGradeBiz.searcheDocResult(null,resultFlow);
				if(result!=null){
					rec.setDeptFlow(result.getDeptFlow());
					rec.setDeptName(result.getDeptName());
					rec.setSchDeptFlow(result.getSchDeptFlow());
				...
				ResDoctorSchProcess process = resGradeBiz.getProcessByResult(resultFlow);
				if(process!=null){
					String processFlow = process.getProcessFlow();
					rec.setProcessFlow(processFlow);
					writeBackProcess.setProcessFlow(processFlow);
			...
			if(StringUtil.isNotBlank(recTypeId)){
				rec.setRecTypeId(recTypeId);
				rec.setRecTypeName(Rec360TypeEnum.getNameById(recTypeId));
			}
			rec.setRecVersion(GlobalConstant.RES_DEFAULT_FORM_VER);
		...
		if(StringUtil.isNotBlank(content))
		{
			...
			try {
				...
				if(dom!=null) {
					Element root = dom.getRootElement();
					Element totalScore=root.element("skillScore");
					if(totalScore!=null)
					{
