package com.pinde.res.ctrl.shzs;
...
import com.pinde.res.biz.shzs.IShzsAppBiz;
import com.pinde.res.biz.shzs.IShzsStudentBiz;
import com.pinde.res.biz.shzs.IShzsTeacherBiz;
...
@Controller
@RequestMapping("/res/shzs")
public class ShzsAppController{
	private static Logger logger = LoggerFactory.getLogger(ShzsAppController.class);

	@Autowired
	private IShzsAppBiz appBiz;
	@Autowired
	private IResSchProcessExpressBiz expressBiz;
	@Autowired
	private IResGradeBiz gradeBiz;
	@Autowired
	private IShzsStudentBiz studentBiz;
	@Autowired
	private IShzsTeacherBiz teacherBiz;
	@Autowired
	...
	public String version(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		return "res/shzs/version";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/shzs/test";
	}
...
//					}
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/shzs/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
	...
	public String viewData(
		...
		if(result!=null){
			...
			SysUser docUser=appBiz.readSysUser(result.getDoctorFlow());

			String grouoFlow = result.getStandardGroupFlow();
			String standardDeptId = result.getStandardDeptId();
			SchRotationDept dept =appBiz.getRotationDeptByResult(grouoFlow, standardDeptId);
			String recordFlow="";
			if(dept!=null)
			{
				recordFlow=dept.getRecordFlow();
			}
			jspPath=GlobalUtil.getFormPath(result.getRotationFlow(),funcFlow,currVer,recForm,"",docUser.getMedicineTypeId(),recordFlow);

			//读取要求
			if(StringUtil.isNotBlank(cataFlow)){
				req = appBiz.readReq(null,recordFlow,cataFlow);
			}
		}
		//要求数据
		model.addAttribute("req",req);
		if(StringUtil.isBlank(jspPath))
		{
			model.addAttribute("resultId", "3010708");
			model.addAttribute("resultType", "数据类型未配置表单");
			return "res/shzs/viewData";
		}
		model.addAttribute("viewPath",jspPath);
	...
	public String dataList(String userFlow,
		...
		{
			model.addAttribute("resultId", "3010708");
			model.addAttribute("resultType", "轮转计划不存在");
			return "res/shzs/dataList";
		}
		SysUser docUser=appBiz.readSysUser(result.getDoctorFlow());
		...
		List<Map<String,String>> views= GlobalUtil.getFormViews(result.getRotationFlow(),funcFlow,null,null,docUser.getMedicineTypeId(),recordFlow);
		model.addAttribute("views",views);
		String jspPath=GlobalUtil.getFormPath(result.getRotationFlow(),funcFlow,null,null,"",docUser.getMedicineTypeId(),recordFlow);
		if(StringUtil.isBlank(jspPath))
		{
			model.addAttribute("resultId", "3010708");
			model.addAttribute("resultType", "数据类型未配置表单");
	...
	public String saveData(String userFlow,
		...
		if(isGrade){
			...
			//编辑这条rec
			dataFlow = appBiz.editRec(dataFlow, operUserFlow, resultFlow, funcFlow, cataFlow, request,currVer,recForm);
			if("Teacher".equals(roleId)){
