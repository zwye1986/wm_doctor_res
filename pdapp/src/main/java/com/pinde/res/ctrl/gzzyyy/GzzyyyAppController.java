package com.pinde.res.ctrl.gzzyyy;
...
import com.pinde.res.biz.gzzyyy.IGzzyyyStudentBiz;
import com.pinde.res.biz.gzzyyy.IGzzyyyTeacherBiz;
...
@Controller
@RequestMapping("/res/gzzyyy")
public class GzzyyyAppController{
	private static Logger logger = LoggerFactory.getLogger(GzzyyyAppController.class);
	...
	@Autowired
	private IGzzyyyAppBiz appBiz;
	@Autowired
	private IResSchProcessExpressBiz expressBiz;
	@Autowired
	private IGzzyyyStudentBiz studentBiz;
	@Autowired
	private IGzzyyyTeacherBiz teacherBiz;
	@Autowired
	...
	public String version(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		return "res/gzzyyy/version";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/gzzyyy/test";
	}
...
//						String isRotationFlag = appBiz.getCfgByCode("jswjw_"+orgFlow+"_P001");
...
//						}
//
//						String isAppFlag = appBiz.getCfgByCode("jswjw_"+orgFlow+"_P005");
...
//								String isSendFlag = appBiz.getCfgByCode("jswjw_sendSchool_"+doctor.getWorkOrgId()+"_P007");
...
//						}//住院医师缩减调整
						...
						if (isReduction) {
//							Map<String, SchDoctorDept> doctorDeptMap = appBiz.getReductionDeptMap(doctor.getDoctorFlow(), doctor.getRotationFlow(), doctor.getSecondRotationFlow());
...
//				return "res/gzzyyy/login";
	...
	public String notice(String userFlow,String roleId,Integer pageIndex,Integer pageSize,HttpServletRequest request,Model model){
		String result = "res/gzzyyy/notice";
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/gzzyyy/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
	...
	public String viewData(
		...
		if(isGrade)
		{
			...
			ResSchProcessExpress dops = null;
			ResSchProcessExpress mini = null;
			String processFlow = "";
			ResDoctorSchProcess process = studentBiz.getProcessByResult(resultFlow);
			if(process!=null){
				//登记表单的科室
			...
			if(ResRecTypeEnum.AfterEvaluation.getId().equals(funcFlow)){
                dops = expressBiz.getExpressByRecType(processFlow,ResRecTypeEnum.DOPS.getId());
                mini = expressBiz.getExpressByRecType(processFlow,ResRecTypeEnum.Mini_CEX.getId());
                if(dops==null || mini==null){
                    model.addAttribute("resultId", "3010607");
                    model.addAttribute("resultType", "迷你临床演练表和临床操作技能考核表必须全部完成");
                    return "res/gzzyyy/viewData";
                }
                String dopesContent = dops.getRecContent();
                Map<String,String> dopesFormDataMap = appBiz.parseRecContent(dopesContent);
                if(dopesFormDataMap!=null&&dopesFormDataMap.size()>0)
                {
                    List<Double> scoreList = new ArrayList<>();
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("skillLevel")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("consentForm")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("readyToWork")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("painAndStabilization")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("SkillAbility")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("asepticTechnique")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("seekAssistance")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("relatedDisposal")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("communicationSkills")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("feelProfessionalDegree")));
                    scoreList.add(Double.parseDouble(dopesFormDataMap.get("overallPerformance")));
                    double sum = 0;
                ...
                String miniContent = mini.getRecContent();
                Map<String,String> miniFormDataMap = appBiz.parseRecContent(miniContent);
                if(miniFormDataMap!=null&&miniFormDataMap.size()>0)
                {
                    List<Double> scoreList2 = new ArrayList<>();
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("medicalInterview")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("physicalExamination")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("humanisticCare")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("clinicalJudgment")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("communicationSkills")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("organization")));
                    scoreList2.add(Double.parseDouble(miniFormDataMap.get("holisticCare")));
                    double sum = 0;
                    if(!scoreList2.isEmpty()){
                        for(double a:scoreList2){
                            sum+=a;
                        }
                    }
	...
	public String saveData(String userFlow,
		...
		if(isGrade){
			dataFlow = gradeBiz.editGradeInfo(dataFlow, operUserFlow, resultFlow, funcFlow, request,GlobalConstant.RES_GZZYYY_DEFAULT_FORM);
			...
			//编辑这条rec
			dataFlow = expressBiz.editJszyExpress(dataFlow, operUserFlow, resultFlow, funcFlow,GlobalConstant.RES_GZZYYY_DEFAULT_FORM, request);
			if("Teacher".equals(roleId)){
				expressBiz.auditDate(userFlow,dataFlow);
			}
		}else {
			//编辑这条rec
			dataFlow = appBiz.editRec(dataFlow, operUserFlow, resultFlow, funcFlow, cataFlow, request,oldContent);
			if("Teacher".equals(roleId)){
...
//		}
	...
	public String getSysNotice(String userFlow,String isDoctor, Integer pageIndex, Integer pageSize, HttpServletRequest request,Model model) {
		...
		if (StringUtil.isBlank(userFlow)) {
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/sysNotices/getSysNotice";
		}
		//验证用户是否存在
		...
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzyyy/sysNotices/no-pic.png";
	...
	public String sysNoticeDetail(String userFlow,String infoFlow,HttpServletRequest request,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/sysNotices/sysNoticeDetail";
		}
		...
		String httpurl=httpRequest.getRequestURL().toString();
		String servletPath=httpRequest.getServletPath();
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzyyy/sysNotices/showSysNotice.jsp?recordFlow="+infoFlow;
		String androidimgurl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzyyy/sysNotices/showSysNotice2.jsp?recordFlow="+infoFlow;
	...
	public String findActivityList(String userFlow,String typeId,String isCurrent,String deptFlow,Integer pageIndex,
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/findActivityList";
		}
	...
	public String getNewLectures(Model model,String userFlow,String roleId,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/getNewLectures";
		}
		if(StringUtil.isEmpty(roleId)){
			model.addAttribute("resultId", "31801");
		...
		PageHelper.startPage(pageIndex, pageSize);
		List<ResLectureInfo> lectureInfos = appBiz.SearchNewLectures(orgFlow,roleId,roleFlow);
	...
	public String getHistoryLectures(Model model,String userFlow,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/getHistoryLectures";
		}
	...
	public synchronized String lectureRegist(String lectureFlow,String roleId,String userFlow,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/lectureRegist";
		}
	...
	public String zpNoticeDetail(String userFlow,String recordFlow,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/zpNoticeDetail";
		}
		...
		String httpurl=httpRequest.getRequestURL().toString();
		String servletPath=httpRequest.getServletPath();
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzyyy/showNotice/showNoticeDetail.jsp?recordFlow="+recordFlow;
		String androidimgurl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzyyy/showNotice/showNoticeDetail2.jsp?recordFlow="+recordFlow;
		model.addAttribute("iosDetailUrl",hostUrl);
		model.addAttribute("androidDetailUrl",androidimgurl);
		return "res/gzzyyy/zpNoticeDetail";
	}
	...
	public String viewError(String processFlow,Model model){
		String result = "res/gzzyyy/viewError";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
	...
	public String viewErrorDetail(String processFlow,String resultsId , Model model){
		String result = "res/gzzyyy/viewErrorDetail";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isBlank(processFlow)){
			model.addAttribute("resultId", "30401");
	...
	public String joinExam(String userFlow,String schDeptFlow,String resultFlow,String processFlow,HttpServletRequest request,HttpServletResponse response,Model model) throws UnsupportedEncodingException {
		...
		ResDoctorSchProcess process=appBiz.getProcessByResultFlow(resultFlow);
		if(process==null)
		{
			model.addAttribute("resultId", "40404");
	...
	public String allAfterDept(String userFlow,Integer pageIndex,Integer pageSize,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30401");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzyyy/allAfterDept";
		}
		//考试地址
		String testUrl = appBiz.getCfgCode("res_mobile_after_url_cfg");
		if(!StringUtil.isNotBlank(testUrl)){
		...
		ResDoctor doctor=appBiz.readResDoctor(userFlow);
		List<SchArrangeResult> resultList = appBiz.getSchArrangeResult(userFlow, doctor.getOrgFlow(),pageIndex, pageSize);
		...
		for(SchArrangeResult result:resultList) {
			ResDoctorSchProcess process=appBiz.getProcessByResultFlow(result.getResultFlow());
			if(process!=null)
			{
