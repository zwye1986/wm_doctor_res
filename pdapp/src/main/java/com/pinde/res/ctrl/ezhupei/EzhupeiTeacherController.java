...
@Controller
@RequestMapping("/res/ezhupei/teacher")
public class EzhupeiTeacherController {
    private static Logger logger = LoggerFactory.getLogger(EzhupeiTeacherController.class);

    @Autowired
    private IEzhupeiStudentBiz ezhupeiStudentBiz;
    @Autowired
    private IEzhupeiAppBiz ezhupeiAppBiz;
    @Autowired
    ...
    public String test() {
        return "res/ezhupei/teacher/test";
    }
    ...
    public String doctorList(String userFlow,
        ...
        if (StringUtil.isEmpty(userFlow)) {
            ...
            return "res/ezhupei/teacher/doctorList";
        }
        if (pageIndex == null) {
        ...
        //获取访问路径前缀
        String uploadBaseUrl = ezhupeiAppBiz.getCfgByCode("upload_base_url");
    ...
    public String funcList(String userFlow, String doctorFlow, HttpServletRequest request, Model model) {
        ...
        if (StringUtil.isBlank(userFlow)) {
            ...
            model.addAttribute("resultType", "用户标识符为空");
            return "res/ezhupei/teacher/funcList";
        }
    ...
    public String index(String userFlow, HttpServletRequest request, Model model) {
        ...
        SysUser user = ezhupeiAppBiz.readSysUser(userFlow);
        ...
        List<SysUserRole> userRoles = ezhupeiAppBiz.getSysUserRole(userFlow);
        if(userRoles!=null&&userRoles.size()>0) {
            //获取当前配置的老师角色
            String teacherRole = ezhupeiAppBiz.getCfgCode("res_teacher_role_flow");
            //获取当前配置的科主任角色
            String headRole = ezhupeiAppBiz.getCfgCode("res_head_role_flow");
        ...
        return "res/ezhupei/teacher/index";
    }
    ...
    public String studentList(String userFlow, String studentName, String dataType, Integer pageIndex, Integer pageSize, Model model, String biaoJi) {
        ...
        if (StringUtil.isBlank(userFlow)) {
            ...
            model.addAttribute("resultType", "用户标识符为空");
            return "res/ezhupei/teacher/studentList";
        }
        //验证用户是否存在
        SysUser userinfo = ezhupeiAppBiz.readSysUser(userFlow);
        if (userinfo == null) {
...
//		后台配置是否校验权限时间
        String isPermitOpen = ezhupeiAppBiz.getCfgByCode("res_permit_open_doc");
        ...
        if (StringUtil.isBlank(dataType) || dataType.equals("fiveData")) {
            for (ResRecTypeEnum e : ResRecTypeEnum.values()) {
                String cfgValue = ezhupeiAppBiz.getCfgByCode("res_registry_type_" + e.getId());
    ...
    public String dataNoAudit(String userFlow, String docFlow, String dataType, String processFlow, String resultFlow, Model model) {
        ...
        if (StringUtil.isBlank(userFlow)) {
            model.addAttribute("resultId", "3011101");
            model.addAttribute("resultType", "用户标识符为空");
            return "res/ezhupei/teacher/dataNoAudit";
        }
    ...
    public String recDataList(String userFlow, String docFlow, String processFlow, String recType, String biaoJi, String resultFlow, Integer pageIndex, Integer pageSize, Model model) {
        ...
        if (StringUtil.isBlank(userFlow)) {
            model.addAttribute("resultId", "3011101");
            model.addAttribute("resultType", "用户标识符为空");
            return "res/ezhupei/teacher/recDataList";
        }
        if (StringUtil.isBlank(docFlow)) {
            model.addAttribute("resultId", "3011101");
            model.addAttribute("resultType", "学生标识符为空");
        ...
        Map<String, Object> processPerMap = ezhupeiStudentBiz.getRegPer(0, docFlow, resultFlow);
        ...
        for (ResRec rec : recList) {
            ...
            Map<String, String> formDataMap = ezhupeiAppBiz.parseRecContent(recContent);
    ...
    public String resRecDeatil(String userFlow, String recFlow, String resultFlow, String cataFlow, String recType, Model model) {
        ...
        if (StringUtil.isBlank(userFlow)) {
            model.addAttribute("resultId", "3011101");
            model.addAttribute("resultType", "用户标识符为空");
            return "res/ezhupei/teacher/resRecDetail";
        }
        ...
        //			SchRotationDept rotationDept = ezhupeiAppBiz.getRotationDeptByResult(standardGroupFlow,standardDeptId);
    ...
    public String viewData(
        ...
        if (result != null) {
            ...
            if (StringUtil.isNotBlank(cataFlow)) {
                ...
                if (rotationDept != null) {
                    ...
                    req = ezhupeiAppBiz.readReq(null, relRecordFlow, cataFlow);
                }
            }
        }
        ...
        if (StringUtil.isNotBlank(dataFlow)) {
            rec = ezhupeiAppBiz.getRecByRecFlow(dataFlow);
            //如果是11模式根据类型和科室查询
            ...
            rec = ezhupeiAppBiz.getRecByRecType(processFlow, funcFlow);
        }
        ...
        if (isGrade && StringUtil.isNotBlank(assessType)) {
            //读取评分配置
            ResAssessCfg assessCfg = ezhupeiAppBiz.getGradeTemplate(assessType);
            model.addAttribute("assessCfg", assessCfg);

            if (assessCfg != null) {
                String content = assessCfg.getCfgBigValue();
                //解析评分xml
                List<Map<String, Object>> assessMaps = ezhupeiAppBiz.parseAssessCfg(content);
        ...
        if (rec != null) {
            ...
            if (!isGrade) {
                formDataMap = ezhupeiAppBiz.parseRecContent(content);
            } else {
                formDataMap = ezhupeiAppBiz.parseDocGradeXml(content);
            }
    ...
    public String viewFourTable(
        ...
        SchArrangeResult result = ezhupeiStudentBiz.searcheDocResult(null, resultFlow);
        model.addAttribute("result", result);
        SysUser tea = ezhupeiAppBiz.readSysUser(userFlow);
        ...
        if (recType.equals(ResRecTypeEnum.AfterEvaluation.getId())) {
            ResDoctorSchProcess process = ezhupeiAppBiz.readSchProcessByResultFlow(resultFlow);
            if (process != null) {
                String res_after_test_switch = ezhupeiAppBiz.getCfgByCode("res_after_test_switch");
                String res_after_url_cfg = ezhupeiAppBiz.getCfgByCode("res_after_url_cfg");
//				String p004=ezhupeiAppBiz.getCfgByCode("jswjw_"+process.getOrgFlow()+"_P004");
//				String docTestSwitch=ezhupeiAppBiz.getCfgByCode("doc_test_switch_"+process.getOrgFlow()+"_"+process.getUserFlow());
                //学员开通出科考试权限
                ...
                ResScore score = ezhupeiAppBiz.getScoreByProcess(process.getProcessFlow());
                model.addAttribute("outScore", score);
                model.addAttribute("doctorSchProcess", process);
                model.addAttribute("currRegProcess", process);

                //百分比算法
                Map<String, Object> perMap = ezhupeiStudentBiz.getRegPer(0, process.getUserFlow(), resultFlow, null, null, true);
    ...
    public String saveData(String userFlow,
        ...
        dataFlow = expressBiz.editExpress(dataFlow, operUserFlow, resultFlow, recTypeId, GlobalConstant.RES_EZHUPEI_DEFAULT_FORM, request,GlobalConstant.RES_ROLE_SCOPE_TEACHER,null, "");
