...
@Controller
@RequestMapping("/res/hbres")
public class HbresAppController{
	private static Logger logger = LoggerFactory.getLogger(HbresAppController.class);
	...
	public String version2(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version2";
	}
	...
	public String version3(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version3";
	}
	/**
	 * 云住培-重庆
	 * @param request
	...
	public String version4(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version4";
	}
	/**
	 * 云住培-遵义五院
	 * @param request
     ...
     * @return
     */
	@RequestMapping(value={"/version5"},method={RequestMethod.GET})
	public String version5(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version5";
	}
	/**
	 * 云住培-广州医科大学
	 * @param request
     ...
     * @return
     */
	@RequestMapping(value={"/version6"},method={RequestMethod.GET})
	public String version6(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version6";
	}
	/**
	 * 云住培-潮州-住院医师
	 * @param request
     ...
     * @return
     */
	@RequestMapping(value={"/version7"},method={RequestMethod.GET})
	public String version7(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version7";
	}
	/**
	 * 云住培-潮州-实习生
	 * @param request
     ...
     * @return
     */
	@RequestMapping(value={"/version8"},method={RequestMethod.GET})
	public String version8(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/hbres/version8";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/hbres/test";
	}
	...
	public String login(String userCode,String userPasswd,String uuid,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		//验证用户是否存在
		SysUser userinfo = hbresAppBiz.getUserByCode(userCode);
		if(userinfo==null){
			...
			//获取当前配置的医师角色
			String doctorRole = hbresAppBiz.getCfgByCode("res_doctor_role_flow");

			//获取当前配置的老师角色
			String teacherRole = hbresAppBiz.getCfgByCode("res_teacher_role_flow");
...
//							如果不校验校验权限时间，并且开通APP权限，则开通APP
				...
				if(processAreaMap!=null){
					...
					if(StringUtil.isNotBlank(minDate)){
						...
						//获取该医师的轮转计划的区间
						Map<String,Object> resultAreaMap = hbresAppBiz.getDocResultArea(userFlow);
				...
				if(StringUtil.isNotBlank(orgFlow)){
					String isAppFlag = hbresAppBiz.getCfgByCode("jswjw_"+orgFlow+"_P006");
					if(!GlobalConstant.FLAG_Y.equals(isAppFlag)){
						model.addAttribute("resultId", "3010110");
				...
				String orgFlow = userinfo.getOrgFlow();
				if(StringUtil.isNotBlank(orgFlow)){
					String isAppFlag = hbresAppBiz.getCfgByCode("jswjw_"+orgFlow+"_P001");
					if(!GlobalConstant.FLAG_Y.equals(isAppFlag)){
						model.addAttribute("resultId", "3010110");
						model.addAttribute("resultType", "登录失败,你暂无权限使用,请联系培训基地管理员!");
						return "res/hbres/login";
	...
	public String notice(String userFlow,String roleId,Integer pageIndex,Integer pageSize,HttpServletRequest request,Model model){
		String result = "res/hbres/notice";
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/hbres/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
	...
	public String viewData(
		...
		if(isGrade)
		{
			...
			if(StringUtil.isNotBlank(dataFlow)){
				...
				if(ResRecTypeEnum.AfterEvaluation.getId().equals(funcFlow) ){
					//百分比算法
					Map<String, Object> perMap = hbresStudentBiz.getRegPer(0, userFlow, resultFlow, null, cataFlow, true);
			...
			if(rec!=null){
				...
				//解析登记信息的xml
				Object formDataMap = hbresAppBiz.parseRecContent(content);
				model.addAttribute("formDataMap",formDataMap);
				//如果读取到数据就使用数据本身的funcFlow
	...
	public String dataList(String userFlow,
		...
		if("Student".equals(roleId)){
			...
			//查询登记数据
			recMaps = hbresAppBiz.getParsedRecs(paramMap);
			...
			//查询登记数据
			recMaps = hbresAppBiz.getTeacherParsedRecs(paramMap);
		}
		model.addAttribute("path",path);
		model.addAttribute("recMaps",recMaps);

		//百分比算法
		Map<String,Object> perMap = hbresStudentBiz.getRegPer(0, userFlow, resultFlow,funcFlow,cataFlow,true);
		model.addAttribute("perMap",perMap);
	...
	public String cataList(String userFlow,
		...
		//根据条件获取要求列表
		PageHelper.startPage(pageIndex, pageSize);
		List<Map<String,Object>> reqMaps = hbresAppBiz.getReqByResult(paramMap);
		model.addAttribute("reqMaps",reqMaps);

		model.addAttribute("dataCount",PageHelper.total);

		//百分比算法
		Map<String,Object> perMap = hbresStudentBiz.getRegPer(0, userFlow, resultFlow,funcFlow);
		model.addAttribute("perMap",perMap);
	...
	public String saveData(String userFlow,
		...
		if(isGrade){
			dataFlow = gradeBiz.editGradeInfo(dataFlow, operUserFlow, resultFlow, funcFlow, request,GlobalConstant.RES_HBRES_DEFAULT_FORM);
			...
			//编辑这条rec
			dataFlow = expressBiz.editExpress(dataFlow, operUserFlow, resultFlow, funcFlow,GlobalConstant.RES_HBRES_DEFAULT_FORM, request,"","", "");
			if("Teacher".equals(roleId)){
				expressBiz.auditDate(userFlow,dataFlow);
			}
		}else {
			//编辑这条rec
			dataFlow = hbresAppBiz.editRec(dataFlow, operUserFlow, resultFlow, funcFlow, cataFlow, request);
			if("Teacher".equals(roleId)){
...
//		}
		...
		if(StringUtil.isEquals(funcFlow, "lectureSignin")){
			...
			if("Student".equals(roleId)) {
				doctor = hbresAppBiz.readResDoctor(currUser.getUserFlow());
				if(doctor==null)
				{
					model.addAttribute("resultId", "30906");
			...
			ResLectureInfo info=hbresAppBiz.read(lectureFlow);
			if(info!=null)
			{
				String signTime = paramMap.get("time");
				String key="res_"+info.getOrgFlow()+"_org_jiangzuo_code_type";
				String cfgv=hbresAppBiz.getResCfgCode(key);
				if(GlobalConstant.FLAG_Y.equals(cfgv)) {
					...
					if (DateUtil.signSecondsBetweenTowDate(DateUtil.transDateTime(scanTime), DateUtil.transDateTime(signTime)) > 5) {
						model.addAttribute("resultId", "3011106");
						model.addAttribute("resultType", "二维码已失效，请重新扫码！");
						return "res/hbres/qrCode";
					}
				}
				String key1=hbresAppBiz.getResCfgCode("res_"+info.getOrgFlow()+"_org_jiangzuo_start_time");
				int startTime=10;
				...
				if(regist!=null)
				{
					...
					List<ResLectureInfo> infos=hbresAppBiz.checkJoinList(lectureFlow,userFlow);
				...
				int count =hbresAppBiz.scanRegist(regist);
				if(count!=1)
				{
			...
			if(info!=null)
			{
				...
				String key2=hbresAppBiz.getResCfgCode("res_"+info.getOrgFlow()+"_org_jiangzuo_end_time");
				int endTime=10;
			...
			ResLectureRandomSign sign=hbresAppBiz.readLectureRandomSign(randomFlow);
			if(sign==null)
			{
				model.addAttribute("resultId", "3011107");
			...
			ResLectureInfo lectureInfo=hbresAppBiz.read(sign.getLectureFlow());
			if(lectureInfo==null)
			{
				model.addAttribute("resultId", "3011107");
				model.addAttribute("resultType", "讲座信息不存在");
			...
			//学员信息
			SysUser docUser=hbresAppBiz.readSysUser(userFlow);
			if(docUser==null){
				model.addAttribute("resultId", "3011101");
			...
			ResLectureScanRegist regist=hbresAppBiz.searchByUserFlowAndLectureFlow(userFlow,sign.getLectureFlow());
			...
			ResLectureRandomScan scan=hbresAppBiz.readLectureRandomScan(userFlow,randomFlow);
			if(scan!=null){
				model.addAttribute("resultId", "3011101");
			...
			int c=hbresAppBiz.saveLectureRandomScan(scan);
			if(c==0){
				model.addAttribute("resultId", "3011106");
			...
			boolean result = hbresAppBiz.signin(userFlow,deptFlow,signinType);
			if(!result){
				model.addAttribute("resultId", "3011108");
			...
			return "res/hbres/qrCode";
				...
				String key1=hbresAppBiz.getResCfgCode("res_"+info.getOrgFlow()+"_org_activity_start_time");
				int startTime=10;
				if(StringUtil.isNotBlank(key1))
				{
					startTime=Integer.valueOf(key1);
				...
				String key2=hbresAppBiz.getResCfgCode("res_"+info.getOrgFlow()+"_org_activity_end_time");
				int endTime=10;
				if(StringUtil.isNotBlank(key2))
				{
					endTime=Integer.valueOf(key2);
	...
	public String schDeptList(String userFlow,String searchStr,Integer pageIndex,Integer pageSize,HttpServletRequest request,
		...
		PageHelper.startPage(pageIndex, pageSize);
		List<SchDeptRel> schDeptList = hbresAppBiz.searchRelByStandard(orgFlow, standardDeptId,searchStr);
	...
	public String teacherList(String schDeptFlow,String searchStr,Integer pageIndex,Integer pageSize,HttpServletRequest request,Model model){
		...
		String roleFlow = hbresAppBiz.getCfgByCode("res_teacher_role_flow");
		if(StringUtil.isNotBlank(roleFlow)){
			PageHelper.startPage(pageIndex, pageSize);
			List<SysUser> userList =  hbresAppBiz.getUserListBySchDept(schDeptFlow,roleFlow,searchStr);
	...
	public String headList(String schDeptFlow,String searchStr,Integer pageIndex,Integer pageSize,HttpServletRequest request,Model model){
		...
		String roleFlow = hbresAppBiz.getCfgByCode("res_head_role_flow");
		if(StringUtil.isNotBlank(roleFlow)){
			PageHelper.startPage(pageIndex, pageSize);
			List<SysUser> userList =  hbresAppBiz.getUserListBySchDept(schDeptFlow,roleFlow,searchStr);
			model.addAttribute("userList", userList);
	...
	public String getNewLectures(Model model,String userFlow,String roleId,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/hbres/getNewLectures";
		}
		if(StringUtil.isEmpty(roleId)){
			model.addAttribute("resultId", "31801");
		...
		PageHelper.startPage(pageIndex, pageSize);
		List<ResLectureInfo> lectureInfos = hbresAppBiz.SearchNewLectures(orgFlow,roleId,roleFlow);
		...
		if(lectureInfos!=null&&lectureInfos.size()>0){
			for(ResLectureInfo lectureInfo:lectureInfos){
				...
				List<ResLectureScanRegist> resLectureScanRegists=hbresAppBiz.searchIsRegist(lectureFlow);
	...
	public String getHistoryLectures(Model model,String userFlow,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/hbres/getHistoryLectures";
		}
	...
	public synchronized String lectureRegist(String lectureFlow,String roleId,String userFlow,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/hbres/lectureRegist";
		}
		...
		if(StringUtil.isBlank(lectureInfo.getLimitNum())||resLectureScanRegists==null||resLectureScanRegists.size()<Integer.valueOf(lectureInfo.getLimitNum())) {
			...
			int count = hbresAppBiz.editLectureScanRegist(lectureFlow, currUser, regist,doctor);
			if(count<0)
			{
				model.addAttribute("resultId", "30111013");
	...
	public synchronized String lectureCannelRegist(String lectureFlow,String roleId,String userFlow,Model model){
		...
		int c=hbresAppBiz.scanRegist(regist);
		if(c==0)
		{
			model.addAttribute("resultId", "30906");
	...
	public String getZhupeiNotices(String userFlow,String roleId,Integer pageIndex,Integer pageSize,String noticeTitle,HttpServletRequest request,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/hbres/noticeList";
		}
		if(pageIndex==null){
		...
		ResDoctor doctor =null;
		if("Student".equals(roleId)){
			doctor=hbresAppBiz.readResDoctor(userFlow);
			if(doctor==null)
			{
				model.addAttribute("resultId", "3011101");
				model.addAttribute("resultType", "医师信息不存在");
		...
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/hbres/showNotice/no-pic.png";
	...
	public String zpNoticeDetail(String userFlow,String recordFlow,HttpServletRequest request,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/hbres/zpNoticeDetail";
		}
		...
		String httpurl=httpRequest.getRequestURL().toString();
		String servletPath=httpRequest.getServletPath();
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/hbres/showNotice/showNoticeDetail.jsp?recordFlow="+recordFlow;
		String androidimgurl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/hbres/showNotice/showNoticeDetail2.jsp?recordFlow="+recordFlow;
