...
@Controller
@RequestMapping("/res/gzzl")
public class GzzlAppController{
	private static Logger logger = LoggerFactory.getLogger(GzzlAppController.class);

	@Autowired
	private IHbresAppBiz appBiz;
	@Autowired
	...
	public String version(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		return "res/gzzl/version";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/gzzl/test";
	}
		...
		if(userinfo==null){
			...
			if(isDoctor){
				...
				if(doctor==null){
					...
					String key1=appBiz.getJsResCfgCode(doctor.getOrgFlow()+"_bind");
					if("Y".equals(key1)) {
						...
						ResUserBindMacid macid = appBiz.readMacidByUserFlow(userinfo.getUserFlow());
...
//				if(macid==null)
//				{
//					macid=appBiz.readMacidByMacid(uuid);
//
...
//				}
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/gzzl/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
...
//		}
	...
	public String getNewLectures(Model model,String userFlow,String roleId,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzl/getNewLectures";
		}
		if(StringUtil.isEmpty(roleId)){
			model.addAttribute("resultId", "31801");
	...
	public String getHistoryLectures(Model model,String userFlow,Integer pageIndex,Integer pageSize) {
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "31801");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzl/getHistoryLectures";
		}
		...
		if(newList!=null&&!newList.isEmpty())
		{
			...
			for(Map<String,String> bean:newList){
				...
				if((lectureEndTime.compareTo(currTime)<0&&lectureTrainDate.compareTo(currDate)==0)||(lectureTrainDate.compareTo(currDate)<0)){
					if("Y".equals(isScan)&&"Y".equals(isScan2)){
	...
	public String getZhupeiNotices(String userFlow,String roleId,Integer pageIndex,Integer pageSize,String noticeTitle,HttpServletRequest request,Model model){
		...
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzl/showNotice/no-pic.png";
	...
	public String zpNoticeDetail(String userFlow,String recordFlow,HttpServletRequest request,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/gzzl/zpNoticeDetail";
		}
		...
		String httpurl=httpRequest.getRequestURL().toString();
		String servletPath=httpRequest.getServletPath();
		String hostUrl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzl/showNotice/showNoticeDetail.jsp?recordFlow="+recordFlow;
		String androidimgurl=httpurl.substring(0,httpurl.indexOf(servletPath))+"/jsp/res/gzzl/showNotice/showNoticeDetail2.jsp?recordFlow="+recordFlow;
