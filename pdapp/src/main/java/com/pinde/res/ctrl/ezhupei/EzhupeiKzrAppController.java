...
import com.pinde.res.biz.ezhupei.*;
...
@Controller
@RequestMapping("/res/ezhupei/kzr")
public class EzhupeiKzrAppController {    
	private static Logger logger = LoggerFactory.getLogger(EzhupeiKzrAppController.class);
	...
	@Autowired
	private IEzhupeiAppBiz ezhupeiBiz;
	@Autowired
	private IEzhupeiStudentBiz studentBiz;
	@Autowired
	...
	@Autowired
	private IEzhupeiKzrBiz ezhupeiKzrBiz;
	@Autowired
	...
	public String index(String userFlow,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			...
			return "res/ezhupei/kzr/index";
		}
		//验证用户是否存在
		SysUser userinfo = ezhupeiBiz.readSysUser(userFlow);
		if(userinfo==null){
		...
		//培训学员总数
		String isOpen = ezhupeiBiz.getCfgByCode("res_permit_open_doc");
		int conut=ezhupeiKzrBiz.schProcessStudentDistinctQuery(userinfo.getDeptFlow(),userFlow,isOpen);
	...
	public String studentList(String userFlow,String roleId,String seachStr,
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/studentList";
		}
		...
		resDoctorSchProcess=ezhupeiKzrBiz.schDoctorSchProcessHead(schArrangeResultMap);
		...
		for (Map<String, String> map : resDoctorSchProcess) {
			...
			if (schArrangeResult!=null) {
				//返回出科异常异常原因
				ResDoctorSchProcess process = ezhupeiKzrBiz.searchByResultFlow(schArrangeResult.getResultFlow());
				if(process!=null) {
	...
	public String deptList(String userFlow,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		if(StringUtil.isEmpty(userFlow)){
			...
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/deptList";
		}
		SysUser user= ezhupeiBiz.readSysUser(userFlow);
		List<SysDept> depts=ezhupeiBiz.getHeadDeptList(userFlow,user.getDeptFlow());
	...
	public String inProcessInfo(String userFlow,String deptFlow,HttpServletRequest request,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			...
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/inProcessInfo";
		}
		...
		SysDept dept=ezhupeiBiz.readSysDept(deptFlow);
		if(dept==null)
		{
			model.addAttribute("resultId", "3020202");
	...
	public String funcList(String userFlow,String doctorFlow,String processFlow,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3020201");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/funcList";
		}
		...
		ResDoctor resDoctor = ezhupeiBiz.readResDoctor(process.getUserFlow());
	...
	public String resRecList(String userFlow,String doctorFlow,String processFlow,String recType,Integer pageIndex,Integer pageSize,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/resRecList";
		}
		...
		//System.out.println("=======================:"+JSON.toJSONString(deptPerMap));
		//验证用户是否存在
		SysUser userinfo = ezhupeiBiz.readSysUser(userFlow);
		if(userinfo==null){
			model.addAttribute("resultId", "3010103");
	...
	public String resRecDeatil(String userFlow,String recFlow,String deptFlow,String cataFlow,String recType,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/resRecDetail";
		}
		...
		ResRec rec = ezhupeiBiz.getRecByRecFlow(recFlow);
		String recContent = rec.getRecContent();
		Map<String, String> formDataMap = parseRecContent(recContent);
		formDataMap.put("auditId",rec.getAuditStatusId());
	...
	public String evaluationSun(String userFlow,
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/evaluationSun";
		}
		if(StringUtil.isBlank(doctorFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "学员标识符为空");
		...
		SysUser currUser=ezhupeiBiz.readSysUser(userFlow);
		model.addAttribute("currUser",currUser);
		if(StringUtil.isNotBlank(doctorFlow)){
			doctor  = ezhupeiBiz.readResDoctor(doctorFlow);
			model.addAttribute("doctor", doctor);
			operUser  = ezhupeiBiz.readSysUser(doctorFlow);
		...
		if(rec!=null){
			String recContent = rec.getRecContent();
			formDataMap = ezhupeiBiz.parseRecContent(recContent);
		...
		//出科考核对接判断
		boolean ckk=false;
		String regScore=ezhupeiBiz.getCfgByCode("res_doc_reg_score");
		if(GlobalConstant.FLAG_Y.equals(regScore))
		{
			String switchFlag=ezhupeiBiz.getCfgByCode("res_after_test_switch");
			String urlCfg=ezhupeiBiz.getCfgByCode("res_mobile_after_url_cfg");
			//学员开通出科考试权限
		...
		if(ckk) {
			ResScore outScore = ezhupeiBiz.readScoreByProcessFlow(processFlow);
	...
	public String mini_cex(String userFlow,
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/mini_cex";
		}
		if(StringUtil.isBlank(doctorFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "学员标识符为空");
		...
		ResDoctor doctor=ezhupeiBiz.readResDoctor(doctorFlow);
	...
	public String teacherGradeList(String userFlow,String teaName,Integer pageIndex,Integer pageSize,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/teacherGradeList";
		}
		SysUser userinfo = ezhupeiBiz.readSysUser(userFlow);
		if(userinfo==null){
			model.addAttribute("resultId", "3010103");
		...
		String cfgTeacher=ezhupeiBiz.getCfgCode("res_teacher_role_flow");
		PageHelper.startPage(pageIndex, pageSize);
		List<SysUser> sysUserList=ezhupeiKzrBiz.teacherRoleCheckUser(userinfo.getDeptFlow(),cfgTeacher,teaName,userFlow);
	...
	public String deptGrade(String userFlow,String deptFlow,Model model) throws DocumentException {
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/deptGrade";
		}
		...
		ResAssessCfg assessCfg = ezhupeiBiz.getGradeTemplate(cfgCodeId);
	...
	public String deptGradeList(String userFlow,String deptFlow,Integer pageIndex,Integer pageSize,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/deptGradeList";
		}
		if(StringUtil.isBlank(deptFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "科室标识符为空");
	...
	public String gradeDetail(String userFlow,String recFlow,String gradeType,Model model) throws DocumentException {
		...
		if (StringUtil.isBlank(userFlow)) {
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/gradeDetail";
		}
		if (StringUtil.isBlank(recFlow)) {
			model.addAttribute("resultId", "3011101");
	...
	public String evalStudentList(String userFlow,String roleId,String seachStr,
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/evalStudentList";
		}
		//验证用户是否存在
		SysUser userinfo = ezhupeiBiz.readSysUser(userFlow);
		if(userinfo==null){
	...
	public String monthEvalList(String userFlow,String doctorFlow,String processFlow,String schResultFlow,Model model) throws ParseException {
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/monthEvalList";
		}
		if(StringUtil.isBlank(doctorFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "学生标识符为空");
	...
	public String showMonthEval(String userFlow,String doctorFlow,String processFlow,String recordFlow,String evalMonth,Model model) throws ParseException {
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/showMonthEval";
		}
		if(StringUtil.isBlank(doctorFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "学生标识符为空");
	...
	public String ownerInfo(String userFlow,Model model) throws ParseException {
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/ownerInfo";
		}
		//验证用户是否存在
		SysUser userinfo = ezhupeiBiz.readSysUser(userFlow);
		if(userinfo==null){
		...
		model.addAttribute("userinfo",userinfo);
		List<SysUserRole> userRoles = ezhupeiBiz.getSysUserRole(userFlow);
		if(userRoles!=null&&userRoles.size()>0) {
			//获取当前配置的老师角色
			String teacherRole = ezhupeiBiz.getCfgCode("res_teacher_role_flow");
			//获取当前配置的科主任角色
			String headRole = ezhupeiBiz.getCfgCode("res_head_role_flow");
	...
	public String afterFormAudit(String userFlow,String sessionNumber,String userName,Integer pageIndex,Integer pageSize,Model model){
		...
		if(StringUtil.isBlank(userFlow)){
			model.addAttribute("resultId", "3011101");
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/afterFormAudit";
		}
		Map<String,Object> paramMap = new HashMap<String, Object>();
		paramMap.put("userFlow",userFlow);
		paramMap.put("sessionNumber",sessionNumber);
		paramMap.put("userName",userName);
		String isOpen = ezhupeiBiz.getCfgByCode("res_permit_open_doc");//权限期间是否开通
		paramMap.put("isOpen",isOpen);
		PageHelper.startPage(pageIndex, pageSize);
		List<ResDoctorSchProcess> processList = ezhupeiKzrBiz.searchProcessByDoctorNew(paramMap);
		...
		if(processList!=null && processList.size()>0){
			...
			List<SysUser> userList = ezhupeiKzrBiz.searchSysUserByuserFlows(userFlows);
			...
			List<ResDoctor> doctorList = ezhupeiKzrBiz.searchDoctorByuserFlow(userFlows);
	...
	public String showRegistryForm(String userFlow, String resultFlow, String recTypeId, String recFlow, Model model) {
		...
		if (StringUtil.isEmpty(userFlow)) {
			...
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/showRegistryForm";
		}
		...
		SysUser tea = ezhupeiBiz.readSysUser(userFlow);
		...
		if (rec != null) {
			...
			formDataMap = ezhupeiBiz.parseRecContent(content);
			model.addAttribute("formDataMap", formDataMap);
		...
		if (recTypeId.equals(com.pinde.res.enums.ezhupei.ResRecTypeEnum.AfterEvaluation.getId())) {
			...
			if (process != null) {
				...
				String res_after_url_cfg = ezhupeiBiz.getCfgByCode("res_after_url_cfg");
				//学员开通出科考试权限
				Map<String, String> paramMap = new HashMap();
				...
				ResScore score = ezhupeiBiz.getScoreByProcess(process.getProcessFlow());
	...
	public String saveRegistryForm(String userFlow, String resultFlow, String recTypeId, String dataFlow, HttpServletRequest request,Model model) {
		...
		if (StringUtil.isEmpty(userFlow)) {
			...
			model.addAttribute("resultType", "用户标识符为空");
			return "res/ezhupei/kzr/saveRegistryForm";
		}
		if (StringUtil.isEmpty(resultFlow)) {
		...
		dataFlow = expressBiz.editExpress(dataFlow, operUserFlow, resultFlow, recTypeId, GlobalConstant.RES_EZHUPEI_DEFAULT_FORM, request,GlobalConstant.RES_ROLE_SCOPE_HEAD,content, "");
		//审核数据
		ezhupeiKzrBiz.auditDate(userFlow, dataFlow);
		return "res/ezhupei/kzr/saveRegistryForm";
	}
}
