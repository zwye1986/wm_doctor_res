...
import com.pinde.res.biz.qpres.IQpresAppBiz;
import com.pinde.res.biz.qpres.IQpresStudentBiz;
import com.pinde.res.biz.qpres.IQpresTeacherBiz;
...
@Controller
@RequestMapping("/res/qpres")
public class QpresAppController {
	private static Logger logger = LoggerFactory.getLogger(QpresAppController.class);

	@Autowired
	private IQpresAppBiz appBiz;
	@Autowired
	private IResSchProcessExpressBiz expressBiz;
	@Autowired
	private IResGradeBiz gradeBiz;
	@Autowired
	private IQpresStudentBiz studentBiz;
	@Autowired
	private IQpresTeacherBiz teacherBiz;
	@Autowired
	...
	public String version2(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/qpres/version2";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/qpres/test";
	}
	...
	public String login(String userCode,String userPasswd,HttpServletRequest request,HttpServletResponse response,Model model){
		...
		if(userinfo==null){
			...
			if(isDoctor){
				...
				//读取是否允许学员自己入科开关 res_doc_in_by_self
...
//				}
	...
	public String notice(String userFlow,String roleId,Integer pageIndex,Integer pageSize,HttpServletRequest request,Model model){
		String result = "res/qpres/notice";
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/qpres/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
...
//		}
		...
		if(StringUtil.isEquals(funcFlow, "lectureSignin")){
			...
			schEntryReport.setReportFlow(result.getResultFlow());
			...
			return "res/qpres/qrCode";
		} else if(StringUtil.isEquals(funcFlow,"qingpuSingin")) {
			...
			List<ResDoctorSchProcess> currentProcesses= appBiz.getCurrentProcesses(userFlow);
			...
			QingpuDoctorSignin searchSignin = new QingpuDoctorSignin();
			searchSignin.setSigninDate(DateUtil.getCurrDate());
			searchSignin.setDoctorFlow(userFlow);
			List<QingpuDoctorSignin> qingpuDoctorSignins = appBiz.searchQingpuDoctorSignin(searchSignin);
			if(qingpuDoctorSignins!=null&&qingpuDoctorSignins.size()>0)
			{
				model.addAttribute("resultId", "3011106");
				model.addAttribute("resultType", "你已成功扫码签到！");
				return "res/qpres/qrCode";
			}
			QingpuDoctorSignin qingpuDoctorSignin = new QingpuDoctorSignin();
			qingpuDoctorSignin.setDoctorFlow(userFlow);
			qingpuDoctorSignin.setTeacherUserFlow(teacherUserFlow);
			qingpuDoctorSignin.setSigninTime(DateUtil.getCurrDateTime2());
			qingpuDoctorSignin.setSigninDate(DateUtil.getCurrDate());
			qingpuDoctorSignin.setRecordStatus("Y");
			qingpuDoctorSignin.setCodeInfo(codeInfo);
			qingpuDoctorSignin.setResultFlow(currentProcesses.get(0).getSchResultFlow());
			int c = appBiz.addQingpuDoctorSignin(qingpuDoctorSignin);
			if(c>0){
				model.addAttribute("resultId", "3011108");
				model.addAttribute("resultType", "你已成功扫码报到！");
			}else{
