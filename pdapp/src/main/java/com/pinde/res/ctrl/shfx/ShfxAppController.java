...
@Controller
@RequestMapping("/res/shfx")
public class ShfxAppController {
	private static Logger logger = LoggerFactory.getLogger(ShfxAppController.class);
	...
	public String version2(HttpServletRequest request,HttpServletResponse response,Model model){
		model.addAttribute("resultId", "200");
		model.addAttribute("resultType", "交易成功");
		return "res/shfx/version2";
	}


	@RequestMapping(value={"/test"},method={RequestMethod.GET})
	public String test(){
		return "res/shfx/test";
	}
		...
		if(userinfo==null){
			...
			for(SysUserRole sur : userRoles){
				...
				if(doctorRole.equals(ur)){
					...
					model.addAttribute("roleName","科室秘书");
				}
			}
...
//				}
				...
				//验证该科秘是否存在培训机构,如果存在则验证该机构是否开启过程
				String orgFlow = userinfo.getOrgFlow();
				if(StringUtil.isNotBlank(orgFlow)){
					String isAppFlag = appBiz.getCfgByCode("jswjw_"+orgFlow+"_P001");
					if(!GlobalConstant.FLAG_Y.equals(isAppFlag)){
					...
					model.addAttribute("resultId", "3010109");
					model.addAttribute("resultType", "登录失败,该人员暂无所属机构!");
					return "res/shfx/login";
				}
			}else{
				model.addAttribute("resultId", "3010109");
	...
	public String noticeCount(String userFlow,String roleId,HttpServletRequest request,Model model){
		String result = "res/shfx/noticeCount";
		model.addAttribute("resultId", ResultEnum.Success.getId());
		model.addAttribute("resultType", ResultEnum.Success.getName());
		if(StringUtil.isEmpty(userFlow)){
			model.addAttribute("resultId", "30201");
	...
	public String saveData(String userFlow,
		...
		if(isGrade){
			dataFlow = appBiz.editGradeInfo(dataFlow, operUserFlow, resultFlow, funcFlow, request,GlobalConstant.RES_SHFX_DEFAULT_FORM);
			...
			//编辑这条rec
			dataFlow = expressBiz.editExpress(dataFlow, operUserFlow, resultFlow, funcFlow,GlobalConstant.RES_SHFX_DEFAULT_FORM, request,"","", "");
			if("Teacher".equals(roleId)){
