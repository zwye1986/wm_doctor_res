...
<!-- 	                		</div> -->
      	 			...
      	 			<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
	      	 			...
	      	 			<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
	      	 				<div style="float: right; margin-right: 270px;">名称：
...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			...
        			<td colspan="3" style="width: 55%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input style="width: 115px;" type="text" name="patientName" value="${formDataMap['patientName']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        		...
        		<tr>
        			...
        			<td>
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" name="hospitaNumber" style="width: 115px;" value="${formDataMap['hospitaNumber']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        			...
        			<td style="width: 21%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text"  readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" style="width: 115px;" name="admissionDate" value="${formDataMap['admissionDate']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['admissionDate']}
        				</c:if>
        			</td>
        			<td style="width: 13%;">主要诊断:</td>
        			<td style="width: 21%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" style="width: 115px;" name="mainDiagnosis" value="${formDataMap['mainDiagnosis']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
