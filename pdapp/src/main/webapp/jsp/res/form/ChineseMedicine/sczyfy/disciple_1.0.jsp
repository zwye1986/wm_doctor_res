...
<script type="text/javascript">
	...
	function audit(auditResult){
		if($("#summaryForm").validationEngine("validate")){
			...
			jboxConfirm("确认提交?一旦提交将无法修改!",function(){
				var url = "<s:url value='/res/rec/saveRegistryFormNew'/>?auditResult="+auditResult;
			...
			<div style="width: 90%;margin-right: auto;margin-left: auto;">
	 		...
	 		<form id="summaryForm" method="post">
				...
				<table class="orgTable" border="0" width="90%" cellspacing="0" cellpadding="0" style="margin-left: auto;margin-right: auto;">
					<caption class="orgCaption" style="color: #399ce0;">门诊跟师个人小结
					...
					<tr class="orgTr">
						<td class="orgTd" colspan="4" style="padding-top:10px;padding-left:0;">
							<c:if test="${empty rec && param.roleFlag eq GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
								<textarea placeholder="结合培训细则对跟师学习；学习的病种、学习内容体会等方面进行小结，要求300字以上。" name="personalSummary" class="validate[required]"></textarea>
							</c:if>
							<c:if test="${empty rec && !(param.roleFlag eq GlobalConstant.RES_ROLE_SCOPE_DOCTOR)}">
								暂无出科小结!
							</c:if>
				...
				<c:if test="${!empty rec}">
				<table class="orgTable" border="0" width="90%" cellspacing="0" cellpadding="0" style="margin-left: auto;margin-right: auto;">
					<caption class="orgCaption" style="color: #399ce0;">导师评价</caption>
					<tr class="orgTr">
						<td class="orgTd" colspan="4" style="padding-top:10px;padding-left:0;">
							<c:if test="${param.roleFlag eq GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty formDataMap['deptAppraise']}">
								<textarea name="deptAppraise"placeholder="主要包括规培态度、遵守劳动纪律、人际沟通、团队合作、临床综合能力等方面情况，要求导师亲笔书写。" class="validate[required]"></textarea>
							</c:if>
