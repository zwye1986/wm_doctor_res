...
<div class="mainright">
	...
	<table class="basic" width="100%" style="margin-top: 10px;">
		...
		<tr>
			<c:if test="${verification}">
				<td colspan="3">病情严重情况：
					&#12288;<label><input class="validate[required]" type="checkbox" onchange="single(this)" name="severity" value="轻" <c:if test="${formDataMap['severity']=='轻'}">checked</c:if>/>&nbsp;轻</label>&nbsp;
					&#12288;<label><input class="validate[required]" type="checkbox" onchange="single(this)" name="severity" value="中" <c:if test="${formDataMap['severity']=='中'}">checked</c:if>/>&nbsp;中</label>&nbsp;
					&#12288;<label><input class="validate[required]" type="checkbox" onchange="single(this)" name="severity" value="重" <c:if test="${formDataMap['severity']=='重'}">checked</c:if>/>&nbsp;重</label>&nbsp;
				</td>
			</c:if>
			<c:if test="${!(verification)}">
				<td colspan="3">病情严重情况：
		...
		<c:if test="${!(verification)}">
			<tr>
				<td colspan="2">1、医疗面谈(称呼病人/自我介绍/病人陈述病史/适当提问/适当回应)</td>
				<td>
