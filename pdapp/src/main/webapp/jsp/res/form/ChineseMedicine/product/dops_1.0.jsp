	...
	<table class="basic" width="100%" style="margin-top: 10px;">
		...
		<tr>
			<c:if test="${verification}">
				<td >
				评估日期：
				 <input  class="inputText" name="assessmentDate" type="text" value="${formDataMap['assessmentDate']}" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});"/>
				</td>
				<td colspan="2">评估地点：
		...
		<tr>
			<td colspan="3">病人来源：
				<c:if test="${verification}"> 
					...
					<label style="padding-left: 2%;"><input type="checkbox" onchange="single(this)" name="patientSource" value="新病人" <c:if test="${formDataMap['patientSource']=='新病人'}">checked</c:if>/>&nbsp;新病人</label>&nbsp;
					<label style="padding-left: 2%;"><input type="checkbox" onchange="single(this)" name="patientSource" value="复诊病人" <c:if test="${formDataMap['patientSource']=='复诊病人'}">checked</c:if>/>&nbsp;复诊病人</label> &nbsp;
				</c:if> 
				<c:if test="${!(verification)}"> 
