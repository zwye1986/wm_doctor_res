...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			...
        			<td style="width: 21%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" style="width: 115px;" name="id" value="${formDataMap['id']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['id']}
        				</c:if>
        			...
        			<td style="width: 21%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" style="width: 115px;" name="patientName" value="${formDataMap['patientName']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['patientName']}
        				</c:if>
        			</td>
        			<td style="width: 14%;">住院号:</td>
        			<td style="width: 20%;">
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" style="width: 111px;" name="hospitaNumber" value="${formDataMap['hospitaNumber']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['hospitaNumber']}
        				</c:if>
        			</td>
        		</tr>
        		<tr>
        			...
        			<td>
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" name="admissionDate" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" style="width: 115px;" value="${formDataMap['admissionDate']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['admissionDate']}
        				</c:if>
        			</td>
        			<td>主要诊断:</td>
        			<td>
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					<input type="text" name="mainDiagnosis" style="width: 115px;" value="${formDataMap['mainDiagnosis']}"/>
        				</c:if>
        				<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
        					${formDataMap['mainDiagnosis']}
        				</c:if>
        			</td>
        			<td>操作人身份:</td>
        			<td>
        				<c:if test="${param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
	        				<select name="operatorIdentity" style="width: 115px;">
	        					<option></option>
	        					<option value="主刀" <c:if test="${'主刀' eq formDataMap['operatorIdentity']}">selected</c:if>>主刀</option>
	        					<option value="一助" <c:if test="${'一助' eq formDataMap['operatorIdentity']}">selected</c:if>>一助</option>
	        					<option value="二助" <c:if test="${'二助' eq formDataMap['operatorIdentity']}">selected</c:if>>二助</option>
	        					<option value="其他人员" <c:if test="${'其他人员' eq formDataMap['operatorIdentity']}">selected</c:if>>其他人员</option>
	        				</select>
	        				<input type="hidden" name="operatorIdentity" value="${formDataMap['operatorIdentity']}"/>
	        			</c:if>
	        			<c:if test="${param.roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
	        				${formDataMap['operatorIdentity']}
	        			</c:if>
        			</td>
        		</tr>	
        	</table>
