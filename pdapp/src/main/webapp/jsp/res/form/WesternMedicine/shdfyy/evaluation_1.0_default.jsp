...
<table class="basic" width="100%" style="margin-top: 10px;">
	...
	<tr>
		<td rowspan="6" colspan="2">临床综合能力考核（60%）</td>
	...
	<tr>
		...
		<td>5.0%</td>
		<td class="sSum">${formDataMap['cFive']*0.05}</td>
	</tr>
	<tr>
		...
		<td>5.0%</td>
		<td class="sSum">${formDataMap['cSix']*0.05}</td>
	</tr>
	<tr>
		<td colspan="2">病例分析</td>
		<td style="text-align: center">
			<c:if test="${(param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId) }">
				<input class="validate[custom[integer],min[0],max[100]]"type="text" name="cSeven" value="${formDataMap['cSeven']}" onchange="count(this)"
					   style="width: 50%;">
			</c:if>
			<c:if test="${!((param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId)) }">
				<c:if test="${!empty formDataMap['cSeven']}">
					${formDataMap['cSeven']}
				</c:if>
				<input type="hidden" name="cSeven" value="${formDataMap['cSeven']}"/>
			</c:if>
		</td>
		<td>15.0%</td>
		<td class="sSum">${formDataMap['cSeven']*0.15}</td>
	</tr>
	<tr>
		<td colspan="2">临床操作</td>
		<td style="text-align: center">
			<c:if test="${(param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId) }">
				<input class="validate[custom[integer],min[0],max[100]]"type="text" name="cEight" value="${formDataMap['cEight']}" onchange="count(this)"
					   style="width: 50%;">
			</c:if>
			<c:if test="${!((param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId)) }">
				<c:if test="${!empty formDataMap['cEight']}">
					${formDataMap['cEight']}
				</c:if>
				<input type="hidden" name="cEight" value="${formDataMap['cEight']}"/>
			</c:if>
		</td>
		<td>10.0%</td>
		<td class="sSum">${formDataMap['cEight']*0.1}</td>
	</tr>
	<tr>
		...
		<td>5.0%</td>
		<td class="sSum">${formDataMap['cNine']*0.05}</td>
	</tr>
	<tr>
