		...
		<form id="dopsForm" style="position: relative">
			<div id="printDiv">
				<div id="title" style="width:100%;text-align: center;font-size: 15px;">
					上海市奉贤区中心医院住院医师规范化培训<br/>腰椎穿刺术操作技能考核评分表
				</div>
				...
				<table class="basic" width="100%" style="margin-top: 10px;">
					...
					<tr>
						<td colspan="5">①协助病人侧卧于硬板床上（3 分）②背部与床面垂直（3 分）③使病人头向前胸部屈曲（3 分）④两手抱膝紧贴腹部，使躯干成弓形（3 分）⑤脊柱尽量后凸以增宽椎间隙（3 分）</td>
						<td>15</td>
					...
					<tr>
						...
						<td colspan="5">定位：①以髂嵴连线与后正中线的交点处为穿刺点（相当于第 3、4 腰椎棘突间隙）（4 分）②也可在上一或下一腰椎见间隙进行（2 分）</td>
						<td>6</td>
					...
					<tr>
						<td rowspan="5">麻醉穿刺（31分）</td>
						<td colspan="5">①检查器械（2 分）②检查穿刺针是否通畅、针芯是否配套（4 分）</td>
						<td>6</td>
					...
					<tr>
						<td colspan="5">①左手固定局部皮肤部位，右手持穿刺针以垂直背部刺入（3 分）②针尖可稍倾向头部方向（3 分）③当突然两次突破感（穿过棘间韧带和硬脊膜）后可将针芯慢慢抽出，见脑脊液流出（3 分）</td>
						<td>9</td>
						<td><input name="part08" type="text" value="${formDataMap['part08']}" class="inputText ctrlInput validate[required,custom[number],max[9],min[0]] scoreCount" style="width:90%; " style="width: 80px;"/></td>
					</tr>
					<tr>
						<td colspan="5">用无菌试管接取缓慢放出的脑脊液 2-5ml </td>
						<td>5</td>
					...
					<tr>
						<td colspan="5">将抽出液送化验（常规、生化、培养及病理）、记量</td>
						<td>5</td>
					...
					<tr>
						<td rowspan="5">穿刺结束后处理（17 分）</td>
						<td colspan="5">①抽完后重新插入针芯②拔出穿刺针③消毒后适当按压后覆盖无菌纱布、稍用力压迫片刻。</td>
						<td>6</td>
					...
					<tr>
						...
						<td>3</td>
						<td><input name="part12" type="text" value="${formDataMap['part12']}" class="inputText ctrlInput validate[required,custom[number],max[3],min[0]] scoreCount" style="width:90%; " style="width: 80px;"/></td>
					</tr>
					<tr>
						<td colspan="5">术后嘱病人保持去枕平卧体位 4-6 小时</td>
						<td>3</td>
						<td><input name="part13" type="text" value="${formDataMap['part13']}" class="inputText ctrlInput validate[required,custom[number],max[3],min[0]] scoreCount" style="width:90%; " style="width: 80px;"/></td>
					</tr>
					<tr>
						<td colspan="5">告诉病人有不适立即通知医护人员</td>
						<td>2</td>
					...
					<tr>
						...
						<td>3</td>
						<td><input name="part15" type="text" value="${formDataMap['part15']}" class="inputText ctrlInput validate[required,custom[number],max[3],min[0]] scoreCount" style="width:90%; " style="width: 80px;"/></td>
					</tr>
					<tr>
						<td>提问（6 分） </td>
						<td colspan="5"></td>
