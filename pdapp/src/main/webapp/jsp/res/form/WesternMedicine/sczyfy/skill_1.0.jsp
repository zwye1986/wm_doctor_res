...
<!-- 	                		</div> -->
      	 			...
      	 			<c:if test="${!empty rec}">
	      	 			...
	      	 			<c:if test="${!doctor}">
	      	 				...
	      	 				<div style="margin-right:10%">${rec.itemName}</div>
	      	 				</c:if>
	      	 			</c:if>
      	 			</c:if>
      	 			<c:if test="${doctor}">
...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			...
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
        					<input type="text" style="width: 160px;" class="validate[required]" name="skill_pName" value="${formDataMap['skill_pName']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
       						<input type="text" style="width: 160px;" class="validate[required]" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" name="skill_operDate" value="${formDataMap['skill_operDate']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<tr>
	        			...
	        			<td colspan="3">
	        				<c:if test="${doctor}">
	        					<textarea placeholder="100字" id="fail_diagnosticResults" style="width: 100%; height:100px; border: none;" name="fail_diagnosticResults">${formDataMap['fail_diagnosticResults']}</textarea>
	        				</c:if>
	        				<c:if test="${!doctor}">
	        					${formDataMap['fail_diagnosticResults']}
	        				</c:if>
	        			</td>
        			</tr>	
        			<tr>
