      ...
      <div class="content">
        ...
        <form id="operationForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		<tr>
        			<td style="width: 12%;"><span style="color: red;">*</span>手术名称：</td>
        			<td colspan="5">
						<c:if test="${empty rec}">
...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			...
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 160px;" name="operation_pName" value="${formDataMap['operation_pName']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['operation_pName']}
        				</c:if>
        			</td>
        			<td style="width: 14%;"><span style="color: red;">*</span>手术日期：</td>
        			<td style="width: 20%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 160px;"  readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" name="operation_operDate" value="${formDataMap['operation_operDate']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<tr>
        			<td><span style="color: red;">*</span>病历号：</td>
        			<td>
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" name="operation_mrNo" style="width: 160px;" value="${formDataMap['operation_mrNo']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['operation_mrNo']}
        				</c:if>
        			</td>
        			<td><span style="color: red;">*</span>术中职务：</td>
        			<td>
        				<c:if test="${doctor}">
        				<select name="operation_operRole" style="width: 164px;" class="autoValue validate[required]">
        					<option></option>
        					<option value="0" <c:if test="${'0' eq formDataMap['operation_operRole_id']}">selected</c:if>>术者</option>
        					<option value="1" <c:if test="${'1' eq formDataMap['operation_operRole_id'] or empty formDataMap['operation_operRole_id']}">selected</c:if>>一助</option>
        					<option value="2" <c:if test="${'2' eq formDataMap['operation_operRole_id']}">selected</c:if>>二助</option>
       					</select>
       					</c:if>
       					<c:if test="${!doctor}">
       						${formDataMap['operation_operRole']}
       						<input type="hidden" name="operation_operRole" value="${formDataMap['operation_operRole']}"/>
       					</c:if>
        			</td>
        			</tr>
        			<tr>
