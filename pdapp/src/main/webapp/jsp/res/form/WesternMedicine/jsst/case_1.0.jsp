        ...
        <form id="dblForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		...
        		<tr>
        			...
        			<td>
        				<c:if test="${doctor}">
       					<select name="mr_diagType" style="width: 144px;">
        					<option></option>
        					<option value="主要诊断" <c:if test="${'主要诊断' eq formDataMap['mr_diagType']}">selected</c:if>>主要诊断</option>
        					<option value="次要诊断" <c:if test="${'次要诊断' eq formDataMap['mr_diagType']}">selected</c:if>>次要诊断</option>
        					<option value="并行诊断" <c:if test="${'并行诊断' eq formDataMap['mr_diagType']}">selected</c:if>>并行诊断</option>
        				</select>
        				</c:if>
        				<c:if test="${!doctor}">
				...
				<tr>
					...
					<td colspan="3">
						<c:set var="imageList" value="${formDataMap['imageList']}"></c:set>
