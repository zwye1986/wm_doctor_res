      ...
      <div class="content">
        ...
        <form id="dblForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		<tr>
        			<td style="width: 12%;"><span style="color: red;">*</span>病种名称：</td>
...
<!-- 	                		</div> -->
		      	 			<input id="viewContainer" type="text"  class="validate[required]" value="${rec.itemName}" style="width: 100%;" readonly="readonly" onclick="viewReqs();" placeholder="点击选择子项"/>
...
<%-- 	      	 				${formDataMap['disease_diagName']}</div> --%>
        		...
        		<tr>
        			...
        			<td style="width: 30%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 160px;" name="disease_pName" value="${formDataMap['disease_pName']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['disease_pName']}
        				</c:if>
        			</td>
        			<td style="width: 20%;"><span style="color: red;">*</span>日期：</td>
        			<td colspan="3" style="width: 30%;">
        				<c:if test="${doctor}">
        					<input style="width: 160px;" class="validate[required]" type="text" name="disease_pDate" value="${formDataMap['disease_pDate']}" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
        				</c:if>
        				<c:if test="${!doctor}">
        		...
        		<tr>
        			...
        			<td>
        				<c:if test="${doctor}">
        					<input type="text" name="disease_mrNo" class="validate[required]" style="width: 160px;" value="${formDataMap['disease_mrNo']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
        				<select name="disease_diagType" style="width: 164px;" class="autoValue validate[required]">
	        					<option></option>
	        					<option value="1" <c:if test="${'1' eq formDataMap['disease_diagType_id'] or empty formDataMap['disease_diagType_id']}">selected</c:if>>主要诊断</option>
        			...
        			<tr>
	        			<td style="width: 13%;"><span style="color: red;">*</span>是否主管：</td>
	        			<td style="width: 21%;">
	        			<c:if test="${doctor}">
	        				<label><input type="radio" name="disease_isCharge" <c:if test="${'1' eq formDataMap['disease_isCharge_id'] or empty formDataMap['disease_isCharge_id']}">checked</c:if> value="1" class="autoValue validate[required]"/>是</label>&#12288;
	        				<label><input type="radio" name="disease_isCharge"  <c:if test="${'0' eq formDataMap['disease_isCharge_id']}">checked</c:if> value="0" class="autoValue validate[required]" />否</label>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['disease_isCharge']}
        				</c:if>
        			</td>
        			<td style="width: 13%;"><span style="color: red;">*</span>是否抢救：</td>
	        			<td style="width: 21%;">
	        			<c:if test="${doctor}">
        					<label><input type="radio" name="disease_isRescue" <c:if test="${'1' eq formDataMap['disease_isRescue_id']}">checked</c:if> value="1" onchange="check(this.value);" class="autoValue validate[required]"/>是</label>&#12288;
        					<label><input type="radio" name="disease_isRescue"  <c:if test="${'0' eq formDataMap['disease_isRescue_id'] or empty formDataMap['disease_isRescue_id']  }">checked</c:if> value="0" onchange="check(this.value);" class="autoValue validate[required]"/>否</label>
        				</c:if>
        				<c:if test="${!doctor}">
