			...
			<form id="diseaseForm" enctype="multipart/form-data" method="post">
				...
				<table class="basic" width="100%">
					<tr>
						...
						<td >
							<c:if test="${doctor}">
							<input style="width: 160px;" class="validate[required]" name="activity_date" type="text" value="${formDataMap['activity_date']}" style="margin-right: 0px" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
							</c:if>
							<c:if test="${!doctor}">
						...
						<td >
							<c:if test="${doctor}">
							<input style="width: 160px;" class="validate[required]" name="activity_title" type="text" value="${formDataMap['activity_title']}" style="margin-right: 0px"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['activity_title']}
							</c:if>
						</td>

					</tr>
					<tr>
						<td style="width: 100px;"><font color="red">*</font>讲者/带教：</td>
						<td >
							<c:if test="${doctor}">
							<input style="width: 160px;" class="validate[required]" name="activity_speaker" type="text" value="${formDataMap['activity_speaker']}" style="margin-right: 0px"/>
							</c:if>
							<c:if test="${!doctor}">
