<%@ page language="java" contentType="aplication/json; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	"values":[
		...
		<c:choose>
			<c:when test="${!testTypeFlag}">
				<c:set var="llkh" value="${formDataMap['llkh']}"/>
			</c:when>
			<c:when test="${testTypeFlag}">
				<c:set var="llkh" value="${empty formDataMap['llkh'] ? (empty outScore.theoryScore ? '':(outScore.theoryScore.toString()+0)*0.25*10/10):formDataMap['llkh']}"/>
			</c:when>
		</c:choose>
		{
			"inputId":"llkh",
			"value":${pdfn:toJsonString(llkh)}
		},
		{
			"inputId":"gradeScore",
