...
<html>
<head>
<title>惠州APP-带教端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(action=="login" || action=="version" || action=="notice" || action=="noticeCount"){
			...
			url = "<s:url value='/res/hzyy/teacher'/>/"+action+"?_method="+method;	
		}
...
<body>
	<form name="appForm" action="${ctxPath}/res/hzyy/teacher/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="skillList" method="get" needDataType="N" <c:if test="${param.reqCode=='skillList'}">selected</c:if> >技能考核列表(skillList)get</option>
						<option value="selectSkill" method="get" needDataType="N" <c:if test="${param.reqCode=='selectSkill'}">selected</c:if> >选择技能考核表(selectSkill)get</option>
						<option value="saveSkillScore" method="post" needDataType="N" <c:if test="${param.reqCode=='saveSkillScore'}">selected</c:if> >保存技能考核表(saveSkillScore)post</option>
						<option value="patientList" method="get" needDataType="N" <c:if test="${param.reqCode=='patientList'}">selected</c:if> >接诊病人列表(patientList)get</option>
						<option value="selectPatient" method="get" needDataType="N" <c:if test="${param.reqCode=='selectPatient'}">selected</c:if> >选择病人评分表(selectPatient)get</option>
						<option value="savePatientScore" method="post" needDataType="N" <c:if test="${param.reqCode=='savePatientScore'}">selected</c:if> >保存病人评分表(savePatientScore)post</option>
						<option value="selectTheoryScore" method="get" needDataType="N" <c:if test="${param.reqCode=='selectTheoryScore'}">selected</c:if> >理论成绩(selectTheoryScore)get</option>
						<option value="saveTheoryScore" method="post" needDataType="N" <c:if test="${param.reqCode=='saveTheoryScore'}">selected</c:if> >保存理论成绩(saveTheoryScore)post</option>
			<option value="AfterEvaluation" method="get" needDataType="N" <c:if test="${param.reqCode=='AfterEvaluation'}">selected</c:if> >出科考核表(AfterEvaluation)get</option>
			<option value="saveAfterEvaluation" method="post" needDataType="N" <c:if test="${param.reqCode=='saveAfterEvaluation'}">selected</c:if> >保存出科考核表(saveAfterEvaluation)post</option>
			...
			<%--<option value="delFile" method="post" needDataType="N" <c:if test="${param.reqCode=='delFile'}">selected</c:if> >删除附件(delFile)post</option>--%>
			<option value="checkFile" method="post"  needDataType="N" <c:if test="${param.reqCode=='checkFile'}">selected</c:if> >下载附件或图片前先校验(checkFile)post</option>
			<option value="downFile" method="post" isForm="Y" needDataType="N" <c:if test="${param.reqCode=='downFile'}">selected</c:if> > 下载附件或图片(downFile)post</option>
			<option value="addFile" method="post" isForm="N" needDataType="Y" <c:if test="${param.reqCode=='addFile'}">selected</c:if> > 上传附件(addFile)post</option>
		</select>
	...
	<div id="skillList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&searchData=&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="patientList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&searchData=&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="selectPatient" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&patientTypeId=</textarea>
	</div>
	<div id="selectSkill" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&skillFlow=</textarea>
	</div>
	<div id="saveSkillScore" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&skillFlow=&其他参数为selectSkill返回接口中的必填字段,附件的参数名为fileFlow，如果是多个就多个fileFlow</textarea>
	</div>
	<div id="savePatientScore" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&patientTypeId=&其他参数为selectPatient返回接口中的必填字段,附件的参数名为fileFlow，如果是多个就多个fileFlow</textarea>
	</div>
	<div id="selectTheoryScore" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}</textarea>
	</div>
	<div id="saveTheoryScore" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&TheoryScore=${TheoryScore}</textarea>
	</div>
	...
	<div id="addData" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&funcTypeId=${funcTypeId}&funcFlow=${funcFlow}&cataFlow=1</textarea>
	</div>
	<div id="viewData" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&funcTypeId=${funcTypeId}&funcFlow=${funcFlow}&cataFlow=1&dataFlow=${dataFlow}</textarea>
	</div>
	<div id="AfterEvaluation" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}</textarea>
	</div>
	<div id="saveAfterEvaluation" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&其他参数为AfterEvaluation返回接口中的必填字段</textarea>
	</div>
	...
	<div id="addFile" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&isActivity=Y&fileName=&uploadFile=${uploadFile}</textarea>
	</div>
</body>
</html>
