...
<html>
<head>
<title>惠州APP-教秘端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(action=="login" || action=="version" || action=="notice" || action=="noticeCount"){
			...
			url = "<s:url value='/res/hzyy/secretarie'/>/"+action+"?_method="+method;
		}
...
<body>
	<form name="appForm" action="${ctxPath}/res/hzyy/secretarie/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="saveTheoryScore" method="get" needDataType="N" <c:if test="${param.reqCode=='saveTheoryScore'}">selected</c:if> >保存理论成绩(saveTheoryScore)get</option>
			...
			<option value="saveNewAfterEvaluation" method="post" needDataType="N" <c:if test="${param.reqCode=='saveNewAfterEvaluation'}">selected</c:if> >保存出科考核表(saveNewAfterEvaluation)post</option>
	...
	<div id="saveTheoryScore" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&UCSID=${UCSID}&TheoryScore=${TheoryScore}</textarea>
	</div>

	<div id="newAfterEvaluation" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&UCSID=${UCSID}</textarea>
	</div>
	<div id="saveNewAfterEvaluation" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&UCSID=${UCSID}&其他参数为newAfterEvaluation返回接口中的必填字段</textarea>
	</div>
</body>
</html>
