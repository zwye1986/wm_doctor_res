		...
		<c:forEach items="${lectureInfos}" var="lecture" varStatus="s">
	     	{
				...
				"operId":
							<c:choose>
								<c:when test="${(empty evaDetailMap[lecture.lectureFlow])&&(evaMap[lecture.lectureFlow]>=0)&&(!empty scanMap[lecture.lectureFlow])&&(!empty scan2Map[lecture.lectureFlow])}">
									"Evaluate",
								</c:when>
								<c:when test="${(!empty evaDetailMap[lecture.lectureFlow])&&(!empty scanMap[lecture.lectureFlow])}">
									"ShowLecture",
								</c:when>
								<c:when test="${(evaMap[lecture.lectureFlow]<0)&&(!empty scanMap[lecture.lectureFlow])&&(!empty scan2Map[lecture.lectureFlow])&&(empty evaDetailMap[lecture.lectureFlow])}">
									"IsClosed",
								</c:when>
								<c:when test="${not empty scanMap[lecture.lectureFlow] and empty scan2Map[lecture.lectureFlow]}">
									"IsScan",
								</c:when>
								<c:when test="${not empty scanMap[lecture.lectureFlow] and not  empty scan2Map[lecture.lectureFlow]}">
									"IsOut",
								</c:when>
								<c:otherwise>
									"",
