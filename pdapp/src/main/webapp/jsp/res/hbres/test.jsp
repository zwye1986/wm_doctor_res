...
<html>
<head>
<title>湖北住院医师APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/hbres'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/hbres/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="zpNoticeDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='zpNoticeDetail'}">selected</c:if> >3.4.2最新指南详情(zpNoticeDetail)post</option>
					...
					<a href="<s:url value='/res/hbres/student/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/hbres/teacher/test'/>">老师端测试工具</a>
					<a href="<s:url value='/res/hbres/kzr/test'/>">科主任端测试工具</a>
					<script>
						function reload(){
	...
	<div id="login" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userCode=student&userPasswd=123456&uuid=</textarea>
	</div>
	...
	<div id="getZhupeiNotices" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&noticeTitle=${noticeTitle}&roleId=${roleId}&pageIndex=1&pageSize=100</textarea>
	</div>
