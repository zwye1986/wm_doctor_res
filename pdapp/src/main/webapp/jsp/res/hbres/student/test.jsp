...
<html>
<head>
<title>湖北住院医师APP学生端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/hbres/student'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/hbres/student/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="getSysNotice" method="post" needDataType="N" <c:if test="${param.reqCode=='getSysNotice'}">selected</c:if> >6.0.3 科教通知列表(getSysNotice)post</option>
			<option value="sysNoticeDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='sysNoticeDetail'}">selected</c:if> >6.0.4 科教通知详情(sysNoticeDetail)post</option>
		</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/hbres/test'/>">测试工具</a>
		</div>
