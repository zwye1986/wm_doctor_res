...
<html>
<head>
<title>青浦住院医师APP学生端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/qpres/student'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/qpres/student/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="newEvaluate" method="post" needDataType="N" <c:if test="${param.reqCode=='newEvaluate'}">selected</c:if> >3.3.5 查看讲座评价2(newEvaluate)post</option>
			<option value="saveNewEvaluate" method="post" needDataType="N" <c:if test="${param.reqCode=='saveNewEvaluate'}">selected</c:if> >3.3.6 保存讲座评价2(saveNewEvaluate)post</option>
			...
			<option value="checkFile" method="post"  needDataType="N" <c:if test="${param.reqCode=='checkFile'}">selected</c:if> >下载文件前先校验(checkFile)post</option>
			<option value="downFile" method="post" isForm="Y" needDataType="N" <c:if test="${param.reqCode=='downFile'}">selected</c:if> > 下载文件(downFile)post</option>
			<option value="viewLectureImage" method="post" needDataType="N" <c:if test="${param.reqCode=='viewLectureImage'}">selected</c:if> >3.3.15 查看佐证材料(viewLectureImage)post</option>
			<option value="addLectureImage" method="post" needDataType="N" <c:if test="${param.reqCode=='addLectureImage'}">selected</c:if> >3.3.15 上传佐证材料图片(addLectureImage)post</option>
			<option value="deleteLectureImage" method="get" needDataType="N" <c:if test="${param.reqCode=='deleteLectureImage'}">selected</c:if> >3.3.15 删除佐证材料图片(deleteLectureImage)get</option>
			<option value="deptActivtiyList" method="get" needDataType="N" <c:if test="${param.reqCode=='deptActivtiyList'}">selected</c:if> >3.3.16 科室活动评价(deptActivtiyList)get</option>
			<option value="deptActivtiyEval" method="get" needDataType="N" <c:if test="${param.reqCode=='deptActivtiyEval'}">selected</c:if> >3.3.17 科室活动评价详情页(deptActivtiyEval)get</option>
			<option value="saveDeptActivtiyEval" method="post" needDataType="N" <c:if test="${param.reqCode=='saveDeptActivtiyEval'}">selected</c:if> >3.3.17 保存科室活动评价(saveDeptActivtiyEval)post</option>
		</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/qpres/test'/>">测试工具</a>
		</div>
	...
	<div id="newEvaluate" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&lectureFlow=${lectureFlow}</textarea>
	</div>
	<div id="saveNewEvaluate" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&lectureFlow=${lectureFlow}&其他参数参照双向评价提交接口</textarea>
	</div>
	...
	<div id="deptActivtiyList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&isEval=Y&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="deptActivtiyEval" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&itemFlow=&evalFlow=</textarea>
	</div>
	<div id="saveDeptActivtiyEval" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&itemFlow=&deptActivtiyEval接口返回的所有inputId</textarea>
	</div>
	...
	<div id="checkFile" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&lectureFlow=</textarea>
	</div>
	<div id="downFile" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&lectureFlow=</textarea>
	</div>
	<div id="viewLectureImage" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">registFlow=&userFlow=${userFlow}</textarea>
	</div>
	<div id="addLectureImage" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">registFlow=&userFlow=${userFlow}&fileName=&其他参数和学员端addImage接口一样</textarea>
	</div>
	<div id="deleteLectureImage" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">registFlow=&userFlow=${userFlow}&imageFlow=</textarea>
	</div>
</body>
</html>
