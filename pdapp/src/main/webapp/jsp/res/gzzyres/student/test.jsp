...
<html>
<head>
<title>广州中医APP-学员端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(action=="login" || action=="version" || action=="notice" || action=="noticeCount"){
			...
			url = "<s:url value='/res/gzzyres/student'/>/"+action+"?_method="+method;	
		}
...
<body>
	<form name="appForm" action="${ctxPath}/res/gzzyres/student/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="suggestions" method="post" needDataType="N" <c:if test="${param.reqCode=='suggestions'}">selected</c:if> >住培反馈(suggestions)post</option>
						<option value="delOpinions" method="post" needDataType="N" <c:if test="${param.reqCode=='delOpinions'}">selected</c:if> >删除意见反馈(delOpinions)post</option>
						<option value="opinionsDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='opinionsDetail'}">selected</c:if> >意见反馈详情(opinionsDetail)post</option>
						<option value="saveOpinions" method="post" needDataType="N" <c:if test="${param.reqCode=='saveOpinions'}">selected</c:if> >住培反馈保存(saveOpinions)post</option>
						<option value="getZhupeiNotices" method="post" needDataType="N" <c:if test="${param.reqCode=='getZhupeiNotices'}">selected</c:if> >最新指南列表(getZhupeiNotices)post</option>
						<option value="zpNoticeDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='zpNoticeDetail'}">selected</c:if> >最新指南详情(zpNoticeDetail)post</option>
						<option value="studentSignIn" method="post" needDataType="N" <c:if test="${param.reqCode=='studentSignIn'}">selected</c:if> >学员签到查询次数(studentSignIn)post</option>
						<option value="saveSignIn" method="post" needDataType="N" <c:if test="${param.reqCode=='saveSignIn'}">selected</c:if> >保存签到信息(saveSignIn)post</option>
