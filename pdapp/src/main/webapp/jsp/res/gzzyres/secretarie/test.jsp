...
<html>
<head>
<title>广州中医APP-教秘端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(action=="login" || action=="version" || action=="notice" || action=="noticeCount"){
			...
			url = "<s:url value='/res/gzzyres/secretarie'/>/"+action+"?_method="+method;
		}
...
<body>
	<form name="appForm" action="${ctxPath}/res/gzzyres/secretarie/remember" method="get">
	...
	<div id="DayEval" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&UCSID=${UCSID}&UserID=${UserID}&stuType=${stuType}</textarea>
	</div>
	...
	<div id="saveDayEval" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&UCSID=${UCSID}&stuType=${stuType}&加DayEval接口返回的所有必填项</textarea>
	</div>
