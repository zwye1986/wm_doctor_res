...
<html>
<head>
<title>蚌埠医附院APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(action=="login" || action=="version" || action=="notice" || action=="noticeCount"){
			url = "<s:url value='/res/bengyify'/>/"+action+"?_method="+method;	
		}else{
			url = "<s:url value='/res/bengyify/teacher'/>/"+action+"?_method="+method;	
		}
...
<body>
	<form name="appForm" action="${ctxPath}/res/bengyify/teacher/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
					    ...
					    <option value="activityCount" method="get" needDataType="N" <c:if test="${param.reqCode=='activityCount'}">selected</c:if>>待办活动数量(activityCount)get</option>
					    <option value="activityList" method="get" needDataType="N" <c:if test="${param.reqCode=='activityList'}">selected</c:if>>活动列表(activityList)get</option>
					    <option value="activityDetail" method="get" needDataType="N" <c:if test="${param.reqCode=='activityDetail'}">selected</c:if>>活动详情(activityDetail)get</option>
					    <option value="saveActivity" method="post" needDataType="N" <c:if test="${param.reqCode=='saveActivity'}">selected</c:if>>保存安排活动(saveActivity)post</option>
					</select>
	...
	<div id="activityCount" class="reqParam" style="display: none">
	    <textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}</textarea>
	</div>
	<div id="activityList" class="reqParam" style="display: none">
	    <textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}&status=Y&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="activityDetail" class="reqParam" style="display: none">
	    <textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}&CaDisID=${CaDisID}</textarea>
	</div>
	<div id="saveActivity" class="reqParam" style="display: none">
	    <textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}&CaDisID=${CaDisID}&uploadFile=&CaDisTime=&CaDisContent=&CaName=&CaEndDisTime=</textarea>
	</div>
</body>
</html>
