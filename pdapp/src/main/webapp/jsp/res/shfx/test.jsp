...
<html>
<head>
<title>奉贤住院医师APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/shfx'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/shfx/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="version2" method="get" needDataType="N" <c:if test="${param.reqCode=='version2'}">selected</c:if> >3.1.2查询最新版本号(version2)get</option>
					...
					<a href="<s:url value='/res/shfx/student/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/shfx/teacher/test'/>">老师端测试工具</a>
					<a href="<s:url value='/res/shfx/kzr/test'/>">科主任端测试工具</a>
					<a href="<s:url value='/res/shfx/jxms/test'/>">教秘端测试工具</a>
					<script>
						function reload(){
	...
	<div id="version2" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly"></textarea>
	</div>
