...
<html>
<head>
<title>奉贤教秘APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/shfx/jxms'/>/"+action;
		//"?_method="+method;
		if(method=="put"){
			method = "post"
		}
...
<body>
	<form name="appForm" action="/pdapp/res/shfx/jxms/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="batchAudit" method="post" needDataType="N" <c:if test="${param.reqCode=='batchAudit'}">selected</c:if> >1.0.23 数据一键审核(batchAudit)post</option>
						<option value="oneAudit" method="post" needDataType="N" <c:if test="${param.reqCode=='oneAudit'}">selected</c:if> >1.0.24 单条审核(oneAudit)post</option>
						<option value="skillEvlList" method="post" needDataType="N" <c:if test="${param.reqCode=='skillEvlList'}">selected</c:if> >1.0.25 临床技能评价列表(skillEvlList)post</option>
						<option value="stuSkillEvlList" method="post" needDataType="N" <c:if test="${param.reqCode=='stuSkillEvlList'}">selected</c:if> >1.0.26 查看学员已有的技能评价(stuSkillEvlList)post</option>
						<option value="saveSkillEvlForm" method="post" needDataType="N" <c:if test="${param.reqCode=='saveSkillEvlForm'}">selected</c:if> >1.0.27 保存技能评价表单(saveSkillEvlForm)post</option>
						<option value="view360SkillEvl" method="post" needDataType="N" <c:if test="${param.reqCode=='view360SkillEvl'}">selected</c:if> >1.0.28 评分详情(view360SkillEvl)post</option>
						<option value="skillTypeList" method="post" needDataType="N" <c:if test="${param.reqCode=='skillTypeList'}">selected</c:if> >1.0.28 技能评价表选择列表(skillTypeList)post</option>
						<option value="evl360List" method="post" needDataType="N" <c:if test="${param.reqCode=='evl360List'}">selected</c:if> >1.0.29 360评价列表(evl360List)post</option>
					</select>
					...
					<a href="<s:url value='/res/shfx/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/shfx/teacher/test'/>">带教端测试工具</a>
					<a href="<s:url value='/res/shfx/kzr/test'/>">科主任端测试工具</a>
		</div>
	...
	<div id="view360SkillEvl" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}&docFlow=${docFlow}&recTypeId=${recTypeId}&recFlow=${recFlow}&resultFlow=${resultFlow}</textarea>
	</div>
	<div id="skillTypeList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${docFlow}&recForm=${recForm}&resultFlow=${resultFlow}</textarea>
	</div>
	<div id="saveSkillEvlForm" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&resultFlow=${resultFlow}&operUserFlow=${operUserFlow}&recTypeId=${recTypeId}&recFlow=${recFlow}</textarea>
	</div>
	<div id="skillEvlList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">roleId=${roleId}&userFlow=${userFlow}&userName=${userName}&recForm=${recForm}&pageIndex=${pageIndex}&pageSize=${pageSize}</textarea>
	</div>
	<div id="stuSkillEvlList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&processFlow=${processFlow}&recForm=${recForm}&pageIndex=${pageIndex}&pageSize=${pageSize}</textarea>
	</div>
	<div id="evl360List" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">roleId=${roleId}&userFlow=${userFlow}&userName=${userName}&recForm=${recForm}&pageIndex=${pageIndex}&pageSize=${pageSize}</textarea>
	</div>
</body>
</html>
