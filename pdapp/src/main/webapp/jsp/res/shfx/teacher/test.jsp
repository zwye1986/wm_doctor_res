...
<html>
<head>
<title>奉贤APP老师端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/shfx/teacher'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/shfx/teacher/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			老师端动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="skillEvlList" method="post" needDataType="N" <c:if test="${param.reqCode=='skillEvlList'}">selected</c:if> >3.3.11 临床技能评价列表(skillEvlList)post</option>
						<option value="stuSkillEvlList" method="post" needDataType="N" <c:if test="${param.reqCode=='stuSkillEvlList'}">selected</c:if> >3.3.12 查看学员已有的技能评价(stuSkillEvlList)post</option>
						<option value="saveSkillEvlForm" method="post" needDataType="N" <c:if test="${param.reqCode=='saveSkillEvlForm'}">selected</c:if> >3.3.13 保存技能评价表单(saveSkillEvlForm)post</option>
						<option value="view360SkillEvl" method="post" needDataType="N" <c:if test="${param.reqCode=='view360SkillEvl'}">selected</c:if> >3.3.14 评分详情(view360SkillEvl)post</option>
