...
<html>
<head>
<title>广州肿瘤省厅APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/gzzl/kzr'/>/"+action;
		//"?_method="+method;
		if(method=="put"){
			method = "post"
		}
...
<body>
	<form name="appForm" action="/pdapp/res/gzzl/kzr/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="effectiveResult" method="post" needDataType="N" <c:if test="${param.reqCode=='effectiveResult'}">selected</c:if> >3.3.15 教学活动评价详情列表-修改状态(effectiveResult)post</option>
					...
					<a href="<s:url value='/res/gzzl/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/gzzl/teacher/test'/>">带教端测试工具</a>
		</div>
