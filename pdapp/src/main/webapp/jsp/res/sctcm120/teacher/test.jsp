...
<html>
<head>
<title>成都中医药大学APP老师端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/sctcm120/teacher'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/sctcm120/teacher/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			老师端动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="leaveAuditList" method="post" needDataType="N" <c:if test="${param.reqCode=='leaveAuditList'}">selected</c:if> >3.3.11 学员请假列表(leaveAuditList)post</option>
						<option value="showLeave" method="post" needDataType="N" <c:if test="${param.reqCode=='showLeave'}">selected</c:if> >3.3.12 学员请假审核详情页(showLeave)post</option>
						<option value="auditLeave" method="post" needDataType="N" <c:if test="${param.reqCode=='auditLeave'}">selected</c:if> >3.3.12 审核请假(auditLeave)post</option>
		</select>
					<select id="dataType" name="dataType" style="display:none;">
						...
						<option value="AfterSummary,AfterEvaluation,DiscipleSummary">出科考核（AfterSummary,AfterEvaluation,DiscipleSummary）</option>
					</select>
	...
	<div id="leaveAuditList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&statusId=Auditing或Audited&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="showLeave" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&recordFlow=</textarea>
	</div>
	<div id="auditLeave" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&recordFlow=&teacherAgreeFlag=Y或N&auditInfo=</textarea>
	</div>
