	...
	"recTypeList":[
		...
		<c:forEach items="${list}" var="doctorMap" varStatus="status">
			...
			</c:if>
			{
				...
				<c:set value="${doctorMap.processFlow}DiscipleSummary" var="discipleSumKey"/>
				...
				"discipleSumRecFlow":"${resRecMap[discipleSumKey].recFlow}",
				"discipleSumStatusId":"<c:if test="${empty resRecMap[discipleSumKey]}">notSumbit</c:if><c:if test="${(not empty resRecMap[discipleSumKey]) and (empty resRecMap[discipleSumKey].auditStatusId)}">notAudit</c:if><c:if test="${not empty resRecMap[discipleSumKey].auditStatusId}">isAudit</c:if>",
				"discipleSumStatusName":"<c:if test="${empty resRecMap[discipleSumKey]}">未提交</c:if><c:if test="${ (not empty resRecMap[discipleSumKey]) and (empty resRecMap[discipleSumKey].auditStatusId)}">未审核</c:if><c:if test="${not empty resRecMap[discipleSumKey].auditStatusId}">已审核</c:if>",
