<%@ page language="java" contentType="aplication/json; charset=UTF-8" pageEncoding="UTF-8"%>
{
    ...
    ,
        ...
        <c:set value="${param.processFlow}DiscipleSummary" var="discipleKey"/>
	...
	"funcLists":[
				...
				},
				{
					...
					"recType":"cr",
                    "reqType":"resRecList",
                    "recTypeId":"CaseRecord",
                    "recFlow":"",
                    "statusId":"",
                    "statusName":""
				},
                <c:if test="${dopsFlag eq 'Y'}">
                ...
                },
                {
                    "a":"10",
                    "typeName":"门诊跟师小结",
                    "recType":"discipleSummary",
                    "recTypeId":"DiscipleSummary",
                    "recFlow":"${resRecMap[discipleKey].recFlow}",
                    "rotationFlow":"${resDoctor.rotationFlow}",
                    "schDeptFlow":"${process.schDeptFlow}",
                    "operUserFlow":"${process.userFlow}",
                    "resultFlow":"${process.schResultFlow}",
                    "processFlow":"${process.processFlow}",
                    "headAuditStatusId":"${!empty resRecMap[discipleKey]?resRecMap[discipleKey].headAuditStatusId:''}",
                    "statusId":"<c:if test="${empty resRecMap[discipleKey].auditStatusId}">notAudit</c:if><c:if test="${not empty resRecMap[discipleKey].auditStatusId}">isAudit</c:if>",
                    "statusName":"<c:if test="${empty resRecMap[discipleKey].auditStatusId}">未审核</c:if><c:if test="${not empty resRecMap[discipleKey].auditStatusId}">已审核</c:if>"
                }
		],
    </c:if>
}