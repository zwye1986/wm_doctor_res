<%@ page language="java" contentType="aplication/json; charset=UTF-8" pageEncoding="UTF-8"%>
{
    ...
    "dataCount": ${dataCount},
		<c:set var="finishkey" value="${param.processFlow}${recTypeId}Finished"></c:set>
		<c:set var="reqkey" value="${param.processFlow}${recTypeId}ReqNum"></c:set>
		"finishNum":"${processPerMap[finishkey]}",
		"reqNum":"${processPerMap[reqkey]}",
	    <c:if test="${param.recType eq 'mr'}">
	    "head":[
	    	{
	            "headId": "mr_pName",
	            "label": "病人姓名"
	    ...
	    <c:if test="${param.recType eq 'disease'}">
	    "head":[
	    	{
	    ...
	    <c:if test="${param.recType eq 'skill'}">
	    "head":[
	    	{
	            "headId": "skill_pName",
	            "label": "病人姓名"
	    ...
	    <c:if test="${param.recType eq 'operation'}">
	    "head":[
	    	{
	    ...
	    <c:if test="${param.recType eq 'activity'}">
	    "head":[
	    	{
