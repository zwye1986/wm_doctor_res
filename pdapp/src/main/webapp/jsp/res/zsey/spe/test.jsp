...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/zsey/spe'/>/"+action;
		//"?_method="+method;
		if(method=="put"){
			method = "post"
		}
...
<body>
	<form name="appForm" action="/pdapp/res/zsey/spe/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="userList" method="post" needDataType="N" <c:if test="${param.reqCode=='userList'}">selected</c:if> >3.3.4 专业学员列表(userList) post</option>
						...
						<option value="auditAbsenceDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='auditAbsenceDetail'}">selected</c:if> >3.3.8 请假待审核或销假详情或已审核详情(auditAbsenceDetail) post</option>
						...
						<option value="doctorStatistics" method="post" needDataType="N" <c:if test="${param.reqCode=='doctorStatistics'}">selected</c:if> >3.3.8 学员信息统计1(doctorStatistics) post</option>
						<option value="doctorStatistics2" method="post" needDataType="N" <c:if test="${param.reqCode=='doctorStatistics2'}">selected</c:if> >3.3.8 学员信息统计2(doctorStatistics2) post</option>
						<option value="teacherStatistics" method="post" needDataType="N" <c:if test="${param.reqCode=='teacherStatistics'}">selected</c:if> >3.3.8 师资信息统计1(teacherStatistics) post</option>
						<option value="teacherList" method="post" needDataType="N" <c:if test="${param.reqCode=='teacherList'}">selected</c:if> >3.3.8 师资信息统计下师资列表(teacherList) post</option>
					</select>
	...
	<div id="studentList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&roleId=Spe&seachStr=${seachStr}&typeId=${typeId}&yearMonth=${yearMonth}&deptFlow=&sessionNumber=&doctorTypeId=&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="teacherList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}seachStr=${seachStr}&deptFlow=&isHave=&pageIndex=1&pageSize=10</textarea>
	</div>
	...
	<div id="teacherGradeList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&teaName=${teaName}&deptFlow=&orderBy=&pageIndex=1&pageSize=10</textarea>
	</div>
	...
	<div id="userList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&deptFlow=${deptFlow}&sessionNumber=${sessionNumber}&studentName=${studentName}&typeId=&doctorTypeId=&isAfter=&isError=&pageIndex=1&pageSize=10</textarea>
	</div>
	...
	<div id="doctorStatistics" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&sessionNumber=${sessionNumber}</textarea>
	</div>
	<div id="teacherStatistics" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}</textarea>
	</div>
	<div id="doctorStatistics2" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}</textarea>
	</div>
</body>
</html>
