...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/zsey'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/zsey/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="getNews" method="post" needDataType="N" <c:if test="${param.reqCode=='getNews'}">selected</c:if> >3.3.11 新闻动态(getNews)post</option>
						<option value="newDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='newDetail'}">selected</c:if> >3.3.12 新闻动态详情(newDetail)post</option>
						...
						<option value="mailList" method="post" needDataType="N" <c:if test="${param.reqCode=='mailList'}">selected</c:if> >3.3.15 通信录列表(mailList)post</option>

					</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/zsey/student/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/zsey/teacher/test'/>">老师端测试工具</a>
					<a href="<s:url value='/res/zsey/kzr/test'/>">科主任端测试工具</a>
					<a href="<s:url value='/res/zsey/spe/test'/>">专业管理员端测试工具</a>
					<a href="<s:url value='/res/zsey/km/test'/>">科秘端测试工具</a>
					<script>
						function reload(){
	...
	<div id="getNews" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="newDetail" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&infoFlow=${infoFlow}</textarea>
	</div>
	<div id="mailList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&userName=&role=</textarea>
	</div>
