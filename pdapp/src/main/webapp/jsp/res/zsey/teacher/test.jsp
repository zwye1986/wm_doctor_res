...
<html>
<head>
<title>中山二院继教科过程管理系统APP老师端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/zsey/teacher'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/zsey/teacher/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			老师端动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="userList" method="post" needDataType="N" <c:if test="${param.reqCode=='userList'}">selected</c:if> >3.3.4学员列表(userList) post</option>
						<option value="studentList" method="post" needDataType="Y" <c:if test="${param.reqCode=='studentList'}">selected</c:if> >3.3.4学员轮转列表(studentList) post</option>
						...
						<option value="evalInfoDetail" method="post" needDataType="N" <c:if test="${param.reqCode=='evalInfoDetail'}">selected</c:if> >1.0.13 360°带教评价详情(evalInfoDetail)post</option>
						<option value="saveEvalData" method="post" needDataType="N" <c:if test="${param.reqCode=='saveEvalData'}">selected</c:if> >3.1.9保存360°带教评价数据(saveEvalData)post</option>
					...
					<a href="<s:url value='/res/zsey/test'/>">测试工具</a>
					<a href="<s:url value='/res/zsey/kzr/test'/>">科主任端测试工具</a>
	...
	<div id="studentList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&doctorFlow=${doctorFlow}&deptFlow=${deptFlow}&speId=${speId}&statusId=${statusId}&sessionNumber=${sessionNumber}&studentName=${studentName}&biaoJi=${biaoJi}&pageIndex=1&pageSize=10</textarea>
	</div>
	<div id="userList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&typeId=&deptFlow=${deptFlow}&speId=${speId}&statusId=${statusId}&sessionNumber=${sessionNumber}&studentName=${studentName}&pageIndex=1&pageSize=10</textarea>
	</div>
	...
	<div id="evalInfoDetail" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&processFlow=${processFlow}&typeId=TeacherDoctorAssess&recFlow=</textarea>
	</div>
	<div id="saveEvalData" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&roleId=${roleId}&processFlow=${processFlow}&resultFlow=&typeId=TeacherDoctorAssess&recFlow=</textarea>
	</div>
</body>
</html>
