...
<html>
<head>
<title>标准产品过程APP老师端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/stdp/section'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/stdp/section/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			学生端动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="funcList" method="get" needDataType="N" <c:if test="${param.reqCode=='funcList'}">selected</c:if> >3.3.2获取轮转计划列表(funcList)get</option>
					</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/stdp/test'/>">测试工具</a>
		</div>
