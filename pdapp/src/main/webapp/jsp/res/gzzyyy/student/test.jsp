...
<html>
<head>
<title>广州中医医院住院医师APP学生端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(isForm=="Y")
		{
			var url = "<s:url value='/res/gzzyyy/student'/>/" + action ;
			...
			var url = "<s:url value='/res/gzzyyy/student'/>/" + action + "?_method=" + method;
			var formData = $("#reqParam").val();
...
<body>
	<form name="appForm" action="${ctxPath}/res/gzzyyy/student/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="showDiscipleMain" method="post"   needDataType="N" <c:if test="${param.reqCode=='showDiscipleMain'}">selected</c:if> >1.0.7 跟师文字提醒(showDiscipleMain)post</option>
		</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/gzzyyy/test'/>">测试工具</a>
		</div>
	...
	<div id="showDiscipleMain" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}</textarea>
	</div>
