	...
	"recTypeList":[
		...
		<c:forEach items="${list}" var="doctorMap" varStatus="status">
			...
			</c:if>
			{
				...
				"score": ${pdfn:toJsonString(empty doctorMap.score ? "暂无": doctorMap.score)},
				...
				<c:set value="${doctorMap.processFlow}MonthlyAssessment_clinic" var="clinicKey"/>
				<c:set value="${doctorMap.processFlow}MonthlyAssessment_inpatientArea" var="inpatientAreaKey"/>
				...
				"clinicRecFlow":"${resRecMap[clinicKey].recFlow}",
				"clinicStatusId":"<c:if test="${empty resRecMap[clinicKey]}">notAudit</c:if><c:if test="${not empty resRecMap[clinicKey]}">isAudit</c:if>",
				"clinicStatusName":"<c:if test="${empty resRecMap[clinicKey]}">未审核</c:if><c:if test="${not empty resRecMap[clinicKey]}">已审核</c:if>",
				"inpatientAreaRecFlow":"${resRecMap[inpatientAreaKey].recFlow}",
				"inpatientAreaStatusId":"<c:if test="${empty resRecMap[inpatientAreaKey]}">notAudit</c:if><c:if test="${not empty resRecMap[inpatientAreaKey]}">isAudit</c:if>",
				"inpatientAreaStatusName":"<c:if test="${empty resRecMap[inpatientAreaKey]}">未审核</c:if><c:if test="${not empty resRecMap[inpatientAreaKey]}">已审核</c:if>",
				<c:set var="showType" value=""></c:set>
				<c:if test="${(empty resRecMap[clinicKey] )and (empty resRecMap[inpatientAreaKey])}">
					<c:set var="showType" value="MonthlyAssessment_clinic,MonthlyAssessment_inpatientArea"></c:set>
				</c:if>
				<c:if test="${(not empty resRecMap[clinicKey] )and (empty resRecMap[inpatientAreaKey])}">
					<c:set var="showType" value="MonthlyAssessment_clinic"></c:set>
				</c:if>
				<c:if test="${(empty resRecMap[clinicKey] )and (not empty resRecMap[inpatientAreaKey])}">
					<c:set var="showType" value="MonthlyAssessment_inpatientArea"></c:set>
				</c:if>
				<c:if test="${isMonthOpen != 'Y' }">
					<c:set var="showType" value=""></c:set>
				</c:if>
				"recTypeId":"${showType}",
				"doctorTypeName":"<c:if test="${doctorMap.doctorTypeId eq 'Company'}">本</c:if><c:if test="${doctorMap.doctorTypeId eq 'CompanyEntrust'}">委</c:if>
					<c:if test="${doctorMap.doctorTypeId eq 'Social'}">行</c:if><c:if test="${doctorMap.doctorTypeId eq 'Graduate'}">硕</c:if>"
			}
			<c:if test="${!status.last}">
				,
			</c:if>
