...
<html>
<head>
<title>江苏中医住院医师APP学生端测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		if(isForm=="Y")
		{
			var url = "<s:url value='/res/jszy/student'/>/" + action ;
			...
			var url = "<s:url value='/res/jszy/student'/>/" + action + "?_method=" + method;
			var formData = $("#reqParam").val();
...
<body>
	<form name="appForm" action="${ctxPath}/res/jszy/student/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			...
			<option value="showDiscipleMain" method="post"   needDataType="N" <c:if test="${param.reqCode=='showDiscipleMain'}">selected</c:if> >1.0.7 跟师文字提醒(showDiscipleMain)post</option>
					...
					<a href="<s:url value='/res/jszy/test'/>">测试工具</a>
		</div>
