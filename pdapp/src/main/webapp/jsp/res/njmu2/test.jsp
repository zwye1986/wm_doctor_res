...
<html>
<head>
<title>南医大实习生APP测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/njmu2'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/njmu2/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="doctorTrainingSpe" method="get" needDataType="N" <c:if test="${param.reqCode=='doctorTrainingSpe'}">selected</c:if> >3.1.12专业信息(doctorTrainingSpe)get</option>
						<option value="sessionNumber" method="get" needDataType="N" <c:if test="${param.reqCode=='sessionNumber'}">selected</c:if> >3.1.13学生届别(sessionNumber)get</option>
						<option value="docCategory" method="get" needDataType="N" <c:if test="${param.reqCode=='docCategory'}">selected</c:if> >3.1.14人员类型(docCategory)get</option>
					</select>
					<input type="button" value="测试" class="search" onclick="test();">
					<a href="<s:url value='/res/njmu2/student/test'/>">学生端测试工具</a>
					<a href="<s:url value='/res/njmu2/teacher/test'/>">老师端测试工具</a>
					<a href="<s:url value='/res/njmu2/hospital/test'/>">医院管理员测试工具</a>
					<a href="<s:url value='/res/njmu2/admin/test'/>">平台管理员测试工具</a>
		</div>
	...
	<div id="doctorTrainingSpe" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly"></textarea>
	</div>
	<div id="sessionNumber" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly"></textarea>
	</div>
	<div id="docCategory" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly"></textarea>
	</div>
</body>
</html>
