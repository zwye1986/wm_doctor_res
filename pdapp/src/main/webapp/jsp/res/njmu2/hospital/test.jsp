...
<html>
<head>
<title>南医大实习生APP医院管理员测试程序<%-- --http://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath} --%></title>
...
<script type="text/javascript">
	function test() {
		...
		var url = "<s:url value='/res/njmu2/hospital'/>/"+action+"?_method="+method;
...
<body>
	<form name="appForm" action="${ctxPath}/res/njmu2/hospital/remember" method="get">
	<div style="width: 60%; float: left">
		<div style="margin-top: 20px; margin-bottom: 20px; margin-left: 100px">
			医院管理员动作：	<select id="reqCode" name="reqCode" onchange="change(this.value);">
						...
						<option value="doctorSchDetail" method="get" needDataType="N" <c:if test="${param.reqCode=='docSchDept'}">selected</c:if> >3.3.2学生轮转详情(doctorSchDetail)get</option>
						<option value="trainDocList" method="get" needDataType="N" <c:if test="${param.reqCode=='trainDocList'}">selected</c:if> >3.3.3培训学员查询(trainDocList)get</option>
		...
		<div style="margin-left: 100px ;width: 800px;">
			...
			<br>
			startDate:<input name="startDate" type="text" value="${startDate}">
			endDate:<input name="endDate" type="text" value="${endDate}">
			<br>
	...
	<div id="docSchDept" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&searchData={"startDate":"2016-07-07","endDate":"2016-08-04","schDeptFlow":"","sessionNumber":""}</textarea>
	</div>
	...
	<div id="trainDocList" class="reqParam" style="display: none">
		<textarea style="width: 350px; height: 400px" readonly="readonly">userFlow=${userFlow}&pageIndex=1&pageSize=10&searchData={"doctorCategoryId":"","sessionNumber":"","trainingSpeId":"","userName":""}</textarea>
	</div>
