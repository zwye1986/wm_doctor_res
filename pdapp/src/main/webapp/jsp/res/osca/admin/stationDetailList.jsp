                ...
                "stations":[
                    ...
                    <c:forEach var="s" varStatus="s1" begin="1" end="${maxStationSize}" step="1">
                        ...
                        <c:if test="${empty station}">
                            <c:set var="examStatusId" value="--"></c:set>
                            <c:set var="examStatusName" value="--"></c:set>
                        ...
                        <c:if test="${not empty station}">
                            ...
                            <c:set var="score" value="${examScore.examScore}"></c:set>
                            <c:set var="allScore" value="${allScore+examScore.examScore}"></c:set>
                            <c:set var="examStatusId" value="${examScore.examStatusId}"></c:set>
                            <c:set var="examStatusName" value="${examScore.examStatusName}"></c:set>
                            <c:if test="${nowDate>data.examEndTime and (empty examScore.examStatusId or examScore.examStatusId ne 'Assessment')}">
                                <c:set var="examStatusId" value="NotAssessment"></c:set>
                                <c:set var="examStatusName" value="缺考"></c:set>
                            </c:if>
                            <c:if test="${nowDate<=data.examEndTime and (empty examScore.examStatusId)}">
                                <c:set var="examStatusId" value="StayAssessment"></c:set>
                                <c:set var="examStatusName" value="待考核"></c:set>
                            </c:if>
                        </c:if>
                        {
                            "id":${pdfn:toJsonString(s)},
                            ...
                            "examStatusId": ${pdfn:toJsonString(examStatusId)},
                            "examStatusName": ${pdfn:toJsonString(examStatusName)}
                        }
                        <c:if test="${not s1.last }">
                            ,
                        </c:if>
