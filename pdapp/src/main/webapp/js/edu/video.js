...
var timer;
var isFinish="";
function initPlayer(chapterFlow){
	var url = rootPath()+"/edu/course/chapterData?chapterFlow="+chapterFlow;
	jboxPost(url,null,function(data){
		if(data){
			...
			thePlayer =  jwplayer("myPlayer").setup({
				...
				image: rootPath()+"/js/jwplayer/player.jpg",
				controls: false,
		        file:data.chapterFile,
		        //title: '',
				//description: "",	
				controlbar: "none",  //控制条位置	//controlbar: "bottom",controlbar: "none"
				...
				events: {
				 onReady: function(event) {
					 this.play();
					 $("#video_length").html(data.chapterTime);//设置视频右下方总时长
					 },
