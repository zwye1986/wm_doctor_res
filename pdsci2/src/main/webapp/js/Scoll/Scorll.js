...
function Map() {  
 ...
 ;(function($) {
	...
	$.fn.Scorll= function(options,style){
		...
		$.fn.Scorll.methods.createElement(table_id);
		if(defaults.colums>0)
		{
			$.fn.Scorll.methods.getScollByColumNums(table_id);
		}else if(defaults.clearClass&&defaults.removeClass)
		{
			$.fn.Scorll.methods.getScollByTwoClass(table_id);
		}
		
	};
	$.fn.Scorll.idsMap=new Map();
	$.fn.Scorll.style={"width":"900px","height":"600px","margin-top":"10px","margin-bottom":"10px","overflow":"auto","margin-left":"30px"};
	$.fn.Scorll.defaults={
	...
	$.fn.Scorll.methods={
		...
		//复制与生成所有需要的表
		createElement:function(table_id)
		{
			//div默认参数
		...
		//通过指定固定的列数，来固定
		getScollByColumNums:function (table_id)
		{
			//
			var defaults=$.fn.Scorll.idsMap.get(table_id);
		...
		//通过class来指定固定的列 
		// table_id 表示表格的id，class1表示 数据表格中要被清除数据的td  class2表示表格中不需要固定的列 options 表示展示的宽与高
		getScollByTwoClass:function (table_id)
		{
			//
			var defaults=$.fn.Scorll.idsMap.get(table_id);
			//去除不需要的数据
			...
			var $ScendTable=$("#"+defaults.secondTableid);
			//去除不固定的表格信息
			$ScendTable.find("thead").find("th."+defaults.removeClass).remove();
			$ScendTable.find("tbody").find("td."+defaults.removeClass).remove();
			...
			$ThreeTable.find("tbody").remove();
			//移除表头不固定部分
			$ThreeTable.find("thead").find("th."+defaults.removeClass).remove();
			
			//最原始的数据
			$("#"+defaults.table_id).find("tbody").find("td."+defaults.clearClass).each(function(){
				$(this).html("");
			});
