...
(function($, document) {
    ...
    function checkForInvalid() {
        ...
        if (invalid) {
            ...
            if (!O.addInvalid) {
                ...
                $display.removeClass(pname + cinvalid).siblings(cp + cddback)
                    .removeClass(pname + cddback + cinvalid);
                $display.siblings('select, ' + cp + cvalue).find("option").attr("selected", false);
                var opt = new Option(invalid, invalid);
                $(opt).attr("selected", "selected");
                $display.siblings('select, ' + cp + cvalue)[0].options.add(opt);
            }
    ...
    $.fn[pname].defaults = {
        ...
        forbidInvalid: false,
        /**
         * 当手动输入时,如果下拉框中没有该选项，将没有的选项以append的形式加入到该下拉框中，同时默认是选中
         */
        addInvalid: false,
        /**
         * 当手动输入时,如果手动输入的文本和下拉框中文本匹配的话就选中该值 默认不选中
         */
        addSameInvalid: false,
        /**
