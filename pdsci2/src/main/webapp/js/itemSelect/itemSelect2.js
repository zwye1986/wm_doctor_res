(function($) {
    $.itemSelect = function(target, data, otherName,defaultItemId,checkData,left,width) {
		var left = $('#' + target).offset().left+((left==undefined||left=='')?0:left);
        var topPx = $('#' + target).offset().top + $('#' + target).outerHeight();
		var suggestContainerWidth = $('#' + target).innerWidth();
		var homeDefaulOption ={
			'color':'#000000',
			'background-color':'white',
			'width':(width==undefined||width=='')?300:width+'px',
			'position': 'absolute',
			'left':left+"px",
			'display':'none',
			'max-height': '200px',
		...
		var _initClick=function()
		{
			...
			$("#"+divhomeId).find(".itemDiv").click(function(e){
				e.stopPropagation();
			});
			$("#"+divhomeId).find(".itemDiv").on("mouseenter mouseleave",function(){
				$(this).toggleClass("on");
			});
			$(document).click(function(){
				$("#"+divhomeId).toggle(!!$("#"+divhomeId).find(".itemDiv.on").length);
			});
		};
