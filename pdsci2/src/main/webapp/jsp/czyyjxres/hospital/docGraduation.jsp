<script type="text/javascript">
    ...
    function searchDoctor() {
        jboxPostLoad("content", "<s:url value='/czyyjxres/hospital/graduationDoctor'/>?isQuery=Y", $("#searchForm").serialize(), true);
    }
    //导出
    function exportInfo() {
        var url = "<s:url value='/czyyjxres/hospital/exportGraduation'/>";
    ...
    function getInfo(userFlow, batchId) {
        jboxOpen("<s:url value='/czyyjxres/doctor/getsingupinfoaudit'/>?userFlow=" + userFlow + "&batchId=" + batchId, "人员信息", 1000, 550);
    }
    //结业
    ...
    function issueCertificate(resumeFlow) {
        var url = "<s:url value='/czyyjxres/hospital/issueCertificate'/>?resumeFlow=" + resumeFlow;
...
//                        $("#fileSrc").find("span").trigger("click");
    ...
    function printCertificate(resumeFlow,flag){
        ...
        if(flag =='print'){
            ...
            jboxOpen("<s:url value='/czyyjxres/hospital/certificateDate?resumeFlow='/>"+resumeFlow+"&isForeign='Y'", tip,500,300,true);
        }

        if(flag =='view'){
            jboxTip("下载中,请稍等....");
            var url = '<s:url value="/czyyjxres/doctor/englishCertificate?resumeFlow="/>'+resumeFlow;
            window.location.href = url;
        }
    ...
    function printInfo(resumeFlow,flag) {
        ...
        if(flag=='print'){
            ...
            jboxOpen("<s:url value='/czyyjxres/hospital/certificateDate?resumeFlow='/>"+resumeFlow+"&isForeign='N'", tip,500,300,true);
        }

        if(flag=='view'){
            jboxTip("下载中,请稍等...");
            var url = '<s:url value="/czyyjxres/doctor/showCertificate?resumeFlow="/>' + resumeFlow;
            window.location.href = url;
        }
        ...
        <form id="searchForm">
            ...
            <table>
                <tr>
                    ...
                    <td class="td_left"><input type="text" name="userName" value="${param.userName}" class="input" style="width: 120px"/></td>
                    ...
                    <td>
                            <c:if test="${user.sysUser.isForeign eq 'N'}">
                                <c:if test="${not empty stuFileMap[user.resumeFlow]}">
                                    <a onclick="showFile('${stuFileMap[user.resumeFlow].fileFlow}');" target="_blank">[查看进修鉴定表]</a>
                                </c:if>
                            </c:if>
                            ...
                            <c:if test="${user.stuStatusId eq stuStatusEnumAdmited.id or (user.stuStatusId eq stuStatusEnumDelayGraduation.id ) and flag1 eq GlobalConstant.FLAG_Y}">
                                ...
                                <%--<a href="<s:url value='/czyyjxres/doctor/showCertificate?flag=view&resumeFlow=${user.resumeFlow}'/>"--%>
