...
<script type="text/javascript">
    ...
    function toPage(page) {
        ...
        jboxPostLoad("content","<s:url value='/czyyjxres/hospital/furtherStudyQuery'/>",$("#searchForm").serialize(),true);
    }
    //编辑
    function edit(flag,recordFlow){
        jboxOpen("<s:url value='/czyyjxres/hospital/showEdit'/>?recordFlow="+recordFlow, flag,800,600,true);
    }
    //删除
    function del(recordFlow){
        jboxConfirm("确定刪除吗？",function(){
            jboxPost("<s:url value='/czyyjxres/hospital/delStudyRegist'/>",{"recordFlow":recordFlow},function(resp){
                furtherStudyQuery();
            },null,true);
        },null);
    }
    function searchSuggest(resp){
        var url = "<s:url value='/czyyjxres/hospital/searchSuggest'/>";
        var searchData = "";
        if(resp == "deptName"){
