...
function openWindow(absenceFlow){
        ...
        var url = "<s:url value='/res/czAbsence/editAbsence'/>?resRoleScope=${GlobalConstant.RES_ROLE_SCOPE_DOCTOR}&absenceFlow=" + absenceFlow;
...
function delAbsence(absenceFlow){
	jboxConfirm("确认删除请假? " ,  function(){
		jboxStartLoading();
		var url = "<s:url value='/res/czAbsence/delAbsence'/>?absenceFlow=" + absenceFlow;
		jboxPost(url, null,
			function(resp){
...
function repealAbsence(absenceFlow){
	jboxConfirm("确认销假?",  function(){
		jboxStartLoading();
		var url = "<s:url value='/res/czAbsence/repealAbsence'/>?absenceFlow=" + absenceFlow;
		jboxPost(url, null,
			function(resp){
...
<body>
	<div class="mainright">
		<div class="content">
			...
			<form id="searchForm" action="<s:url value="/res/czAbsence/absenceList"/>" method="post">
