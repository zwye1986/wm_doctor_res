...
//            $(".submit_btn").hide();
    ...
    function uploadFile(type, typeName) {
        jboxOpen("<s:url value='/inx/czyyjxrecruit/uploadFile'/>?operType=" + type, "Upload "+ typeName, 450, 150);
    }
    /**
     * 删除证书及文件
     */
...
//                jboxTip("You can add only 3 at most!") ;
    ...
    function search(stuBatId,flag){
        ...
        var url = "<s:url value='/czyyjxres/doctor/singup_en?batchFlow='/>"+stuBatId+"&flag="+flag;
...
//        saveInfo();
        ...
        jboxConfirm("Confirm the submission? Application information cannot be modified after submission!" , function(){
            jboxStartLoading();
            jboxPost("<s:url value='/czyyjxres/doctor/saveSingupInfo?flag=true&stuBatId='/>"+stuBatId,null,function(resp){
                if(resp=="1"){
                    jboxTip("Save successfully!");
                    search(stuBatId);
                    //自动跳转到主页（仅第一次点击提交）
                    location.href="/pdsci/czyyjxres/doctor/index_en";
                }else{
                    jboxTip(resp);
                }
            },null,false);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <%--  <select name="user.nationalityId" class="select validate[required]" style="width: 157px;margin-left: 0px;">
                                                    <option value=""></option>
                                                    <c:forEach items="${dictTypeEnumCzyyjxNationalList}" var="dict">
