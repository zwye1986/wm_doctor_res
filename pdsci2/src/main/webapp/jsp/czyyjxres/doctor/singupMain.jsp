...
//            }
    ...
    function search(stuBatId){
        ...
        var url = "<s:url value='/czyyjxres/doctor/singup?batchFlow='/>"+stuBatId;
...
//        $('#registerFormUri').removeClass("validate[required]");
        ...
        jboxPost("<s:url value='/czyyjxres/doctor/valideExist'/>",{"batchFlow":batchFlow},function(resp){
            if(null != resp && resp != ""){
                jboxTip("下载中,请稍等...");
                var url = '<s:url value="/czyyjxres/hospital/printApplForm?resumeFlow="/>'+resp+"&printFlag=doctor"+"&templeteName="+encodeURI(encodeURI(templeteName));
    ...
    function saveInfo(formId){
       ...
       $("#titleName").val(titleName);
        ...
        jboxEndLoading();
        jboxPost("<s:url value='/czyyjxres/doctor/saveSingupInfo?stuBatId='/>"+stuBatId,$("#"+formId).serialize(),function(resp){
            var data = eval("("+resp+")");
    ...
    function submiteApply(){
        ...
        jboxConfirm("确认提交吗？提交后申请信息不可修改！" , function(){
            jboxStartLoading();
            jboxPost("<s:url value='/czyyjxres/doctor/saveSingupInfo?flag=true&stuBatId='/>"+stuBatId+"&registerFormUriValue="+registerFormUriValue,null,function(resp){
                if(resp=="1"){
                    jboxTip("保存成功");
                    search(stuBatId);
                    //自动跳转到主页（仅第一次点击提交）
                    location.href="/pdsci/czyyjxres/doctor/index";
    ...
    function loadTitle(){
        ...
        var titleType = 'CzyyjxUserTitle.'+$("#titleTypeId").val();
        if (titleType) {
            var url = "<s:url value='/czyyjxres/doctor/loadTitle'/>?titleTypeId=" + titleType;
//            jboxStartLoading();
    ...
    function reApply(){
        var stuBatId = $("#batchFlow").find("option:selected").val()
        jboxPost("<s:url value='/czyyjxres/doctor/saveSingupInfo?flag=reApply&stuBatId='/>"+stuBatId,null,function(resp){
            if(resp=="1"){
                jboxTip("操作成功");
                search(stuBatId);
            }else{
                        ...
                        <div id="infoDiv2" class="infoDiv search_table doctorInfo" style="margin-top:20px;display: none;" >
                            ...
                            <form id="socialForm"  style="position:relative;" >
                                ...
                                <table style="width: 100%;" class="searchTable" >
                                    ...
                                    <tr>
                                        <td colspan="5" style="height: 10px"><font style="color: red;font-size: 10px">（请下载<a href="<s:url value='/jsp/czyyjxres/print/agreementTemeplete.doc'/>">遵守纪律协议书模板</a>，再上传审核盖章的协议书扫描件并以PDF格式上传。）</font></td>
                                    </tr>
                                    <tr>
