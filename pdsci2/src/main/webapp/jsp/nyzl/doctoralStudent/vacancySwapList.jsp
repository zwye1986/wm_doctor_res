...
<head>
    ...
    <script type="text/javascript">
        ...
        function leadTo(){
            jboxOpen("<s:url value='/nyzl/vacancySwap/leadTo'/>?stuSignFlag=${stuSignFlag}","导入",500,150);
        }

        function exportVacancySwap(){
            if(false==$("#recSearchForm").validationEngine("validate")){
                return;
            }
            var url = "<s:url value='/nyzl/vacancySwap/exportVacancySwap?stuSignFlag=${stuSignFlag}'/>";
            jboxTip("导出中…………");
            jboxSubmit($("#recSearchForm"), url, null, null, false);
            jboxEndLoading();
        }

        function editRecord(orgFlow,recordFlow){
            var adminFlag='${adminFlag}';
            var recruitYear=$("#"+orgFlow+"TrData").find("td[id='recruitYear']").html();
            var tkAcademicNum=$("#"+orgFlow+"TrData").find("input[id='tkAcademicNum']").val();
            var tkSpecialNum=$("#"+orgFlow+"TrData").find("input[id='tkSpecialNum']").val();
            var tmsAcademicNum=$("#"+orgFlow+"TrData").find("input[id='tmsAcademicNum']").val();
            var tmsSpecialNum=$("#"+orgFlow+"TrData").find("input[id='tmsSpecialNum']").val();
            var data = {
...
<div class="mainright">
    <div class="content">
        <form id="recSearchForm" method="post" action="<s:url value='/nyzl/vacancySwap/vacancySwapList?stuSignFlag=${stuSignFlag}&adminFlag=${adminFlag}'/>">
            ...
            <table class="basicData" style="width: 100%;">
                ...
                <c:forEach items="${recordList}" var="record">
                    <tr>
                        ...
                        <td>${record.fullNum}</td>
                        <td>${record.vacancyNum}</td>
                        <td>${record.basciConditions}</td>
