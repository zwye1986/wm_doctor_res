...
function reloadVerifyCode() {
	jboxPost("<s:url value='/sys/user/edit/captchaEmail'/>?userFlow=${user.userFlow}"+"&userEmail="+$("#newEmail").val(),null,function(resp){
...
function step2(){
	...
	jboxPost("<s:url value='/sys/user/edit/checkNewEmail'/>?userEmail="+newEmail,null,function(resp){
		if(resp=="${GlobalConstant.OPRE_SUCCESSED}"){
			$("#step3Div").show();
...
function step3(){
	...
	jboxPost("<s:url value='/sys/user/edit/editUserEmail'/>?userFlow=${user.userFlow}"
		...
		<table border="0" cellspacing="0" cellpadding="0" style="line-height: 50px;font-size: 15px;">
				<tr>
					...
					<td style="text-align: left;">
						<input type="text" class="validate[required,custom[email]] xltext" id="newEmail" value="">&#12288;此邮箱可作为登录邮箱，请选择常用邮箱，避免遗忘
					</td>
				</tr>
				<tr>
