...
<script type="text/javascript">
	function saveAllot() {
		...
		var url = '<s:url value="/sys/user/saveAllot"/>';
...
<form id="allotForm">
				<table width="800" cellpadding="0" cellspacing="0" class="basic">
					...
					<c:if test="${!(sessionScope.currWsId==GlobalConstant.GCP_WS_ID and sysRole.roleFlow == pdfn:getSysCfg(GlobalConstant.DECLARER_ROLE_FLOW))}">
					<tr>
						<th></th>
						<td>
							...
							<c:if test="${sessionScope.userListScope==GlobalConstant.USER_LIST_LOCAL or sessionScope.userListScope==GlobalConstant.USER_LIST_CHARGE}">
								<%-- 如果循环的角色当前用户具有，显示 --%>
								<%--  如果循环的角色父角色当前用户具有，显示--%>
