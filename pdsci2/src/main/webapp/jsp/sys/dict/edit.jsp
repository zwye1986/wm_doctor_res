			...
			<div class="tagContent selectTag" id="tagContent0">
				<table style="width: 400px" cellpadding="0" cellspacing="0" class="basic">
						...
						<td width="200px">
							<select class="validate[required] xlselect" name="dictTypeId">
								...
								<c:forEach var="dictTypeEnum" items="${dictTypeEnumList}">
									...
									<option value="${dictTypeEnum.id}" <c:if test="${dictTypeEnum.id==dict.dictTypeId or dictTypeEnum.id==param.dictTypeId}">selected="selected"</c:if>>${dictTypeEnum.name}</option>
									</c:if>
									</c:forEach>
							</select>
						</td>
					...
					<tr>
						<th>代码描述：</th>
						<td
							><input class="xltext validate[maxSize[200]]" name="dictDesc" type="text"
