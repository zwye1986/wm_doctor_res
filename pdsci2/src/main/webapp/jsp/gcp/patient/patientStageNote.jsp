...
function offPatient() {
	jboxConfirm("确认该受试者脱落？",  function() {
		jboxPost( "<s:url value='/gcp/researcher/changePatientStage?patientFlow=${patient.patientFlow}'/>&patientStageId=${patientStageEnumOff.id}",$('#saveForm').serialize(),function(resp){
...
<div class="mainright">
	...
	<form id="saveForm">
		<table class="xllist nofix">
				...
				<tr>
					<td>
					<textarea name="patientStageNote" rows="6" placeholder="请填写脱落原因" style="width: 99.5%" class="text-input validate[required]">${patient.patientStageNote }</textarea>
					</td>
				</tr>
			</tbody>
		</table>
