...
<script type="text/javascript">
	...
	function goDeptFile(deptFlow){
		window.location.href="<s:url value='/gcp/fin/deptList'/>?type=fund&deptFlow="+deptFlow;
	}
	...
	function toDeptProjList(deptFlow){
		window.location.href="<s:url value='/gcp/fin/projFundList'/>?applyDeptFlow="+deptFlow+"&deptFlag=y";
	}
</script>
</head>
<body>
				...
				<c:if test="${status.index % 5 eq 0}">
					<tr>
						<c:forEach items="${deptList }" var="dept" begin="${status.index}" end="${status.index+4}">
							<td>
								<table onmouseover="this.className='dept_over'" onmouseout="this.className='dept_out'" onclick="toDeptProjList('${dept.deptFlow}')">
									<tr>
