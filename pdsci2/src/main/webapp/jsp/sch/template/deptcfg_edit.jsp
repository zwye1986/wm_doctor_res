...
// 		});
		...
		function groupInfoView(){
			...
			if(!isRequired && isFree){
				...
				$("[id^='schMaxMonth']").addClass("validate[custom[integer],max[99],min[1]]");
				$("[id^='schMaxMonth']").removeClass("validate[custom[integer],max[99],min[1]]");
				</c:if>
				<c:if test="${!isWeek}">
				...
				$("[id^='schMaxMonth']").addClass("validate[custom[number],max[99],min[0.5]]");
				$("[id^='schMaxMonth']").removeClass("validate[custom[number],max[99],min[0.5]]");
				</c:if>
			}else{
				<c:if test="${isWeek}">
...
// 			$("#schMonth").val("");
				...
				var schMaxMonthG = 0;
				$(".data [id^=schMaxMonth]").each(function(){
					var val = this.value;
					if($.trim(val)!="" && !isNaN(val)){
						schMaxMonthG+=(parseFloat(val));
					}
				});
				debugger
				$("#schMaxMonth").val(schMaxMonthG);
			}else{
			...
			selDepts.each(function(){
				...
				var obj = {
					...
					"schMaxMonth":$("#schMaxMonth"+regValue).val(),
...
<%--								alert(${dictTypeEnumStandardDeptList.size()});--%>
			...
			<div id="dept" style="width:65%;margin-bottom: 10px;float: left;overflow: auto;height: 500px;" class="side">
				<table width="100%" class="basic" style="font-size: 14px">
					...
					<tr>
						...
						<th style="text-align: center; width: 150px;padding: 0;">轮转最小时间</th>
						<th style="text-align: center; width: 150px;padding: 0;">轮转最大时间</th>
					...
					<c:forEach items="${dictTypeEnumStandardDeptList}" var="dict">
						...
						<tr class="data ${dict.dictId}">
							...
							<td style="text-align: center">
								<input type="text" value="${rotationDeptMap[dict.dictId].schMaxMonth}" id="schMaxMonth${dict.dictId}" style="width: 25px;text-align: center;" onchange="valueCheck(this);"/>
								<font class="schReq">默认最小值</font>
							</td>
							<td>
						...
						<c:forEach items="${applicationScope[dictKey]}" var="sDict">
							...
							<tr class="data ${sDictId}">
								...
								<td style="text-align: center">
									<input type="text" value="${rotationDeptMap[sDictId].schMaxMonth}" id="schMaxMonth${sDictId}" style="width: 25px;text-align: center;" onchange="valueCheck(this);"/>
								</td>
								<td>
							...
							<c:forEach items="${applicationScope[dictKey]}" var="tDict">
								...
								<tr class="data ${tDictId}">
									...
									<td style="text-align: center">
										<input type="text" value="${rotationDeptMap[tDictId].schMaxMonth}" id="schMaxMonth${tDictId}" style="width: 25px;text-align: center;" onchange="valueCheck(this);"/>
									</td>
									<td>
					...
					<tr class="groupInfo">
						...
						<td colspan="2">
							...
							~
							<input id="schMaxMonth" type="text" class="validate[required,custom[number]]" value="${rotationGroup.schMaxMonth}" style="width: 30px"/>
						</td>
					</tr>
					<tr class="groupInfo">
