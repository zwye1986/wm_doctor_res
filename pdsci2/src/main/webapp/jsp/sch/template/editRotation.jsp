	...
	<div class="title1 clearfix" >
		...
		<table class="basic" style="width: 100%;">
			...
			<tbody>
					...
					<td>
						...
						<div style="display: none;">
							<option 
								...
								${recDocCategoryEnumAssistantGeneral.id} |
								...
								${recDocCategoryEnumGeneralPractitioner.id} |
								${recDocCategoryEnumAssistantGeneralDoc.id} |
								${recDocCategoryEnumGPractitionerTransf.id} |
								${recDocCategoryEnumGeneralPractitionerStation.id} |
								${recDocCategoryEnumPediatricianTransfe.id} |
								${recDocCategoryEnumObstetricianTransfer.id} |
								" value="">请选择</option>
							<c:forEach items="${dictTypeEnumDoctorTrainingSpeList}" var="dict">
								...
								${recDocCategoryEnumAssistantGeneralDoc.id} |
								${recDocCategoryEnumGPractitionerTransf.id} |
								${recDocCategoryEnumGPractitionerStat.id} |
								${recDocCategoryEnumPediatricianTransf.id} |
								${recDocCategoryEnumObstetricianTransf.id} |
							...
							<c:forEach items="${dictTypeEnumSecondTrainingSpeList}" var="dict">
								<option class="${recDocCategoryEnumChineseMedicine.id}  |  ${recDocCategoryEnumTCMGeneral.id}" value="${dict.dictId}" ${rotation.speId eq dict.dictId?'selected':''}>${dict.dictName}(二级专业)</option>
							</c:forEach>
						</div>
						</c:if>
						<c:if test="${GlobalConstant.FLAG_Y eq rotation.publishFlag}">
				...
				<tr id="work" style="${rotation.doctorCategoryId ne recDocCategoryEnumIntern.id ? 'display: none;':''}">
					<th><font color="red" >*</font>派送院校：</th>
					<td>
						<c:if test="${!(GlobalConstant.FLAG_Y eq rotation.publishFlag)}">
							<select name="workOrgId" class="validate[required]"  id="workOrgId" >
								<option value=""></option>
								<c:forEach items="${dictTypeEnumSendSchoolList}" var="dict">
									<option value="${dict.dictId}" ${rotation.workOrgId eq dict.dictId?'selected="selected"':''}>${dict.dictName}</option>
								</c:forEach>
							</select>
						</c:if>
						<c:if test="${GlobalConstant.FLAG_Y eq rotation.publishFlag}">
							<font style="font-weight: bold;">&#12288;${rotation.workOrgName}</font>
							<input type="hidden" name="workOrgId" value="${rotation.workOrgId}"/>
						</c:if>
					</td>
					<th></th>
					<td>
				...
				<tr>
					<th><font color="red" >*</font>要求数是否翻倍：</th>
					<td id="">
						<label>
							<input type="radio" name="isDouble" value="${GlobalConstant.FLAG_Y}" <c:if test="${rotation.isDouble eq GlobalConstant.FLAG_Y}">checked</c:if>  class="validate[required]"/>
							是
						</label>
						&#12288;
						<label>
							<input type="radio" name="isDouble" value="${GlobalConstant.FLAG_N}" <c:if test="${rotation.isDouble eq GlobalConstant.FLAG_N}">checked</c:if>  class="validate[required]"/>
							否
						</label>
					</td>
					<td colspan="2">
						翻倍说明：<br>
						&#12288;&#12288;如果选择“是”，则每一个科室的要求数=轮转月数/标准月数*要求数。<br>
						&#12288;&#12288;如果选择“否”，则要求数不变。
					</td>
				</tr>
				<tr>
