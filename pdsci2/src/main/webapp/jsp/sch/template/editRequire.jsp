	...
	<head>
		...
		<script type="text/javascript">
			...
			<c:if test="${typeId eq jszyTCMPracticEnumBasicPractice.id}">
				var itemMap = {
					<c:forEach items="${practicRegistryTypeEnumList}" var="reg">
						${reg.id}:
						"${reg.haveItem}",
					</c:forEach>
				}
			...
			<c:if test="${typeId eq jszyTCMPracticEnumTheoreticalStudy.id}">
				var itemMap = {
					<c:forEach items="${theoreticalRegistryTypeEnumList}" var="reg">
						${reg.id}:
						"${reg.haveItem}",
					</c:forEach>
				}
				...
				<tr>
					...
					<td>
						<select name="recTypeId" class="xlselect validate[required]" style="margin-right: 0px" onchange="checkReqType();">
							...
							&& pdfn:findChineseOrWestern(rotation.rotationTypeId,regType.id)}">
