...
<%-- 		</jsp:include> --%>
		...
		<script type="text/javascript">
			function loadRequire(recTypeId,selTab){
				jboxLoad("tagContent","<s:url value='/sch/template/requireList'/>?roleFlag=${param.roleFlag}&typeId=${typeId}&relRecordFlow=${rotationDept.recordFlow}&recTypeId="+recTypeId,true);
				if(selTab){
					$("#"+recTypeId).click();
				}
			}
...
// 				});
			...
			function editDeptReq(reqFlow,typeId){
				var title = "新增";
				if(reqFlow!=null && reqFlow!=''){
					title = "编辑";
				}
				jboxOpen("<s:url value='/sch/template/editDeptReq'/>?rotationFlow=${rotation.rotationFlow}&relRecordFlow=${rotationDept.recordFlow}&reqFlow="+reqFlow+"&recTypeId="+$(".selectTag").prop("id")+"&typeId="+typeId,title+"轮转要求",500,350);
			}

			function impDeptReq(typeId){
				jboxOpen("<s:url value='/sch/template/importDeptReq'/>?rotationFlow=${rotation.rotationFlow}&relRecordFlow=${rotationDept.recordFlow}&typeId="+typeId,"导入",500,350);
			}
				...
				<tr>
					<td style="text-align: left;padding-left: 10px;">
						<select style="height: 27px;width: 400px;" onchange="selRotationDept(this.value);">
							<%--<c:forEach begin="1" end="3" var="step">--%>
								<c:forEach items="${rotationDeptList}" var="rotationDeptTemp">
									<%--<c:if test="${groupMap[rotationDeptTemp.groupFlow].ordinal eq step}">--%>
										<option
...
<!-- 						<input type="button" value="同步要求" class="search" onclick="synch();"/> -->
			...
			<div style="float: left;width: 100%;">
				<c:if test="${sysCfgMap['sys_index_url'] eq '/inx/shangHaiRuiJing'}">
					<input type="button" value="导&#12288;入" class="search" onclick="impDeptReq('${typeId}');" style="float: right;margin-right: 10px;"/>
				</c:if>
				<input id="reqAddBtn" type="button" value="新&#12288;增" class="search" onclick="editDeptReq('','${typeId}');" style="float: right;margin-right: 10px;"/>
				<ul id="tags" style="width: 90%;">
