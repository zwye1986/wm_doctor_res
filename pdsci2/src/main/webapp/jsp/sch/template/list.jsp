...
<head>
	...
	<script type="text/javascript">
		...
		function search(){
			...
			$("#rotationNameExp").val(rotationName);
			console.log($("#rotationNameExp").val());
			$("#rotationForm").submit();
		}
		...
		function viewForm(form,type,typeName){
			if(form){
				jboxOpen("<s:url value='/res/rec/viewForm'/>?type=open&recTypeId="+type+"&formId="+form+"&viewFlag=Y",typeName,1000,550);
			}else{
		...
		function gToLocation(orgFlow){
			jboxGet("<s:url value='/sch/template/gToLocalList'/>?orgFlow="+orgFlow,null,function(){
				checkRotationSetStatus(orgFlow);
			},null,false);
		}
		</c:if>

		//检测方案调整状态
		function checkRotationSetStatus(orgFlow){
			orgFlow = orgFlow || "${sessionScope.currUser.orgFlow}";
			if(orgFlow){
				$(".setStatus").each(function(){
					...
					jboxGet("<s:url value='/sch/template/getUnrelCount'/>?orgFlow="+orgFlow+"&rotationFlow="+rotationFlow,null,function(resp){
						if(resp>0){
							$this.html("需调整<font color='red'>"+resp+"</font>处");
						}else{
							$this.text("已完成");
						}
					},null,false);
		...
		function guanLian(flow){
			jboxOpen("<s:url value='/sch/template/guanLian'/>?flow="+flow,"关联基地",850,500);
		}
...
<div class="mainright">
	<div class="content">
		<div class="title1 ">
			<form id="form"></form>
			<form id="rotationForm" action="<s:url value='/sch/template/list'/>" method="post">
				<input type="hidden" name="roleFlag" value="${param.roleFlag}"/>
				<input id="rotationNameExp" type="hidden" name="rotationName" value="">
				...
				<div class="queryDiv">
					...
					<div class="inputDiv">
						...
						<input id="rotationName" type="text" class="qtext" value="${rotationName}"/>
					</div>
					<div class="inputDiv">
					...
					<c:if test="${sysCfgMap['res_sch_type'] eq 'Intern'}">
						<div class="inputDiv">
							<label class="qlable">派送院校：</label>
							<select name="workOrgId" id="workOrgId" class="qselect">
								<option value=""></option>
								<c:forEach items="${dictTypeEnumSendSchoolList}" var="dict">
									<option value="${dict.dictId}" ${schRotation.workOrgId eq dict.dictId?'selected="selected"':''}>${dict.dictName}</option>
								</c:forEach>
							</select>
						</div>
					</c:if>
		...
		<div id="toOutConfirm" style="display: none;" >
			<span style="padding-left: 100px;">
				<label>
					...
					<input type="checkbox" name="standardDoc" checked="checked" value="${dictTypeEnumStandardDept.id}" style="display: none;"/>
				</label>
			</span>
			<span style="padding-left: 100px;"><label><input type="checkbox" name="standardDoc" value="${dictTypeEnumDoctorTrainingSpe.id}"/>是否导出培训专业</label></span>
			<span style="padding-left: 100px;"><input type="button" value="导&#12288;出" onclick="top.document.mainIframe.schExports($(':checkbox:checked'));"  class="search"></span>
		</div>
		...
		<table class="xllist rotationData" style="width: 100%;">
			<tr>
				...
				<c:if test="${sysCfgMap['res_sch_type'] eq 'Intern'}">
					<th width="10%" style="text-align: center">派送院校</th>
				</c:if>
			...
			<c:forEach items="${rotationList}" var="rotation" varStatus="seq">
				<tr class="body _${rotation.speId}">
					...
					<c:if test="${!(GlobalConstant.USER_LIST_GLOBAL eq param.roleFlag)}">
						<td class="setStatus" rotationFlow="${rotation.rotationFlow}">
							<c:if test="${rotationOrgUnrelMap[rotation.rotationFlow]>0}">
								需调整<font color='red'>${rotationOrgUnrelMap[rotation.rotationFlow]}</font>处
							</c:if>
							<c:if test="${rotationOrgUnrelMap[rotation.rotationFlow]<=0}">
								已完成
							</c:if>
					...
					<c:if test="${sessionScope.currUser.userFlow eq GlobalConstant.ROOT_USER_FLOW}">
						<td style="display: none;">
							...
							<form id="saveCfgForm${rotation.rotationFlow}">
								...
								<select id="${rotation.rotationFlow}Form" name="${key}" onchange="save('${rotation.rotationFlow}');">
									<option></option>
							...
							<a style="color: blue;cursor: pointer;" id="${rotation.rotationFlow}EditDeptForm" onclick="settingDeptForm('${rotation.rotationFlow}');">编辑科室表单</a>
						</td>
						<td style="display: none;">
							<c:forEach items="${preRecTypeEnumList}" var="eu">
								<c:set var="key" value="res_${eu.id}_form_flag"/>
								<c:if test="${(GlobalConstant.FLAG_Y eq sysCfgMap[key])&&(!(eu.id eq 'InProcessInfo'))}">
