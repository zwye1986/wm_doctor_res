...
<div class="mainright">
    <div class="content">
        <div class="title1 clearfix">
            ...
            <table class="basic" style="width:100%;margin-bottom: 10px;border: none;">
                <tr>
                    <td style="border: none;">
                        ...
                        &#12288;Tip:加<font color="red">*</font>表示为外院科室
                    </td>
                </tr>
            </table>
                ...
                <table class="basic" style="width: 100%">
                    <tr>
                        <th style="text-align: left;padding-left: 20px;">
                            医院科室
                            <a style="float: right; padding-right: 10px;" href="javascript:add('${orgFlow}')"><img
                                    title="新增" src="<s:url value='/css/skin/${skinPath}/images/add.png'/>"></a>
                        </th>
                    </tr>
                    <c:forEach items="${deptList}" var="dept">

                        <tr class="viewTd TR_${dept.deptFlow}" deptFlow="${dept.deptFlow}" <c:if
                                test="${schDeptCountMap[dept.deptFlow].size()+0 == 0}"> needMap</c:if>>
                            <td valign="top" style="text-align: left;padding-left: 20px;">
                                <span onclick="viewDept('${dept.deptFlow}');" style="cursor: pointer;">${dept.deptName}（<font
                                        id="${dept.deptFlow}">${schDeptCountMap[dept.deptFlow].size()+0}</font>）</span>
                                ...
                                <c:if test="${GlobalConstant.RECORD_STATUS_Y eq dept.recordStatus}">
                                    ...
                                    <a style="float: right;color: blue;cursor: pointer;"
                                       onclick="edit('${dept.deptFlow}');">编辑</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:forEach>
                    ...
                    <table class="xllist basic">
                        ...
                        <tr>
                            <th colspan="7" style="text-align: left;padding-left: 10px;">
                                <span id="showAllBtn" onclick="viewDept('');"
                                      style="cursor: pointer;font-weight: normal;color: blue;">显示全部</span>
                                <a style="float: right; padding-right: 10px;margin-top: 7px;"
                                   href="javascript:addDept()"><img title="新增"
                                                                    src="<s:url value='/css/skin/${skinPath}/images/add.png'/>"></a>
                                <font style="float: right;margin-right: 10px;margin-top: 5px;">总轮转科室数：<font
                                        id="schDeptCountT"></font></font>
                                <input style="float: right;margin-right: 10px;" type="button" class="search"
                                       value="导&#12288;出" onclick="exportDept();"/>
                            </th>
                        </tr>
                        <tr>
                        ...
                        <c:forEach items="${deptList}" var="dept">
                            ...
                            <c:forEach items="${schDeptCountMap[dept.deptFlow]}" var="schDept">
                                <tr
                                    ...
                                    <td style="text-align: center">${schDept.schDeptName}</td>
                                    <td style="text-align: center">
                                        <c:forEach items="${deptRelMap[schDept.schDeptFlow]}" var="deptRel"
                                                   varStatus="status">
                                    ...
                                    <td style="text-align: center">
                                        <c:if test="${schDept.recordStatus == GlobalConstant.RECORD_STATUS_Y }">
                                            <c:if test="${empty schStandarMap[schDept.schDeptFlow]}"><a
                                                    style="color: blue;"
                                                    href="javascript:modifyCfg('${schDept.schDeptFlow}','');"
                                                    class="btn">未关联</a></c:if>
                                            <c:if test="${not empty schStandarMap[schDept.schDeptFlow]}"><a
                                                    href="javascript:modifyCfg('${schDept.schDeptFlow}','${schStandarMap[schDept.schDeptFlow].recordFlow}');"
                                                    class="btn">已关联（${schStandarMap[schDept.schDeptFlow].standardDeptName}）</a></c:if>
                                        </c:if>
                                    ...
                                    <td style="text-align: center">${schDept.deptNum}</td>
                                    <td style="text-align: center">
                                    ...
                                    <td style="text-align: center;">
                                        <c:if test="${GlobalConstant.RECORD_STATUS_Y eq schDept.recordStatus}">
                                            ...
                                            <c:if test="${schDept.external eq GlobalConstant.FLAG_Y and (sysCfgMap['res_sch_dept_external'] eq GlobalConstant.FLAG_Y)}">
                                                <a href="javascript:setExternalDept('${schDept.schDeptFlow}')"
                                                   style="color: blue">设置</a>
                                                &nbsp;&nbsp;<br/>
                                            </c:if>
                                        </c:if>
                                        <a href="javascript:delDept('${schDept.schDeptFlow}','${GlobalConstant.RECORD_STATUS_Y eq schDept.recordStatus?GlobalConstant.RECORD_STATUS_N:GlobalConstant.RECORD_STATUS_Y}')"
                                           style="color: blue">
                                            <c:if test="${GlobalConstant.RECORD_STATUS_Y eq schDept.recordStatus}">
                                                停用
                                            </c:if>
                        ...
                        <tr id="recordStatus" style="<c:if test="${!empty schDeptCountMap}">display: none;</c:if>">
                            <td align="center" colspan="7">暂无轮转科室</td>
                        </tr>
                    </table>
