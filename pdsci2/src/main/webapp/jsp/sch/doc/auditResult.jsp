			...
			function(resp){
				...
				if(resultList){
					for(var index in resultList){
						...
						docInfo = docInfo.htmlFormart(resultList[index].doctorName,resultList[index].schStartDate==null?'':resultList[index].schStartDate,resultList[index].schEndDate==null?'':resultList[index].schEndDate,resultList[index].doctorFlow,schDeptFlow,month,display,doctor.schStatusId,resultList[index].resultFlow);
		...
		jboxPost("<s:url value='/sch/template/doctorResultList'/>",{doctorFlow:doctorFlow},function(resp){
			...
			if(resultList){
				...
				for(var index in resultList){
					...
					docInfo = docInfo.htmlFormart(resultList[index].schDeptName,resultList[index].schStartDate==null?'':resultList[index].schStartDate,resultList[index].schEndDate==null?'':resultList[index].schEndDate);
					var styleStr = "";
	...
	<div class="content">
		<div class="title1 clearfix">
			<form action="<s:url value='/sch/arrange/auditResult'/>" id="doctorSearchForm" method="post">
				<div class="queryDiv" style="min-width: 850px;max-width: 850px;">
					<div class="inputDiv">
					...
					<div class="inputDiv" style="min-width: 315px;max-width: 315px;">
						<label class="qlable">审核区间：</label>
						<input type="text" id="startDate" name="schStartDate" value="${ param.schStartDate}" style="width: 90px;"
							   readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"  class="qtext"/>
						~
						<input type="text" id="endDate" name="schEndDate" value="${param.schEndDate}" style="width: 90px;"
							   readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"  class="qtext"/>
					</div>
					<div class="inputDiv">
					...
					<div class="inputDiv">
						...
						<input name="doctorName" type="text" value="${doctor.doctorName}" class="qtext"/>
					</div>
					<div class="lastDiv" style="max-width: 200px;min-width: 200px;">
						<input type="button" value="查&#12288;询" class="searchInput" onclick="search();"/>
						<input type="button" value="批量审核" class="searchInput" style="margin-left: 10px;" onclick="auditComplate();"/>
					</div>
				</div>
			</form>
		</div>
