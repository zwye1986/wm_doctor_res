...
function queryProcess(obj)
{
	...
	var url = "<s:url value='/jsres/temp/queryProcess'/>?userCode="+userCode;
...
function delProcessType(obj)
{
	...
	var url = "<s:url value='/jsres/temp/delProcessType'/>?processFlow="+processFlow+"&recTypeId="+recTypeId;

	jboxConfirm("确认退回出科表单？",function() {
		postAction(url, "退回出科表单中，请稍后。。。", obj);
	});
}
...
function batchOscaSubmit(obj)
{
	...
	var url = "<s:url value='/jsres/temp/batchOscaSubmit'/>?orgFlow="+orgFlow;
	postAction(url,"学员考核成绩提交中，请稍后。。。",obj);
}
function updateMonthStatistics(obj)
{
	var month=$('#month').val();
	if(!month){
		jboxTip("请选择月份！");
	...
	var url = "<s:url value='/jsres/temp/updateMonthStatistics'/>?month="+month;
	postAction(url,month+"月填写数据更新中，请稍后。。。",obj);
}
function updateMonthAppStatistics(obj)
{
	var month=$('#month').val();
	if(!month){
		jboxTip("请选择月份！");
	...
	var url = "<s:url value='/jsres/temp/updateMonthAppStatistics'/>?month="+month;
	postAction(url,month+"月App使用率更新中，请稍后。。。",obj);
}
function backAttend(obj)
{
	...
	var url = "<s:url value='/jsres/temp/backAttend'/>?userCode="+userCode+"&startDate="+startDate+"&endDate="+endDate;

	jboxConfirm("确认退回考勤审核？",function() {
		postAction(url, "退回考勤审核中，请稍后。。。", obj);
	});
}
function delProcess(obj)
{
	...
	var url = "<s:url value='/jsres/temp/delProcess'/>?processFlow="+processFlow;

	jboxConfirm("确认删除轮转记录？",function(){
		postAction2(url,"删除轮转记录中，请稍后。。。",obj,function(resp){
			if(resp=="操作成功！")
			{
				$("#"+processFlow).remove();
			}
			...
			<table class="searchTable" style="width: 100%;border-collapse:separate; border-spacing:0px 10px;">
				...
				<tr>
					<td class="td_left" style="width: 60%;text-align: center" colspan="2">
						<input class="btn_green" type="button" value="删除整个轮转科室" onclick="delProcess(this);">
						<input class="btn_green" type="button" value="退回出科表" onclick="delProcessType(this);">
					</td>
				</tr>
				...
				<tr>
					...
					<td class=""style="width: 30%;text-align: left">
						<input type="text" id="userCode2" name="userCode2"  class="input"  style="width: 200px;margin-left: 0px"/>
					</td>
					<td rowspan="2">

						<input class="btn_green" type="button" value="退回考勤" onclick="backAttend(this);">
					</td>
				</tr>
				<tr>
					<td class="td_left" style="width: 30%;text-align: right">
				...
				<tr>
					...
					<td class=""style="width: 50%;text-align: left">
						<input class="btn_green" type="button" value="开始更新" onclick="batchOscaSubmit(this);"/>
					</td>
				</tr>
				<tr>
					<td class="td_left" style="width: 50%;text-align: right">
						处理每月统计数据:<input type="text" id="month" name="month"  class="input" readonly="readonly" style="width: 100px;margin-left: 0px"/>
					</td>
					<td class=""style="width: 50%;text-align: left">
						<input class="btn_green" type="button" value="更新填写数据" onclick="updateMonthStatistics(this);"/>
						<input class="btn_green" type="button" value="更新APP使用情况" onclick="updateMonthAppStatistics(this);"/>
					</td>
				</tr>
			</table>
		</div>
