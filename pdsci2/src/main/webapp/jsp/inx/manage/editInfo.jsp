...
<script type="text/javascript">
		...
		function uploadImage(){
			...
			$.ajaxFileUpload({
				url:"<s:url value='/inx/manage/info/uploadImg'/>",
				secureuri:false,
		...
		function saveInfo(){
			...
			if($("#editForm").validationEngine("validate")){
				...
				var url = "<s:url value='/inx/manage/info/save'/>";
		...
		function delImg(infoFlow){
			...
			var url = "<s:url value='/inx/manage/info/deleteImg'/>";
			jboxPost(url,requestData,function(resp){
   		...
   		<table class="basic" width="100%" style="margin: 0 auto;">
            ...
            <tr>
                ...
                <td colspan="3">
					...
					<select name="roleFlow"id="roleFlow" class="validate[required] text-input xselect">
						...
						<c:forEach items="${sysRoleList}" var="role">
							<c:if test="${fn:contains(roleFlows.roleName, role.roleFlow) or sessionScope.currUser.userCode==GlobalConstant.ROOT_USER_CODE}">
								<option value="${role.roleFlow }" <c:if test="${info.roleFlow eq  role.roleFlow}">selected</c:if>>${role.roleName}</option>
							</c:if>
						</c:forEach>
					</select>
				</td>
