...
var tip = {
	...
	function modPasswd() {
		...
		var url = "<s:url value='/inx/hbzy/savePasswd'/>?actionId=${actionId}";
		...
		jboxPost(url, data, function(res) {
			if ("${GlobalConstant.RESET_SUCCESSED}"==res) {
				...
				setTimeout(function(){
					window.location.href="<s:url value='/inx/jszy'/>";
				},1000);
...
<body>
  <div class="register_top">${sysCfgMap['sys_title_name']}</div>
...
<div class="footer">主管单位：湖北省卫生厅科教处 | 协管单位：湖北省毕业后医学继续教育委员会</div>
