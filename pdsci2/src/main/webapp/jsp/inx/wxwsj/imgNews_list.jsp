...
<c:if test="${fn:length(imgNewsList)>0 }">
	<ul class="Limg">
	 <c:forEach items="${imgNewsList}" var="info">
		<li>
			<a href="<s:url value='/inx/wxwsj/getByInfoFlow?infoFlow=${info.infoFlow}&columnId=${param.columnId}'/>" target="_blank">
			<img src="${imageBaseUrl}${info.titleImg}" alt="${info.infoTitle}" /></a>
			<p><a href="<s:url value='/inx/wxwsj/getByInfoFlow?infoFlow=${info.infoFlow}&columnId=${param.columnId}'/>" target="_blank">${pdfn:cutString(info.infoTitle,16,true,3)}</a></p>
		</li>
	</c:forEach>
	</ul>
	<cite class="Nubbt">
