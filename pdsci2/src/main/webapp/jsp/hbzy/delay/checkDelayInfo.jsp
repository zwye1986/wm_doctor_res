    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#graduationYear').datepicker({
                startDate: "${recruit.graduationYear+1}",
                startView: 2,
                maxViewMode: 2,
                minViewMode: 2,
                format: 'yyyy'
        ...
        function submit() {
            if ($("#editForm").validationEngine("validate")) {
                jboxStartLoading();
                var options = {
                    url: "<s:url value='/hbzy/delayReturn/checkDelayInfo'/>",
                    type: "post",
                    cache: false,
                    beforeSend: function () {
                    },
...
<div class="div_search">
    ...
    <form id="editForm" style="position: relative;" method="post"
          action="<s:url value='/hbzy/delayReturn/checkDelayInfo'/>"
          enctype="multipart/form-data">
        <input type="hidden" name="recordFlow" value="${delayInfo.recordFlow}"/>
        <table border="0" cellpadding="0" cellspacing="0" class="base_info">
            ...
            <tr>
                ...
                <td>
                    <input class="validate[required]" type="radio" id="agreeChangeStatusId" name="delayStatusId"
                           value="${agree}" onchange="editOpinon('Y');"/>&nbsp;同意&#12288;
                    <input class="validate[required]" type="radio" id="disAgreeChangeStatusId" name="delayStatusId"
                           value="${disAgree}"
                           onchange="editOpinon('N');"/>&nbsp;不同意
                </td>
            </tr>
            <tr id="graduationYearTr" style="display: none;">
                <th><b class="red">*</b>&#12288;结业考核年份：</th>
                <td>
                    <input type="text" class="input validate[required]" id="graduationYear"
                           name="graduationYear" readonly="readonly" style="width: 100px;height: 20px"/>
                </td>
            </tr>
