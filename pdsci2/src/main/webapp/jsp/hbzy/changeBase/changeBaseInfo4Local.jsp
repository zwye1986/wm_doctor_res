...
<script type="text/javascript">
    ...
    function showCheckChangeOrg(recordFlow) {
        var url = "<s:url value='/hbzy/manage/showCheckChangeOrg'/>?roleId=${roleId}&recordFlow=" + recordFlow;
    ...
    function searchIn() {
        var url = "<s:url value='/hbzy/manage/changeBaseManage'/>";
            ...
            <c:if test="${operType eq 'isCheck'}">
                <table class="searchTable">
                    <tr>
                        ...
                        <td class="td_left">记录来源：</td>
                        <td>
                            <select name="outOrIn" id="outOrIn" class="select" style="width: 107px;">
                                <option value="all">全部</option>
                                <option value="changeOut"
                                        <c:if test="${param.outOrIn=='changeOut'}">selected="selected"</c:if>>转出
                                </option>
                                <option value="changeIn"
                                        <c:if test="${param.outOrIn=='changeIn'}">selected="selected"</c:if>>转入
                                </option>
                            </select>
                        </td>
                        <td colspan="2">
                        ...
                        <c:forEach items="${historysMap[docOrgHistoryExt.userFlow]}" var="docOrgHistory"
                            ...
                            <c:if test="${num.count > 1}">
                                <tr>
                                    ...
                                    <c:if test="${operType eq 'isQuery'}">
                                        <td class="${docOrgHistoryExt.userFlow}Show" style="display: none;"
                                            title="${pdfn:transDate(hostory.modifyTime)}">${docOrgHistory.changeStatusName}</td>
                                        <td class="${docOrgHistoryExt.userFlow}Show"
