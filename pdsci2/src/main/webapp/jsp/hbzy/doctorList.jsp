...
function toPage(page) {
	...
	jboxStartLoading();
	jboxPostLoad("doctorListZi","<s:url value='/hbzy/doctorRecruit/doctorTrendListSun'/>?"+data+"&roleFlag=${roleFlag}&zlFlag=${zlFlag}",$("#searchForm").serialize(),false);
}

function doctorPassedList(doctorFlow,recruitFlow,zlFlag){
	var hideApprove="hideApprove";
	var url = "<s:url value='/hbzy/manage/province/doctor/doctorPassedList'/>?info=${GlobalConstant.FLAG_Y}&liId="+recruitFlow+"&recruitFlow="+recruitFlow+"&openType=open&doctorFlow="+doctorFlow+"&hideApprove="+hideApprove+"&zlFlag="+zlFlag;
	jboxOpen(url,"学员信息",1050,550);
	
}

function updateDoctorTrend(recruitFlow,doctorFlow){
	var url = "<s:url value='/hbzy/manage/updateDoctorRecruit'/>?doctorFlow="+doctorFlow+"&recruitFlow="+recruitFlow;
...
function exportExcel(){
	...
	var url = "<s:url value='/hbzy/doctor/exportDoctor'/>?"+data+"&sessionNumber="+sessionNumber+"&zlFlag=${zlFlag}";
	jboxTip("导出中…………");
...
function exportForDetail(){
	...
	var url = "<s:url value='/hbzy/doctor/exportForDetail'/>?"+data+"&sessionNumber="+sessionNumber+"&zlFlag=${zlFlag}";
	jboxTip("导出中…………");
	jboxSubmit($("#searchForm"), url, null, null, false);
	jboxEndLoading();
}
...
function changeOrgFlow(obj){
 ...
 function changeDerateBox(obj){
	...
	function exTrainingDocPdf(){
		location.href = '<s:url value="/hbzy/manage/exTrainingDocPdf"/>?orgFlow=${currUser.orgFlow}';
	}
