...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var session = $("#archive_select").find("option:first").attr("session");
		changeSessionNumber(session);
		$('#sessionNumber').val(session);
		toPage();
	...
	function toPage(page) {
		...
		jboxStartLoading();
		jboxPostLoad("doctorListZi","<s:url value='/hbzy/archive/doctorTrendListSun'/>?"+data+"&roleFlag=${roleFlag}",$("#searchForm").serialize(),false);
	}

	function doctorPassedList(doctorFlow,recruitFlow,archiveFlow){
		var hideApprove="hideApprove";
		var url = "<s:url value='/hbzy/archive/province/doctor/doctorPassedList'/>?info=${GlobalConstant.FLAG_Y}&liId="+recruitFlow+"&recruitFlow="+recruitFlow+"&archiveFlow="+archiveFlow+"&openType=open&doctorFlow="+doctorFlow+"&hideApprove="+hideApprove;
	...
	function exportExcel(){
		...
		var url = "<s:url value='/hbzy/archive/exportDoctor'/>?"+data+"&sessionNumber="+sessionNumber;
		jboxTip("导出中…………");
	...
	function exportForDetail(){
		...
		var url = "<s:url value='/hbzy/archive/exportForDetail'/>?"+data+"&sessionNumber="+sessionNumber;
		jboxTip("导出中…………");
		jboxSubmit($("#searchForm"), url, null, null, false);
		jboxEndLoading();
	}
...
// alert($("#"+orgFlow));
// 	var url = "<s:url value='/hbzy/doctor/searchJointOrgFlag'/>?orgFlow="+orgFlow;
...
// 		}, null , false);
	...
	function InformationQuery(doctorFlow,roleFlag){
		...
		jboxPost("<s:url value='/hbzy/doctor/checkDoctorAuth'/>?doctorFlow="+doctorFlow+"&roleFlag="+roleFlag,null,function(resp){
			if(resp=="${GlobalConstant.FLAG_Y}"){
				jboxLoad("content","<s:url value='/hbzy/doctor/trainRegister'/>?hideApprove=hideApprove&doctorFlow="+doctorFlow+"&roleFlag="+roleFlag+"&search="+formdate,true);
			}else{
				jboxTip("该学员尚未上传诚信声明！");
			}
		},null,false);
	...
	function exTrainingDocPdf(){
		...
		location.href = '<s:url value="/hbzy/manage/exTrainingDocLogPdf"/>?orgFlow=${currUser.orgFlow}&archiveFlow='+ archiveFlow + '&sessionNumber='+sessionNumber;
	...
	function exZLTrainingDocPdf(){
		...
		location.href = '<s:url value="/hbzy/manage/exZLTrainingDocLogPdf"/>?orgFlow=${currUser.orgFlow}&archiveFlow='+ archiveFlow + '&sessionNumber='+sessionNumber;
	}
	function archiveDoctor(){
		var url = "<s:url value="/hbzy/archive/addArchive"/>";
		jboxOpen(url,"添加存档",500,200,true);
	}
	function changeSessionNumber(vari){
		$("#archiveFlow").find("option").remove();
		$("#archive_select").find("option[session*='"+vari+"']").each(function(i,e){
			$("#archiveFlow").append($(e).clone());
		});
	}
</script>
<script type="text/javascript">
			...
			<form id="searchForm">
				...
				<table class="searchTable">
					<tr>
						...
						<td>
							<input type="text" id="sessionNumber" name="sessionNumber"  onchange="changeSessionNumber(this.value);"
								   <c:if test="${empty param.sessionNumber }">value="${pdfn:getCurrYear()}"</c:if>value="${param.sessionNumber}"
					...
					<tr>
						...
						<td>
							<select name="archiveFlow" id="archiveFlow"  class="select" style="width:107px;">
							</select>
						</td>
						<td colspan="2">
							...
							<label  id="derateFlagLabel" style="cursor: pointer;">
								<input type="checkbox" id="derateFlag"
									   <c:if test="${param.derateFlag eq GlobalConstant.FLAG_Y}">checked="checked"</c:if>
									   name="derateFlag" value="${GlobalConstant.FLAG_Y}">&nbsp;只显示减免
...
<div style="display: none;">
	...
	<select id="archive_select">
		<c:forEach items="${archiveSequenceList}" var="archive">
			<option session="${archive.sessionNumber}" value="${archive.archiveFlow}">${pdfn:transDateTime(archive.archiveTime)}</option>
		</c:forEach>
	</select>
</div>
<div>
