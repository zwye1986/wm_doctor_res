...
<script type="text/javascript">
    ...
    function toPage(page,scoreYear) {
        ...
        jboxStartLoading();
        jboxPostLoad("doctorListZi", "<s:url value='/hbzy/doctorTheoryScore/doctorSkillListSun'/>?" + data + "&roleFlag=${roleFlag}", $("#searchForm").serialize(), false);
    }
    function importExcel() {
        var url = "<s:url value='/hbzy/doctorTheoryScore/importSkillScore'/>";
    ...
    /*function changeTrainSpes() {
        ...
        var url = "<s:url value='/hbzy/doctor/searchResOrgSpeList'/>?sessionNumber=" + sessionNumber + "&orgFlow=" + orgFlow + "&speTypeId=" + trainCategoryid;
     ...
     * @constructor
     */
    function deleteSkillScore(scoreFlow, roleFlag,page) {
        jboxConfirm("确认删除？", function () {
            jboxPost("<s:url value='/hbzy/doctorTheoryScore/deleteSkillScore'/>?scoreFlow=" + scoreFlow + "&roleFlag=" + roleFlag, null, function (resp) {
    ...
    //var map=new Map();
    //map.put("firstStationScore","第一站");
    //map.put("secondStationScore","第二站");
    //map.put("thirdStationScore","第三站");
    //map.put("fourthStationScore","第四站");
    //map.put("fifthStationScore","第五站");
    //map.put("sixthStationScore","第六站");
    //map.put("seventhStationScore","第七站");
    //map.put("eighthStationScore","第八站");
    //map.put("ninthStationScore","第九站");
    function saveScore2(scoreFlow, k, obj, stationName, roleFlag) {
        //k 是表示为第几站
        var score = obj.value;
        var data = {};
        data.scoreFlow = scoreFlow;
        jboxPost("<s:url value='/hbzy/doctorTheoryScore/saveSkillIsPass?roleFlag='/>" + roleFlag + "&stationName=" + k + "&isPass=" + score, data, function (resp) {
            if ("1" == resp) {
    ...
    function saveScore(scoreFlow, k, obj, stationName, roleFlag) {
        ...
        jboxPost("<s:url value='/hbzy/doctorTheoryScore/saveSkillScore?roleFlag='/>" + roleFlag + "&stationName=" + k + "&skilScore=" + score, data, function (resp) {
            if ("1" == resp) {
        ...
        <form id="searchForm" style="margin-left: -30px;">
            ...
            <c:if test="${sessionScope.userListScope eq GlobalConstant.USER_LIST_GLOBAL}">
                <div id="provCityAreaId" class="searchCss">
                    ...
                    <select id="cityId" name="cityId" onchange="searchOrgList();"
                            class="city select validate" data-value="" data-first-title="选择市"  style="width: 106px;">
                    </select>
                </div>
                <div class="searchCss">
                    <label>&#12288;&#12288;培训基地：</label>
            ...
            <c:if test="${sessionScope.userListScope eq GlobalConstant.USER_LIST_CHARGE}">
                <div class="searchCss">
                    ...
                    <input id="trainOrg" oncontextmenu="return false" class="toggleView input" type="text"
                           autocomplete="off" style="margin-left: 0px;width: 100px" onkeydown="changeStatus();"
                           onkeyup="changeStatus();"/>
                </div>
                <div id="pDiv"
                     style="width: 0px;height: 0px;overflow: visible;float: left; position:relative; top:35px;left:-125px">
                    <div class="boxHome trainOrg" id="trainOrgSel"
                         style="max-height: 250px;overflow: auto; border: 1px #ccc solid;background-color: white;min-width: 100px;border-top: none;position: relative;display:none;">
            ...
            <div class="searchCss">
                ...
                <input type="text" id="sessionNumber" name="sessionNumber" value="${param.sessionNumber}" class="input"
                       readonly="readonly" style="width: 100px;margin-left: 0px"/>
            </div>
                    ...
                    <label>
                        ...
                        <select id="trainingYears" class="select" name="trainingYears" style="width: 106px">
                            ...
                            <c:forEach items="${jsResTrainYearEnumList}" var="trainYear">
                                <option value="${trainYear.id}"
                                        <c:if test="${doctor.trainingYears eq trainYear.id}">selected="selected"</c:if>
            ...
            <div class="searchCss">
                &#12288;&#12288;姓&#12288;&#12288;名：<input type="text" name="userName" value="${param.userName}"
                                                          class="input"
                                                          style="width: 100px;"/>
            </div>
            <div class="searchCss">
            ...
            <c:if test="${sessionScope.userListScope != GlobalConstant.USER_LIST_LOCAL }">
                <div style="float: left">
                    &#12288;&#12288;<label style="cursor: pointer;display: none;" id='jointOrg'><input type="checkbox"
                                                                                                       id="jointOrgFlag"
                                                                                                       name="jointOrgFlag"
            ...
            <c:if test="${sessionScope.userListScope eq GlobalConstant.USER_LIST_LOCAL }">
                <div style="float: left">
                    &#12288;&#12288;<label style="cursor: pointer;" id='jointOrg'><input type="checkbox"
                                                                                         id="jointOrgFlag"
                                                                                         name="jointOrgFlag"
                                                                                         value="${GlobalConstant.FLAG_Y}"/>&nbsp;显示协同基地</label>
                </div>
            ...
            <c:if test="${sessionScope.userListScope != GlobalConstant.USER_LIST_LOCAL}">
                <div style="float: left">
                    <label id="derateFlagLabel" style="cursor: pointer;display: none;">
                        &#12288;&#12288;<input type="checkbox" id="derateFlag" name="derateFlag"
            ...
            <div class="searchCss">
                ...
                <c:if test="${roleFlag eq GlobalConstant.USER_LIST_GLOBAL}">
                    <input class="btn_brown" type="button" value="导入成绩" onclick="importExcel();"/>
                </c:if>
            </div>
