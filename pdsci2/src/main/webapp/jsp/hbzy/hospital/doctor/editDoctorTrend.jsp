...
function saveInfo(){
	var url="<s:url value='/hbzy/manage/updateDoctorTrend'/>";
...
function checkStatus(doctorFlow,callBack){
	var url="<s:url value='/hbzy/manage/checkStatus'/>";
...
function changePolicy(obj){
	if($(obj).val()=="2"){
		jboxTip("违约退培学员将5年内无法参加湖北省住院医师规培");
		$("#policy").show();
	}else{
		if($("#policy").is(":hidden")){
			return false;
...
function uploadFile(){
	...
	var url = "<s:url value='/hbzy/doctor/uploadDelayFile'/>";
...
<div class="div_table">
	...
	<form id="editForm" action="<s:url value="/hbzy/manage/updateDoctorTrend"/>" method="post" enctype="multipart/form-data">
