...
<script>
	...
	function saveOper(){
		...
		$data.each(function(){
			var 	rotationFlow=$(".rotationFlow",this).val() || "";
			var 	rotationName="";
			if("${doctor.rotationFlow}"==rotationFlow)
			{
				rotationName="${doctor.rotationName}";
			}
			if("${doctor.secondRotationFlow}"==rotationFlow)
			{
				rotationName="${doctor.rotationName}";
			}
			var obj = {
		...
		$("#saveBtn").hide();
		jboxPostJson("<s:url value='/hbzy/manage/saveReductionOper'/>",
...
// 		}
	...
	function checkFile(obj){
		...
		$.ajaxFileUpload({
			url:"<s:url value='/hbzy/doctor/uploadTrainYearFileAndSaveRecruit'/>?recruitFlow="+flow,
			secureuri:false,
			fileElementId:"file",
			dataType: 'json',
...
<body>
	<div style="height: 440px;overflow: auto;">
		<table id="operDiv" border="0" cellpadding="0" cellspacing="0" class="grid">
					...
					<tr id="${rotationDept.recordFlow}" class="isData">
						...
						<c:if test="${currOrg}">
							<td>
								...
								<input type="hidden" class="rotationFlow" value="${rotationDept.rotationFlow}"/>
			...
			<c:if test="${not empty doctor.secondRotationFlow}">
				...
				<c:forEach items="${groupList2}" var="group">
					<c:set var="sumStandardMonth" value="${sumStandardMonth+group.schMonth}"/>
					<c:set var="rotationDeptList" value="${rotationDeptListMap2[group.groupFlow]}" />
			...
			<tr>
				...
				<th>${sumStandardMonth}</th>
				<th>
					<label id="schMonth"></label>
				</th>
				<th></th>
				<c:if test="${currOrg}">
					<th></th>
	...
	<div align="center" style="margin-top: 10px;">
		<c:if test="${currOrg}">
			<input id="saveBtn" type="button" value="保&#12288;存" class="btn_brown" onclick="saveOper();"/>
		</c:if>
<!-- 		<input id="reductionBtn" type="button" value="查看减免证明" toggleVal="方案减免维护" class="btn_brown" onclick="provToggle();"/> -->
		<input type="button" value="关&#12288;闭" class="btn_brown" onclick="closeWindow();"/>
	</div>
