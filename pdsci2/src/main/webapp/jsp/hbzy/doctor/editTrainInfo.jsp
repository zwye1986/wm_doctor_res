...
//			jboxStartLoading();
		...
		function searchResOrgSpeList(){
			...
			jboxStartLoading();
			var url = "<s:url value='/hbzy/doctor/searchResOrgSpeList?orgFlow='/>" + orgFlow +"&speTypeId="+catSpeId+"&sessionNumber="+year;
			jboxGet(url, null, function(resp){
				jboxEndLoading();
				var dataObj = resp;
				if(dataObj.length>0){
		...
		function validate(){
			...
			if(sessionNum !=""){
				jboxPost("<s:url value='/hbzy/doctorRecruit/validate'/>?sessionNumber="+sessionNum+"&doctorFlow="+doctorFlow , null , function(resp){
		...
		function saveRecruit(){
			...
			var url = "<s:url value='/hbzy/doctor/saveResDoctorRecruit'/>";
				...
				<tr>
					...
					<td>
						<select id="orgFlow" name="orgFlow" class="validate[required] select" style="width: 160px;"onchange="searchResOrgSpeList();">
							<option value="">请选择</option>
							<option orgCityId="320100" value="7a6eb182b89d4cdfb5bb789524c8dff8" style="display: none;">湖北省中医院</option>
							<option orgCityId="320100" value="6bb5b7409fc14038bb28a874e5fd501b" style="display: none;">湖北省中西医结合医院</option>
