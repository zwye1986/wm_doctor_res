<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<link rel="stylesheet" type="text/css" href="<s:url value='/jsp/hbzy/css/exam.css'/>?v=${applicationScope.sysCfgMap['sys_version']}"/>
...
$(function(){
	...
	if("${doctor.doctorTypeId}"=="${jszyResDocTypeEnumCompany.id}" || "${doctor.doctorTypeId}"=="${jszyResDocTypeEnumCompanyEntrust.id}"){
	...
	if(result==0){
		changeWorkAdress("${jszyResDocTypeEnumList[0].id}");
	}
...
function changeWorkAdress(doctorTypeId){
	if(doctorTypeId == "${jszyResDocTypeEnumCompany.id}" || doctorTypeId == "${jszyResDocTypeEnumCompanyEntrust.id}"){
		$(".school").hide();
		$(".address").show();
		$("#psxx").val("");
...
function saveDoctorInfo(){
	...
	var url = "<s:url value='/hbzy/doctor/addInfo'/>";
...
<div class="search_table" style="margin-top:20px;">
	...
	<div class="btn_info" style="margin-top: 50px">
		<input type="button" style="width:100px;" class="btn_blue" onclick="saveDoctorInfo();" value="保&#12288;存"/>
	</div>
</div>
</form>
</body>
