...
function searchOrgList(){
	...
	jboxStartLoading();
	var url = "<s:url value='/hbzy/doctor/searchOrgList'/>?orgCityId=" + cityId;
...
function uploadFile(){
	...
	var url = "<s:url value='/hbzy/doctor/uploadChangeSpeFile'/>";
...
function submit(){
	...
	var data = {
		...
		orgFlow : orgFlow,
		orgName : orgName,
		changeStatusId : "${jszyResChangeApplyStatusEnumOutApplyWaiting.id}",
		speChangeApplyFile:speChangeApplyFile
	};
	var url = "<s:url value='/hbzy/doctor/submitDoctorOrgHistory'/>";
...
function checkResOrgSpe(orgFlow){
	...
	if("${recruit.sessionNumber}">="2015"){
		jboxStartLoading();
		var url = "<s:url value='/hbzy/doctor/checkResOrgSpe'/>?catSpeId=${param.catSpeId}&speId=${param.speId}&orgFlow=" +orgFlow;
		jboxGet(url, null, function(resp){
				jboxEndLoading();
	...
	<table border="0" cellpadding="0" cellspacing="0" class="base_info" style="margin-top: 25px;">
            <colgroup>
              ...
              <col width="70%"/>
	           ...
	           <tbody>
        	   <tr>
	               <th><b class="red">*</b>&#12288;所在地区：</th>
	               <td>
	               		<div id="provCityAreaId">
							<select id="cityId" name="cityId" onchange="searchOrgList();" class="city select validate[required]" data-value="" data-first-title="选择市"></select>
							<!-- <select id="areaId" name="areaId" class="area select validate[required]" data-value="" data-first-title="选择地区"></select> -->
						</div>
					</td>
	           </tr>
	           <tr>
	               <th><b class="red">*</b>&#12288;培训基地：</th>
	               <td>
			...
			<td colspan="3">
	           ...
	           <tr>
					<th>模板文件：</th>
					<td colspan="3"><a href="<s:url value='/jsp/hbzy/daochu/changeBase.docx'/>">培训基地变更申请表.docx</a></td>
				</tr>
	           </tbody>
          </table>
          </form>
