...
<script>
    ...
    function saveSingup(flag){
        ...
        if(flag){
            ...
            jboxConfirm("是否确认提交，提交后不能再修改？" , function(){
                jboxPost("<s:url value='/gzzyjxres/doctor/submitSingup'/>" , $("#doctorForm").serialize() , function(resp){
                    if(resp=="1"){
                        jboxTip("保存成功");
                        search();
                    }else{
    ...
    function printApplForm(batchFlow){
        ...
        jboxPost("<s:url value='/gzzyjxres/doctor/valideExist'/>",{"batchFlow":batchFlow},function(resp){
            if(null != resp && resp != ""){
                jboxTip("打印中,请稍等...");
                var url = '<s:url value="/gzzyjxres/hospital/printApplForm?resumeFlow="/>'+resp+"&printFlag=doctor";
