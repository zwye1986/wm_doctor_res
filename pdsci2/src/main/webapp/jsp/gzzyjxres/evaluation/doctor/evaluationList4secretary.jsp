...
<head>
    <script type="text/javascript">
        ...
        function toPage(page) {
            ...
            jboxPostLoad("content", "<s:url value='/gzzyjxres/evaluation/evaluationManage'/>", $('#searchForm').serialize(), true);
        }
        //展示评价窗口
        function showEvaluation(processFlow) {
            var url = "<s:url value='/gzzyjxres/evaluation/showEvaluation?kaoherenRoleId=Secretary&roleId=Doctor&processFlow='/>" + processFlow;
            var typeName = "学员评价";
            jboxOpen(url, typeName, 800, 500);
        }
        //展示查看评价窗口
        function showEvalDetail(evalRecordFlow) {
            var url = "<s:url value='/gzzyjxres/evaluation/showEvalDetail?roleId=Doctor&evalRecordFlow='/>" + evalRecordFlow;
            var typeName = "学员评价查看";
            jboxOpen(url, typeName, 800, 500);
        }
    </script>
...
<div class="main_bd" id="div_table_0">
    <div class="div_table">
        <form id="searchForm" action="<s:url value="/res/doc/backTrainCheck" />" method="post">
            ...
            <input id="kaoherenRoleId" type="hidden" name="kaoherenRoleId" value="Secretary"/>
            <table>
                <tr>
                    <td>
                        进修批次：
                        ...
                        <select name="speId" class="select" style="width:107px;">
                            ...
                            <c:forEach items="${dwjxSpeLst}" var="dict">
                                <option value="${dict.dictId}"
                                        <c:if test="${param.speId eq dict.dictId}">selected</c:if>>${dict.dictName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        &#12288;状态：
                        <select name="statueId" class="select" style="width:107px;">
                            <option value="">全部</option>
                    ...
                    <td>
                        &#12288;学员姓名：
                        <input class="input" type="text" value="${param.doctorName}" name="doctorName"
                               style="width: 100px;"/>
                    </td>
                    <td>
