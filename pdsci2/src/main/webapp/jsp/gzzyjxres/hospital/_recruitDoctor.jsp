...
<script type="text/javascript">
    ...
    //学员录取
    function search(){
        jboxPostLoad("content","<s:url value='/gzzyjxres/hospital/_recruitDoctor'/>?isQuery=Y",$('#auditForm').serialize(),true);
    }
    //打印进修申请表
    ...
    function showPublish(userFlow,batchId,resumeFlow){
        ...
        jboxConfirm("确认发布录取通知书?", function(){
            jboxPost("<s:url value='/gzzyjxres/hospital/publish'/>",{"resumeFlow":resumeFlow}, function(resp){
                jboxTip(resp);
                if(resp=="发布成功")
                {
    ...
    function  exportInfo()
    {
        var url = "<s:url value='/gzzyjxres/hospital/exportAuditRecruitted'/>";
