...
<script type="text/javascript">
    ...
    //查询
    function search(){
        jboxPostLoad("content","<s:url value='/gzzyjxres/hospital/searchSetting'/>",{"batchDate":$("#batchDate").val()},true);
    }
    //新增、编辑
    function edit(flag,batchFlow){
        ...
        jboxOpen("<s:url value='/gzzyjxres/hospital/edit'/>?batchFlow="+batchFlow+"&isAdd="+isAdd, flag,800,500,true);
    }
    //删除
    function del(batchFlow){
        jboxConfirm("确定删除吗？",function(){
            jboxPost("<s:url value='/gzzyjxres/hospital/delSetting'/>",{"batchFlow":batchFlow},function(resp){
    ...
    function defaultSet(batchFlow,batchStatus){
        ...
        jboxConfirm("此操作将覆盖已设系统默认批次，请确认?",  function() {
            jboxPost('<s:url value="/gzzyjxres/hospital/defaultSet"/>', {"batchFlow": batchFlow,"batchStatus":batchStatus}, function (resp) {
                if (resp == "1") {
                    jboxTip("操作成功");
                    search();
                }
