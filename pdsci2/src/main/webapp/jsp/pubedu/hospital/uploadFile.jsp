...
//		jboxStartLoading();
		...
		jboxSubmitNoLoad($("#uploadFileForm"),"<s:url value='/pubedu/hospital/checkUploadFile'/>",function(flag){
			if(flag=="fileErrorMsg"){
				jboxTip("图片大小超过限制！");
				return;
			}else{
				returnUrl(flag);
...
function returnUrl(flag){
	...
	window.parent.$('#${operType}Value').val(flag);
	<%--var filePath = "${sysCfgMap['upload_base_url']}/flag";--%>
	<%--window.parent.$('#${operType}Del').show();--%>
	window.parent.$('#${operType}Span').show();
	window.parent.$('#${operType}Span').find("a").attr('href',flag);
	doClose();
}
...
//	window.parent.doClose();
	var openDialog = window.parent.dialog.get('jbox-message-iframe');
...
<body>
<form id="uploadFileForm" method="post" action="<s:url value='/pubedu/hospital/checkUploadFile'/>" enctype="multipart/form-data">
