...
<%@ taglib prefix="any" uri="http://www.anychart.com" %>
    <any:chart width="100%" height="400px">
		 <charts>
			...
			<x_axis>
			<title>
              <text>${applicationScope.sysCfgMap['study_platform'] eq 'szslyy'?'基地':'学校'}</text>
			...
			<data>
				...
				<c:forEach items="${orgList }" var="org">
				 <c:set var="allCount" value="0"/>
				 <c:forEach items="${org.majorFormList }" var="majorForm"> 
				  	<c:set var="allCount" value="${allCount+questionCountFormMap[njmuEduQuestionStatusEnumAnswered.id][org.orgFlow][majorForm.majorId]}"/>
				 </c:forEach>
				<point name="${org.orgName}" y="${allCount }" />
				</c:forEach>
				</series>
				...
				<c:forEach items="${orgList }" var="org">
				 <c:set var="allCount" value="0"/>
				 <c:forEach items="${org.majorFormList }" var="majorForm"> 
				  	<c:set var="allCount" value="${allCount+questionCountFormMap[njmuEduQuestionStatusEnumUnanswered.id][org.orgFlow][majorForm.majorId]}"/>
				 </c:forEach>
				<point name="${org.orgName}" y="${allCount }" />
				</c:forEach>
				</series>
