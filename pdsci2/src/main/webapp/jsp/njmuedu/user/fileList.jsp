...
function uploadFile(){
	...
	jboxStartLoading();
	var url = "<s:url value='/njmuedu/user/saveCourseFile?courseFlow=${param.courseFlow}'/>";
...
function delCourseFile(recordFlow){
	jboxConfirm("确认删除此课程资料?", function(){
		var url = "<s:url value='/njmuedu/user/delCourseFile?recordFlow='/>" + recordFlow;
...
function searchFile(){
	var url ="<s:url value='/njmuedu/user/courseFile?courseFlow=${param.courseFlow}'/>";
		...
		<ul style="display:block;position:static">
			<li>
				<form id="fileForm" style="position: relative;" enctype="multipart/form-data" method="post">
        			<input type="file" id="file" name="file" class="validate[required,custom[eduFileType]]" style="display: none" />
