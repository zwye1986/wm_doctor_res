...
<script type="text/javascript">
	function checkPasswd(){
		...
		var userPasswd3 = $("#userPasswd3").val();
		if(userPasswd2 != "" && userPasswd3 != ""){
			if(userPasswd2 != userPasswd3){
				jboxInfo("新密码与确认新密码不一致！");
	...
	function saveUserPasswd(){
		...
		jboxConfirm("确认修改密码?",  function(){
			var url = "<s:url value='/njmuedu/user/saveUserPasswd'/>";
...
<div class="right fr" style="margin-top: 10px;">
	<h3>修改密码</h3>
	<form id="passwdForm" action="<s:url value='/njmuedu/user/saveUserPasswd'/>" style="position: relative;" method="post">
    ...
    <table width="372" border="0">
      ...
      <tr>
        <td colspan="2"><input class="txt validate[required,minSize[6],maxSize[16]]" type="password" name="userPasswd2" id="userPasswd2" placeholder="新密码（6-16位字母、数字或符号的组合）" onchange="checkPasswd()" value=""/></td>
      </tr>
      <tr>
        <td class="txtinfo" colspan="2">确认新密码</td>
      </tr>
      <tr>
        <td colspan="2"><input class="txt validate[required,minSize[6],maxSize[16]]" type="password" name="userPasswd3"  id="userPasswd3" placeholder="确认新密码（6-16位字母、数字或符号的组合）" onchange="checkPasswd()" value=""/></td>
      </tr>
      <tr>
