...
$(function(){
	/*课程评分*/
	$("#courseRaty").raty({
		score:${empty chapterExt.chapterScore?0:chapterExt.chapterScore},
		cancel:false,
...
function submitEvaluate(){
	var form=$("#sEvaluate");
	var url="<s:url value='/njmuedu/course/submitEvaluate'/>";
...
function submitQuestion(){
    	var form=$("#cQuestion");
    	var url="<s:url value='/njmuedu/course/submitQuestion'/>";
    	if(form.validationEngine("validate")){
...
function submitAnswer(questionFlow){
	...
	var url="<s:url value='/njmuedu/course/submitAnswer'/>";
...
function savePraise(){
	var url="<s:url value='/njmuedu/course/savePraise'/>?recordFlow=${sch.recordFlow}";
	jboxPost(url,null,function(resp){
		...
		if(resp=='${GlobalConstant.FLAG_Y}'){
			...
			$obj.html(count + 1);
			$a.attr("style","background-image:url(<s:url value='/jsp/njmuedu/css/images/hand-r.png'/>)");
		}
	},null,false);
}
function collection(){
	 var url = "<s:url value='/njmuedu/course/collection'/>?collectionTypeId=${njmuEduCollectionTypeEnumChapter.id}&resourceFlow=${chapterExt.chapterFlow}";
	 jboxPost(url,null,
				function(resp){
					...
					if(resp == "${GlobalConstant.FLAG_N}"){
						...
						$("#sc-a").css("background-image","url(<s:url value='/jsp/njmuedu/css/images/head-off.png'/>)");
					}else if(resp == "${GlobalConstant.FLAG_Y}"){
						//alert("收藏成功！");
						$("#collectionCount").text(collectionCount+1);
						$("#sc-a").css("background-image","url(<s:url value='/jsp/njmuedu/css/images/head.png'/>)");
					}
				},
				function(resp){},false);
}
...
function selectTag(showContent, selfObj,studyStatusId) {
	if(studyStatusId!='${njmuEduStudyStatusEnumFinish.id}'){
		if("${sysCfgMap['njmuedu_finish_vedio_test']}"=="${GlobalConstant.FLAG_Y}" && showContent=="tagContent3"){
...
function nextChapter(nowChapterFlow,courseFlow){
	var url = "<s:url value='/njmuedu/course/nextChapter'/>?nowChapterFlow="+nowChapterFlow+"&courseFlow="+courseFlow;
	jboxPost(url,null,function(resp){
		if(resp=='${GlobalConstant.LAST_CHAPTER}'){
			...
			url="<s:url value='/njmuedudu/course/chapter'/>"+"/"+resp;
...
function goBack(){
	...
	jboxPost(url,null,function(resp){
		...
		if(resp=="${GlobalConstant.STUDENT_ROLE}"){
			toUrl = "<s:url value='/njmuedu/course/stuCourseDetail/${chapterExt.course.courseFlow}'/>";
		}else if(resp!=""){
			toUrl = "<s:url value='/njmuedu/course/courseDetail/${chapterExt.course.courseFlow}'/>";
		}else{
			toUrl = "<s:url value='/inx/njmuedu'/>";
		}
...
function nextChapters(chapterFlow,courseFlow){
	var url="<s:url value='/njmuedu/course/nextChapters?chapterFlow='/>"+chapterFlow+'&courseFlow='+courseFlow;
	jboxPost(url,null,function(rep){
		var Url;
		if(rep=="${GlobalConstant.LAST_CHAPTER}"){
			jboxTip("已经是最后一章了！！！");
		}else{
			Url="<s:url value='/njmuedu/course/chapter/'/>"+rep;
			window.location.href=Url;
		}
	},null,false);
}
...
function loadChapterVideo(flag){
	...
	var url="<s:url value='/njmuedu/course/loadChapterVideo'/>?chapterFlow=${chapterExt.chapterFlow}&level="+level;
...
<div class="videoPage cbody clearfix" >
     ...
     <div class="fr">
        <p class="fl p1"><a href="javascript:void(0)" onclick="collection();" id="sc-a"
        	<c:if test="${not empty collectionList}">
        		style="background-image: url(<s:url value='/jsp/njmuedu/css/images/head.png'/>)"
        	</c:if> >收藏（<span id="collectionCount"><c:if test="${empty chapterExt.collectionCount}">0</c:if>${chapterExt.collectionCount}</span>）</a>
       	</p>
        <p class="fl p2"><a href="javascript:void(0)"  onclick="savePraise()" <c:if test="${sch.praiseStatus==GlobalConstant.FLAG_Y }">style="background-image:url(<s:url value='/jsp/njmuedu/css/images/hand-r.png'/>)"</c:if> >点赞（<span id="praiseCount"><c:out value="${chapterExt.chapterPraise}" default="0"/></span>）</a></p>    
     </div>
  </div>
  ...
  <div class="video-l fl">
   ...
   <table width="760" border="0">
        ...
        <Td colspan="2">
        	<textarea name="questionContent" placeholder="请输入问题内容" class="validate[required custom[maxSize[500]]]"></textarea>
        </Td>
     </Tr>
     <Tr height="45">
     	<Td></Td>
   		...
   		<dl>
        	<dt>
       	        <div class="fl infoT-l">
			  			<img class="t-face" src="${sysCfgMap['upload_base_url']}/${qSysUserMap[question.user.userFlow].userHeadImg}" onerror="this.src='<s:url value="/jsp/njmuedu/css/images/head-icon.png"/>'" />
                <br/>${question.user.userName }
                </div> 
                <div class="fr infoT-r">
                	<p class="p1"><img src="<s:url value='/jsp/njmuedu/css/images/aicon.png'/>" width="16" height="16" />${question.questionContent}</p>
            ...
            <dd id="answer_${question.questionFlow}" style="display:none;">
            	...
            	<div class="huif clearfix">
                	<textarea name="answerContent" placeholder="请输入回复内容" class="validate[required custom[maxSize[500]]]"></textarea>
                ...
                <ul>
                	<li>
			  			<img class="fl t-face" src="${sysCfgMap['upload_base_url']}/${qSysUserMap[answer.user.userFlow].userHeadImg}" onerror="this.src='<s:url value="/jsp/njmuedu/css/images/head-icon.png"/>'"/>
   ...
   <table width="760" border="0">
    <Tr>
        ...
        <Td colspan="2">
           ...
           <input type="hidden" name="courseFlow" value="${chapterExt.courseFlow}"/>
        	<textarea name="evaluate" id="evaluate" placeholder="请输入评价内容" class="validate[required custom[maxSize[500]]]"></textarea>
        </Td>
     </Tr>
   ...
   <div class="video-info" id="evaluateContent">
   		...
   		<dl <c:if test="${schedule.userFlow eq sessionScope.currUser.userFlow }">class="vedio-own"</c:if>>
        	<dt>
       	        <div class="fl infoT-l">
			  			<img class="t-face" src="${sysCfgMap['upload_base_url']}/${sysUserMap[schedule.userFlow].userHeadImg}" onerror="this.src='<s:url value="/jsp/njmuedu/css/images/head-icon.png"/>'" />
                ...
                <div class="fr infoT-r" <c:if test="${schedule.userFlow eq sessionScope.currUser.userFlow }">id="div_${schedule.userFlow }"</c:if>>
                	<p class="p1"><img src="<s:url value='/jsp/njmuedu/css/images/aicon.png'/>" width="16" height="16" />${schedule.evaluate }</p>
  ...
  <div class="video-r fr">
  	<dl>
        ...
        <dd class="video-rtype">
           <c:choose>
           <c:when test="${chapterExt.course.courseTypeId eq njmuEduCourseTypeEnumRequired.id }">
             <input class="fl r-bnt" type="button" value="${chapterExt.course.courseTypeName }" />
           </c:when>
           <c:when test="${chapterExt.course.courseTypeId eq njmuEduCourseTypeEnumOptional.id }">
             <input class="fl i-bnt" type="button" value="${chapterExt.course.courseTypeName }" />
           </c:when>
           <c:when test="${chapterExt.course.courseTypeId eq njmuEduCourseTypeEnumPublic.id }">
        ...
        <ul>
    	<li class="h3">课程评价<p><span id="courseRaty"></span>&nbsp;<span id="chapterScore">（<c:out value="${chapterExt.chapterScore }" default="0.0"/>分）</span></p></li>
        </ul>
    </dl>
    
    <dl>
        <dt>授课老师：${chapterExt.teacher.userName}</dt>
        <dd>教师简介：${chapterExt.eduUser.intro }<br/></dd>
    </dl>
  </div>
...
<!--/content-->
<%-- <jsp:include page="/jsp/njmuedu/include/foot.jsp"/> --%>
</div>
<div id="qClone" style="display: none">
<dl>
  	<dt>
 	        <div class="fl infoT-l">
		  			<img class="t-face" src="${sysCfgMap['upload_base_url']}/${sessionScope[GlobalConstant.CURRENT_USER].userHeadImg}" onerror="this.src='<s:url value="/jsp/njmuedu/css/images/head-icon.png"/>'"  />
          <br/>{0}
          </div> 
          <div class="fr infoT-r">
          	<p class="p1"><img src="<s:url value='/jsp/njmuedu/css/images/aicon.png'/>" width="16" height="16" />{1}</p>
 ...
 <div id="aClone" style="display: none">
 	 <ul>
     	<li>
		  			<img class="fl t-face" src="${sysCfgMap['upload_base_url']}/${sessionScope[GlobalConstant.CURRENT_USER].userHeadImg}" onerror="this.src='<s:url value="/jsp/njmuedu/css/images/head-icon.png"/>'"/>
        ...
        <dl id="eClone">
        	...
        	<div class="fl infoT-l">
                ...
                <div class="fr video-own infoT-r" style="border-bottom:0;">
                	<p class="p1" id="p1eclone"><img src="<s:url value='/jsp/njmuedu/css/images/aicon.png'/>" width="16" height="16" /></p>
