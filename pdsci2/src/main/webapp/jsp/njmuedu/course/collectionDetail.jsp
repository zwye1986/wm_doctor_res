...
<div class="module-tabs" style="display: block;">
      <ul class="fl">
		  <c:forEach items="${njmuEduCollectionTypeEnumList}" var="type">
 	...
 	<c:if test="${not empty courseList}">
   	<table style="width:100%;" class="collection-table">
   		...
   		<tr>
			...
			<td class="collectiondelete"><a href="javascript:void(0);"
											onclick="delConllection('${course.courseFlow}','${njmuEduCollectionTypeEnumCourse.id}');"></a>
			</td>
   		</tr>
   		</c:forEach>
 	...
 	<c:if test="${not empty chapterList}">
   	<table style="width:100%;" class="collection-table">
   		...
   		<tr>
   			...
   			<td>
   				 <a href="<s:url value='/njmuedu/course/chapter/${chapter.chapterFlow}'/>">${chapter.chapterName }</a>
   			</td>
   			<td>${courseFlowMap[chapter.chapterFlow]}</td>
   			<td>${teacherMap[chapter.teacherId]}</td>
			<td class="collectiondelete"><a href="javascript:void(0);"
											onclick="delConllection('${chapter.chapterFlow}','${njmuEduCollectionTypeEnumChapter.id}');"></a>
			</td>
   		</tr>
   		</c:forEach>
   	</table>
