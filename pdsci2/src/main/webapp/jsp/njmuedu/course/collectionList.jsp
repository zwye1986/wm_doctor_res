...
var id=$("#content").attr("id");
	var url="<s:url value ='/njmuedu/user/showCollection'/>?collectionTypeId=${njmuEduCollectionTypeEnumCourse.id}";
...
function selectCollectionTypeId(typeId){
	var id=$("#content").attr("id");
	var url="<s:url value ='/njmuedu/user/showCollection'/>?collectionTypeId=" + typeId;
	$("#content").html("");
	jboxLoad(id,url,false);
}
...
function delConllection(resourceFlow, collectionTypeId){
	jboxConfirm("确认删除？", function(){
		var url = "<s:url value ='/njmuedu/course/delCollection'/>?resourceFlow=" + resourceFlow +"&collectionTypeId=" + collectionTypeId;
		jboxPost(url,null,
				function(resp){
