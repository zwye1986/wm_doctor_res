...
function saveAudit(agreeFlag,reseachrepFlow){
	var tip =  agreeFlag=="${GlobalConstant.FLAG_Y}"?"审核通过":"退回";
	var url = "<s:url value='/srm/ach/reseachrep/saveAudit'/>?agreeFlag="+agreeFlag+"&reseachrepFlow="+reseachrepFlow;
		...
		<form method="post" id="auditForm">
			<div style="text-align: center;">
				<textarea id="auditContent" name="auditContent" style="width: 100%" rows="7"></textarea><br/><br/><br/>
			     <input class='search' onclick="saveAudit('${GlobalConstant.FLAG_Y}','${param.reseachrepFlow }')" type='button'  value='同&#12288;意' />&#12288;
				 <input class='search' onclick="saveAudit('${GlobalConstant.FLAG_N}','${param.reseachrepFlow }')" type='button' value='退&#12288;回'/>&#12288;
			</div>
		</form>
	</div>
</div> 
