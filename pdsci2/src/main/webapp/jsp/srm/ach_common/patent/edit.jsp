...
//            var datas = [];
            ...
            var trlength = $('#test').find("tr").length;
        ...
        function initProj(obj) {
            ...
            var userFlow = '${patent.applyUserFlow}';
            var datas = [];
                ...
                <form id="srmPatent" enctype="multipart/form-data" method="post">
                    ...
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="radio" class="validate[required]" name="isPropProj" <c:if test="${patent.isPropProj eq 'Y'}">checked="checked"</c:if> value="Y" id="Y" /><label for="Y">依托课题</label>
                                <input type="radio" class="validate[required]" name="isPropProj" <c:if test="${patent.isPropProj eq 'N'}">checked="checked"</c:if> value="N" id="N" /><label for="N">非依托课题</label>
                            </td>
                            <th><span class="redspan isPropProj" style="color: red;padding: 0px;margin-left: 10px;<c:if test="${patent.isPropProj ne 'Y'}">display: none</c:if>">*</span>项目来源：</th>
                            <td>
                                <select id="projSourceId" name="projSourceId" <c:if test="${patent.isPropProj ne 'Y'}">disabled="disabled"</c:if> class="xlselect validate[required]" onchange="clearProj(this.value)">
                                    <option value="">请选择</option>
                                    <c:forEach items="${dictTypeEnumProjTypeSourceList}" var="dict">
                                        <option value="${dict.dictId}"
                                                <c:if test="${patent.projSourceId eq dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th><span class="redspan isPropProj" style="color: red;padding: 0px;margin-left: 10px;<c:if test="${patent.isPropProj ne 'Y'}">display: none</c:if>">*</span>选择依托课题：
                            </th>
                            <td>
                                <input id="trainProj" class="validate[required] xltext" name="propProjName" type="text" style="<c:if test="${patent.isPropProj ne 'Y'}">display: none</c:if>"
                                       value="${patent.propProjName}" autocomplete="off"/>
                                <input id="projFlow" name="propProjFlow" class="input" value="${patent.propProjFlow}" type="text"
                        ...
                        <tr>
                            <th><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>授权公告号：
                            </th>
