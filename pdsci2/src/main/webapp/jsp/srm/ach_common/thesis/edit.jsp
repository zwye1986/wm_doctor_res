        ...
        function saveThesis(agreeFlag) {
            ...
            if($('#authorList tr').length < 1){
                ...
                $.each($('#authorList tr'),function (i,n) {
                    ...
                    var corresponding  = $(n).find("input[name='isCorresponding']");
//                    alert($(corresponding).prop("checked"));
                    if($(corresponding).prop("checked")){
                        tongxun = true;
                    }
                    if(typeName == '第一作者'){
                        diyi = true;
...
//            var datas = [];
            ...
            var trlength = $('#authorList').find("tr").length;
        ...
        function initProj(obj) {
            ...
            var userFlow = '${thesis.applyUserFlow}';
            var datas = [];
...
//                $(tr).find("input[name='deptName']").attr("readonly",false);
...
//                $(tr).find("select[name='scoreFlow']").val(n.scoreFlow);
        ...
        function checkFile(obj){
            var array = new Array('jpg','jpeg','png','pdf');  //可以上传的文件类型   'gif', 'jpeg', 'png'
            if (obj.value == '') {
                jboxTip("让选择要上传的文件！");
                return false;
            } else {
...
<div class="mainright">
    <div class="content">
        <div class="title1 clearfix">
            <div id="look">
                <form id="srmThesis" action="<s:url value="/srm/ach/thesis/save/${roleScope}"/>" method="post" enctype="multipart/form-data">
                    ...
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <input class="validate[required,maxSize[100]] name xltext" type="text" name="thesisName"
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="radio" class="validate[required]" name="isPropProj" <c:if test="${thesis.isPropProj eq 'Y'}">checked="checked"</c:if> value="Y" id="Y" /><label for="Y">依托课题</label>
                                <input type="radio" class="validate[required]" name="isPropProj" <c:if test="${thesis.isPropProj eq 'N'}">checked="checked"</c:if> value="N" id="N" /><label for="N">非依托课题</label>
                            </td>
                            <th><span class="redspan isPropProj" style="color: red;padding: 0px;margin-left: 10px;<c:if test="${thesis.isPropProj ne 'Y'}">display: none</c:if>">*</span>项目来源：</th>
                            <td>
                                <select id="projSourceId" name="projSourceId" <c:if test="${thesis.isPropProj ne 'Y'}">disabled="disabled"</c:if> class="xlselect validate[required]" onchange="clearProj(this.value)">
                        ...
                        <tr>
                            <th><span class="redspan isPropProj" style="color: red;padding: 0px;margin-left: 10px;<c:if test="${thesis.isPropProj ne 'Y'}">display: none</c:if>">*</span>选择依托课题：
                            </th>
                            <td>
                                <input id="trainProj" class="validate[required] xltext" name="propProjName" type="text" style="<c:if test="${thesis.isPropProj ne 'Y'}">display: none</c:if>"
                                       value="${thesis.propProjName}" autocomplete="off"/>
                                <input id="projFlow" name="propProjFlow" class="input" value="${thesis.propProjFlow}" type="text"
                        ...
                        <tr>
                            ...
                            <th><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>统计年度：</th>
                            <td>
                                <input class="validate[required] xltext ctime" style="width:160px;" type="text"
                                       name="authorizeYear"
                                       value="${thesis.authorizeYear}" onClick="WdatePicker({dateFmt:'yyyy'})"
                                       readonly="readonly"/>
                            </td>
                        </tr>
                        <tr>
                    ...
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            <th colspan="4" class="bs_name">
                                附件上传（需上传杂志封面、目录、全文，格式只支持pdf、jpg、jpeg、png；全文上传建议维普、万方数据库下载。）
                                <c:if test="${param.editFlag != GlobalConstant.FLAG_N}">
                ...
                <form id="list">
                    <table class="basic" id="mubiao" style="width: 100%">
                        <tr>
                            <th colspan="12" class="bs_name"> 论文作者<c:if
                        ...
                        <tr>
                            ...
                            <th style="text-align: center; width: 7%;">通讯作者</th>
                            ...
                            <th style="text-align: center; width: 9%;"><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>科室</th>
                            <th style="text-align: center; width: 7%;"><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>性别</th>
                            <th style="text-align: center; width: 8%;"><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>工号</th>
                            <th style="text-align: center; width: 8%;"><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>学历</th>
                            <th style="text-align: center; width: 8%;"><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>学位</th>
                        ...
                        <c:forEach var="author" items="${authorList}" varStatus="status">
                            <tr>
                                ...
                                <td>
                                    <input type="hidden" name="deptFlow" <c:if test="${author.authorPart ne '外院' }">id="deptFlow_${status.index}"</c:if>  value="${author.deptFlow}"/>
                                    <input type="text" style="width: 85%;" <c:if test="${author.authorPart ne '外院' }">id="deptName_${status.index}"</c:if> name="deptName" value="${author.deptName}"/>
                                </td>
                                <td>
...
<div id="menuContent" class="menuContent" style="background-color:#ffffff;display:none; position:absolute;z-index:1000;">
    <ul id="treeDemo" class="ztree" style="margin-top:0; width:150px;height: 140px"></ul>
</div>
</body>
</html>