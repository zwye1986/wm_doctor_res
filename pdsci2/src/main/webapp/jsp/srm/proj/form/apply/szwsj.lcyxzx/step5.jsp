	...
	<table class="bs_tb" style="width: 100%; margin-top: 10px;">
		<tr>
			<th colspan="8" class="theader">
				表6：近五年为第一负责人已完成的市级及以上科研项目共 <input type="text" name="completeNum" id="completeNum" value="${resultMap.completeNum}" class="validate[custom[integer]] inputText" style="width: 30px;" />项。（限填15项代表作）
				<c:if test="${param.view!=GlobalConstant.FLAG_Y }">
				...
				<td>
					<select name="complete_projSource" class="inputText" style="width: 90%;">
						...
						<option value="国家级科研项目（科技部）"
							<c:if test="${complete.objMap.complete_projSource eq '国家级科研项目（科技部）'}">selected="selected"</c:if>>国家级科研项目（科技部）</option>
