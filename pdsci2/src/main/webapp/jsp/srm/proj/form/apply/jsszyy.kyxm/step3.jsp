...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
    ...
    <font style="font-size: 14px; font-weight:bold;color: #333;">二、项目人员名单</font>
        ...
        <tr>
                ...
                <select name="leader_gender" class="validate[required] inputText" style="width: 80%;">
                    ...
                    <c:forEach var="sex" items="${userSexEnumList}">
                        <c:if test="${sex.id != userSexEnumUnknown.id}">
                            ...
                            <c:if test="${(empty resultMap.leader_gender) and (user.sexName eq sex.name)}">selected="selected"</c:if>
                            >${sex.name}</option>
                        </c:if>
                    </c:forEach>
                </select>
            ...
            <td>
                <input class="inputText validate[required,custom[number]]" type="text" name="leader_age"
                       value="${resultMap.leader_age}" style="width: 80%"/>
                </td>
            <td>
                ...
                <select class="validate[required] inputText" name="leader_title" style="width: 80%">
                    ...
                    <c:forEach items="${dictTypeEnumUserTitleList}" var="title">
                        <option value="${title.dictName}"
                                ...
                                <c:if test="${(empty resultMap.leader_title) and (user.titleName eq title.dictName)}">selected="selected"</c:if>
                        >${title.dictName}</option>
                    </c:forEach>
                </select>
            </td>
            <td><input type="text" name="leader_major" value="${resultMap.leader_major}" class="inputText validate[required]" style="width: 90%"/></td>
            <td><input type="text" name="leader_labor" value="${resultMap.leader_labor}" class="inputText validate[required]" style="width: 90%"/></td>
            <td><input type="text" name="leader_dept" value="${resultMap.leader_dept}<c:if test="${(empty resultMap.leader_dept) and (param.view!=GlobalConstant.FLAG_Y)}">${user.deptName}</c:if>" class="inputText validate[required]" style="width: 90%"/></td>
        </tr>
        <tr>
