...
<head>
	...
	<script type="text/javascript">
		...
		function add(tb,total){
			var length = $("#"+tb+"Tb").children().length;
			if(length > total-1 && "labwareList" != tb){
				jboxTip('该项限填'+total+'项！');
				return false;
			}
			//计算行数
			$("#"+tb+"Num").val(length+1);
			...
			<div style="margin-top: 10px;">
				<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" style="position: relative;" id="projForm">
					...
					<table class="bs_tb" style="width: 100%;margin-top: 10px;">
						<tr>
							<th colspan="8" class="theader">十一、近五年为主编/副主编出版的学术专著共<input type="text" name="totalmainAuthorProList" id="totalmainAuthorProList" value="${resultMap.totalmainAuthorProList}" class="inputText" style="width: 70px;"/>本。（限填15项代表作）
								<c:if test="${param.view!=GlobalConstant.FLAG_Y }">
									<span style="float: right;padding-right: 10px">
									<img title="新增" src="<s:url value="/css/skin/${skinPath}/images/add3.png" />" style="cursor: pointer;" onclick="add('mainAuthorProList',15)"/>&#12288;
					...
					<table class="bs_tb" style="width: 100%;margin-top: 10px;">
						<tr>
							<th colspan="9" class="theader">十二、近五年学会任职情况，共<input type="text" name="totaluserDeptList" id="totaluserDeptList" value="${resultMap.totaluserDeptList}" class="inputText" style="width: 70px;"/>个职务。（限填15项最高职务）
								<c:if test="${param.view!=GlobalConstant.FLAG_Y }">
									<span style="float: right;padding-right: 10px">
									<img title="新增" src="<s:url value="/css/skin/${skinPath}/images/add3.png" />" style="cursor: pointer;" onclick="add('userDeptList',15)"/>&#12288;
