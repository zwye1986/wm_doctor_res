        	    	...
        	    	<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
						...
						<table class="basic" style="width: 100%;margin-top: 10px;">
						  <tr>
  						  ...
  						  <tr>
   							<td width="30%" style="text-align: right">重点专科名称：</td>
    						<td>
  						  ...
  						  <tr>
   							<td width="30%" style="text-align: right">专科负责人：</td>
    						<td><input type="text" name="subjectDirector" value="<c:if test='${empty resultMap.subjectDirector and param.view!=GlobalConstant.FLAG_Y}'>${proj.applyUserName}</c:if><c:if test='${! empty resultMap.subjectDirector}'>${resultMap.subjectDirector}</c:if>" class="inputText" style="width: 46%"/></td>
  						  </tr>
  						  <tr>
    						...
    						<td>
    							<input type="text" name="constructOrg" 
    									 <c:choose>
    									 	<c:when test="${empty resultMap.constructOrg}">value="苏州市卫生和计划生育委员会"</c:when>
