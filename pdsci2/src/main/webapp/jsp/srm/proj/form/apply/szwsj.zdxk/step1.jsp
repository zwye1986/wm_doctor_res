...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
	...
	<table class="bs_tb" style="width: 100%">
		...
		<tr>
			...
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" class="validate[required] inputText" name='projName'
				...
				style="width: 46%" /> <span class="redspan"
				style="color: red; padding: 0px; margin-left: 10px;">*</span></td>
		</tr>
		<tr>
			<td style="text-align: center">单位名称：</td>
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" class="validate[required] inputText" name="orgName"
				value="<c:if test='${empty resultMap.orgName and param.view!=GlobalConstant.FLAG_Y}'>${proj.applyOrgName}</c:if><c:if test='${! empty resultMap.orgName}'>${resultMap.orgName}</c:if>"
				style="width: 46%" /> <span class="redspan"
				style="color: red; padding: 0px; margin-left: 10px;">*</span></td>
		</tr>
		<tr>
			...
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" class="inputText" name="orgAddress"
		...
		<tr>
			...
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" class="inputText" name="postcode"
				value="<c:if test='${empty resultMap.postcode and param.view!=GlobalConstant.FLAG_Y}'>${org.orgZip}</c:if><c:if test='${! empty resultMap.postcode}'>${resultMap.postcode}</c:if>"
				style="width: 46%" /></td>
		</tr>
		<tr>
			<td style="text-align: center">联系人：</td>
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" class="validate[required] inputText" name="linkman"
				value="<c:if test='${empty resultMap.linkman and param.view!=GlobalConstant.FLAG_Y}'>${proj.applyUserName}</c:if><c:if test='${! empty resultMap.linkman}'>${resultMap.linkman}</c:if>"
				style="width: 46%" /> <span class="redspan"
				style="color: red; padding: 0px; margin-left: 10px;">*</span></td>
		</tr>
		<tr>
			...
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" name="telephone" class="validate[required] inputText"
				...
				class="validate[custom[phone]]" style="width: 46%" /> <span
				class="redspan" style="color: red; padding: 0px; margin-left: 10px;">*</span>
			</td>
		</tr>
		<tr>
			<td style="text-align: center">邮箱地址：</td>
			<td style="text-align: left; padding-left: 10px;"><input
				type="text" name="email" class="validate[required] inputText"
				...
				class="validate[custom[email]]" style="width: 46%" /> <span
				class="redspan" style="color: red; padding: 0px; margin-left: 10px;">*</span>
			</td>
		</tr>
	</table>
	...
	<table class="bs_tb" style="width: 100%; margin-top: 10px;">
		...
		<tr>
			<td
				style="text-align: left; padding-left: 10px; padding-top: 10px; padding-bottom: 10px;">
				<textarea placeholder="&#12288;概述本学科主要技术特色、优势，规模、国内外学术地位等（限1000字内）" class="validate[required] validate[maxSize[1000]] xltxtarea" style="height: 300px;" name="introductionContent">${resultMap.introductionContent}</textarea>
			</td>
		</tr>
	</table>
	...
	<table class="bs_tb" style="width: 100%; margin-top: 10px;">
		...
		<tr>
			<td style="text-align: left; padding-left: 10px; padding-top: 10px; padding-bottom: 10px;">
				<textarea placeholder="&#12288;围绕1-2个主攻方向，概述本学科今后3年在应用基础研究、临床研究方面的建设目标、研究内容、预期成果、年度安排等。（限2000字内）。" style="height: 300px;" class="validate[required] validate[maxSize[2000]] xltxtarea" name="attackContent">${resultMap.attackContent}</textarea>
			</td>
		</tr>
	</table>
