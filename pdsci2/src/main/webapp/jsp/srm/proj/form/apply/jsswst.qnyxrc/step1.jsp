...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
    ...
    <table class="bs_tb" style="width: 100%">
        ...
        <!--
        <tr>
            <td width="15%" style="text-align: center">课题名称：</td>
            <td width="35%" style="text-align: left;padding-left: 10px;">
                <input type="text"  class="inputText" name="topics"
                       value="<c:if test='${empty resultMap.topics and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.topics}</c:if><c:if test='${!empty resultMap.topics}'>${resultMap.topics}</c:if>" style="width: 90%"/>
            </td>
            <td width="15%" style="text-align: center">单位名称：</td>
            <td width="35%" style="text-align: left;padding-left: 10px;">
                <input type="text"  class="validate[custom[chinaZip]] inputText" name="company_name"
                       value="<c:if test='${empty resultMap.company_name and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.company_name}</c:if><c:if test='${!empty resultMap.company_name}'>${resultMap.company_name}</c:if>" style="width: 46%"/>
            </td>
        </tr>
        <tr>
            <td style="text-align: center">单位地址：</td>
            <td style="text-align: left;padding-left: 10px;">
                <input type="text" class="validate[required] inputText" name="company_address"
                       value="<c:if test='${empty resultMap.company_address and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.company_address}</c:if><c:if test='${!empty resultMap.company_address}'>${resultMap.company_address}</c:if>" style="width: 46%"/>

            </td>
            <td style="text-align: center">单位邮编：</td>
            <td style="text-align: left;padding-left: 10px;">
                <input type="text" class="inputText validate[required,custom[phone]]" name="company_postCode"
                       value="<c:if test='${empty resultMap.company_postCode and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.company_postCode}</c:if><c:if test='${! empty resultMap.company_postCode}'>${resultMap.company_postCode}</c:if>" style="width: 46%"/>

            </td>
            <td style="text-align: center">联系人：</td>
            <td style="text-align: left;padding-left: 10px;">
                <input type="text" class="inputText validate[required,custom[phone]]" name="contacts"
                       value="<c:if test='${empty resultMap.contacts and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.contacts}</c:if><c:if test='${! empty resultMap.contacts}'>${resultMap.contacts}</c:if>" style="width: 46%"/>

            </td>
            <td style="text-align: center">联系电话：</td>
            <td style="text-align: left;padding-left: 10px;">
                <input type="text" class="inputText validate[required,custom[phone]]" name="phoneNum"
                       value="<c:if test='${empty resultMap.phoneNum and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.phoneNum}</c:if><c:if test='${! empty resultMap.phoneNum}'>${resultMap.phoneNum}</c:if>" style="width: 46%"/>
