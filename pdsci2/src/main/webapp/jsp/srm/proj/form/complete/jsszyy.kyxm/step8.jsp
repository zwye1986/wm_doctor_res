...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" enctype="multipart/form-data"
    ...
    <table width="100%" cellspacing="0" cellpadding="0" class="bs_tb">
            ...
            <th colspan="2" class="theader">附件信息
                <c:if test="${param.view!=GlobalConstant.FLAG_Y}"><span style="float: right;padding-right: 10px"><a
                        href="javascript:void(0)"><img title="新增"
                                                       ...
                                                       style="cursor: pointer;" onclick="addFile('projFileTb')"/></a>&#12288;<a
        ...
        <c:if test="${param.view != GlobalConstant.FLAG_Y}">
            <tr>
                <td colspan="2" style="text-align: left">
                    <a href="<s:url value="/jsp/srm/proj/form/complete/jsszyy.kyxm/研究病历清单目录.docx" />">[研究病历清单附件模板]</a>
                </td>
            </tr>
        </c:if>
        <tr>
        ...
        <c:forEach var="file" items="${pageFileMap}" varStatus="status">
            <c:choose>
                <c:when test="${param.view eq GlobalConstant.FLAG_Y}">
                    <tr>
                        <td style="text-align: center;">${status.count}</td>
                        <td><input name="file" value="${file.key}" type="hidden"/><a
                ...
                <c:otherwise>
                    <tr>
                        <td style="text-align: center;"><input type="checkbox"/></td>
                        <td style="text-align: left;">&#12288;<input name="file" value="${file.key}" type="hidden"/><a
                                href='<s:url value="/pub/file/down?fileFlow=${file.key}"/>'>${file.value.fileName}</a>
                        </td>
                    </tr>
