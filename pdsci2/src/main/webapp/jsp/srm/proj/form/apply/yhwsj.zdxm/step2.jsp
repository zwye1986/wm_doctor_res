...
<%-- 	${fe.objMap.fileEdit_fileName}--
    ...
    ${fe.flow}--<br/> --%>
   		...
   		<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" enctype="multipart/form-data" style="position: relative;">
			...
			<font style="font-size: 14px; font-weight:bold;color: #333; ">二、附件信息</font>
    			...
    			<tr>
		             ...
		             <td>
			             	<c:if test="${not empty file1}">
								<a href='<s:url value="/pub/file/down?fileFlow=${file1}"/>'>立项查新报告</a>
								<input type="hidden" name="fileEdit_fileRemark" value="1"/>
								<input type="hidden" name="fileEdit_fileName" value="立项查新报告"/>
								<input type="hidden" name="fileEdit_file" value="${file1}"/>
							</c:if>
			             	<c:if test="${empty file1}">立项查新报告</c:if>
		             </td>
		             <td>
						<c:if test="${not empty file1}">
				...
				<tr>
		             ...
		             <td>
			             	<c:if test="${not empty file2}">
								<a href='<s:url value="/pub/file/down?fileFlow=${file2}"/>'>项目可行性报告</a>
								<input type="hidden" name="fileEdit_fileRemark" value="2"/>
							    <input type="hidden" name="fileEdit_fileName" value="项目可行性报告"/>
							    <input type="hidden" name="fileEdit_file" value="${file2}"/>
							</c:if>
			             	<c:if test="${empty file2}">项目可行性报告</c:if>
		             </td>
		             <td>
						<c:if test="${not empty file2}">
							<img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
				...
				<tr>
		             ...
		             <td>
			             	<c:if test="${not empty file3}">
								<a href='<s:url value="/pub/file/down?fileFlow=${file3}"/>'>承诺书</a>
								<input type="hidden" name="fileEdit_fileRemark" value="3"/>
							    <input type="hidden" name="fileEdit_fileName" value="承诺书"/>
							    <input type="hidden" name="fileEdit_file" value="${file3}"/>
							</c:if>
			             	<c:if test="${empty file3}">承诺书</c:if>
		             </td>
		             <td>
						<c:if test="${not empty file3}">
							<img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
