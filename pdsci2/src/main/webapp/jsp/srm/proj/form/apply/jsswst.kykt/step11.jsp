...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" enctype="multipart/form-data" style="position: relative;">
    ...
    <table width="100%" cellspacing="0" cellpadding="0" class="bs_tb" style="margin-top: 10px;">
        <tr>
            <th colspan="5" class="theader">附件信息</th>
        </tr>
        <tr>
            <td width="3%"></td>
            <td width="5%" style="font-weight:bold;padding: 0px;">序号</td>
            <td width="50%" style="font-weight:bold;padding: 0px;">附件内容</td>
            <td width="15%">有（√）</td>
            <c:if test="${param.view!=GlobalConstant.FLAG_Y }">
                <td width="30%">操作</td>
            </c:if>
        ...
        <tr>
            ...
            <td>
                <c:if test="${not empty file1}">
                    <a href='<s:url value="/pub/file/down?fileFlow=${file1}"/>'>身份证、学位证、职称证明文件</a>
                    <input type="hidden" name="fileEdit_fileRemark" value="1"/>
                    <input type="hidden" name="fileEdit_fileName" value="身份证、学位证、职称证明文件"/>
                    <input type="hidden" name="fileEdit_file" value="${file1}"/>
                </c:if>
                <c:if test="${empty file1}">身份证、学位证、职称证明文件</c:if>
            </td>
            <td>
                <c:if test="${not empty file1}">
        ...
        <tr>
            ...
            <td>
                <c:if test="${not empty file2}">
                    <a href='<s:url value="/pub/file/down?fileFlow=${file2}"/>'>文章</a>
                    <input type="hidden" name="fileEdit_fileRemark" value="2"/>
                    <input type="hidden" name="fileEdit_fileName" value="文章"/>
                    <input type="hidden" name="fileEdit_file" value="${file2}"/>
                </c:if>
                <c:if test="${empty file2}">文章</c:if>
            </td>
            <td>
                <c:if test="${not empty file2}">
                    <img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
        ...
        <tr>
            ...
            <td>
                <c:if test="${not empty file3}">
                    <a href='<s:url value="/pub/file/down?fileFlow=${file3}"/>'>查新报告</a>
                    <input type="hidden" name="fileEdit_fileRemark" value="3"/>
                    <input type="hidden" name="fileEdit_fileName" value="查新报告"/>
                    <input type="hidden" name="fileEdit_file" value="${file3}"/>
                </c:if>
                <c:if test="${empty file3}">查新报告</c:if>
            </td>
            <td>
                <c:if test="${not empty file3}">
                    <img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
        ...
        <tr>
            ...
            <td>
                <c:if test="${not empty file4}">
                    <a href='<s:url value="/pub/file/down?fileFlow=${file4}"/>'>单位信用承诺书</a>
                    <input type="hidden" name="fileEdit_fileRemark" value="4"/>
                    <input type="hidden" name="fileEdit_fileName" value="单位信用承诺书"/>
                    <input type="hidden" name="fileEdit_file" value="${file4}"/>
                </c:if>
                <c:if test="${empty file4}">单位信用承诺书</c:if>
            </td>
            <td>
                <c:if test="${not empty file4}">
                    <img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
        ...
        <tr>
            ...
            <td>
                <c:if test="${not empty file5}">
                    <a href='<s:url value="/pub/file/down?fileFlow=${file5}"/>'>其他</a>
                    <input type="hidden" name="fileEdit_fileRemark" value="5"/>
                    <input type="hidden" name="fileEdit_fileName" value="其他"/>
                    <input type="hidden" name="fileEdit_file" value="${file5}"/>
                </c:if>
                <c:if test="${empty file5}">其他</c:if>
            </td>
            <td>
                <c:if test="${not empty file5}">
                    <img src="<s:url value='/css/skin/${skinPath}/images/gou.gif'/>"/>
