...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
    <input type="hidden" name="pageName" value="step0"/>
    ...
    <table  class="bs_tb" style="width: 100%">
        ...
        <tr>
            <td width="200px;" style="text-align: center">申&nbsp;请&nbsp;者：</td>
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                <input type="text" class="validate[required] inputText" name="applName" style="width: 46%"
                       value="<c:if test='${empty resultMap.applName and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.applName}</c:if><c:if test='${! empty resultMap.applName}'>${resultMap.applName}</c:if>"/>
                <span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>
            </td>
        </tr>
        <tr>
            <td width="200px;" style="text-align: center">从事专业：</td>
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                <input type="text" class="validate[required] inputText" name="speName" style="width: 46%"
                       value="<c:if test='${empty resultMap.speName and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.speName}</c:if><c:if test='${! empty resultMap.speName}'>${resultMap.speName}</c:if>"/>
                <span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>
            </td>
        </tr>
        <tr>
            <td width="200px;" style="text-align: center">课题名称：</td>
            <td style="text-align: left;padding-left: 10px;" colspan="3">
        ...
        <tr>
            ...
            <td width="35%" style="text-align: left;padding-left: 10px;">
                <input type="text"  class="inputText" name="jcrcAddress"
                       value="<c:if test='${empty resultMap.jcrcAddress and param.view!=GlobalConstant.FLAG_Y}'>${org.orgAddress}</c:if><c:if test='${!empty resultMap.jcrcAddress}'>${resultMap.jcrcAddress}</c:if>" style="width: 90%"/>
            </td>
            <td width="15%" style="text-align: center">邮政编码：</td>
            <td width="35%" style="text-align: left;padding-left: 10px;">
...
<c:if test="${param.view!=GlobalConstant.FLAG_Y}">
    <div align="center" style="margin-top: 10px">
        ...
        <input id="nxt" type="button" onclick="nextOpt('step1')" class="search" value="下一步"/>
    </div>
</c:if>