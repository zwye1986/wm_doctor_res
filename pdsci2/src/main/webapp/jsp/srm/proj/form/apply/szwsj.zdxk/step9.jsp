...
function addProj(tb,obj){
	...
	'<td><input type="text" class="inputText" name="certificate_awardTime" style="width:80%;" style="width:80%;" onclick="WdatePicker({dateFmt:'+"'yyyy-MM-dd'"+'})" readonly="readonly" /></td>'+
		...
		$("."+tb+"Serial").each(function(){
			...
			$(this).text(serial);
					 ...
					 <form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
						...
						<tr>
							<th colspan="7" class="theader">表10：近五年为第一完成人的发明专利、新药证书共 <input type="text" name="certificateNum" value="${resultMap.certificateNum}" class="validate[custom[integer]] inputText" style="width:30px;"/> 张。（限填10项代表作）
							<c:if test="${param.view!=GlobalConstant.FLAG_Y }">
          				...
          				<c:if test="${empty resultMap.certificate}">
          				   <tr>
					           ...
					           <td width="20%">
					           		<input type="text" class="inputText" name="certificate_awardTime"  style="width:80%;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
					           </td>
					           <td>
						...
						<c:forEach var="certificate" items="${resultMap.certificate}" varStatus="status">
					        <tr>
					           ...
					           <td width="20%">
					           		<input type="text" class="inputText" name="certificate_awardTime" value="${certificate.objMap.certificate_awardTime}" style="width:80%;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
					           </td>
					           <td>
