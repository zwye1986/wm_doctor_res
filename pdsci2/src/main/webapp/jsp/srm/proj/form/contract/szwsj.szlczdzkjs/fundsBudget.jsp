  					...
  					<table width="100%" cellspacing="0" cellpadding="0" class="basic" style="margin-top: 10px;">
						...
						<tr>
							<td style="text-align: center;">合      计：</td>
  							<td>
  								<input name="amountFund" value="${resultMap.amountFund}" type="text" class="inputText validate[custom[number]]" style="margin-right: 0px; width:26%; margin-left: 22px;" onchange="calculate()"/>（万元）
  							</td>
						</tr>
					</table>
					...
					<table width="100%" cellspacing="0" cellpadding="0" class="basic" style="border-top: none;">
						<tr>
							...
							<td style="text-align: center;" colspan="2">
				                ...
				                <input name="year1" class='inputText'  type="text" <c:if test='${empty resultMap.year1 and param.view != GlobalConstant.FLAG_Y}'>value="${bb}"</c:if><c:if test='${!empty resultMap.year1}'>value="${resultMap.year1}"</c:if>  />年
				            </td>
				            <td style="text-align: center;" colspan="2">
				            	<input name="year2" class='inputText' type="text" <c:if test='${empty resultMap.year2 and param.view != GlobalConstant.FLAG_Y}'>value="${bb+1}"</c:if><c:if test='${!empty resultMap.year2}'>value="${resultMap.year2}"</c:if>  />年
				            </td> 
				            <td style="text-align: center;" colspan="2">
				            	<input name="year3" class='inputText' type="text" <c:if test='${empty resultMap.year3 and param.view != GlobalConstant.FLAG_Y}'>value="${bb+2}"</c:if><c:if test='${!empty resultMap.year3}'>value="${resultMap.year3}"</c:if>  />年
				            </td>
						</tr>
						<tr>
						...
						<tfoot>
						<tr>
							<td colspan="7">填表说明</td>
						</tr>
						<tr>
							<td colspan="7">1、仪器设备费：指项目专用仪器的购置费和运杂、包装、安装费、自制仪器设备的材料、配件和外协加工费。</td>
						</tr>
						<tr>
							<td colspan="7">2、大型仪器设备应充分利用本单位、本地区现有条件。</td>
						</tr>
						<tr>
							<td colspan="7">3、交通运输设备不得列入。</td>
						</tr>
						<tr>
							<td colspan="7">4、单台件在二万元以上的仪器设备须附加设备名称、规格、型号、单价、数量。</td>
						</tr>
						<tr>
							<td colspan="7">5、实验材料费：指科研用消耗性材料、试剂、药品等购置等，标本、样品采集加工和运杂包装费。</td>
						</tr>
						<tr>
							<td colspan="7">6、其它费用需说明用途</td>
						</tr>
						</tfoot>
					</table>
               		...
               		<c:if test="${param.view == GlobalConstant.FLAG_Y}">display:none</c:if>">
                	    ...
                	    <input id="nxt" type="button" onclick="nextOpt('zkFinish')" class="search" value="完&#12288;成"/>
					</div>