                ...
                <tr>
                    ...
                    <td>
                        <c:if test="${param.expert != GlobalConstant.FLAG_Y}">
                            <input type="text" name="mainResearcher_userName"
                                   value="${mr.objMap.mainResearcher_userName}" class="inputText" style="width: 80%"/>
                        </c:if>
                    </td>
                    <td>
                        <select name="mainResearcher_gender" class="inputText" style="width: 80%;">
                            ...
                            <c:forEach var="sex" items="${userSexEnumList}">
                                <c:if test="${sex.id != userSexEnumUnknown.id}">
                                    <option value="${sex.id}"
                                            <c:if test="${mr.objMap.mainResearcher_gender eq sex.id}">selected="selected"</c:if>>${sex.name}</option>
                                </c:if>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <input class="inputText ctime" type="text" name="mainResearcher_birthday"
                               value="${mr.objMap.mainResearcher_birthday}" onClick="WdatePicker({dateFmt:'yyyy-MM'})"
                        ...
                        <select class="inputText" name="mainResearcher_title" style="width: 80%">
                            ...
                            <c:forEach items="${dictTypeEnumUserTitleList}" var="title">
                                <option value="${title.dictId}"
                                        <c:if test='${mr.objMap.mainResearcher_title==title.dictId}'>selected="selected"</c:if>>${title.dictName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <select class="inputText" name="mainResearcher_duty" style="width: 80%">
                            ...
                            <c:forEach items="${dictTypeEnumUserPostList}" var="post">
                                <option value="${post.dictId}"
                                        <c:if test='${mr.objMap.mainResearcher_duty==post.dictId}'>selected="selected"</c:if>>${post.dictName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <c:if test="${param.expert != GlobalConstant.FLAG_Y}">
                            <input type="text" name="mainResearcher_dept" value="${mr.objMap.mainResearcher_dept}"
                                   class="inputText" style="width: 80%"/>
                        </c:if>
                    </td>
                    <td><input type="text" name="mainResearcher_labor" value="${mr.objMap.mainResearcher_labor}"
                               class="inputText" style="width: 80%"/></td>
                    <td><input type="text" name="mainResearcher_month" value="${mr.objMap.mainResearcher_month}"
        ...
        <c:if test="${not empty resultMap.joinUser}">
            <c:forEach var="mr" items="${resultMap.joinUser}" varStatus="status">
                <tr>
                    ...
                    <td>
                        <c:if test="${param.expert != GlobalConstant.FLAG_Y}">
                            <input type="text" name="joinUser_userName" value="${mr.objMap.joinUser_userName}"
                                   class="inputText" style="width: 80%"/>
                        </c:if>
                    </td>
                    <td>
                        <select name="joinUser_gender" class="inputText" style="width: 80%;">
                            ...
                            <c:forEach var="sex" items="${userSexEnumList}">
                                <c:if test="${sex.id != userSexEnumUnknown.id}">
                                    <option value="${sex.id}"
                                            <c:if test="${mr.objMap.joinUser_gender eq sex.id}">selected="selected"</c:if>>${sex.name}</option>
                                </c:if>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <input class="inputText ctime" type="text" name="joinUser_birthday"
                               value="${mr.objMap.joinUser_birthday}" onClick="WdatePicker({dateFmt:'yyyy-MM'})"
                               readonly="readonly" style="width: 80%;"/>
                    </td>
                    <td>
                        <select class="inputText" name="joinUser_title" style="width: 80%">
                            ...
                            <c:forEach items="${dictTypeEnumUserTitleList}" var="title">
                                <option value="${title.dictId}"
                                        <c:if test='${mr.objMap.joinUser_title==title.dictId}'>selected="selected"</c:if>>${title.dictName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <select class="inputText" name="joinUser_duty" style="width: 80%">
                            ...
                            <c:forEach items="${dictTypeEnumUserPostList}" var="post">
                                <option value="${post.dictId}"
                                        <c:if test='${mr.objMap.joinUser_duty==post.dictId}'>selected="selected"</c:if>>${post.dictName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <c:if test="${param.expert != GlobalConstant.FLAG_Y}">
                            <input type="text" name="joinUser_dept" value="${mr.objMap.joinUser_dept}" class="inputText"
                                   style="width: 80%"/>
                        </c:if>
                    </td>
                    <td><input type="text" name="joinUser_labor" value="${mr.objMap.joinUser_labor}" class="inputText"
                               style="width: 80%"/></td>
                    <td><input type="text" name="joinUser_month" value="${mr.objMap.joinUser_month}"
                               class="inputText validate[custom[number]]" style="width: 80%"/></td>
                </tr>
            </c:forEach>
        </c:if>
...
<div style="display: none">
    ...
    <table class="basic" id="mainResearcherTemplate" style="width: 100%">
        <tr>
            ...
            <td>
                <input class="inputText ctime" type="text" name="mainResearcher_birthday"
                       onClick="WdatePicker({dateFmt:'yyyy-MM'})" readonly="readonly" style="width: 80%;"/>
            </td>
            <td>
                <select class="inputText" name="mainResearcher_title" style="width: 80%">
                    <option value="">请选择</option>
            ...
            <td><input type="text" name="mainResearcher_month" class="inputText  validate[custom[number]]"
                       style="width: 80%"/></td>
    ...
    <table class="basic" id="joinUserTemplate" style="width: 100%">
        <tr>
            ...
            <td><input type="text" name="joinUser_month" class="inputText  validate[custom[number]]"
                       style="width: 80%"/></td>
