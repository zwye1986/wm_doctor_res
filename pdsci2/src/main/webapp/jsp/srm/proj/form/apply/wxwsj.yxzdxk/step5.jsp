...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post"
    ...
    <font style="font-size: 14px; font-weight:bold;color: #333;">四、重点学科（发展学科）带头人主要业绩</font>
    <table class="basic" style="width: 100%;margin-top: 10px;">
        <tr>
            <th colspan="6" style="text-align: left;padding-left: 15px;">
                1、近五年为第一完成人的市局级及以上科研成果（含专利、新药证书）共<font color="red">*</font>
                <input type="text" name="scientificResult" class="validate[required,custom[number],min[0]] inputText"
                       value="<c:if test='${empty resultMap.scientificResult and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.scientificResult}</c:if><c:if test='${! empty resultMap.scientificResult}'>${resultMap.scientificResult}</c:if>" style="width: 20px;"/>
                项，限填5项代表作。
                <c:if test="${param.view!=GlobalConstant.FLAG_Y }">
        ...
        <c:if test="${!(param.view eq GlobalConstant.FLAG_Y)}">
            <c:forEach var="scientificResults" items="${resultMap.scientificResults}" varStatus="scientificResultsStatus">
                <tr>
                    ...
                    <td>
                        <input type="text" class="inputText ctime validate[required]" name="scientificResults_grantDate" value="${scientificResults.objMap.scientificResults_grantDate}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
                    </td>
                    <td>
    ...
    <table class="basic" style="width: 100%;margin-top: 10px;">
        <tr>
            <th colspan="7" style="text-align: left;padding-left: 15px;">
                2、近五年为第一负责人的市局级及以上科研课题共<font color="red">*</font>
                <input type="text" name="scientificTopic" class="validate[required] inputText"
                       value="<c:if test='${empty resultMap.scientificTopic and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.scientificTopic}</c:if><c:if test='${! empty resultMap.scientificTopic}'>${resultMap.scientificTopic}</c:if>" style="width: 20px;"/>
                项，限填5项代表作。
                <c:if test="${param.view!=GlobalConstant.FLAG_Y }">
                    <span style="float: right;padding-right: 10px">
    ...
    <table class="basic" style="width: 100%;margin-top: 10px;">
        <tr>
            <th colspan="6" style="text-align: left;padding-left: 15px;">
                ...
                <input type="text" name="learningPaper" class="validate[required] inputText"
                       value="<c:if test='${empty resultMap.learningPaper and param.view!=GlobalConstant.FLAG_Y}'>${resultMap.learningPaper}</c:if><c:if test='${! empty resultMap.learningPaper}'>${resultMap.learningPaper}</c:if>" style="width: 20px;"/>
        ...
        <tr>
            ...
            <td style="text-align: center;" width="5%">序号</td>
            <td style="text-align: center;" width="20%"><font color="red">*</font>发表时间</td>
        ...
        <c:if test="${!(param.view eq GlobalConstant.FLAG_Y)}">
            <c:forEach var="learningPapers" items="${resultMap.learningPapers}" varStatus="learningPapersStatus">
                <tr>
                    ...
                    <td>
                        <input type="text" class="inputText ctime validate[required]" name="learningPapers_publishDate" value="${learningPapers.objMap.learningPapers_publishDate}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
                    </td>
                    <td>
