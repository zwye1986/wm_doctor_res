...
<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
    ...
    <table class="bs_tb" style="width: 100%">
        ...
        <tr>
            ...
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                <input type="text" class="validate[required] inputText" name="topics"
        ...
        <tr>
            ...
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                    <input type="text" class="inputText validate[required]" name="company_postCode"
                       value="<c:if test='${empty resultMap.company_postCode and param.view!=GlobalConstant.FLAG_Y}'>${org.orgZip}</c:if><c:if test='${! empty resultMap.company_postCode}'>${resultMap.company_postCode}</c:if>"
                       style="width: 46%"/>
                <span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>
            </td>
        </tr>
        <tr>
            ...
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                <input type="text" class="inputText validate[required]" name="contacts"
                       value="<c:if test='${empty resultMap.contacts and param.view!=GlobalConstant.FLAG_Y}'>${proj.applyUserName}</c:if><c:if test='${! empty resultMap.contacts}'>${resultMap.contacts}</c:if>"
                       style="width: 46%"/>
                <span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>
            </td>
        </tr>
        <tr>
            ...
            <td style="text-align: left;padding-left: 10px;" colspan="3">
                <input type="text" class="inputText validate[required,custom[phone]]" name="phoneNum"
                       value="<c:if test='${empty resultMap.phoneNum and param.view!=GlobalConstant.FLAG_Y}'>${user.userPhone}</c:if><c:if test='${! empty resultMap.phoneNum}'>${resultMap.phoneNum}</c:if>"
                       style="width: 46%"/>
                <span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>
            </td>
        </tr>
