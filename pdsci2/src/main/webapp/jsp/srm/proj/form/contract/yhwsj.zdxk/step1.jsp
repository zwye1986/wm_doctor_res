...
<script type="text/javascript">
	...
	<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" style="position: relative;"
		...
		<font style="font-size: 14px; font-weight:bold;color: #333;">一、学科基本情况</font>
		<table class="basic" style="width: 100%; margin-top: 10px;">
			<tr>
				<td width="14%" style="text-align: right;">是否原区级重点学科：</td>
				<td width="9%">
					是 <input type="checkbox" name="isOriginal" value="是" onchange="selectSingle(this)" <c:if test="${resultMap.isOriginal eq '是' or applyMap.isOriginal eq '是'}">checked="checked"</c:if> class="inputText" style="margin: 0px;padding: 0px;"/>
					&#12288;否 <input type="checkbox" name="isOriginal" value="否" onchange="selectSingle(this)" <c:if test="${resultMap.isOriginal eq '否' or applyMap.isOriginal eq '否'}">checked="checked"</c:if> class="inputText" style="margin: 0px;padding: 0px;"/>
				</td>
           		<td width="17%" style="text-align: right;">原医学重点学科名称：</td>
				<td colspan="6"><input type="text" name="originalSubjectName" value="${empty resultMap.originalSubjectName?applyMap.originalSubjectName:resultMap.originalSubjectName}" class="inputText" style="width: 90%"/></td>
			</tr>
			<tr>
			...
			<tr>
           		<td colspan="2" style="text-align: right;">学科业务收入（单位：万元）：</td>
				<td ><input type="text" name="businessIncome" value="${resultMap.businessIncome}" onchange="percent()" class="validate[custom[number]] inputText" style="width: 90%"/></td>
           		<td colspan="2" style="text-align: right;" width="22%">医院业务总收入（单位：万元）：</td>
				<td colspan="2" ><input type="text" name="hospitalBusinessTotalIncome" value="${resultMap.hospitalBusinessTotalIncome}" onchange="percent()" class="validate[custom[number]] inputText" style="width: 90%"/></td>
           		<td width="8%" style="text-align: right;">占比%：</td>
			...
			<tr>
           		<td style="text-align: right;">与其他单位协作机制：</td>
           		<td>
           			是 <input type="checkbox" name="isOtherOrgCoordination" value="是" onchange="selectSingle(this)" <c:if test="${resultMap.isOtherOrgCoordination eq '是'}">checked="checked"</c:if> class="inputText" style="margin: 0px;padding: 0px;"/>
           			&#12288;否 <input type="checkbox" name="isOtherOrgCoordination" value="否" onchange="selectSingle(this)" <c:if test="${resultMap.isOtherOrgCoordination eq '否'}">checked="checked"</c:if> class="inputText" style="margin: 0px;padding: 0px;"/>
           		</td>
           		<td style="text-align: right;">已开展学科协作的单位/<br/>希望开展学科协作的单位：</td>
				<td colspan="2" ><input type="text" name="coordinationOrg" value="${resultMap.coordinationOrg}" class="inputText" style="width: 90%"/></td>
           		<td width="10%" style="text-align: right;">协作的学科：</td>
				<td><input type="text" name="coordinationSubject" value="${resultMap.coordinationSubject}" class="inputText" style="width: 90%"/></td>
           		<td style="text-align: right;">协作专家：</td>
				<td><input type="text" name="coordinationSpecialist" value="${resultMap.coordinationSpecialist}" class="inputText" style="width: 90%"/></td>
			</tr>
		</table>
			
		<table width="100%" class="basic" style="border-top-style: none;">
			<tr>
           		<td width="14%" rowspan="2" style="text-align: center;">单位科技管理<br/>部门联系人：</td>
			...
			<tr>
				<td colspan="2"><input type="text" name="manageDeptLinkmanName" value="${resultMap.manageDeptLinkmanName}" class="inputText" style="width: 90%"/></td>
				<td colspan="2"><input type="text" name="manageDeptLinkmanPhone" value="${resultMap.manageDeptLinkmanPhone}" class="validate[custom[phone2]] inputText" style="width: 90%"/></td>
				<td colspan="2"><input type="text" name="manageDeptLinkmanMobilePhone" value="${resultMap.manageDeptLinkmanMobilePhone}" class="validate[custom[mobile]] inputText" style="width: 90%"/></td>
				<td colspan="2"><input type="text" name="manageDeptLinkmanEmail" value="${resultMap.manageDeptLinkmanEmail}" class="validate[custom[email]] inputText" style="width: 90%"/></td>
			</tr>
		</table>
		...
		<table width="100%" class="bs_tb" style="border-top-style: none;">
		    ...
		    <tr>
		       ...
		       <td width="10%">学历</td>
		       <td width="20%">所学专业</td>
		       <td width="13%">专业技术职务</td>
		       <td width="5%">是否博<br/>（硕）<br/>士导</td>
