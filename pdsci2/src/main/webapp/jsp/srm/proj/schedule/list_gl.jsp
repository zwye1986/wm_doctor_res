            ...
            <form id="searchForm"
                ...
                <div class="searchDiv">
                    ...
                    <input type="text" class="xltext ctime" name="startYear" readonly="readonly" style="width: 69px"
                           ...
                           value="${param.startYear }<c:if test="${empty param.startYear and defaultTerm eq 'Y'}">${pdfn:getCurrYear()}</c:if>"/>
                    -
                    <input type="text" class="xltext ctime" name="endYear" readonly="readonly" style="width: 69px"
                           onClick="WdatePicker({dateFmt:'yyyy'})"
                           value="${param.endYear }<c:if test="${empty param.endYear and defaultTerm eq 'Y'}">${pdfn:getCurrYear()}</c:if>"/>
                </div>
                <div class="searchDiv">
                    科&#12288;&#12288;室：
                ...
                <c:if test="${sysCfgMap['srm_for_use'] eq 'local'}">
                    <div class="searchDiv">
                        报告状态：
                ...
                <tr>
                    ...
                    <td>
                        <c:if test="${proj.projStatusId eq 'Submit'}">
                            <a href="javascript:viewScheduleRec('${proj.projFlow}');">[审核]</a>
                        </c:if>
                        <c:if test="${proj.projStatusId ne 'Submit'}">
                            <a href="javascript:viewScheduleRec('${proj.projFlow}');">[查看]</a>
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
