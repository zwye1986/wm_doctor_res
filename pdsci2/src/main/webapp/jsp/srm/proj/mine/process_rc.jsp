...
<div id="main">
   <div class="mainright">
      <div class="content">
	   <div>
	   	<table class="xllist">
	   		<tr><th colspan="4" style="text-align: left;">&#12288;人才信息</th></tr>
			<tr>
	            <td width="200px" style="text-align: left" colspan="2">&#12288;<b>人才名称：</b><a style="color: blue" href="<s:url value='/srm/proj/mine/projView?projFlow=${proj.projFlow}'/>" target="_blank">${proj.projName}</a></td>
	            <td width="200px" style="text-align: left">&#12288;<b>年&#12288;&#12288;份：</b>${proj.projYear}</td>
	            <td width="200px" style="text-align: left">&#12288;<b>人才编号：</b>${proj.projNo}</td>
	        </tr>
		    <tr>
	            <td width="250px" style="text-align: left">&#12288;<b>人才类型：</b>${proj.projTypeName}</td>
		...
		<div class="flow_list" id="icn">
			<ul>
		    	<li class="list_1_${icnMap['0']}" ><span>1</span><br/><a <c:if test="${icnMap['0']=='2'}">onclick="selectTag('0',this)"</c:if> href="javascript:void(0)">人才申报</a></li>
		        ...
		        <li class="list_2_${icnMap['3']}" ><span>4</span><br/><a <c:if test="${icnMap['3']=='2'}">onclick="selectTag('3',this)"</c:if> href="javascript:void(0)">人才进展</a></li>
		        <li class="list_3_${icnMap['4']}" ><span>5</span><br/><a <c:if test="${icnMap['4']=='2'}">onclick="selectTag('4',this)"</c:if> href="javascript:void(0)">人才验收</a></li>
		    </ul>
		</div> 
		<div  id="tagContent">
			<!-- 申报 -->
            <div id="tagContent_0" <c:if test='${empty tab||tab==projStageEnumApply.id}'>style="display: block;"  </c:if> <c:if test='${tab!=projStageEnumApply.id}'>style="display: none;" </c:if>>
			    <c:if test="${projStageEnumApply.id==proj.projStageId and projApplyStatusEnumApply.id==proj.projStatusId}">
			        <a href='<s:url value="/srm/proj/mine/editProjInfo"/>?projFlow=${proj.projFlow}' target="_self" class="butt_see" title="编辑人才基本信息">人才基本信息</a>&nbsp;&nbsp;
            ...
            <!-- 立项开始 -->
            <div  id="tagContent_1" <c:if test='${tab==projStageEnumApprove.id}'>style="display: block;" </c:if> <c:if test='${tab!=projStageEnumApprove.id}'>style="display: none;" </c:if>>
            	<!-- 评审信息开始 -->
           ...
           <c:if test="${!empty projFundPlanList}">
           	<table width="1000" class="xllist">
                 <thead>
                    <tr>
                        <th colspan="14" class="bs_name" style="text-align: left">&#12288;下拨信息(单位：万)</th>
                    </tr>
                 </thead>
                 <tbody>
                 	<tr>
           ...
           <!-- 下坡信息结束 -->
            ...
            <!-- 合同阶段开始 -->
            <div id="tagContent_2" <c:if test='${tab==projStageEnumContract.id}'>style="display: block;" </c:if> <c:if test='${tab!=projStageEnumContract.id}'>style="display: none;" </c:if>>
                <c:if test="${empty contractExists}">该人才类型无需填写合同</c:if>
            ...
            <!-- 实施阶段开始 -->
            <div id="tagContent_3" <c:if test='${tab==projStageEnumSchedule.id}'>style="display: block;" </c:if> <c:if test='${tab!=projStageEnumSchedule.id}'>style="display: none;" </c:if>>
                        ...
                        <tr>
                            ...
                            <td class="xlming">
                                <c:if test='${report.projStageId==projStageEnumSchedule.id and report.projStatusId==projScheduleStatusEnumApply.id}'>
                                    ...
                                    |&#12288;<!-- <a href="javascript:del('${report.recFlow}');">删&#12288;除</a> --><a href="javascript:delRecContent('' , '' , '${report.recFlow}');">清&nbsp;空</a>&#12288;
                                </c:if>
                ...
                <c:if test="${proj.projStageId==projStageEnumSchedule.id}">        
                    <div class="button">
			            <input type="button" onclick="addReport('${proj.projFlow}','${projRecTypeEnumScheduleReport.id }' , '${applicationScope.sysCfgMap['srm_for_use']}');" class="search" value="进展报告"/>
            ...
            <!-- 人才验收开始 -->
            <div id="tagContent_4" <c:if test='${tab==projStageEnumComplete.id}'>style="display: block;"</c:if> <c:if test='${tab!=projStageEnumComplete.id}'>style="display: none;" </c:if>>
                <c:if test="${proj.projStageId==projStageEnumComplete.id and proj.projStatusId==projCompleteStatusEnumApply.id}">
                    ...
                    <c:if test="${projRec.recTypeId==projRecTypeEnumTerminateReport.id}">
                	    <a href="javascript:void(0)" onclick="editTerminateReport('${proj.projFlow}','${projRecTypeEnumTerminateReport.id}' , '${completeFlag}');" class="butt_see" title="中止是指人才因某些原因无法进一步实施，人才承担单位向市卫生局提出中止该人才。">中止申请书</a>    
                    </c:if>
