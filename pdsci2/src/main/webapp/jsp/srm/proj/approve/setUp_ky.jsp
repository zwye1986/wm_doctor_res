...
function ckeckNum(obj){
    var maxLimit = $(obj).val();
    var re =/^[0-9]+.?[0-9]*$/;//正数正则
    var reg = /^([0-9]|[0-9]+.?[0-9]{1,2})$/;
    if(re.test(maxLimit)){
        if(!reg.test(maxLimit)){
	        	  	...
	        	  	<tr>
						...
						<tr>
					  	...
					  	<tr>
							...
							<td>
								下拨金额：<input name="goveFund" id="goveFund" onblur="ckeckNum(this)" class="validate[required,maxSize[10]]"/>
							</td>
							<td>
								配套金额：<input name="orgFund" id="orgFund" onblur="ckeckNum(this)" class="validate[required,maxSize[10]]"/>
							</td>
							<td>
								总金额：<input id="amountFund" name="amountFund" readonly="readonly" class="validate[required,maxSize[10]]"/>
							</td>
						</tr>
						<tr>
	                        <th>
