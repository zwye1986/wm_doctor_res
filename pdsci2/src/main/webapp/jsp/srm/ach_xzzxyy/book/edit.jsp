...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        if (isbnCode == null || isbnCode == "" || isbnCode == undefined) {
            return false;
        }
        var codes = isbnCode.split(" ");
        $("#isbnCode_1").val(codes[0]);
        $("#isbnCode_2").val(codes[2]);
    });
    function buildCode(){
        var isbnCode_1 = $("#isbnCode_1").val();
        var isbnCode_2 = $("#isbnCode_2").val();
        $("#isbnCode").val(isbnCode_1+" 第 "+ isbnCode_2 +" 号");
        validateISBNCode();
    }

    var bookIsExist ;
    function validateISBNCode(){
        var isbnCode = '${book.isbnCode}';
        bookIsExist = false;
        $("#bookMessage").text("");
        var changeCode = $("#isbnCode").val().trim();
        // alert(changeCode);
        if(isbnCode == changeCode){
            return true;
        }else {
            var url = "<s:url value='/srm/ach/book/bookIsExist'/>";
            var data = {"isbnCode":changeCode};
            jboxPost(url, data, function (resp) {
                if(resp){
                    bookIsExist = true;
                    $("#bookMessage").text("该著作已存在，如需增加作者信息，请联系登记人'"+resp.applyUserName+"'！");
                }
            }, null, false);
        }
    }
                ...
                <form id="book" style="position: relative;" enctype="multipart/form-data" method="post"
                    ...
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            ...
                            <th><span class="redspan" style="color: red;padding: 0px;margin-left: 10px;">*</span>出版年份：
                            </th>
                            <td>
                                <input onClick="WdatePicker({dateFmt:'yyyy'})" style="width: 160px;"
                        ...
                        <tr>
                            ...
                            <td>
                                （<input class="validate[required,custom[onlyLetterNumber],maxSize[10]] inputText" id="isbnCode_1" onchange="buildCode()"
                                       style="width:100px;" type="text" />）第
                                （<input class="validate[required,custom[onlyLetterNumber],maxSize[10]] inputText" id="isbnCode_2" onchange="buildCode()"
                                        style="width:100px;" type="text" />）号
                                <p id="bookMessage" style="color: #f00"></p>
                                <input class="validate[required,custom[onlyLetterNumber]] xltext" id="isbnCode" name="isbnCode"
                                       type="hidden" value="${book.isbnCode}"/>
                            </td>
                        </tr>
                        <tr>
                            <th>出版地：</th>
                ...
                <form id="authorList">
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            ...
                            <th width="13%" style="text-align: center;">编委会</th>
                            <th width="8%" style="text-align: center;">性别</th>
                            <th width="10%" style="text-align: center;">学历</th>
                            <th width="12%" style="text-align: center;">参编字数(万字)</th>
