...
function saveAudit(agreeFlag,bookFlow){
	var tip =  agreeFlag=="${GlobalConstant.FLAG_Y}"?"审核通过":"退回";
	var url = "<s:url value='/srm/ach/book/saveAudit'/>?agreeFlag="+agreeFlag+"&bookFlow="+bookFlow;

	jboxConfirm("确认"+tip+"?" ,  function(){
        if(!saveBookAndAuthor()) return false;
		jboxStartLoading();
...
function updateEdit(agreeFlag,tip) {
	jboxConfirm("确认" + tip + "?", function () {
		if (saveBookAndAuthor(agreeFlag)) {
			jboxTip("保存成功");
...
<div class="mainright">
    <div class="content">
		...
		<c:if test="${book.operStatusId ne achStatusEnumFirstAudit.id}">
		<h2>审核意见：</h2>
		<hr/>
		<form method="post" id="auditForm">
			<div style="text-align: center;">
				<textarea id="auditContent" name="auditContent" style="width: 100%" rows="7"></textarea><br/><br/><br/>
		     	<input class='search' onclick="saveAudit('${GlobalConstant.FLAG_Y}','${book.bookFlow }')" type='button'  value='同&#12288;意' />&#12288;
			 	<input class='search' onclick="saveAudit('${GlobalConstant.FLAG_N}','${book.bookFlow }')" type='button' value='退&#12288;回'/>&#12288;
			</div>
		</form>
		</c:if>
