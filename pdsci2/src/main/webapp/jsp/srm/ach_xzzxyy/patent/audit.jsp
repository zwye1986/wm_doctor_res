...
function saveAudit(agreeFlag,patentFlow){
	var tip =  agreeFlag=="${GlobalConstant.FLAG_Y}"?"审核通过":"退回";
	var url = "<s:url value='/srm/ach/patent/saveAudit'/>?agreeFlag="+agreeFlag+"&patentFlow="+patentFlow;

	jboxConfirm("确认"+tip+"?" ,  function(){
        if(!savePat()) return false;
...
<div class="mainright">
    <div class="content">
	<!-- <iframe id="view" width="900" height="800"></iframe> -->
	...
	<jsp:include page="edit.jsp">
		...
		<c:if test="${patent.operStatusId ne achStatusEnumFirstAudit.id}">
		<h2>审核意见：</h2>
		<hr/>
		<form method="post" id="auditForm">
			<div style="text-align: center;">
				<textarea id="auditContent" class="validate[maxSize[100]]" name="auditContent" style="width: 100%" rows="7"></textarea><br/><br/><br/>
		     	<input class='search' onclick="saveAudit('${GlobalConstant.FLAG_Y}','${patent.patentFlow }')" type='button'  value='同&#12288;意' />&#12288;
			 	<input class='search' onclick="saveAudit('${GlobalConstant.FLAG_N}','${patent.patentFlow }')" type='button' value='退&#12288;回'/>&#12288;
			</div>
		</form>
		</c:if>
