...
<head>
    ...
    <script type="text/javascript">
        ...
        function hospitalFund(fundFlow) {
            var width = $('#mainright').width();
            jboxStartLoading();
            jboxOpen("<s:url value='/srm/surplus/hospitalFund/${sessionScope.projListScope}'/>?fundFlow=" + fundFlow, "经费管理", width, 500);
        }
...
<div class="mainright" id="mainright">
    <div class="content">
        ...
        <table class="xllist" style="margin-top: 20px">
            <thead>
            <tr>
                <th width="5%">序号</th>
                <th width="15%">账户编号</th>
                <th width="10%">经费总额(元)</th>
                <th width="10%">已报销金额(元)</th>
                <th width="10%">剩余金额(元)</th>
                <th width="20%">操&#12288;作</th>
            </tr>
            </thead>
            <c:forEach items="${fundInfoList}" var="fundInfo" varStatus="status">
                <tr>
                    <td>${status.count}</td>
                    ...
                    <td>
                        <c:if test="${sessionScope.projListScope eq 'local'}">
                            <a href="javascript:void(0)" onclick="hospitalFund('${fundInfo.fundFlow}')">经费查询</a>&#12288;
                        </c:if>
                        <c:if test="${sessionScope.projListScope eq 'finance'}">
                            <a href="javascript:void(0)" onclick="hospitalFund('${fundInfo.fundFlow}')">经费管理</a>&#12288;
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
