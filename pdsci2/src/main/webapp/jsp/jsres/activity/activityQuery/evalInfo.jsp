...
<head>
    ...
    <script>
        ...
        function save() {
            ...
            var remarks=inputs["${t.targetFlow}"]||'';
            var ele = $("${t.targetFlow}");
            if(param==0 && ele==undefined)
            {
                ...
                eval.evalRemarks=remarks;
                eval.recordStatus="Y";
                eval.isText="${t.isText}";
                evals.push(eval);
            }
            </c:forEach>
            var param={
            ...
            jboxStartLoading();
            jboxPost("<s:url value='/jsres/activityQuery/saveEvalInfo'/>",param,function(resp){
                jboxEndLoading();
    ...
    <script type="text/javascript">
        ...
        // 文本框输入，将内容保存到 inputs   06d0e5aa407a4e728ab1b24143f7fece："adhgasd"
        function writeRemarks(obj,targetFlow) {
            inputs[targetFlow]=$(obj).val();
        }
    </script>
</head>
<body>
    ...
    <div class="basic">
        ...
        <form id="addForm">
            <table class="grid" style="width:100%; margin-bottom: 10px; margin-top: 10px;">
                <tr>
                    <td style="text-align: center;width: 45%" >指标</td>
                    <td style="text-align: center;width: 20%" colspan="2">评分&#12288;&#12288;&#12288;</td>
                        ...
                        <tr>
                            ...
                            <c:if test="${t.isText eq 'N'}">
                                ...
                                <td style="text-align: center">
                                    <textarea onchange="writeRemarks(this,'${t.targetFlow}');"
                                            class="validate[maxSize[100]]" placeholder="请输入备注" style="height: 60px;width: 100%"></textarea>
                                </td>
                            </c:if>
                            <c:if test="${t.isText eq 'Y'}">
                                <td colspan="3">
                                    <textarea onchange="writeRemarks(this,'${t.targetFlow}');" id="${t.targetFlow}"
                                            class="validate[maxSize[100]]" placeholder="请输入内容" style="height: 60px;width: 100%"></textarea>
                                </td>
                            </c:if>
                        </tr>
                    </c:forEach>
