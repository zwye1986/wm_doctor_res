...
<script type="text/javascript">
	...
	function toPage(page) {
		...
		jboxStartLoading();
		jboxPostLoad("doctorListZi","<s:url value='/jsres/doctorActivityStatistics/list'/>?roleFlag=${roleFlag}",$("#searchForm").serialize(),false);
	}
				...
				<c:otherwise>
					<table style="width:100%">
						<tr>
							...
							<td>
								<input id="trainOrg" oncontextmenu="return false"  class="toggleView input" type="text" autocomplete="off" value="${gxOrg.orgName}" style="margin-left: 0px;width: 130px"  onkeydown="changeStatus();" onkeyup="changeStatus();" />
									...
									<div class="boxHome trainOrg" id="trainOrgSel" style="max-height: 250px;overflow: auto; border: 1px #ccc solid;background-color: white;min-width: 140px;border-top: none;position: relative;display:none;">
										<c:forEach items="${orgs}" var="org">
											<p class="item trainOrg allOrg orgs"  flow="${org.orgFlow}" value="${org.orgName}" <c:if test="${empty org.orgLevelId}">type="allOrg"</c:if><c:if test="${!empty org.orgLevelId }">type="${org.orgLevelId}"</c:if> style="line-height: 20px; padding:10px 0;cursor: default; "
											   <c:if test="${gxOrg.orgFlow==org.orgFlow }">style="display: none;"</c:if>
											>${org.orgName}</p>
										</c:forEach>
									</div>
									<input type="text" name="orgFlow" id="orgFlow" value="${gxOrg.orgFlow}" style="display: none;"/>
								</div>
							</td>
