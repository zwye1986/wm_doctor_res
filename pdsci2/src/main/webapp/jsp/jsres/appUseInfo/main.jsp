	...
	<script type="text/javascript">
		...
		function appUser(tag){
			...
			if("${param.userListScope}"=="local"){
				jboxLoad("hosContent","<s:url value='/jsres/statistic/statisticsAppUser?userListScope=${param.userListScope}'/>",true);
			}else if("${param.userListScope}"=="global"||"${param.userListScope}"=="charge") {
				jboxLoad("hosContent", "<s:url value='/jsres/statistic/statisticsAppUserForOrg'/>", true);
			}
		}
		function appUserTable(tag){
			...
			$(tag).removeClass('tab');
			$(tag).addClass('tab_select');
			jboxLoad("hosContent","<s:url value='/jsres/appUseInfo/appUserTable?userListScope=${param.userListScope}'/>",true);
			jboxStartLoading();
		}
	</script>
</head>
