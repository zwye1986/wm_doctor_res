...
//				return false;
	...
	function chargeResetOrg(recruitFlow,doctorFlow){
		if($("#returnNotice").is(":hidden")){
			...
			var globalNotice=$("#globalNotice").val();
...
// 	jboxOpenContent2($("#returnData").html(),"提示",350,230,true);
	...
	function saveAuditRecruit(recruitFlow, doctorFlow, auditFlag){
		...
		jboxConfirm("确认审核"+title+"?" ,  function(){
			var url = "<s:url value='/jsres/manage/local/doctor/saveAuditRecruit'/>";
    ...
    function saveGlobalAuditRecruit(recruitFlow, doctorFlow, auditFlag){
        ...
        jboxConfirm("确认审核"+title+"?" ,  function(){
            var url = "<s:url value='/jsres/manage/saveGlobalAuditRecruit'/>";
            jboxPost(url, data, function(resp){
                if("${GlobalConstant.OPRE_SUCCESSED}" == resp){
                	debugger
					window.parent.searchDoctor('${doctorSignupFlag}');
	...
	function editDoctorRecruit(recruitFlow,doctorFlow){
		if(doctorFlow == ""){
			jboxTip("请先完善基本信息！");
			return false;
		}
		var url = "<s:url value='/jsres/doctor/saveDoctorCatSpe2'/>?doctorFlow="+doctorFlow+"&recruitFlow="+recruitFlow;
		var title = "编辑";
...
<!-- <div class="infoAudit"> -->
<div>
	<div class="${'open' eq param.openType?'infoAudit':'main_bd' }" style="max-height: 500px">
		<div class="div_table">
			<!-- 		 <input type="hidden" id="proveFileUrlFlag"/> -->
			<!-- 	     <input type="hidden" id="completeFileUrlFlag"/> -->
			<input type="hidden" id="upFileId"/>
			<h4>培训信息
				<c:if test="${nowYear == doctorRecruit.sessionNumber}">
					<c:if test="${sessionNumber != doctorRecruit.sessionNumber}">
						<span style="color: red">(届别有误，请重新编辑！)</span>
					</c:if>
				</c:if>
			</h4>
			...
			<form id="infoForm" style="position: relative;" method="post">
				<table border="0" cellpadding="0" cellspacing="0" class="base_info">
					...
					<c:if test="${doctorRecruit.catSpeName eq '助理全科'}">
						<tr>
							<th style="${auditNotPassed?'color: red':'' }">审核状态：</th>
							<c:set var="modifyTime" value="${pdfn:transDateTime(jsresRecruitDocInfo.modifyTime)}"></c:set>
							<c:set var="modifyTime2" value="${pdfn:transDateTime(doctorRecruit.modifyTime)}"></c:set>
					...
					<tr>
						...
						<td>
							...
							<c:choose>
								...
								<c:otherwise>
									<c:set var="graduactionYear" value="${doctorRecruit.sessionNumber + 3}" />
								</c:otherwise>
							</c:choose>
					...
					<c:if test="${not empty doctorRecruit.globalNotice}">
						<tr>
							<th style="color: red;">省厅<c:if test="${GlobalConstant.USER_LIST_PERSONAL != sessionScope.userListScope}">历史</c:if>审核意见：</th>
							<td colspan="3" class="red">${doctorRecruit.globalNotice}</td>
						</tr>
					</c:if>
					<%-- 	           </c:if> --%>
					...
					<c:if test="${param.auditFlag == GlobalConstant.FLAG_Y and (('WaitGlobalPass' eq doctorRecruit.auditStatusId  and param.exType ne 'view') or param.exType eq 'reExamine')
					                and GlobalConstant.USER_LIST_GLOBAL eq sessionScope.userListScope}">
						<tr>
							<th>省厅审核意见：</th>
							<td colspan="3">
								<textarea class="validate[required] xltxtarea" style=" margin: 0px; padding: 0px;" id="globalNotice" name="globalNotice"></textarea>
							</td>
						</tr>
					</c:if>
					<c:if test="${GlobalConstant.USER_LIST_CHARGE eq sessionScope.userListScope || GlobalConstant.USER_LIST_GLOBAL eq sessionScope.userListScope }">
						<tr id="returnNotice">
							<th>退回意见：</th>
							<td colspan="3">
								<textarea class="validate[required] xltxtarea" id="globalNotice"></textarea>
							</td>
						</tr>
					</c:if>
					</tbody>
			...
			<c:if test="${empty param.isRetrunShow}">
					...
					<div align="center" style="margin-top: 20px; margin-bottom:20px;">
						<c:if test="${param.exType ne 'view'}">
							...
							<c:if test="${empty sysCfgMap['jsres_is_train'] or sysCfgMap['jsres_is_train'] eq 'Y'}">
								<input type="button" class="btn_green" onclick="editDoctorRecruit('${doctorRecruit.recruitFlow}','${doctorRecruit.doctorFlow}');" value="编辑"/>&nbsp;
							</c:if>
				...
				<c:if test="${'WaitGlobalPass' eq doctorRecruit.auditStatusId and GlobalConstant.USER_LIST_GLOBAL eq sessionScope.userListScope}">
					<div class="btn_info">
						<c:if test="${param.exType eq 'examine'}">
							<input type="button" style="width:100px;" class="btn_green" onclick="saveGlobalAuditRecruit('${doctorRecruit.recruitFlow}','${doctorRecruit.doctorFlow}','${GlobalConstant.FLAG_Y}')" value="通过"></input>
							<input type="button" style="width:100px;" class="btn_red" onclick="saveGlobalAuditRecruit('${doctorRecruit.recruitFlow}','${doctorRecruit.doctorFlow}','${GlobalConstant.FLAG_N}')" value="不通过"></input>
						</c:if>
						<c:if test="${param.exType ne null}">
							<input type="button" style="width:100px;" class="btn_green" onclick="jboxClose();" value="关闭"></input>
						</c:if>
				...
				<%--<c:if test="${jszyResDoctorAuditStatusEnumPassed.id eq doctorRecruit.auditStatusId and GlobalConstant.USER_LIST_LOCAL eq sessionScope.userListScope}">--%>
					<%--<div class="btn_info">--%>
						<%--<c:if test="${param.exType eq 'reExamine'}">--%>
							<%--<input type="button" style="width:100px;" class="btn_red" onclick="saveAuditRecruit('${doctorRecruit.recruitFlow}','${doctorRecruit.doctorFlow}','${GlobalConstant.FLAG_N}')" value="不通过"/>--%>
						<%--</c:if>--%>
