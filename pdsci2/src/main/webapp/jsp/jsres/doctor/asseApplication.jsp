				...
				<tr>
					...
					<td>
						<select <c:if test="${(empty jsresGraduationApply.auditStatusId) or (jsresGraduationApply.auditStatusId eq 'Auditing') or (jsresGraduationApply.auditStatusId eq 'WaitChargePass') or (jsresGraduationApply.auditStatusId eq 'WaitGlobalPass')
							...
							<option value="2" <c:if test="${not empty userResumeExt.registeManua and userResumeExt.registeManua eq GlobalConstant.NORMAL}">selected="selected"</c:if>>正常</option>
						</select>
					</td>
					<th>实际轮转时间（月）：</th>
					<td>
...
<%--				</tr>--%>
	...
	<div style="text-align: center;padding-top: 20px;padding-bottom: 50px;">
		<c:if test="${'Y'eq apply}">
			<input type="button" class="btn_green" id="submitBtn" onclick="apply('${resDoctor.doctorFlow}','${canApply}');" value="提交"/>&#12288;&#12288;
		</c:if>
        <%-- 禅道需求449
        ...
        <%-- 结业申请审核状态 --%>
        <c:set var="auditStatusId" value="${not empty jsresGraduationApply.auditStatusId and (jsresGraduationApply.auditStatusId eq 'LocalNotPassed') or (jsresGraduationApply.auditStatusId eq 'ChargeNotPassed')}"></c:set>

		<c:if test="${applyTime and  auditStatusId}">
			<input type="button" class="btn_green" id="submitBtn2" onclick="apply2('${resDoctor.doctorFlow}','${jsresGraduationApply.applyYear}');" value="重新提交"/>
		</c:if>
		<%--<c:if test="${not empty jsresGraduationApply.globalReason}">--%>
