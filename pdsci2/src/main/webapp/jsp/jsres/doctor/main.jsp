...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        <c:if test="${empty recruitList or recruitList.get(recruitList.size()-1).doctorStatusId eq '21' or isRetrain eq 'Y'  or (recruitList.get(recruitList.size()-1).auditStatusId eq 'NotPassed' and recruitList.get(recruitList.size()-1).signupWay eq 'DoctorSignup')}">
        $("#addRecruitBtn").show();
        </c:if>
    });
    ...
    function editDoctorRecruit(){
        ...
        var url = "<s:url value='/jsres/doctor/saveDoctorCatSpe2'/>?doctorFlow="+doctorFlow;
        var title = "新增";
    ...
    function backTrain(userFlow){
        var url = "<s:url value='/jsres/doctor/backTrainInfo?currentPage=1&userFlow='/>"+userFlow;
        jboxLoad("doctorContent", url, true);
    }
...
<div class="main_hd">
    <h2>培训信息&#12288;&#12288;
        <c:if test="${empty sysCfgMap['jsres_is_train'] or sysCfgMap['jsres_is_train'] eq 'Y'}">
            <c:if test="${empty recruitList or recruitList.get(recruitList.size()-1).doctorStatusId eq '21' or (recruitList.get(recruitList.size()-1).auditStatusId eq 'NotPassed' and recruitList.get(recruitList.size()-1).signupWay eq 'DoctorSignup') or isRetrain eq 'Y'}">
                <input class="btn_green" type="button" id="addRecruitBtn" onclick="editDoctorRecruit('');" value="添加培训记录" style="display: none;">
            </c:if>
        </c:if></h2>
    <div class="title_tab" id="toptab">
        <ul>
            ...
            <c:forEach items="${recruitList}" var="recruit">
                <c:if test="${recruit.signupWay eq 'DoctorSend'}">
