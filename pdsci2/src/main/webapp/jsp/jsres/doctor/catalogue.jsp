...
<div class="main_hd">
    <div class="title_tab" style="margin-top: 10px;">
        <ul id="reducationTab">
            <li class="${param.recTypeId eq resRecTypeEnumCaseRegistry.id?'tab_select':'tab'}"
                onclick="setType('${resRecTypeEnumCaseRegistry.id}');"><a>${resRecTypeEnumCaseRegistry.name}</a></li>
            <li class="${param.recTypeId eq resRecTypeEnumDiseaseRegistry.id?'tab_select':'tab'}"
                onclick="setType('${resRecTypeEnumDiseaseRegistry.id}');"><a>${resRecTypeEnumDiseaseRegistry.name}</a>
            </li>
            <li class="${param.recTypeId eq resRecTypeEnumSkillRegistry.id?'tab_select':'tab'}"
                onclick="setType('${resRecTypeEnumSkillRegistry.id}');"><a>${resRecTypeEnumSkillRegistry.name}</a></li>
            <li class="${param.recTypeId eq resRecTypeEnumOperationRegistry.id?'tab_select':'tab'}"
                onclick="setType('${resRecTypeEnumOperationRegistry.id}');">
                <a>${resRecTypeEnumOperationRegistry.name}</a></li>
            <li class="${param.recTypeId eq resRecTypeEnumCampaignRegistry.id?'tab_select':'tab'}"
                onclick="setType2('${resRecTypeEnumCampaignRegistry.id}');">
                <a>${resRecTypeEnumCampaignRegistry.name}</a></li>
            <li class="${param.recTypeId eq resRecTypeEnumAfterEvaluation.id?'tab_select':'tab'}"
                onclick="setType('${resRecTypeEnumAfterEvaluation.id}');"><a>${resRecTypeEnumAfterEvaluation.name}</a>
            </li>
...
<div class="div_search">
    <form id="searchForm" action="<s:url value='/jsres/manage/catalogue'/>" method="post">
        ...
        <input type="hidden" id="schRotationDeptFlow" name="schRotationDeptFlow" value="${param.schRotationDeptFlow}">
    </form>
</div>
<div class="search_table" style="overflow-x: auto;height: 380px;">
    <c:if test="${param.recTypeId==resRecTypeEnumCampaignRegistry.id}">
        <table border="0" cellpadding="0" cellspacing="0" class="grid" id="myTable">
            <thead>
            <tr style="">
                <th>时间</th>
                <th>活动形式</th>
    ...
    <c:if test="${param.recTypeId==resRecTypeEnumTeacherGrade.id||param.recTypeId==resRecTypeEnumDeptGrade.id}">
        <c:if test="${not empty resRecList}">
                    ...
                    <c:forEach items="${title.itemList}" var="item">
                        ...
                        <tr>
                            <td style="text-align: left;width: 100px;">
                                &#12288;&#12288;分值：${(assessCfg.assessTypeId eq resAssessScoreTypeEnumPercentile.id)?item.score:9}<input
                                    type="hidden" name="assessId" value="${item.id}"/></td>
                            <td style="text-align: left;width: ${(assessCfg.assessTypeId eq resAssessScoreTypeEnumPercentile.id)?150:500}px;">
                                ...
                                <input id="${item.id}scoreSel" name="score" type="hidden"
                                       value="${gradeMap[item.id]['score']}"/>
                            </td>
            ...
            <div style="padding-left: 140px;padding-top: 5px;">总分：<input style="width:100px;" type="text"
                                                                         name="totalScore" id="totalScore"
                                                                         class="inputText"
                                                                         value="${empty gradeMap['totalScore']?0:gradeMap['totalScore']}"
...
<div style="text-align: center;"><input type="button" class="btn_green" value="关&#12288;闭" onclick="top.jboxClose();"/>
</div>
</body>
</html>