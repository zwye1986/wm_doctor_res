...
<head>
	...
	<script type="text/javascript">
		...
		function searchResOrgSpeList(){
			...
			//清空原来协同基地！！！
			$("select[name=orgFlowSub] option[value != '']").remove();
				...
				if(dataObj2.length>0){
					for(var i = 0; i<dataObj2.length; i++){
						...
						if(addFlag){
							...
							$option.attr("value",speId);
							$option.text(speName);
							$("select[name=orgFlowSub]").append($option);
						}
					}
				}
			}, null , false);
		...
		function reportOperation(){
			...
			top.jboxPost(url, $("#editForm").serialize(), function(resp){
				top.jboxEndLoading();
				top.jboxTip(resp);
				setTimeout(function(){
					top.parent.getPlanList('',1);
		...
		function showUploadFileTr(){
			...
			if("" == trainYearId || "${jsResTrainYearEnumThreeYear.id}" == trainYearId){
				$("#proveFileUrlTr").hide();
...
<div class="infoAudit">
	<div class="div_table">
		...
		<form id="editForm" style="position: relative;" method="post">
			...
			<input type="hidden" id="catSpeId" name="catSpeId" value="${doctorRecruit.catSpeId}"/>
			...
			<input type="hidden" id="speId" name="speId" value="${doctorRecruit.speId}"/>
			<%-- 所在地区:市、地区 --%>
		...
		<div align="center" style="margin-top: 20px; margin-bottom:20px;">
			<input type="button" id="saveBtn" class="btn_green" onclick="reportOperation();" value="确认报到"/>&nbsp;
