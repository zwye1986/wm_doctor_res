...
<script type="text/javascript">
	...
	//所有详情
	function showAllInfo(recruitFlow,applyYear)
	{
		var url = "<s:url value='/jsres/asse/showAllInfoMain?recruitFlow='/>"+recruitFlow+"&applyYear="+applyYear;
	...
	<div class="main_bd clearfix">
			<table class="grid" style="width: auto;" id="dataTable">
				...
				<c:forEach items="${list}" var="doctor">
					<tr class="fixTrTd">
						<td style="min-width: 60px; max-width: 60px; " class="by"><a href="javascript:void(0);" onclick="showAllInfo('${doctor.recruitFlow}','${doctor.applyYear}')">${doctor.userName}</a></td>
						...
						<td style="min-width: 200px; max-width: 200px; " class="by"><a href="javascript:void(0);" onclick="showPeiDetail('${doctor.recruitFlow}','${doctor.applyYear}')">${doctor.startDate}~${doctor.endDate}</a></td>
						...
						<td style="min-width: 100px; max-width: 100px; " class="by">
							<c:if test="${ doctor.applyYear eq pdfn:getCurrYear() }">
								<c:if test="${ (empty doctor.globalAuditStatusId) and (doctor.auditStatusId eq jsResAuditStatusEnumWaitChargePass.id)}">
									<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">审核</a>
								</c:if>
								<c:if test="${ (empty doctor.globalAuditStatusId) and (not empty doctor.cityAuditStatusId)}">
