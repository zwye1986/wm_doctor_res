		...
		<table border="0" cellpadding="0" cellspacing="0" class="base_info">
			...
			<tbody>
				<tr>
					...
					<th>是否全科定向学员：</th>
			        <td>&nbsp;<c:if test="${userResumeExt.isGeneralOrderOrientation eq GlobalConstant.FLAG_Y }">是</c:if>
			        	 <c:if test="${userResumeExt.isGeneralOrderOrientation eq GlobalConstant.FLAG_N }">否</c:if>
			        </td>
				</tr>
			     <tr>
			    	<th>本科毕业院校：</th>
		...
		<colgroup>
			...
			<tbody>
			    <tr>
			        ...
			        <th>取得日期：</th>
			        <td colspan="2">${userResumeExt.technologyQualificationDate}</td>
			    </tr>
			     <tr>
			    	<th>执业资格材料：</th>
