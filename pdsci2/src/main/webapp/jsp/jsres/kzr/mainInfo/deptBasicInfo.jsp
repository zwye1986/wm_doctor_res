    ...
    <div class="main_bd" <c:if test="${isJoin eq 'Y'  or isglobal eq 'Y'}"> style="position: relative;overflow-y: auto;height: 600px" </c:if>  >
        <div class="div_table">
            ...
            <c:if test="${viewFlag ne 'Y'}">
                <img src="<s:url value='/jsp/res/images/test.png'/>"  onclick="editInfo('DeptBasicInfo','${deptFlow}');"
        ...
        <%--<div class="div_table">
            ...
            <c:if test="${viewFlag ne 'Y'}">
                <img src="<s:url value='/jsp/res/images/test.png'/>"  onclick="editInfo('DeptBasicInfo','${deptFlow}');"
                     style="cursor: pointer;height: 20px;width: 20px;float: right;margin-top: -30px"/>
            </c:if>
            <table id="cd" style="display: none" border="0" cellspacing="0" cellpadding="0" class="base_info">
                ...
                <tr>
                    <th>承担继续医学教育情况：</th>
                    <td colspan="3"><textarea readonly name="deptBasicInfoForm.cdjjyx">${deptBasicInfoForm.cdjjyx}</textarea></td>
                </tr>
                </tbody>
            </table>
        </div>
