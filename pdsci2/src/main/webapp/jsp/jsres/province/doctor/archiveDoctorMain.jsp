...
<head>
	...
	<script type="text/javascript">
		...
		function getDoctorRecruit(recruitFlow, doctorFlow, backFlag,archiveFlow){
			var url = "<s:url value='/jsres/archive/getDoctorRecruit?info=${param.info}&openType=open&recruitFlow='/>"+recruitFlow+"&doctorFlow="+doctorFlow+"&backFlag="+backFlag+"&archiveFlow="+archiveFlow;
			jboxLoad("doctorContent", url, false);
		}
		function doctorInfo(doctorFlow,archiveFlow){
			var url = "<s:url value='/jsres/archive/doctorInfo'/>?viewFlag=${GlobalConstant.FLAG_Y}&openType=open&userFlow=" + doctorFlow+"&hideApprove=${param.hideApprove}"+"&archiveFlow="+archiveFlow;
			...
			<c:if test="${GlobalConstant.FLAG_Y != studyFlag }">
				...
				<c:forEach items="${recruitList}" var="recruit" varStatus="status">
					<c:if test="${jsResDoctorAuditStatusEnumPassed.id eq recruit.auditStatusId }">
						<li <c:if test="${param.recruitFlow ==recruit.recruitFlow }">class="tab_select"</c:if>id="${recruit.recruitFlow}" onclick="getDoctorRecruit('${recruit.recruitFlow}','${param.doctorFlow}','${status.last}','${recruit.archiveFlow}');"><a>${recruit.catSpeName}（${empty recruit.speName?'--':recruit.speName}）</a></li>
					</c:if>
				</c:forEach>
