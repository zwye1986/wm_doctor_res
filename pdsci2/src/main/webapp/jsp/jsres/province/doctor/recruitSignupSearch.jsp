...
function auditRecruit(recruitFlow, doctorFlow, exType){
	var url = "<s:url value='/jsres/manage/province/doctor/auditRecruitSignup'/>?openType=open&recruitFlow="+recruitFlow+"&doctorFlow="+doctorFlow + "&exType=" + exType;
	jboxOpen(url," 学员报到审核",1050,550);
}
	...
	<form id="searchForm">
		...
		<table class="searchTable">
			...
			<tr>
				...
				<td>
					<select name="auditStatusId" class="select">
						<option value="">请选择</option>
						<c:forEach items="${jsResDoctorAuditStatusEnumList}" var="auditStatusEnum">
							<c:if test="${auditStatusEnum.id ne 'NotSubmit'}">
								<option name="${auditStatusEnum.id}" value="${auditStatusEnum.id}"
										<c:if test="${param.auditStatusId==auditStatusEnum.id || auditStatusEnum.id == 'Auditing'}">selected="selected"</c:if>>${auditStatusEnum.name}</option>
							</c:if>
						</c:forEach>
					</select>
				</td>
