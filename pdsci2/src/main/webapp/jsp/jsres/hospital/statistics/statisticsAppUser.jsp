...
//		        x:'800'
    ...
    function changeTrainSpes(catSpeId, sessionNumber) {
        ...
        jboxLoad("hosContent", "<s:url value='/jsres/statistic/statisticsAppUser'/>?catSpeId=" + catSpeId + "&sessionNumber=" + sessionNumber + "&graduate=" + graduate + "&userListScope=local", true);
        </c:if>
        if ("${GlobalConstant.USER_LIST_LOCAL}" != "${param.userListScope}") {
            var orgFlow = "${param.orgFlow}";
            var url = "<s:url value='/jsres/statistic/statisticsAppUser'/>?open=Y&orgFlow=" + orgFlow + "&catSpeId=" + catSpeId + "&sessionNumber=" + sessionNumber + "&graduate=" + graduate + "&userListScope=";
