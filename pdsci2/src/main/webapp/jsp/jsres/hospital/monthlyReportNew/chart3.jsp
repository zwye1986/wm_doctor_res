...
<script>
    ...
    function search() {
        var url = "<s:url value='/jsres/monthlyReportNew/chart3'/>";
        jboxPost(url,$("#searchForm").serialize(),function (resp) {
            $("#content1").html(resp);
        },null,false)
    }
    function exportChart3(){
        var url="<s:url value='/jsres/monthlyReportNew/exportChart3'/>?"+$("#searchForm").serialize();
        window.open(url);
    }
    function exportChart3Detail(){
        var url="<s:url value='/jsres/monthlyReportNew/exportChart3Detail'/>?"+$("#searchForm").serialize();
        window.open(url);
    }
</script>
