...
<head>
    ...
    <script type="text/javascript">
        ...
        function selectTea(stationFlow,stationName){//房间流水号recordFlow，用来判定编辑则清空$("#teaTD")内容
            ...
            console.log(teaFlows);
            var url ="<s:url value='/jsres/skillTimeConfig/roomTeacher?recordFlow=${param.recordFlow}&stationFlow='/>"+stationFlow+"&stationName="+encodeURI(encodeURI(stationName))+"&exitTeaLst="+exitTeaLst;
            // var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
            // jboxMessager(iframe,null,300,400,false);
            jboxOpen(url,'编辑考官',300,320,true);
        }
...
<div class="mainright">
        <form id="myForm">
            ...
            <table class="grid" style="width:100%; margin-bottom: 10px; margin-top: 30px;">
                <tr>
                    <th style="text-align:right;width:30%;">房间名称：</th>
                    <td><select id="roomFlow" name="roomFlow" style="width:137px;" class="validate[required] select">
                        <option/>
                ...
                <tr>
                    <th style="text-align:right;line-height:18px;">房间考核站点及考官选择：</th>
                    <td>
                        <c:forEach items="${stationList}" var="sta">
                            <input type="checkbox" name="staCheck" value="${sta.stationFlow}" stationName="${sta.stationName}"
                                   onchange="bindSelectTea(this);" disabled="disabled"
                                   ${stationFlow eq sta.stationFlow?'checked':''}>
                                    &nbsp;<span style="cursor:pointer;" class="station">${sta.stationName}</span><span style="padding-left:20px;"></span>
                        </c:forEach>
                    </td>
                </tr>
                <tr>
                    <th style="text-align:right;">考官：</th>
                    <td id="teaTD">
            ...
            <div style="text-align: center;margin-top:20px;">
                ...
                <input type="button" class="btn_green" onclick="jboxCloseMessager2();" value="取&#12288;消"/>
            </div>
        </form>
</div>
</body>
