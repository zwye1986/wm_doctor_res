...
function importExcel(){
	...
	jboxStartLoading();
	var url = "<s:url value='/jsres/hospital/impGradeExcel'/>";
	jboxSubmit(
		$('#planForm'),
		url,
		function(data){
			var length = 100;
			data = eval("("+data+")");
			if (data.errorInfoNum == '1') {
				jboxEndLoading();
				$("#failCount").html(data.errorInfo);
				jboxOpenContent($("#returnData").html(),"提示",400,length,true);
				return;
			}
...
function closeWP(){
	setTimeout("window.parent.toPage(1);jboxClose();", 1000);
}
	...
	<div style="padding:0px 20px;margin-top:20px;">
		...
		<form id="planForm" method="post" enctype="multipart/form-data">
			<table width="100%" class="basic">
				...
				<tr>
					<th>导入模板：</th>
					<td><a href="<s:url value='/jsp/jsres/hospital/achievement/gradeTemp.xlsx'/>">成绩单导入模板.xlsx</a></td>
				</tr>
				<tr>
					<th>备注：</th>
					<td><font color="red">导入成绩前请仔细核对，否则影响总成绩</font></td>
				</tr>
			</table>
			<br/>
			<p style="text-align: center;">
				<input type="button" onclick="importExcel();" value="操作成绩单导入" class="btn_green"/>
			</p>
		</form>
	</div>
</div>
...
<div id="returnData"style="display: none;">
	...
	<div style="bottom:5px;width:350px; text-align:center; position:absolute;">
		<input type="button" style="text-align: center;" class="btn_green" onclick="closeWP();"value='确&#12288;定'/>
	</div>
</div>
</body>
</html>