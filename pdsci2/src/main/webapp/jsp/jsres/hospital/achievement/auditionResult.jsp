...
<script type="text/javascript">
    ...
    function toPage(page) {
        ...
        jboxStartLoading();
        jboxPostLoad("achievementContent","<s:url value='/jsres/hospital/getAchievementDetails?scoreType=audition'/>",$("#searchForm").serialize(),false);
    }
    ...
    function confireResult(recruitFlow,userName){
        jboxButtonConfirm("审核学员"+ userName +"面试成绩？","通过","不通过",function(){
            jboxPost("<s:url value='/jsres/hospital/confireResult'/>",{"recruitFlow":recruitFlow,"auditionStatusId":"Passed"}, function(resp){
                setTimeout(function(){
                    toPage(1);
                },300);
            } , null , true);
            ...
            jboxStartLoading();
            jboxPost("<s:url value='/jsres/hospital/confireResult'/>",{"recruitFlow":recruitFlow,"auditionStatusId":"NotPassed"}, function(resp){
                setTimeout(function(){
                    toPage(1);
                },300);
            } , null , true);
    ...
    function batchAuditOpt(){
        ...
        jboxButtonConfirm("确认一键审核勾选学员面试成绩？","一键通过","一键不通过",function(){

            jboxPost("<s:url value='/jsres/hospital/confireResultAll?recruitFlowList='/>"+recruitFlowList, {"scoreType":"audition","resultFlag":"Passed"}, function(resp){
                setTimeout(function(){
                    toPage(1);
                },300);
            } , null , true);
            ...
            jboxStartLoading();
            jboxPost("<s:url value='/jsres/hospital/confireResultAll?recruitFlowList='/>"+recruitFlowList,{"scoreType":"audition","resultFlag":"NotPassed"}, function(resp){
                setTimeout(function(){
                    toPage(1);
                },300);
            } , null , true);
    ...
    function exportForDetail(){
        ...
        var url = "<s:url value='/jsres/hospital/expertAchievementList?scoreType=audition'/>";
        jboxTip("导出中…………");
            ...
            <input id="auditStatusId" type="hidden" name="auditStatusId" value ="Passed"/>
             ...
             <div class="form_search">
                ...
                <div class="form_item form_item_hide">
                    ...
                    <div class="form_content">
                        <select style="width: 161px" name="auditionStatusId" class="select" >
                            <option value="">全部</option>
                            <option value="Auditing" ${param.auditionStatusId eq 'Auditing'?'selected':''}>待审核</option>
                            <option value="Passed" ${param.auditionStatusId eq 'Passed'?'selected':''}>审核通过</option>
                            <option value="NotPassed" ${param.auditionStatusId eq 'NotPassed'?'selected':''}>审核不通过</option>
                        </select>
                    </div>
                ...
                <div class="form_item form_item_hide" style="width:400px ;">
                 ...
                 <a style="color: #54B2E5;float: right; margin: auto 0 auto 15px;" onclick="showOrHide()" id="open">展开</a>
...
<%--                <div>--%>
<%--                    <label class="form_label">证件号：</label>--%>
<%--                    <input style="width: 161px" type="text" name="idNo" value="${param.idNo}" class="input"/>--%>
<%--                </div>--%>
<%--            </div>--%>
...
<%--                    <td >--%>
<%--                        <select name="auditionStatusId" class="select" >--%>
<%--                            <option value="">全部</option>--%>
<%--                            <option value="Auditing" ${param.auditionStatusId eq 'Auditing'?'selected':''}>待审核</option>--%>
<%--                            <option value="Passed" ${param.auditionStatusId eq 'Passed'?'selected':''}>审核通过</option>--%>
<%--                            <option value="NotPassed" ${param.auditionStatusId eq 'NotPassed'?'selected':''}>审核不通过</option>--%>
            ...
            <table border="0" cellpadding="0" cellspacing="0" class="grid">
                ...
                <c:forEach items="${recruitList}" var="recruit" varStatus="a">
                    <tr>
                        ...
                        <td>${empty recruit.auditionResult ?"-":recruit.auditionResult}</td>
                        <td>
                                ${empty recruit.auditionStatusName?"待审核":recruit.auditionStatusName}
                        </td>
                        <c:choose>
                            <c:when test="${recruit.auditionStatusId eq 'Auditing' or empty recruit.auditionStatusId }">
