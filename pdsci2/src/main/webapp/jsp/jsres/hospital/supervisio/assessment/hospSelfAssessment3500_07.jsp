...
<head>
    ...
    <script type="text/javascript">
        ...
        function subInfo() {
            ...
            window.parent.frames["jbox-message-iframe"].$("#scoreInfo62")[0].value = allScore;
            var scoreOneAll = 0;
            var oneNum = 0;
...
<div class="div_table" style="overflow: auto;max-height: 570px;">
    <table cellpadding="4" style="width: 1000px">
        ...
        <tr height="34" class="firstRow">
            <th colspan="5">
                <h2 style="font-size:150%">基本技能操作评分表（气管插管术）</h2>
            </th>
        </tr>
        <tr height="28">
        ...
        <tr>
            <th rowspan="3">物品准备</th>
            <td>洗手,戴帽子、ロ罩,签署知情同意书；核对病人身份,评估患者状态, 判断是否存在困难插管可能性(口述完成)</td>
        ...
        <tr >
            <td>根据患者情况选择气管导管,并检查气囊通畅,无漏气，准备喉镜,选择镜片,检查灯光</td>
            <td style="text-align: center">5</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr >
            <td>其他：手套、吸痰器、球囊面罩、气插导芯、注射器、胶布、牙垫、听诊器</td>
            <td style="text-align: center">5</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr >
            <td rowspan="6">操作</td>
            <td>1.将患者仰卧,头后仰,颈上抬,使口、咽部和气管成一直线以便直视插管。球囊/面罩给氧(口述通气2分钟)</td>
        ...
        <tr >
            <td>2.右手拇指推开患者下唇和下领,食指抵住门齿,必要时使用开口器,清除呼吸道内异物</td>
            <td style="text-align: center">10</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr >
            <td>3.左手持喉镜沿右侧口角进入口腔,压住舌背,显露悬雍垂。慢推镜达舌根,见到会厌,上提镜片显露声门</td>
            <td style="text-align: center">10</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr >
            <td>
                4.气管导管沿喉镜压舌板凹槽放入,到声门时轻旋导管进入气管,同时取出导芯,把气管导管轻轻送入,注意插管深度,安置牙垫,拔出喉镜。
                (置入导芯超过管尖扣5分；各类插管失败不得分；再次插管未先通气扣5分)
            </td>
            <td style="text-align: center">15</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
                    <input onchange="saveScore(this,15);" itemId="7" name="first" class="input" type="text" value="${scoreMap['7']}" style="height:25px; text-align: center"/>
                </c:if>
                <c:if test="${type ne 'Y'}">
                    ${scoreMap['7']}
                </c:if>
        ...
        <tr >
            <td>
                5.先向导管前端气囊内充气3-5ml,再用简易人工呼吸器压入气体,观察胸廓起伏情况,或者用听诊器听双肺呼吸音有无对称,以确定导管已在气管内
                (未发现插入气管并固定,不得分)
            </td>
            <td style="text-align: center">15</td>
            <td style="text-align: center">
