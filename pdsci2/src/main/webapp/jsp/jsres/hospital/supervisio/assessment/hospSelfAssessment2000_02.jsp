...
<div class="div_table" style="overflow: auto;max-height: 600px;">
    <table cellpadding="4" style="width: 1000px">
        ...
        <tr>
            <th colspan="6" style="text-align: left;">教学阅片主题：
                <c:if test="${type eq 'Y'}">
        ...
        <tr style="height:32px;">
            <th colspan="3" style="text-align: left;" width="55%">患者病历号：
                <c:if test="${type eq 'Y'}">
            ...
            <th colspan="3" style="text-align: left;" width="45%">诊断名称：
                <c:if test="${type eq 'Y'}">
                    <input onchange="saveInpuDetail(this);" value="${detailedMap['zdmc']}" itemId="zdmc" class="input" type="text" style="width:200px; text-align: left"/>
                </c:if>
                <c:if test="${type ne 'Y'}">
                    ${detailedMap['zdmc']}
                </c:if>
        ...
        <tr style="height:32px;">
            ...
            <th colspan="3" style="text-align: left;">职称：
                <c:if test="${type eq 'Y'}">
                    <input onchange="saveInpuDetail(this);" value="${detailedMap['zc']}" itemId="zc" class="input" type="text" style="width:200px; text-align: left"/>
                </c:if>
                <c:if test="${type ne 'Y'}">
                    ${detailedMap['zc']}
                </c:if>
        ...
        <tr style="height:32px;">
            <th colspan="3" style="text-align: left;">主阅住院医师：
                <c:if test="${type eq 'Y'}">
            ...
            <th colspan="3" style="text-align: left;">学习对象：
                <c:if test="${type eq 'Y'}">
        ...
        <tr style="height:32px;">
            <th colspan="3" style="text-align: left;">参加人数：
                <c:if test="${type eq 'Y'}">
                    <input onchange="saveInpuDetail(this);" value="${detailedMap['cjrs']}" itemId="cjrs" class="input" type="text" style="width:200px; text-align: left"/>
                </c:if>
                <c:if test="${type ne 'Y'}">
                    ${detailedMap['cjrs']}
                </c:if>
            </th>
            <th colspan="3" style="text-align: left;">教学时长：
                <c:if test="${type eq 'Y'}">
        ...
        <tr style="height:32px;">
            <th width="15%">评价项目</th>
            <th colspan="3" width="60%">内容要求</th>
            <th width="10%">分值</th>
            <th width="15%">得分</th>
        </tr>
        <tr style="height:32px">
            <th rowspan="4">阅片准备 <br>（15分）</th>
            <td colspan="3">病例紧扣培训细则，诊断明确，资料完整，病理形态较典型</td>
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导医师针对住院医师所描述的病理形态的特点给予充分点评，适时肯
                定、纠正和补充征象描述的不足或错误，并指导专业术语的规范使用
            </td>
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导住院医师对病史、辅助检查结果和病理形态特点进行归纳总结，合
                理地提取诊断及鉴别诊断所需的相关信息，并提出个人见解
            </td>
            <td style="text-align: center">8</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导住院医师提出为明确诊断所需进一步检查的计划和方案，并进行点
                评和修正
            </td>
            <td style="text-align: center">4</td>
            <td style="text-align: center">
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导医师分层次设置问题并引导不同层次的住院医师展开讨论、寻求答
                案，充分体现教学互动
            </td>
        ...
        <tr style="height:32px">
            <td colspan="3">
                合理教授专业英语词汇，适当介绍相关领域的最新进展，并引导住院医
                师阅读相关书籍、文献及参考资料等
            </td>
            <td style="text-align: center">5</td>
            <td style="text-align: center">
        ...
        <tr style="height:32px">
            <td colspan="3">
                融入医学人文和思政教育元素，注重培养住院医师的同理心、爱伤观念
                以及团队合作能力
            </td>
            <td style="text-align: center">5</td>
            <td style="text-align: center">
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导医师对本次教学阅片的知识点进行归纳总结，并布置课后拓展作
                业；师生双方针对本次教学阅片的整体表现进行互评
            </td>
            <td style="text-align: center">6</td>
            <td style="text-align: center">
                <c:if test="${type eq 'Y'}">
        ...
        <tr style="height:32px">
            <td colspan="3">
                鼓励住院医师在实践中坚持将病理形态与临床病例相结合，不断提高阅
                片的准确度与综合诊疗思维能力
            </td>
            <td style="text-align: center">6</td>
            <td style="text-align: center">
        ...
        <tr style="height:32px">
            <td colspan="3">
                指导医师通过提问、假设、推理等多种方式，及时指导住院医师归纳并
                小结阅片内容
            </td>
            <td style="text-align: center">6</td>
            <td style="text-align: center">
