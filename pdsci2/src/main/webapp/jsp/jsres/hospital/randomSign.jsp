    ...
    <script type="text/javascript">
        ...
        function showSecondCode(tabTag)
        {
            ...
            if(canvas!=null)
            {
                ...
                {
                    ctx.fillText("                                                                ", 0, 415);
                    ctx.fillText("【签退二维码】（此二维码在讲座结束前${endTime}钟至结束后${endTime}分钟内有效）", 0, 415);   // 文字
//                    $("#qrcode2").hide();
                }else{
                    ctx.fillText("                                                                ", 0, 415);
                    ctx.fillText("【签到二维码】（此二维码在讲座开始前${startTime}钟至结束后${startTime}分钟内有效）", 0, 415);   // 文字
//                    $("#qrcode").hide();
                }
//                $("#qrcode3").show();
            }
        ...
        function  down(name)
        {
            $('#qrcode canvas').downCanvasImg("【签到二维码】（此二维码在讲座开始前${startTime}钟至结束后${startTime}分钟内有效）.png");
        }
...
<div class="main_hd">
    <div class="title_tab">
        <ul id="reducationTab">
            <li id="Singn" class="tab" onclick="selTag('Singn');"><a id="regiestA">随机签到二维码</a></li>
            <li id="SingnOut" class="tab" onclick="searchRandomSign('${randomSign.lectureFlow}');"><a id="scanA">随机签到设置</a></li>
        </ul>
    </div>
</div>
...
<div id="SingnDiv" style="display: none;">
    ...
    <div>
        【签到二维码】（此二维码在讲座开始前${randomSign.codeStartTime}钟至结束后${randomSign.codeEndTime}分钟内有效）
    </div>
    <c:if test="${randomSign.codeStatusType == 'N'}">
    <div id="downDiv"  style="text-align: center;margin-top: 5px;">
        ${pdfn:jsresPowerCfgMap(key) eq 'Y'? '<font color="red">此二维码为动态二维码，每5秒钟刷新一次</font>':''}
