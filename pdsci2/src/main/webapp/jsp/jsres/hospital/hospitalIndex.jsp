...
//	});
        ...
        // 这里因为是异步调用，可能会有问题：这是菜单代码，当不在欢迎页时，下面的元素会找不到，会弹报错，有问题，所以先判断当前是不是欢迎页面
        function getDocInitData(fun) {
            // 通过欢迎页元素判断是不是欢迎页， 不是就返回
            if((!$("#doctorNumSearch") && !$("#trainTypeId")) || (!$("#trainTypeId").length && !$("#doctorNumSearch").length)) {
                return;
            }
    ...
    <script>
        ...
        function doctorList() {
            ...
            jboxLoad("content", "<s:url value='/jsres/doctorRecruit/provinceDoctorList'/>?roleFlag=" + roleFlag + "&orgFlow=${sessionScope.currUser.orgFlow}" + "&sessionNumber=" + sessionNumber, true);
        }
        ...
        function recruitReport() {
            var url = "<s:url value='/jsres/recruitDoctorInfo/recruitReport'/>";
            jboxLoad("content", url, true);
        }
        ...
        function zljh() {
            <%--var roleFlag="${GlobalConstant.USER_LIST_LOCAL}";--%>
            jboxLoad("content", "<s:url value='/jsres/message/plan'/>", true);
        }
        ...
        function hospitalSelfAssessment() {
            var url="<s:url value='/jsp/jsres/hospital/supervisio/hospitalSelfTab.jsp'/>";
            jboxLoad("content", url, false);
        }

        function hospitalSpeSelfAssessment() {
            jboxLoad("content","<s:url value='/jsres/supervisio/selfAssessmentMain'/>",true);
        }
        ...
        <div class="yw">
            ...
            <div class="head">
                <div class="head_inner">
                    ...
                    <div class="account">
                        <h2 class="head_right" style="margin-top: 0;margin-bottom: 0">${sessionScope.currUser.orgName }</h2>
...
<%--                            <a class="side-button" href="#" onclick="showSelectMenu();">切<br/>换<br/>新<br/>版<br/>菜<br/>单</a>--%>
                            ...
                            <dd class="menu_item"><a href="javascript:hosMain('${sessionScope.currUser.orgFlow}', '');">基地信息管理</a></dd>
                            <c:if test="${isJointOrg eq 'N'}">
                            <dd class="menu_item"><a href="javascript:hosMain('${sessionScope.currUser.orgFlow}', 'mainAssistOrg');">协同单位管理</a></dd>
                            </c:if>
                            <dd class="menu_item"><a href="javascript:hosMain('${sessionScope.currUser.orgFlow}', 'mainSpeBase');">专业基地管理</a></dd>
                            <%--<dd class="menu_item" id="evaluation"><a onclick="searchEvaluationInfo();">培训基地评估</a></dd>--%>
                            <dd class="menu_item"><a href="javascript:baseInfo();">科室信息管理</a></dd>
...
<%--                            <dd class="menu_item"><a onclick="djxxgl();">带教信息管理</a></dd>--%>
                      ...
                      <%--  <dl class="menu">
                            ...
                            &lt;%&ndash;<dd class="menu_item"><a onclick="graduationExam()">结业理论模拟考核</a></dd>&ndash;%&gt;
                        </dl>--%>
                        <%--<dl class="menu">
                            <dt class="menu_title">
