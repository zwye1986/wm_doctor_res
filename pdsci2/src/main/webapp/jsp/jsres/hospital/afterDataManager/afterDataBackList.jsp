...
<script type="text/javascript">
	...
	function backProcess(processFlow)
	{
		if(!processFlow){
			jboxTip("请选择轮需要恢复的轮转记录！");
			return false;
		}
		var url = "<s:url value='/jsres/afterDataManager/backProcess'/>?processFlow="+processFlow;
	...
	function backProcessType(processFlow)
	{
		if(!processFlow){
			jboxTip("请选择轮转记录！");
			return false;
		...
		var url = "<s:url value='/jsres/afterDataManager/backProcessType'/>?processFlow="+processFlow+"&recTypeId="+recTypeId+"&recFlow="+recFlow;
		jboxConfirm("确认恢复出科表单？",function() {
			jboxPost(url, null, function (resp) {
		...
		<table border="0" cellpadding="0" cellspacing="0" class="grid">
			...
			<c:forEach items="${list}" var="bean">
				<tr>
					...
					<td>
						<c:if test="${count>0}">
							<a class="btn"  onclick="backProcessType('${bean.processFlow}');" style="margin-top: 5px;margin-bottom: 5px;">恢复出科表</a>
						</c:if>
						<c:if test="${bean.recordStatus eq 'N'}">
							<a class="btn"  onclick="backProcess('${bean.processFlow}');" style="margin-top: 5px;margin-bottom: 5px;">恢复科室</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
