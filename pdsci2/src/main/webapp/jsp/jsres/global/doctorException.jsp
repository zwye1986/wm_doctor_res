...
<head>
    ...
    <script>
        ...
        jboxPostAsync("<s:url value='/jsres/manage/doctorExceptionListByOrg'/>",data, function (res) {
            if(res.length>0){
                if(res[0].error){
                    jboxTip(res[0].error);
                    initTable([]);
        ...
        function search() {
            ...
            data=$("#orgForm").serialize()+data;
            jboxStartLoading();
            jboxPost("<s:url value='/jsres/manage/doctorExceptionListByOrg'/>",data, function (res) {
                if(res.length>0){
                    if(res[0].error){
                        jboxTip(res[0].error);
                        initTable([]);
...
<div class="main_bd define_bd" id="div_table_0">
    <div class="div_search" style="padding-left: 40px; box-sizing: border-box;">
            ...
            <ul id="body-tab" class="title_tab">
                <li><a href="javascript:;" class="current" onclick="table()">学员轮转异常统计</a></li>
            </ul>
