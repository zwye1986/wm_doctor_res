<%@include file="/jsp/common/doctype.jsp" %>
<html>
<head>
<title>江苏省住院医师规范化培训管理平台</title>
<jsp:include page="/jsp/jsres/htmlhead-jsres.jsp">
	<jsp:param name="basic" value="true"/>
	<jsp:param name="jbox" value="true"/>
	<jsp:param name="font" value="true"/>
</jsp:include>
<script type="text/javascript">
</script>
</head>

<body>
<div class="main_bd">
	<div class="div_search">
		<h2>江苏省2015届各专业在培医师统计</h2><span>统计单位：人</span></br>
		届别：<input name="" type="checkbox" value="" />2011&#12288;
			<input name="" type="checkbox" value="" />2012&#12288;
			<input name="" type="checkbox" value="" />2013&#12288;
			<input name="" type="checkbox" value="" />2014&#12288;
			<input name="" type="checkbox" value="" />2015
		
	</div>
	<div class="search_table">
        <table border="0" cellpadding="0" cellspacing="0" class="grid">
        	<colgroup>
        		<col colspan="2" style="text-align: left;padding-left: 40px;"/>
        		<col style="text-align: left;padding-left: 40px;"/>
        		<col style="text-align: left;padding-left: 40px;"/>
        		<col style="text-align: left;padding-left: 40px;"/>
        		<col style="text-align: left;padding-left: 40px;"/>
        	</colgroup>
        	<tr>
                <th colspan="2">医师类别</th>
                <th>专业</th>
                <th>在培</th>
                <th>专业</th>
                <th>在培</th>
            </tr>
             <tr>
                <td rowspan="2" colspan="2" class="heb">全科医师</td>
                <td>全科医学（中医）</td>
                <td>0</td>
                <td></td>
                <td></td>
            </tr>
            <tr>
                <td>全科医学（西医）</td>
                <td>0</td>
                <td>合计</td>
                <td>1</td>
            </tr>
            <tr>
                <td rowspan="27" class="heb">住院医师</td>
                <td rowspan="10" class="heb">一阶段</td>
                <td>内科</td>
                <td>1</td>
                <td>外科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>妇产科</td>
                <td>0</td>
                <td>医学检验科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>急诊科</td>
                <td>0</td>
                <td>医学影像科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>病理科</td>
                <td>0</td>
                <td>儿科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>神经内科</td>
                <td>0</td>
                <td>皮肤性病科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>眼科</td>
                <td>0</td>
                <td>耳鼻咽喉科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>精神科</td>
                <td>0</td>
                <td>儿外科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>康复医学科</td>
                <td>0</td>
                <td>麻醉科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>口腔科</td>
                <td>0</td>
                <td>全科医学（西医）</td>
                <td>0</td>
            </tr>
            <tr>
                <td>全科医学（中医）</td>
                <td>0</td>
                <td>合计</td>
                <td>1</td>
            </tr>
            <tr>
            	<td class="heb" rowspan="17">二阶段</td>
                <td>心血管内科</td>
                <td>0</td>
                <td>呼吸内科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>消化内科</td>
                <td>0</td>
                <td>血液内科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>内分泌科</td>
                <td>0</td>
                <td>肾脏内科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>风湿免疫科</td>
                <td>0</td>
                <td>神经内科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>感染科</td>
                <td>0</td>
                <td>普通外科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>神经外科</td>
                <td>0</td>
                <td>心胸外科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>烧伤整形科</td>
                <td>0</td>
                <td>泌尿外科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>骨科</td>
                <td>0</td>
                <td>妇产科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>儿科</td>
                <td>0</td>
                <td>急诊科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>全科医学（中医）</td>
                <td>0</td>
                <td>合计</td>
                <td>1</td>
            </tr>
            <tr>
                <td>皮肤科</td>
                <td>0</td>
                <td>麻醉科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>耳鼻咽喉科</td>
                <td>0</td>
                <td>眼科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>口腔科</td>
                <td>0</td>
                <td>康复医学科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>病理科</td>
                <td>0</td>
                <td>检验科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>放射科</td>
                <td>0</td>
                <td>超声诊断科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>核医学科</td>
                <td>0</td>
                <td>精神科</td>
                <td>0</td>
            </tr>
            <tr>
                <td>儿外科</td>
                <td>0</td>
                <td>合计</td>
                <td>0</td>
            </tr>
            <tr>
                <td colspan="2" class="heb">合计</td>
                <td colspan="4">在培：2</td>
            </tr>
        </table>
	</div>
    <div style="text-align:center;margin:20px 0;">
    	<input type="button" style="width:100px;" class="btn_blue" value="打印"/>
    	<input type="button" style="width:100px;" class="btn_blue" value="导出Excel"/>
    </div>
</div>
</body>
</html>