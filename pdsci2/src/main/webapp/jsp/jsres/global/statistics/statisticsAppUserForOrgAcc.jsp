...
// 		var endTime=$("#endTime").val().substring(0,4)+$("#endTime").val().substring(5,7);
		...
		jboxLoad("hosContent","<s:url value='/jsres/statistic/statisticsAppUserForOrgAcc'/>?catSpeId="+catSpeId+"&sessionNumber="+sessionNumber,true);
// 	}else{
...
// 		var endTime=$("#endTime").val().substring(0,4)+$("#endTime").val().substring(5,7);
		var catSpeId = $("#catSpeId").val();
		jboxOpen("<s:url value='/jsres/statistic/statisticsAppUserAcc'/>?open=Y&catSpeId="+catSpeId+"&orgFlow="+orgFlow+"&sessionNumber="+sessionNumber,"APP使用情况折线图",1050,550);
// 	}else{
// 		jboxTip("开始时间不能大于结束时间！");return;
// 	}
}
...
<body>
<div class="search_table" style="margin-top: 10px;padding-left: 0px;">
	<form id="searchForm">
...
<%-- 			结束时间：<input type="text" id="endTime" name="endTime" <c:if test="${empty param.startTime }">value="${pdfn:getCurrDateTime('yyyy-MM')}"</c:if>value="${pdfn:transDateTimeForPattern(param.endTime,'yyyyMM','yyyy-MM')}" class="input" readonly="readonly" style="width: 100px;margin-left: 0px"/>&#12288; --%>
			...
			年级：<input type="text" id="sessionNumber" name="sessionNumber" value="${not empty sessionNumber?sessionNumber:(pdfn:getCurrYearByMonth())}"  class="input" readonly="readonly" style="width: 100px;margin-left: 0px"/>&#12288;
	...
	<div id="tableContext" style="margin-top: 10px;margin-bottom: 10px;overflow: auto;margin-left: 30px;height: 700px" onscroll="tableFixed(this);">
		...
		<table border="0" cellpadding="0" cellspacing="0" class="grid" style="border-bottom: 0px;width: 100%;" >
			<tr>
			...
			<c:forEach items="${sysOrgList}" var="org" varStatus="b">
				<tr>
					<td style="width: 30%"><label style="cursor: pointer;" onclick="showView('${org.orgFlow}','${sessionNumber}');"><font color="#459ae9">${org.orgName}</font></label></td>
							...
							<c:forEach items="${jointOrgListMap[org.orgFlow]}" var="jointOrg" varStatus="a">
								<tr style="border-right: 0px;">
									<td class="bo" <c:if test="${a.count == fn:length(jointOrgListMap[org.orgFlow]) }">style="border-bottom: 0px"</c:if>>
										<label style="cursor: pointer;" onclick="showView('${jointOrg.jointOrgFlow}','${sessionNumber}');"><font color="#459ae9">${jointOrg.jointOrgName}</font></label>
									</td>
