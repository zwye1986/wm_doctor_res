    ...
    function InformationQuery(doctorFlow,roleFlag,yearStr){
        ...
        jboxPost("<s:url value='/jsres/doctor/checkDoctorAuth'/>?doctorFlow="+doctorFlow+"&roleFlag="+roleFlag,null,function(resp){
            if(resp=="${GlobalConstant.FLAG_Y}"){
                ...
                jboxLoad("content","<s:url value='/jsres/doctor/trainRegister'/>?hideApprove=hideApprove&doctorFlow="+doctorFlow+"&roleFlag="+roleFlag+"&yearStr="+yearStr+"&search="+encodeURIComponent(encodeURIComponent(formdate))+"&currentPage="+currentPage,true);
            }else{
                jboxTip("该学员尚未上传诚信声明！");
            }
        },null,false);
