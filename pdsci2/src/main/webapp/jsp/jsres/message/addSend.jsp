...
<jsp:include page="/jsp/jsres/htmlhead-jsres.jsp">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sendStartTime').datepicker({
                startView: 0,
                maxViewMode: 0,
                minViewMode:0,
                format:'yyyy-mm-dd'
            });
            $('#sendEndTime').datepicker({
                startView: 0,
                maxViewMode: 0,
                minViewMode:0,
                format:'yyyy-mm-dd'
        ...
        function showOrgSpe(orgFlow,assignYear){
            if (assignYear == null || assignYear == '') {
                jboxLoad("gridTable","<s:url value='/jsres/message/addSendSpe'/>?assignYear=${param.assignYearEdit}&orgFlow=" + orgFlow+"&flag=${param.flag}",true);
            } else {
                jboxLoad("gridTable","<s:url value='/jsres/message/addSendSpe'/>?assignYear=" + assignYear + "&orgFlow=" + orgFlow+"&flag=${param.flag}",true);
            }
        }
        ...
        function saveAssign(){
            ...
            var startTime = $("#sendStartTime").val();
            var endTime = $("#sendEndTime").val();
            // 招生计划信息
            var assignDatas = [];
            $("#gridTable").find("tr").each(function(){
                var recordFlow = $(this).find("input[name='recordFlow']").val();
                var sendPlan = $(this).find("input[name='sendPlan']").val();
            ...
            var url = "<s:url value='/jsres/message/saveSend'/>";
            jboxPost(url, data, function (resp) {
                if (resp == '${GlobalConstant.SAVE_SUCCESSED}') {
                    setTimeout(function () {
                        window.parent.getSendList('${param.assignYear}',1);
                        jboxCloseMessager();
                    },800);
                }
            }, null, true);
        ...
        <form id="editForm" method="post">
            ...
            <table class="grid">
                <tr>
                    ...
                    <th style="text-align: right;"><span class="red">*</span>报送年份：</th>
                    <td style="text-align: left;">
                ...
                <tr>
                    ...
                    <td style="text-align: left;">
                        <input class="input validate[required]" name="sendStartTime" id="sendStartTime" readonly="readonly" value="${param.sendStartTime}" <c:if test="${param.flag eq 'audit'}">disabled</c:if> type="text"/>
                    </td>
                    <th style="text-align: right;"><span class="red">*</span>结束时间：</th>
                    <td style="text-align: left;">
                        <input class="input validate[required]" name="sendEndTime" id="sendEndTime" readonly="readonly" value="${param.sendEndTime}" <c:if test="${param.flag eq 'audit'}">disabled</c:if> type="text"/>
                    </td>
                </tr>
            </table>
