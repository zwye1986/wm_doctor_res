...
<script type="text/javascript">
    ...
    function editDoctorRecruit(recruitFlow){
        ...
        var url = "<s:url value='/jsres/doctor/editDoctorRecruit'/>?openType=open&recruitFlow="+recruitFlow;
        var title = "新增";
        if(recruitFlow){
            title = "修改";
        }
        jboxOpen(url, title + "报考记录", 900, 500);
    }
    ...
    <div class="title_tab" id="toptab">
        <ul>
            ...
            <c:forEach items="${recruitList}" var="recruit">
                ...
                <c:if test="${recruit.auditStatusId eq 'Passed'}">
                    <li class="tab" id="li_${recruit.recruitFlow}" onclick="getDoctorRecruit('${recruit.recruitFlow}','${sessionScope.currUser.userFlow}');">
                        <a>培训记录${recruit.catSpeName}（${empty recruit.speName?'--':recruit.speName}）</a></li>
                </c:if>
            </c:forEach>
