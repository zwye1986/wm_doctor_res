...
<c:if test="${empty list}">
    <div class="search_table" style="width: 100%;padding: 0px 0px">
        <table border="0" cellpadding="0" cellspacing="0" class="grid">
            <tr>
                <th>
                    <c:if test="${param.tabTag eq 'SecondWait' or param.tabTag eq 'SecondWait2'}">审核</c:if>
                    <c:if test="${param.tabTag ne 'SecondWait' and param.tabTag ne 'SecondWait2'}">操作</c:if>
                </th>
...
<c:if test="${not empty list}">
    <div class="main_bd clearfix" style="width: 100%;">
        <table id="dataTable" border="0" cellpadding="0" cellspacing="0" class="grid">
            ...
            <tr>
                <c:if test="${f eq 'Y'}">
                    <th style="width: 50px " class="toFiexdDept">
                        <c:if test="${s.auditStatusId eq 'WaitChargePass'}">
                            <a href="javascript:void(0);"
                               onclick="selectAll();">全选</a>/<a href="javascript:void(0);" onclick="Contrary();">反选</a>
                        </c:if>
                    </th>
                </c:if>
                <th class="toFiexdDept">
                    <c:if test="${s.auditStatusId eq 'WaitChargePass'}">审核</c:if>
                    <c:if test="${s.auditStatusId ne 'WaitChargePass'}">操作</c:if>
                </th>
                <c:if test="${param.tabTag ne 'SecondWait' and param.tabTag ne 'SecondWait2'}">
            ...
            <c:forEach items="${list}" var="s">
                ...
                <tr class="fixTrTd">
                    <c:if test="${f eq 'Y'}">
                        <td style=" " class="by"><input value="${s.signupFlow}"
                                                        name="signupFlow"
                                                        type="checkbox"
                                                        onclick="checkboxOnclick(this,'${s.auditStatusId}')"/>
                        </td>
                    </c:if>
                    <td class="by">
                        <c:if test="${f eq 'Y'}">
                            ...
                            <c:if test="${s.auditStatusId ne 'WaitChargePass'}">
