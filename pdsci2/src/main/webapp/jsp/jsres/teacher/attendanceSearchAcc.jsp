    ...
    <script type="text/javascript">
        ...
        function attendanceSearch2(form)
        {
            <c:if test="${GlobalConstant.USER_LIST_LOCAL eq sessionScope.userListScope || GlobalConstant.USER_LIST_BASE eq sessionScope.userListScope}">
                jboxPostLoad("mainDiv", "<s:url value='/jsres/teacher/attendanceSearchAcc/${roleId}'/>?baseFlag=${param.baseFlag}", form, true);
            </c:if>
        ...
        function exportAttendance() {
            ...
            var roleId = "${roleId}";
            var url = "<s:url value='/jsres/teacher/exportAttendanceAcc?roleId='/>" + roleId+"&baseFlag=${param.baseFlag}";
            console.log(url);
            jboxTip("导出中…………");
