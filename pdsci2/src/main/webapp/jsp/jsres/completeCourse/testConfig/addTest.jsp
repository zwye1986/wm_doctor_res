<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@include file="/jsp/common/doctype.jsp" %>
<html>
<head>
    <jsp:include page="/jsp/jsres/htmlhead-jsres.jsp">
        <jsp:param name="basic" value="true"/>
        <jsp:param name="jbox" value="true"/>
        <jsp:param name="font" value="true"/>
        <jsp:param name="jquery_validation" value="true"/>
        <jsp:param name="jquery_form" value="true"/>
    </jsp:include>
    <script type="text/javascript"
            src="<s:url value='/js/DatePicker/WdatePicker.js'/>?v=${applicationScope.sysCfgMap['sys_version']}"></script>
    <style type="text/css">
        td {
            text-align: left
        }

        textarea {
            width: 90%;
            height: 100px;
            max-height: 100px;
            max-width: 96%;
            margin: 5px 0;
        }
    </style>
    <script type="text/javascript">
        function jointLocalAuditShow() {
            $("#jointLocalAudit").show();
        }

        function jointLocalAuditHide() {
            $("#jointLocalAudit").hide();
        }

        function localAuditShow() {
            $("#localAudit").show();
        }

        function localAuditHide() {
            $("#localAudit").hide();
        }

        function chargeAuditShow() {
            $("#chargeAudit").show();
        }

        function chargeAuditHide() {
            $("#chargeAudit").hide();
        }

        function checkApplyStart() {
            if ($("#applyEndTime").val()) {
                if ($("#applyStartTime").val() > $("#applyEndTime").val()) {
                    jboxTip("报名开始时间不能大于结束时间")
                }
            }
        }

        function checkApplyEnd() {
            if ($("#applyStartTime").val()) {
                if ($("#applyStartTime").val() > $("#applyEndTime").val()) {
                    jboxTip("报名开始时间不能大于结束时间")
                }
            }
        }

        function checkJointLocalStart() {
            if ($("#jointLocalAuditEndTime").val()) {
                if ($("#jointLocalAuditStartTime").val() > $("#jointLocalAuditEndTime").val()) {
                    jboxTip("协同单位审核开始时间不能大于结束时间")
                }
            }
        }

        function checkJointLocalEnd() {
            if ($("#jointLocalAuditStartTime").val()) {
                if ($("#jointLocalAuditStartTime").val() > $("#jointLocalAuditEndTime").val()) {
                    jboxTip("协同单位审核开始时间不能大于结束时间")
                }
            }
        }

        function checkLocalStart() {
            if ($("#localAuditEndTime").val()) {
                if ($("#localAuditStartTime").val() > $("#localAuditEndTime").val()) {
                    jboxTip("基地审核开始时间不能大于结束时间")
                }
            }
        }

        function checkLocalEnd() {
            if ($("#localAuditStartTime").val()) {
                if ($("#localAuditStartTime").val() > $("#localAuditEndTime").val()) {
                    jboxTip("基地审核开始时间不能大于结束时间")
                }
            }
        }

        function checkChargeStart() {
            if ($("#chargeAuditEndTime").val()) {
                if ($("#chargeAuditStartTime").val() > $("#chargeAuditEndTime").val()) {
                    jboxTip("市局审核开始时间不能大于结束时间")
                }
            }
        }

        function checkChargeEnd() {
            if ($("#chargeAuditStartTime").val()) {
                if ($("#chargeAuditStartTime").val() > $("#chargeAuditEndTime").val()) {
                    jboxTip("市局审核开始时间不能大于结束时间")
                }
            }
        }

        function checkTestEnd() {
            if ($("#applyEndTime").val()) {
                if ($("#applyEndTime").val() > $("#testEndTime").val()) {
                    jboxTip("考试结束时间不能小于报名结束时间")
                }
            } else {
                jboxTip("请先填写报名结束时间")
            }
        }

        function save(testFlow, flag) {
            if (flag == 'N') {
                jboxTip("已结束或者已关闭的考试不能编辑");
                return false;
            }
            if (false == $("#saveCfgForm").validationEngine("validate")) {
                return false;
            }
            if ($("#applyStartTime").val() > $("#applyEndTime").val()) {
                jboxTip("报名开始时间不能大于结束时间");
                return false;
            }
            if ($("#jointLocalAuditStartTime").val() > $("#jointLocalAuditEndTime").val()) {
                jboxTip("协同单位审核开始时间不能大于结束时间");
                return false;
            }
            if ($("#localAuditStartTime").val() > $("#localAuditEndTime").val()) {
                jboxTip("基地审核开始时间不能大于结束时间");
                return false;
            }
            if ($("#chargeAuditStartTime").val() > $("#chargeAuditEndTime").val()) {
                jboxTip("市局审核开始时间不能大于结束时间");
                return false;
            }
            if ($("#applyEndTime").val() > $("#testEndTime").val()) {
                jboxTip("考试结束时间不能小于报名结束时间");
                return false;
            }
            if ($("#localAuditEndTime").val() != "") {
                if ($("#localAuditEndTime").val() > $("#testEndTime").val()) {
                    jboxTip("报名结束时间不能小于基地审核时间");
                    return false;
                }
            }
            if ($("#chargeAuditEndTime").val() != "") {
                if ($("#chargeAuditEndTime").val() > $("#testEndTime").val()) {
                    jboxTip("报名结束时间不能小于市局审核时间");
                    return false;
                }
            }
            if ($("#localAuditEndTime").val() != "" && $("#chargeAuditEndTime").val() != "") {
                if ($("#localAuditEndTime").val() > $("#chargeAuditEndTime").val()) {
                    jboxTip("市局审核的结束时间不能小于基地审核的结束时间");
                    return false;
                }
            }
            jboxPost("<s:url value='/jsres/testConfig/insertTest'/>?testFlow=" + testFlow, $("#saveCfgForm").serialize(), function (resp) {
                jboxTip(resp);
                if (resp == "${GlobalConstant.SAVE_SUCCESSED}") {
                    setTimeout(
                        function () {
                            window.parent.search();
                            jboxClose();
                        }, 2000
                    );
                }
            }, null, false)
        }
    </script>
</head>
<body>
<div class="mainright" style="max-height: 700px; padding: 0px 20px;  ">
    <form id="saveCfgForm" style="position: relative;">
        <table class="grid" style="width:100%; margin-bottom: 10px; margin-top: 30px;">
            <tr>
                <td colspan="2" style="text-align: center;width: 180px"> 考试名称<font color="red">*</font></td>
                <td colspan="2" style="text-align: left;width: 620px">
                    <input name="testName"
                           value="${empty resTestConfig.testFlow ? testName:resTestConfig.testName}" class="input validate[required]"
                           readonly="readonly"
                           <%--<c:if test="${not empty flag}">readonly="readonly"</c:if>--%>
                    >
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 报名时间<font color="red">*</font></td>
                <td colspan="2" style="text-align: left">
                    <input name="applyStartTime" id="applyStartTime"
                    <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                           onchange="checkApplyStart()"
                           class="input validate[required]"
                           value="${resTestConfig.applyStartTime}"
                           <c:if test="${not empty flag}">readonly="readonly"</c:if>>&nbsp;&nbsp;至&nbsp;&nbsp;
                    <input name="applyEndTime" id="applyEndTime"
                    <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                           onchange="checkApplyEnd()"
                           class="input validate[required]"
                           <c:if test="${empty flag}">value="${applyEndTime}"</c:if>
                           <c:if test="${not empty flag}">value="${resTestConfig.applyEndTime}"</c:if>
                           <c:if test="${not empty flag}">readonly="readonly"</c:if>>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 地市<font color="red">*</font></td>
                <td colspan="2" style="text-align: left">
                    <c:forEach items="${citys}" var="city">
                        <input type="checkbox" name="citysId" class="validate[required]" value="${city.cityId}"
                               <c:if test="${pdfn:contain(city.cityId, cityList) or empty cityList}">checked</c:if>
                        <c:if test="${not empty flag}"> onclick="return false"</c:if>
                        ><font
                            style="font-size: 15px"
                    >${city.cityName}</font>
                    </c:forEach>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 协同单位审核时间<font color="red">*</font></td>
                <td colspan="2" style="text-align: left; align-items: center;">
                    <input type="radio" name="jointLocalAudit" class="validate[required]" value="Y"
                           <c:if test="${resTestConfig.jointLocalAudit eq 'Y'}">checked</c:if>
                           onclick="jointLocalAuditShow()"
                           <c:if test="${not empty flag}">disabled</c:if>>开通
                    <input type="radio" name="jointLocalAudit" class="validate[required]" value="N"
                           <c:if test="${resTestConfig.jointLocalAudit eq 'N'}">checked</c:if>
                           onclick="jointLocalAuditHide()"
                           <c:if test="${not empty flag}">disabled</c:if>>关闭
                    <span
                            <c:if test="${resTestConfig.jointLocalAudit eq 'N'}">style="display: none"</c:if>
                            <c:if test="${ empty resTestConfig.jointLocalAudit }">style="display: none"</c:if>
                            id="jointLocalAudit">
                        <input name="jointLocalAuditStartTime" id="jointLocalAuditStartTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkJointLocalStart()"
                               value="${resTestConfig.jointLocalAuditStartTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if>> &nbsp;&nbsp;至&nbsp;&nbsp;
                        <input name="jointLocalAuditEndTime" id="jointLocalAuditEndTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkJointLocalEnd()"
                               value="${resTestConfig.jointLocalAuditEndTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if> >
                            </span>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 基地审核时间<font color="red">*</font></td>
                <td colspan="2" style="text-align: left; align-items: center;">
                    <input type="radio" name="localAudit" class="validate[required]" value="Y"
                           <c:if test="${resTestConfig.localAudit eq 'Y'}">checked</c:if>
                           onclick="localAuditShow()"
                           <c:if test="${not empty flag}">disabled</c:if>>开通
                    <input type="radio" name="localAudit" class="validate[required]" value="N"
                           <c:if test="${resTestConfig.localAudit eq 'N'}">checked</c:if>
                           onclick="localAuditHide()"
                           <c:if test="${not empty flag}">disabled</c:if>>关闭
                    <span
                            <c:if test="${resTestConfig.localAudit eq 'N'}">style="display: none"</c:if>
                            <c:if test="${ empty resTestConfig.localAudit }">style="display: none"</c:if>
                            id="localAudit">
                        <input name="localAuditStartTime" id="localAuditStartTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkLocalStart()"
                               value="${resTestConfig.localAuditStartTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if>> &nbsp;&nbsp;至&nbsp;&nbsp;
                        <input name="localAuditEndTime" id="localAuditEndTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkLocalEnd()"
                               value="${resTestConfig.localAuditEndTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if> >
                            </span>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 市局审核时间<font color="red">*</font></td>
                <td colspan="2" style="text-align: left;  align-items: center;">
                    <input type="radio" name="chargeAudit" value="Y"
                           <c:if test="${resTestConfig.chargeAudit eq 'Y'}">checked</c:if>
                           onclick="chargeAuditShow()"
                           <c:if test="${not empty flag}">disabled</c:if>>开通
                    <input type="radio" name="chargeAudit" value="N"
                           <c:if test="${resTestConfig.chargeAudit eq 'N'}">checked</c:if>
                           onclick="chargeAuditHide()"
                           <c:if test="${not empty flag}">disabled</c:if>>关闭
                    <span   <c:if test="${resTestConfig.chargeAudit eq 'N'}"> style="display: none"</c:if>
                            <c:if test="${empty resTestConfig.chargeAudit }"> style="display: none"</c:if>
                            id="chargeAudit">
                        <input name="chargeAuditStartTime" id="chargeAuditStartTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkChargeStart()"
                               value="${resTestConfig.chargeAuditStartTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if> >&nbsp;&nbsp;至&nbsp;&nbsp;
                        <input name="chargeAuditEndTime" id="chargeAuditEndTime"
                        <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                               onchange="checkChargeEnd()"
                               value="${resTestConfig.chargeAuditEndTime}" class="input validate[required]"
                               <c:if test="${not empty flag}">readonly="readonly"</c:if>>
                            </span>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: center"> 报名结束时间<font color="red">*</font></td>
                <td colspan="2" style="text-align: left">
                    <input name="testEndTime" id="testEndTime"
                    <c:if test="${empty flag}"> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"</c:if>
                           onchange="checkTestEnd()"
                           value="${resTestConfig.testEndTime}" class="input validate[required]"
                           <c:if test="${not empty flag}">readonly="readonly"</c:if>>
                </td>
            </tr>
        </table>
    </form>
    <p style="text-align: center;">
        <input type="button" onclick="save('${resTestConfig.testFlow}','${flag}');" style="background: #34A6E3"
               class="btn_green"
               value="保&#12288;存"/>&#12288;
    </p>
</div>
</body>
</html>
