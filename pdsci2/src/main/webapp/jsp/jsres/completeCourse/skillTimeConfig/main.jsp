...
<script type="text/javascript">
    ...
    function addTest() {
        var url = "<s:url value ='/jsres/skillTimeConfig/addTest'/>";
        jboxOpen(url, "新增考试", 800, 400);
    }

    function search() {
        var roleFlag = "${GlobalConstant.USER_LIST_GLOBAL}";
        jboxLoad("content", "<s:url value='/jsres/testConfig/testMain'/>?roleFlag=" + roleFlag+"&tabTag=SkillTest", true);
    }

    function edit(skillTimeFlow) {
        var url = "<s:url value ='/jsres/skillTimeConfig/addTest'/>?skillTimeFlow=" + skillTimeFlow;
        jboxOpen(url, "编辑考试", 800, 400);
    }

    function lookOver(skillTimeFlow, flag) {
        var url = "<s:url value ='/jsres/skillTimeConfig/addTest'/>?skillTimeFlow=" + skillTimeFlow + "&flag=" + flag;
        jboxOpen(url, "查看考试", 800, 400);
    }

    function closeTest(skillTimeFlow) {
        var url = "<s:url value ='/jsres/skillTimeConfig/closeTest'/>?skillTimeFlow=" + skillTimeFlow;
...
<%--<div class="main_hd">--%>
    <%--<h2 class="underline">技能考核时间配置</h2>--%>
<%--</div>--%>
...
<div style="padding: 0px 40px;">
    <div class="main_bd clearfix" style="margin-top:20px;">
        ...
        <c:if test="${empty resSkillTimeConfigs}">
        ...
        <c:if test="${not empty resSkillTimeConfigs}">
            <div class="main_bd clearfix" style="margin-top:20px;">
                <table id="dataTable" border="0" cellpadding="0" cellspacing="0" class="grid">
                    ...
                    <tbody>
                    <c:forEach items="${resSkillTimeConfigs}" var="s" varStatus="vs">
                        <tr>
                            <td>${vs.index + 1}</td>
                            <td>${s.testId}</td>
                            <td>${s.testName}</td>
                            ...
                            <td>
                                <c:if test="${map[s.skillTimeFlow]}">
                                    <a style="margin-left:35px;margin-right: 30px;color: #459AE9;"
                                       onclick="lookOver('${s.skillTimeFlow}','N')"
                                    >查看</a>
                                </c:if>
                                <c:if test="${!map[s.skillTimeFlow]}">
                                    <c:if test="${s.recordStatus eq 'Y'}">
                                        <a style="margin-left:35px;margin-right: 30px;color: #459AE9;"
                                           onclick="edit('${s.skillTimeFlow}')"
                                        >编辑</a>
                                    </c:if>
                                ...
                                <c:if test="${!map[s.skillTimeFlow]}">
                                    <c:if test="${s.recordStatus eq 'Y'}">
                                        <a style="margin-left:35px;margin-right: 30px;color: #FFB45B;"
                                           onclick="closeTest('${s.skillTimeFlow}')"
                                        >关闭</a>
                                    </c:if>
                                    <%--<c:if test="${s.recordStatus eq 'N'}">--%>
