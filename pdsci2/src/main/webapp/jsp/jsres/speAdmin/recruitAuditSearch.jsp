...
<head>
    ...
    <script type="text/javascript">
        ...
        function details(doctorFlow, processFlow) {
            var url = "<s:url value='/jsres/speAdmin/details'/>?doctorFlow=" + doctorFlow + "&processFlow=" + processFlow;
...
<div class="main_bd" id="div_table_0">
    <div class="div_search">
        <form id="searchForm" action="<s:url value='/jsres/teacher/recruitAuditSearch'/>" method="post">
            ...
            <table border="0" cellpadding="0" cellspacing="0" class="grid" style="border: none">
                <tr>
                    ...
                    <td style="border: none">
                        轮转科室：
                        <select name="schDeptFlow" class="select" style="width: 150px;">
                            <option value="">请选择</option>
                            <c:forEach var="d" items="${depts}">
                                <option value="${d.deptFlow}" <c:if test="${param.schDeptFlow==d.deptFlow}">selected="selected"</c:if>>${d.deptName}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
