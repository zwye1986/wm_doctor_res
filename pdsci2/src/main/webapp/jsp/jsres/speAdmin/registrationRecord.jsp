...
<body>
	<div class="main_hd" >
	    <div class="title_tab" style="margin-top: 10px;">
	        <ul id="reducationTab">
					<li class="${param.recTypeId eq resRecTypeEnumCaseRegistry.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumCaseRegistry.id}');"><a>${resRecTypeEnumCaseRegistry.name}</a></li>
					<li class="${param.recTypeId eq resRecTypeEnumDiseaseRegistry.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumDiseaseRegistry.id}');"><a>${resRecTypeEnumDiseaseRegistry.name}</a></li>
					<li class="${param.recTypeId eq resRecTypeEnumSkillRegistry.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumSkillRegistry.id}');"><a>${resRecTypeEnumSkillRegistry.name}</a></li>
					<li class="${param.recTypeId eq resRecTypeEnumOperationRegistry.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumOperationRegistry.id}');"><a>${resRecTypeEnumOperationRegistry.name}</a></li>
					<li class="${param.recTypeId eq resRecTypeEnumCampaignRegistry.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumCampaignRegistry.id}');"><a>${resRecTypeEnumCampaignRegistry.name}</a></li>
					<li class="${param.recTypeId eq resRecTypeEnumAfterEvaluation.id?'tab_select':'tab'}" onclick="setType('${resRecTypeEnumAfterEvaluation.id}');"><a>${resRecTypeEnumAfterEvaluation.name}</a></li>
	...
	<div class="div_search">
		<form id="searchForm" action="<s:url value='/jsres/speAdmin/details'/>" method="post">
        	...
        	<c:if test="${param.recTypeId==resRecTypeEnumCampaignRegistry.id}">
	        	 <table border="0" cellpadding="0" cellspacing="0" class="grid">
		        	...
		        	<c:forEach items="${resRecList}" var="resRec">
		        		<tr>
			        		<td>${resRec['activity_date']}</td>
			        		<td>${resRec['activity_way']}</td>
			        		<td>${resRec['activity_period']}</td>
			        		<td>${resRec['activity_speaker']}</td>
			        		<td>${resRec['activity_content']}</td>
		        		</tr>
		        	</c:forEach>
