...
<form id='BaseInfoForm' style="position:relative;">
    <div class="main_bd"
            ...
            <div class="div_table">
                <h4 style="border-left: 4px solid #54b2e5">医疗设备仪器</h4>
                如某项的数量为0，可省略不填。
                <c:forEach items="${resultList}" var="t" varStatus="status">
                    <c:set var="infoList" value="${t.infoList}"/>
                    <c:set var="infoList2" value="${t.infoList2}"/>
                    <c:set var="tableAllArrNum" value="${t.tableAllArrNum}"/>
                    <c:set var="tableAllArrNum2" value="${t.tableAllArrNum2}"/>

                    <%--<span style="color: blue;cursor: pointer" onclick="showTable('${status.index}')"> ${t.standardDeptName}</span>--%>
                            ...
                            <c:forEach items="${infoList2}" var="t" varStatus="status">
                                <tr>
                                    <c:if test="${t.isTh eq 'Y'}">
                                        ...
                                        <th colspan="${t.arrThreeClo}" rowspan="${t.arrThreeRow}" style="<c:if test="${status.index==0}">background-color: #f4f5f9;</c:if> -align: center;border-right: 1px solid #e7e7eb">${t.arrThree}</th>
