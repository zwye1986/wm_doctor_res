...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        <c:forEach items="${orgList}" var="org">
            ...
            $("#count_${org.orgFlow}").text(sum);
        </c:forEach>
        <c:set var="dictName" value="dictTypeEnumDoctorTrainingSpeList" />
        <c:forEach items="${applicationScope[dictName]}" var="dict">
            var dictSum = 0;
            $(".count_${dict.dictId}").each(function () {
                var text = $(this).text();
                dictSum = dictSum + Number(text);
            })
            $("#count_${dict.dictId}").text(dictSum);
        </c:forEach>
        var totalCount = 0;
        $(".total_count").each(function () {
            var text = $(this).text();
            totalCount = totalCount + Number(text);
        })
    ...
    function exportExcel(){
        var url = "<s:url value='/jsres/manage/exportRecruitStatistics'/>";
        jboxTip("导出中…………");
        jboxSubmit($("#searchForm"), url, null, null, false);
        jboxEndLoading();
    }
    function toPage() {
        jboxPostLoad("content","<s:url value='/jsres/manage/recruitStatisticsReport'/>",$("#searchForm").serialize(),true);
    }
    ...
    <div id="tableContext"style="width:900px; margin-top: 10px;margin-bottom: 10px;overflow: auto;margin-left: 0px;" onscroll="tableFixed(this);">
        <table border="0" cellpadding="0" cellspacing="0" class="grid" style="width: auto;">
            ...
            <tr>
                ...
                <c:forEach items="${applicationScope[dictName] }" var="dict">
                    <th style="width: 40px">${dict.dictName}</th>
                </c:forEach>
            ...
            <c:forEach items="${orgList}" var="org">
                ...
                <tr>
                    ...
                    <td title="${org.orgName}">${pdfn:cutString(org.orgName,6,true,3) }</td>
                    <c:forEach var="speInfo" items="${orgSpeInfo}" varStatus="status" >
                        <td style="width: 40px" class="count_${org.orgFlow} count_${orgSpeInfo[status.index].speId}">${orgSpeInfo[status.index].count}</td>
                    </c:forEach>
                    <td style="width: 40px" id="count_${org.orgFlow}"></td>
                </tr>
            </c:forEach>
            <tr>
                <td style="width: 200px">合计</td>
                <c:forEach items="${applicationScope[dictName]}" var="dict">
                    <td style="width: 40px" id="count_${dict.dictId}" class="total_count"></td>
                </c:forEach>
                <td style="width: 40px" id="totalCount"></td>
            </tr>
        </table>
    </div>
