...
function toPage(page) {
	...
	jboxStartLoading();
	jboxPostLoad("doctorListZi","<s:url value='/jszy/asseGlobal/waitAuditList'/>?roleFlag=${roleFlag}",$("#searchForm").serialize(),false);
}
...
function exportInfo()
{
	...
	var url = "<s:url value='/jszy/asseGlobal/exportList'/>?roleFlag=${roleFlag}&auditStatusId=${jszyResAsseAuditListEnumAuditing.id}&isWaitAudit=Y";
	</c:if>
	<c:if test="${roleFlag eq GlobalConstant.USER_LIST_GLOBAL}">
	var url = "<s:url value='/jszy/asseGlobal/exportList'/>?roleFlag=${roleFlag}&auditStatusId=${jszyResAsseAuditListEnumWaitGlobalPass.id}&isWaitAudit=Y";
	</c:if>
	<c:if test="${roleFlag eq GlobalConstant.USER_LIST_CHARGE}">
	var url = "<s:url value='/jszy/asseGlobal/exportList'/>?roleFlag=${roleFlag}&auditStatusId=${jszyResAsseAuditListEnumWaitChargePass.id}&isWaitAudit=Y";
	</c:if>
...
function  batchApply()
{
	...
	var url = "<s:url value='/jszy/asseGlobal/batchAuditApply?'/>"+applyFlow;
...
function getCityArea(){
	...
	function  changInfo(id)
	{
		if(id=="Y")
		{
			...
			$(".examType").hide();
			$("#isTheroy").removeAttr("checked");
			$("#isSkill").removeAttr("checked");
		}
	}
</script>
<script type="text/javascript">
	...
	<form id="searchForm">
		<input type="hidden" id="currentPage" name="currentPage"/>
			<table class="searchTable" style="width: 100%;border-collapse:separate; border-spacing:0px 10px;">
				...
				<tr>
					<td class="td_left">是否补考：</td>
					<td>
						<select name="makeUp" id="makeUp" class="select"  onchange="changInfo(this.value);" style="width:107px;">
							<option value="">请选择</option>
							<option value="Y">是</option>
							<option value="N">否</option>
						</select>
					</td>
					<td class="td_left examType">补考类型：</td>
					<td class="examType">
						<input  name="isTheroy" id="isTheroy" type="checkbox" value="Y" /><label style="width:50px; display: initial" for="isTheroy">理论补考</label>
						<input  name="isSkill"  id="isSkill"  type="checkbox" value="Y" /><label style="width:50px; display: initial" for="isSkill">技能补考</label>
					</td>
				...
				<tr>
					<td class="td_left">考试批次：</td>
					<td colspan="3">
						<input  name="isFive" id="isFive"  type="checkbox" value="Y" /><label style="width:50px; display: initial" for="isFive">五月</label>
						<input  name="isTen" id="isTen"  type="checkbox" value="Y" /><label style="width:50px; display: initial" for="isTen">十月</label>
					</td>
					<td colspan="10">
