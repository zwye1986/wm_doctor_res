		...
		<table class="grid" style="width: auto;" id="dataTable">
			...
			<c:forEach items="${list}" var="doctor">
				<tr class="fixTrTd">
					...
					<td style="min-width: 100px; max-width: 100px; " class="by">${empty doctor.cityReason ? '-' : doctor.cityReason}</td>
						...
						<td style="min-width: 100px;max-width: 100px; " class="by">
							<c:if test="${ doctor.applyYear eq pdfn:getCurrYear() }">
								...
								<c:if test="${ param.roleFlag eq GlobalConstant.USER_LIST_CHARGE }">
									<c:if test="${(not empty doctor.cityAuditStatusId and empty doctor.globalAuditStatusId)}">
										<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">重新审核</a>
									</c:if>
								...
								<c:if test="${ param.roleFlag eq GlobalConstant.USER_LIST_LOCAL  and org.orgFlow eq doctor.orgFlow }">
									<c:if test="${(not empty doctor.localAuditStatusId and empty doctor.cityAuditStatusId and empty doctor.globalAuditStatusId)}">
										<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">重新审核</a>
									</c:if>
