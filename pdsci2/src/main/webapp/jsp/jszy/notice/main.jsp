...
<script type="text/javascript">
	...
	function saveNotice(){
		...
		jboxStartLoading();
		var url = "<s:url value='/jszy/notice/saveNotice/${roleFlag}'/>";
	...
	function edit(infoFlow){
		var url = "<s:url value='/jszy/notice/findNoticeByFlow'/>?infoFlow="+infoFlow;
	...
	function delNotice(infoFlow){
		jboxConfirm("确认删除？" , function(){
			var url = "<s:url value='/jszy/notice/delNotice'/>?infoFlow="+infoFlow;
			jboxGet(url , null , function(resp){
				toPage(1);
				$("#infoFlow").val("");
				$("#title").val("");
...
function toPage(page) {
	...
	jboxPostLoad("doctorListZi","<s:url value='/jszy/notice/notice'/>",$("#searchForm").serialize(),false);
}
