...
function submitRecruit(recruitFlow){
	if($("select[name=secondSpeId]").hasClass("validate[required]")){
		...
		var url = "<s:url value='/jszy/doctor/checkUserInfoInBlack'/>?userCode=${user.userCode}&userEmail=${user.userEmail}&userFlow=${user.userFlow}&userPhone=${user.userPhone}&idNo=${user.idNo}";
...
//				return false;
				...
				jboxConfirm("确认提交？提交后培训信息将不可修改",  function(){
					...
					var url = "<s:url value='/jszy/doctor/submitResDoctorRecruit'/>?auditStatusId=${jszyResDoctorAuditStatusEnumAuditing.id}&recruitFlow="+recruitFlow;
					jboxPost(url, null, function(resp){
...
function save(recruitFlow){
	jboxConfirm('确认保存?',function(){
		jboxPost("<s:url value='/jszy/doctorRecruit/saveTrainInfo'/>?recruitFlow="+recruitFlow , $("#infoForm").serialize() , function(resp){
			setTimeout(function(){
...
function save2(recruitFlow){
	...
	jboxConfirm('请确认二级专业信息，一旦保存无法修改二级专业信息?',function(){
		jboxPost("<s:url value='/jszy/doctorRecruit/saveSecondSpe'/>?recruitFlow="+recruitFlow , $("#infoForm").serialize() , function(resp){
			setTimeout(function(){
...
function delDoctorRecruit(recruitFlow){
	jboxConfirm("确认删除这条培训信息?",  function(){
		 var url= "<s:url value='/jszy/doctorRecruit/delDoctorRecruit'/>?recruitFlow="+recruitFlow;
		 jboxPost(url, null, function(resp){
...
function resetStatus(recruitFlow,doctorFlow,flag){
	...
	jboxConfirm("确认将该学员退回"+info+"?",  function(){
		var url = "<s:url value='/jszy/doctor/backResDoctorRecruit'/>?recruitFlow="+recruitFlow +"&auditStatusId=" + auditStatusId+"&doctorFlow="+doctorFlow;
		jboxPost(url, null, function(resp){
		    if("${GlobalConstant.OPRE_FAIL}" != resp){
...
function chargeResetOrg(recruitFlow,doctorFlow){
	if($("#returnNotice").is(":hidden")){
		...
		jboxConfirm("确认将该学员退回重审?",  function(){
			var url = "<s:url value='/jszy/doctor/backResDoctorRecruit'/>?doctorFlow="+doctorFlow+"&auditStatusId="+auditStatusId+"&recruitFlow="+recruitFlow ;
...
function applyChangeBase(){
	var url = "<s:url value='/jszy/doctor/applyChangeBase'/>?doctorFlow=${doctorRecruit.doctorFlow}&recruitFlow=${doctorRecruit.recruitFlow}&catSpeId=${doctorRecruit.catSpeId}&speId=${doctorRecruit.speId}&orgFlow=${doctorRecruit.orgFlow}";
...
function saveAuditRecruit(recruitFlow, doctorFlow,sessionNumber, auditFlag){
	...
	jboxConfirm("确认审核"+title+"?" ,  function(){
		var url = "<s:url value='/jszy/manage/province/doctor/saveAuditRecruit'/>";
...
function validateChange(type){
	var url = "<s:url value='/jszy/doctor/validateChange'/>?type="+type+"&doctorFlow=${doctorRecruit.doctorFlow}&orgFlow=${doctorRecruit.orgFlow}";
	jboxPost(url, null, function(resp){
...
function applyChangeSpe(){
	var url = "<s:url value='/jszy/doctor/applyChangeSpe'/>?doctorFlow=${doctorRecruit.doctorFlow}&recruitFlow=${doctorRecruit.recruitFlow}&catSpeId=${doctorRecruit.catSpeId}&speId=${doctorRecruit.speId}&orgFlow=${doctorRecruit.orgFlow}";
