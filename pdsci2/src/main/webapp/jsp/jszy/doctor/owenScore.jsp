...
$(document).ready(function(){
	...
	{
		var url = "<s:url value='/jszy/doctorTheoryScore/publicScoreDetail?scoreFlow='/>"+scoreFlow;
		jboxOpen(url, "非全科专业住院医师成绩详情",800,200);
	}
	//技能成绩详情
	function skillScoreDetail(doctorFlow,scoreFlow,scoreType)
	{
		var url = "<s:url value='/jszy/doctorTheoryScore/scoreDetail?scoreFlow='/>"+scoreFlow+"&doctorFlow="+doctorFlow+"&scoreType="+scoreType;
...
//		}
		jboxConfirm(msg+"确认申请？" , function() {
			jboxPost("<s:url value='/jszy/doctorTheoryScore/doctorSubmit?doctorFlow='/>" + doctorFlow, null, function (resp) {
				if ("1" == resp) {
					jboxTip("申请成功!");
	...
	{
		var url = "<s:url value='/jszy/doctorTheoryScore/guiPeiDetail?doctorFlow='/>"+doctorFlow;
...
<div class="main_bd" id="div_table_0" style="display: none;" >
	...
	<table border="0" cellpadding="0" cellspacing="0" class="base_info" >
		...
		<table border="0" cellpadding="0" cellspacing="0" class="base_info" >
			...
			<tr>
				<th style="text-align: center;" <c:if test="${not empty theoryList}"> rowspan="${fn:length(theoryList)+1}"</c:if><c:if test="${ empty theoryList}">rowspan="2"</c:if>>理论成绩</th>
