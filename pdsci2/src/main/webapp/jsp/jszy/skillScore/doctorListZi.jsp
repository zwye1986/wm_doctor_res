             ...
             <c:forEach items="${doctorList}" var="doctor">
				 ...
				 <c:if test="${roleFlag eq GlobalConstant.USER_LIST_GLOBAL}">
					 <tr>
						 ...
						 <td>
							 <%--<input style="width: 30px;border: 1px solid #e7e7eb;" class="inputText isPass" readOnly="true" onclick="$(this).removeAttr('readOnly');"--%>
							 ...
							 <%--<input class="isPass1" hidden value="<c:if test="${doctor.resScore.skillScore eq GlobalConstant.PASS}">是</c:if><c:if test="${doctor.resScore.skillScore eq GlobalConstant.UNPASS}">否</c:if>"/>--%>
								 <select class="select" onchange="saveScore2('${doctor.resScore.scoreFlow}','isPass',this,'是否合格','${roleFlag}');">
									 <option value="1" ${doctor.resScore.skillScore eq '1'?'selected':''}>合格</option>
									 <option value="0" ${doctor.resScore.skillScore eq '0'?'selected':''}>不合格</option>
									 <option value="2" ${doctor.resScore.skillScore eq '2'?'selected':''}>缺考</option>
								 </select>
						 </td>
						 <td style="">
							 <div style="width: 100px">
		          			<span>
							<!-- 培训基地 -->
				          	<a class="btn"  onclick="deleteSkillScore('${doctor.resScore.scoreFlow}','${roleFlag}','${param.currentPage}');" style="margin-top: 5px;margin-bottom: 5px;">删除</a>
							</span>
							 </div>
						 </td>
					 </tr>
				 ...
				 <c:if test="${roleFlag eq GlobalConstant.USER_LIST_CHARGE}">
					 <tr>
                         ...
                         <td>${doctor.resScore.skillScore eq '1'?'合格':(doctor.resScore.skillScore eq '0'?'不合格':'缺考')}</td>
                     </tr>
				 </c:if>
