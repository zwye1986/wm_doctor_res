...
function caculate(startDate,endDate){
	...
	var url = "<s:url value='/jszy/statistic/statisticsNoAppUser'/>?open=${param.open}&currentPage=${param.currentPage}&catSpeId=${param.catSpeId}&orgFlow=${param.orgFlow}&endDate="+endDate+"&startDate="+startDate+"&sessionNumber="+sessionNumber;
...
function changeTrainSpes(catSpeId,sessionNumber){
	<c:if test="${GlobalConstant.USER_LIST_LOCAL eq param.userListScope}">
		jboxLoad("content","<s:url value='/jszy/statistic/statisticsAppUser'/>?catSpeId="+catSpeId+"&sessionNumber="+sessionNumber+"&userListScope=local",true);
	</c:if>
	if("${GlobalConstant.USER_LIST_LOCAL}"!="${param.userListScope}"){
		var orgFlow="${param.orgFlow}";
		var url="<s:url value='/jszy/statistic/statisticsAppUser'/>?open=Y&orgFlow="+orgFlow+"&catSpeId="+catSpeId+"&sessionNumber="+sessionNumber+"&userListScope=";
...
function showNoAppUser(orgFlow,catSpeId,currentPage,sessionNumber){
	...
	var url = "<s:url value='/jszy/statistic/statisticsNoAppUser'/>?open=${param.open}&currentPage="+currentPage+"&catSpeId="+catSpeId+"&orgFlow="+orgFlow+"&sessionNumber="+sessionNumber;
	jboxPostLoad("othersDiv", url, function(resp){
		if(resp!=null){
			top.jboxEndLoading();
		}
