...
function operReduction(doctorFlow,rotationFlow,secondRotationFlow,orgFlow){
	...
	jboxOpen("<s:url value='/jszy/manage/operReduction'/>?doctorFlow="+doctorFlow+"&rotationFlow="+rotationFlow+"&secondRotationFlow="+secondRotationFlow+"&orgFlow="+orgFlow,"减免证明及方案减免维护",800,500);
}

function writeStudy(recruitFlow){
	var url = "<s:url value='/jszy/manage/province/doctor/doctorPass'/>?studyFlag=${GlobalConstant.FLAG_Y}&openType=open&recruitFlow="+recruitFlow;
...
function setType(flag){
	...
	function viewZpPic(doctorFlow){
		var url = "<s:url value='/jszy/manage/zpReductionFiles'/>?doctorFlow="+doctorFlow;
		jboxOpen(url,"减免证明",560,260);
	}
</script>
<div class="main_hd">
	<h2>方案减免维护</h2>
		...
		<table border="0" cellpadding="0" cellspacing="0" class="grid">
			...
			<c:forEach items="${doctorList}" var="doctor">
				<tr>
					...
					<td>
						<a class="btn" onclick="operReduction('${doctor.doctorFlow}','${doctor.rotationFlow}','${doctor.secondRotationFlow}','${doctor.orgFlow}');">方案减免维护</a>
						<%--查看在培状态减免证明--%>
						<c:set var="fileCount" value="${fn:length(reductionFilesMap[doctor.doctorFlow])}"/>
						<a class="btn"
						   <c:if test="${fileCount gt 1}">onclick="viewZpPic('${doctor.doctorFlow}')"</c:if>
						   <c:if test="${fileCount eq 1}"> target="_blank" href="${sysCfgMap['upload_base_url']}/${reductionFilesMap[doctor.doctorFlow][0].filePath}"</c:if> >查看减免证明</a>
