...
function turnOutOrg(recordFlow, auditFlag, doctorFlow){
	...
	if(title=="不通过"){
		var url="<s:url value='/jszy/manage/auditInfo'/>?recordFlow="+recordFlow +"&changeStatusId="+changeStatusId+"&doctorFlow="+doctorFlow;
		jboxOpen(url, "审核意见", 680, 400);
	}else{
		jboxConfirm("确认转出" + title +"?",  function(){
			var url = "<s:url value='/jszy/manage/turnOutOrg'/>?recordFlow="+recordFlow +"&changeStatusId="+changeStatusId+"&doctorFlow="+doctorFlow;
...
function searchOut(){
	var url ="<s:url value='/jszy/manage/turnOutMain'/>";
