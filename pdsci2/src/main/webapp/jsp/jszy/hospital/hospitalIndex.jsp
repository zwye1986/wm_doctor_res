...
function shouye(page,needScorll){
	...
	var url="<s:url value='/jszy/manage/local'/>?currentPage="+page+"&needScorll="+needScorll;
...
function baseInfoManage(){
	jboxLoad("content","<s:url value='/jszy/base/baseInfoManage'/>",true);
}
function activityTarget(){
	var roleFlag="${GlobalConstant.USER_LIST_LOCAL}";
	jboxLoad("content","<s:url value='/jszy/base/activityTarget/manageMain'/>?roleFlag="+roleFlag,true);
}
</script>
<style>
	.docTj{
 ...
 <div class="body">
   <div class="container">
	   <div class="content_side">
		   <%--国家基地：基地信息维护--%>
		   <c:if test="${applicationScope.sysOrgMap[sessionScope.currUser.orgFlow].orgLevelId eq 'CountryOrg'}">
			   <dl class="menu menu_first">
				   <dt class="menu_title">
					   <i class="icon_menu menu_management"></i>基地信息管理
				   </dt>
				   <dd class="menu_item"><a onclick="baseInfoManage();">基地信息维护</a></dd>
			   </dl>
		   </c:if>
		   <dl class="menu menu_first">
			   <dt class="menu_title">
			   ...
			   <dd class="menu_item" id="reductionRotationOper"><a onclick="reductionRotationOper();">方案减免维护
				   <img id="reducationCount" style="display: none;"
						src="<s:url value="/jsp/jszy/images/gantanhao.png" />"/></a>
			   </dd>
		   </dl>
		   <dl class="menu">
			   <dt class="menu_title">
