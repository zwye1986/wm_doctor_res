...
function zpxytj(){
	var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
	jboxLoad("content","<s:url value='/jszy/doctorRecruit/zpxytj'/>?roleFlag="+roleFlag,true);
}
//招录学员信息查询
function doctorListZl(){
    var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
    jboxLoad("content","<s:url value='/jszy/doctorRecruit/provinceDoctorList'/>?roleFlag="+roleFlag+"&zlFlag=Y",true);
}
function noticeManage(resNoticeSysId,typeId){
	var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
	jboxLoad("content","<s:url value='/jszy/notice/main'/>?roleFlag="+roleFlag+"&sysId="+resNoticeSysId+"&typeId="+typeId,true);
}
//基地专业管理
...
function parameterConfig(){
    jboxLoad("content","<s:url value='/jszy/sysCfg/parameterConfigMain'/>",true);
}
...
function reductionTab(){
	var url = "<s:url value='/jszy/reduction/reductionTab?roleId=${GlobalConstant.USER_LIST_GLOBAL}'/>";
	currentJboxLoadNoData("content", url, true);
}

//基地变更管理
function changeBaseTab(){
	var url = "<s:url value='/jszy/manage/changeBaseTab?roleId=${GlobalConstant.USER_LIST_GLOBAL}'/>";
	currentJboxLoadNoData("content", url, true);
}
//医师专业变更
function speMain(){
	var url = "<s:url value='/jszy/manage/changeSpeTab?roleId=${GlobalConstant.USER_LIST_GLOBAL}'/>";
	jboxLoad("content", url, true);
}
//退培医师查询
function backTrain(){
	var url = "<s:url value='/jszy/delayReturn/backTrainTab?roleId=${GlobalConstant.USER_LIST_GLOBAL}'/>";
	currentJboxLoadNoData("content", url, true);
}
//延期医师查询
function delayInfoManage(){
	var url = "<s:url value='/jszy/delayReturn/delayTab?roleId=${GlobalConstant.USER_LIST_GLOBAL}'/>";
	currentJboxLoadNoData("content", url, true);
}
//师资培训统计
...
function searchBlackInfo(){
	var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
	var url = "<s:url value='/jszy/blackList/blackListInfo?currentPage=1'/>"+"&roleFlag="+roleFlag;
	currentJboxLoadNoData("content", url, true);
}
function resMain(){
...
//住院医师招录统计
function zlxytj(){
	var url = "<s:url value='/jszy/statistic/zlxytj'/>";
	jboxLoad("content", url, true);
}
//招录学员统计
function zlxytj2(){
	var url = "<s:url value='/jszy/statistic/zlxytj2'/>?trainTypeId=${jszyTrainCategoryEnumChineseMedicine.id}";
	jboxLoad("content", url, true);
}
//招录学员统计-专业
function zlxytj3(){
	var url = "<s:url value='/jszy/statistic/zlxytj3'/>";
	jboxLoad("content", url, true);
}
//招录学员统计-汇总
function zlxytj4(){
	var url = "<s:url value='/jszy/statistic/zlxytj4'/>";
	jboxLoad("content", url, true);
}
//招录学员统计（此数据为固定数据）
function zlxytj5(){
    var url = "<s:url value='/jszy/statistic/zlxytj5'/>";
    jboxLoad("content", url, true);
}
//存档学员查询
function archiveDoctorList(){
	var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
	jboxLoad("content","<s:url value='/jszy/doctorRecruit/provinceDoctorList'/>?isArchive=Y&roleFlag="+roleFlag,true);
}
function CertificateManage(){
	var roleFlag="${GlobalConstant.USER_LIST_GLOBAL}";
	jboxLoad("content","<s:url value='/jszy/certificateManage/main'/>?roleFlag="+roleFlag,true);
}
function graduationManager(pageType,tabType){
	jboxLoad("content","<s:url value='/jszy/graduationManager/main'/>?roleFlag=${GlobalConstant.USER_LIST_GLOBAL}&pageType="+pageType+"&tabType="+tabType,true);
}
function asseManager(){
	jboxLoad("content","<s:url value='/jszy/asseGlobal/main'/>?roleFlag=${GlobalConstant.USER_LIST_GLOBAL}",true);
}
function shouye(){
	var url="<s:url value='/jszy/manage/global'/>";
   ...
   <div class="head_inner">
     ...
     <div class="account">
       ...
       <div class="head_right">
        <a href="<s:url value='/jszy/manage/global'/>">首页</a>&#12288;
        ...
        <dl class="menu">
          <dt class="menu_title">
           ...
           <dd class="menu_item"><a onclick="changeBaseTab();">基地变更管理
			   <img  id="changeOrgFlag" style="display: none;" src="<s:url value="/jsp/jszy/images/gantanhao.png" />"/></a>
		   </dd>
          <dd class="menu_item"><a onclick="speMain();">专业变更管理
           <img  id="speFlag" style="display: none;" src="<s:url value="/jsp/jszy/images/gantanhao.png" />"/></a>
          </dd>
		  <dd class="menu_item"><a href="javascript:backTrain();">退培学员管理
			  <img  id="backFlag" style="display: none;" src="<s:url value="/jsp/jszy/images/gantanhao.png" />"/></a>
		  </dd>
          <dd class="menu_item"><a onclick="delayInfoManage();">延期学员管理
			  <img id="delayFlag" style="display: none;" src="<s:url value="/jsp/jszy/images/gantanhao.png" />"/></a>
		  </dd>
