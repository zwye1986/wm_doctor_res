...
function toPage(page) {
	...
	var url="<s:url value='/jszy/base/baseInfo'/>?type=${param.type}&currentPage="+page;
	jboxLoad("hosContent",url,false);
	} 
function searchJointOrg(orgFlow){
	var url="<s:url value='/jszy/base/searchJOrgInfo'/>?flag=${GlobalConstant.FLAG_Y}&orgFlow="+orgFlow;
            ...
            <c:forEach items="${orgList}" var="org" varStatus="status">
				<c:set var="rowspan" value="${fn:length(jointMap.get(org.orgFlow))}"></c:set>
					<tr>
						...
						<td rowspan="${rowspan eq 0?1:rowspan}"><a style="cursor:pointer;color:blue;" href="<s:url value='/jszy/base/baseInfoManage?orgFlow=${org.orgFlow}&viewFlag=Y'/>" target="_blank">${org.orgName}</a></td>
