...
function searchInfo(sessionNumber,trainTypeId){
	...
	jboxLoad("content","<s:url value='/jszy/statistic/statisticCountryOrg'/>?orgLevel="+orgLevel+""+data+"&viewBox="+viewBox+"&trainTypeId="+trainTypeId+"&sessionNumber="+sessionNumber,true);
}
...
function exportExcel(){
	...
	jboxTip("导出中…………");
	var url = "<s:url value='/jszy/statistic/exportExcel'/>?"+data+"&"+$("#searchForm").serialize();
								...
								<td style="width: 170px;min-width:170px;max-width:170px;" class="by">
									...
									<c:if test="${orgSpeFlagMap[flow] eq GlobalConstant.FLAG_Y}">
										...
										<div class="docNameView" style="display: none;">
											<img  style="vertical-align:middle; margin-right:10px;" src="<s:url value='/jsp/jszy/images/gou.gif'/>"/>
										</div>
									</c:if>
									<div class="kong">
			...
			<tr >
				<td colspan="${sysOrgList.size()}">
					<label>说明：<br>
						...
						(3) <img  style="vertical-align:middle; margin-right:10px;" src="<s:url value='/jsp/jszy/images/gou.gif'/>"/>
						代表有此专业。
					</label>
