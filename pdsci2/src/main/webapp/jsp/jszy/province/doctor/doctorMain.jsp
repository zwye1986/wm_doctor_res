...
function getDoctorRecruit(recruitFlow, doctorFlow, backFlag){
    <c:if test="${GlobalConstant.FLAG_Y == zlFlag }">
        var url = "<s:url value='/jszy/doctor/getDoctorRecruitInfo?info=${param.info}&openType=open&recruitFlow='/>"+recruitFlow+"&doctorFlow="+doctorFlow+"&backFlag="+backFlag+"&zlFlag=${zlFlag}";
    </c:if>
    <c:if test="${GlobalConstant.FLAG_Y != zlFlag }">
        var url = "<s:url value='/jszy/doctor/getDoctorRecruit?info=${param.info}&openType=open&recruitFlow='/>"+recruitFlow+"&doctorFlow="+doctorFlow+"&backFlag="+backFlag;
    </c:if>
	jboxLoad("doctorContent", url, false);
}
function doctorInfo(doctorFlow){
	var url = "<s:url value='/jszy/doctor/doctorInfo'/>?viewFlag=${GlobalConstant.FLAG_Y}&openType=open&userFlow=" + doctorFlow+"&hideApprove=${param.hideApprove}";
