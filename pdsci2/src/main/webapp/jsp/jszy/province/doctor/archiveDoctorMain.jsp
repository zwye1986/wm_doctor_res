...
<head>
	...
	<script type="text/javascript">
		...
		function getDoctorRecruit(recruitFlow, doctorFlow, backFlag,archiveFlow){
			var url = "<s:url value='/jszy/archive/getDoctorRecruit?info=${param.info}&openType=open&recruitFlow='/>"+recruitFlow+"&doctorFlow="+doctorFlow+"&backFlag="+backFlag+"&archiveFlow="+archiveFlow;
			jboxLoad("doctorContent", url, false);
		}
		function doctorInfo(doctorFlow,archiveFlow){
			var url = "<s:url value='/jszy/archive/doctorInfo'/>?viewFlag=${GlobalConstant.FLAG_Y}&openType=open&userFlow=" + doctorFlow+"&hideApprove=${param.hideApprove}"+"&archiveFlow="+archiveFlow;
			jboxLoad("doctorContent", url, true);
			jboxEndLoading();
		}
		function trainInfo(){
			jboxLoad("doctorContent","<s:url value='/jsp/jszy/province/doctor/trainInfo.jsp'/>?type=${param.type}",false);
		}
		function getInfo(recruitFlow){
			var url = "<s:url value='/jszy/doctor/getDocRecruitForReduction'/>?studyFlag=${GlobalConstant.FLAG_Y}&openType=open&recruitFlow="+recruitFlow;
			jboxLoad("doctorContent", url, false);
		}
		function backTrain(userFlow){
			var url = "<s:url value='/jszy/doctor/backTrainInfo?seeFlag=Y&userFlow='/>"+userFlow;
			jboxLoad("doctorContent", url, false);
		}
	...
	<div class="title_tab" style="margin-top: 0;">
		<ul>
			<c:if test="${GlobalConstant.FLAG_Y != studyFlag }">
				<li><a onclick="doctorInfo('${param.doctorFlow}','${archiveFlow}');">基本信息</a></li>
				<c:forEach items="${recruitList}" var="recruit" varStatus="status">
					<c:if test="${jszyResDoctorAuditStatusEnumPassed.id eq recruit.auditStatusId }">
						<li <c:if test="${param.recruitFlow ==recruit.recruitFlow }">class="tab_select"</c:if>id="${recruit.recruitFlow}" onclick="getDoctorRecruit('${recruit.recruitFlow}','${param.doctorFlow}','${status.last}','${archiveFlow}');"><a>${recruit.catSpeName}（${empty recruit.speName?'--':recruit.speName}）</a></li>
					</c:if>
				</c:forEach>
