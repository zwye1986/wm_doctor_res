...
<head>
    ...
    <script type="text/javascript">
        ...
        function showInfo(userFlow){
            jboxStartLoading();
            var url= "<s:url value='/xjgl/user/userCertificateInfo'/>?userFlow="+userFlow;
...
<div class="mainright">
    <div class="content">
        <form id="searchForm" action="<s:url value="/xjgl/partyBranch/partyMemberList"/>" method="post">
        ...
        <table class="xllist" style="width:100%;">
            ...
            <c:forEach items="${dataList}" var="info" varStatus="i">
                <tr>
                    ...
                    <td>
                        <c:if test="${not empty info.firstTeacherFlow and empty info.secondTeacherFlow}">
                            ${info.firstTeacher}
                        </c:if>
                        <c:if test="${empty info.firstTeacherFlow and not empty info.secondTeacherFlow}">
                            ${info.secondTeacher}
                        </c:if>
                        <c:if test="${not empty info.firstTeacherFlow and not empty info.secondTeacherFlow}">
