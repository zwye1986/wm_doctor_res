...
<head>
    ...
    <script type="text/javascript">
        ...
        function editInfo(tutorFlow,changeFlag){
            var url = "<s:url value='/xjgl/tutor/changeTutor?tutorFlow='/>"+tutorFlow+"&role=xwk"+"&changeFlag="+changeFlag;
        ...
        function blockInfo(doctorFlow,flag){
            ...
            jboxConfirm("确认"+msg+"此条信息？", function(){
                var url = "<s:url value='/xjgl/tutor/blockInfo?doctorFlow='/>"+doctorFlow+"&flag="+flag;
        ...
        function exportTutors(){
            var url = "<s:url value='/xjgl/tutor/exportFromChange'/>";
            jboxTip("导出中…………");
        ...
        function batchBlock(flag){
            ...
            jboxConfirm("确认批量"+msg+"当前勾选的导师？", function(){
                var url = "<s:url value='/xjgl/tutor/blockInfoBatchOpt?recordLst='/>"+recordLst+"&opeFlag="+flag;
                jboxPost(url, null, function(resp){
                    if (resp == "${GlobalConstant.OPERATE_SUCCESSED}") {
                        location.reload();
                    }
...
<div class="mainright">
    <div class="content">
        <form id="searchForm" action="<s:url value="/xjgl/tutor/tutorChange?role=${param.role}"/>" method="post">
