            ...
            <table class="basic" style="width: 100%;margin: 10px 0px;">
                <tr>
                    <td style="text-align: left;" colspan="2">
                        &ensp;问卷名称：<input class="validate[required]" name="questionName" style="width: 500px;text-align: left;"
                                          value="${courseQuestion.questionName}"/>
                    </td>
                </tr>
                <tr>
                    <td style="text-align: left;" colspan="2">
                        ...
                        <c:forEach items="${dictTypeEnumXjCourseTypeList}" var="dict">
                            <option <c:if test="${courseQuestion.courseTypeId eq dict.dictId}">selected="selected"</c:if> value="${dict.dictId}">${dict.dictName}</option>
                        </c:forEach>
                    </select><input type="hidden" name="courseTypeName" value="${courseQuestion.courseTypeName}">
                    </td>
                </tr>
                <tr>
                ...
                <tbody id="questionTbody">
                    <c:forEach items="${dataList}" var="data">
                        <tr id='question_${data.questionDetail.serial}'>
                            <td style='text-align:center;'>${data.questionDetail.questionTitle}</td>
                            <td style='text-align:center;'>
                                <a onclick="editThis(this);" style='cursor:pointer;color:blue;'>[编辑]</a>
                                <a onclick="delThis(this);" style='cursor:pointer;color:blue;'>[删除]</a>
                            </td>
                            <input type='hidden' name='detailJsonData' value='${jsonMap[data.questionDetail.serial]}'/>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
