...
<head>
	...
	<script type="text/javascript">
		function importExl(){
			jboxOpen("<s:url value='/xjgl/majorCourse/importCourseMajor'/>","导入",600,180);
		}
		var year='${year}';
		function edit(flow){
			var url = "<s:url value='/xjgl/term/manage/edit'/>?flow="+flow+"&isEdit=Y";
			var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
			jboxMessager(iframe,'课表基本信息维护',756,300,null,false);
		}
		function add(){
			var url = "<s:url value='/xjgl/term/manage/edit'/>";
			var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
		...
		function loadDetail(flow,weekNumber,refreshFlag)
		{
			...
			var url = "<s:url value='/xjgl/term/manage/loadDetail?roleFlag=${roleFlag}&recordFlow='/>" + flow+"&assumeOrgFlow="+assumeOrgFlow+"&courseFlow="+courseFlow
		...
		function arrangePlan(obj,day){
			...
			var url = "<s:url value='/xjgl/term/manage/courseAdd?roleFlag=${roleFlag}&termFlow='/>"+termFlow+"&classTime="+classTime+"&classOrder="+encodeURI(encodeURI(classOrder))+"&recordFlow="+recordFlow+"&courseTypeId=${courseTypeId}&recordMap=";//+JSON.stringify(map);
			jboxOpen(url, "增加排课",460,260);
		}
		function delInfo(termFlow,recordFlow,courseFlow){
			jboxConfirm("确定删除此课吗？",function(){
				jboxPost("<s:url value='/xjgl/term/manage/delClass'/>?courseTypeId=${courseTypeId}&recordFlow="+recordFlow+"&courseFlow="+courseFlow,null,function(resp){
		...
		function settingOpt(){
			...
			var url = "<s:url value='/xjgl/term/manage/scheduleSetting?sessionNumber='/>"+$("[name='sessionNumber']").val();
			jboxOpen(url, "排课设置",660,460);
		}
		function exportExcel(recordFlow){
			var url = "<s:url value='/xjgl/term/manage/exportClass?recordFlow='/>"+recordFlow;
			jboxTip("导出中…………");
...
<div class="mainright">
	<div class="content">
		<form id="recSearchForm" method="post" action="<s:url value='/xjgl/term/manage/list'/>">
				...
				<c:if test="${roleFlag ne 'skz'}">
					<tr>
						<td style="border: none;">
							...
							<select name="gradeTermId" style="width:141px;">
								...
								<c:forEach items="${dictTypeEnumTermSeasonList}" var="recruitSeason">
					...
					<tr>
						<td>
							...
							<c:if test="${trainTypeId eq '1' and className eq '2018年级硕士四教班'}">
								&#12288;&#12288;<font style="color: red;">各专业课表，以发放的纸质版课表为准。</font>
							</c:if>
						</td>
					</tr>
				</c:if>
