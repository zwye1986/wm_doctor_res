...
<script type="text/javascript">
    ...
    function save(){
        ...
        jboxConfirm(title, function(){
            ...
            var url = "<s:url value='/xjgl/student/course/saveStudentCourse'/>?studentPeriod=${param.period}&userFlow=${param.userFlow}";
            jboxPostJson(url,
    ...
    function saveBefore(){
        ...
        var courseCreditSum = $("#courseCreditSum").text();
        var coursePeriodSum = $("#coursePeriodSum").text();
        var title = "您当前总共选修课程<font color='red'>"+ courseCount + "</font>门，总学时<font color='red'>" + coursePeriodSum + "</font>，总学分<font color='red'>" + courseCreditSum + "</font>分，确认保存？";
        jboxConfirm(title, function(){
            var $courseFlows = $("input[name='courseFlow']:checked");
            var datas = [];
            $.each($courseFlows, function(i,n){
            ...
            var requestData = {"studentCourseList":datas};
            var url = "<s:url value='/xjgl/student/course/saveStudentCourse'/>?studentPeriod=${param.period}&userFlow=${param.userFlow}&saveBeforeFlag=Y";
            jboxPostJson(url,
                    JSON.stringify(requestData),
                    function(resp){
                        if("${GlobalConstant.OPRE_SUCCESSED}" == resp){
    ...
    function courseOutline(courseFlow,courseName){
        var url = "<s:url value ='/xjgl/course/manage/searchOutline'/>?reqFlag=outline&courseFlow="+courseFlow;
        jboxOpen(url,courseName+"课程大纲查看",800,500);
    }
    function courseTeaching(courseFlow,courseName){
        var url = "<s:url value ='/xjgl/course/manage/searchOutline'/>?reqFlag=teaching&courseFlow="+courseFlow;
            ...
            <c:forEach items="${resultMap['courseTypeMajorMap'][courseType.id]}" var="courseMajorExt" varStatus="status">
                ...
                <tr>
                    ...
                    <td style="text-align:left;">&#12288;
                        ...
                        <font color="${((chooseStudentCourse.replenishFlag eq GlobalConstant.FLAG_Y || courseMajorExt.majorId ne majorId || course.gradationId ne trainTypeId) && course.gradationId ne '3')?'red':''}">[${course.courseCode}]${course.courseName}</font>
                    ...
                    <td><input type="checkbox" name="degreeCourseFlag" ${chooseStudentCourse.degreeCourseFlag eq 'Y'?'checked':''} onclick="calculate();" <c:if test="${!inChooseTime}">disabled="disabled"</c:if>/></td>
                    <td>
                        ...
                        <input type="checkbox" class="${courseType.id}_checkBox ${course.gradationId}_checkBox" id="${course.courseFlow}" name="courseFlow" value="${course.courseFlow}" onclick="chooseCourse('${course.courseFlow}');"
                               <c:if test="${not empty chooseStudentCourse}">checked="checked"</c:if><c:if test="${((courseMajorExt.majorId ne majorId || course.gradationId ne trainTypeId ) and course.gradationId ne '3')||!inChooseTime}">disabled="disabled"</c:if> />
                    </td>
                </tr>
