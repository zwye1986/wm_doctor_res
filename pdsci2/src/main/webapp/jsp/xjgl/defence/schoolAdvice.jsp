...
<head>
    ...
    <script type="text/javascript">
        ...
        function expExcel(){
            var url = "<s:url value='/xjgl/paper/expAdvice'/>";
            jboxTip("导出中…………");
...
<div class="mainright">
    <div class="content">
        ...
        <c:if test="${param.role ne 'xs'}">
            <form id="searchForm" action="<s:url value="/xjgl/paper/schoolAdvice"/>" method="post">
                <div class="choseDivNewStyle">
                    <input id="currentPage" type="hidden" name="currentPage" value="1"/>
                    <input type="hidden" name="role" value="${param.role}"/>
                    <input type="hidden" name="funcFlag" value="xxAdvice"/>
            ...
            <c:forEach items="${defenceList}" var="defence" varStatus="i">
                <tr>
                    ...
                    <td>
                        ...
                        <input type="radio" id="xxwhAdviceFlagN${i.index}" name="xxwhAdviceFlag${i.index}" ${defence.xxwhAdviceFlag eq 'N'?'checked':''} onchange="updateOpt('${defence.recordFlow}','N','xxwhAdviceFlag')" ${(param.role eq 'xx' and readOnlyFlag ne 'Y')?'':'disabled'}><label for="xxwhAdviceFlagN${i.index}">建议缓授</label>
                    </td>
                </tr>
            </c:forEach>
