...
<head>
    ...
    <script type="text/javascript">
        ...
        function detailInfo(recordFlow){
            var url = "<s:url value='/xjgl/paper/editDefence?recordFlow='/>"+recordFlow+"&viewFlag=view&role=${param.role}&isReplenish=${isReplenish}";
            jboxOpen(url, "查看",800,460);
        }
        function auditInfo(recordFlow,tutorFlag){
            var url = "<s:url value='/xjgl/paper/editDefence?role=${param.role}&auditFlag=Y&recordFlow='/>"+recordFlow+"&tutorFlag="+tutorFlag+"&isReplenish=${isReplenish}";
            jboxOpen(url, "审核",800,460);
        }
        function expExcel(){
            var url = "<s:url value='/xjgl/paper/expExcel'/>?role=${param.role}";
            jboxTip("导出中…………");
            jboxSubmit($("#searchForm"), url, null, null, false);
            jboxEndLoading();
        }
        function leadTo(){
            jboxOpen("<s:url value='/xjgl/paper/leadTo'/>?isReplenish=${isReplenish}","导入",360,150);
        }
        $(function(){
...
<div class="mainright">
    <div class="content">
        <form id="searchForm" action="<s:url value="/xjgl/paper/paperAudit?role=${param.role}"/>" method="post">
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <th style="min-width:60px;line-height: 130%;">培养单位</th>
                <th style="min-width:60px;line-height: 130%;">分委会</th>
                <th style="min-width:60px;line-height: 130%;">导师审核</th>
                <th style="min-width:55px;line-height: 130%;">培养单位审核</th>
