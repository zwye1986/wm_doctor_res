...
<head>
    ...
    <script type="text/javascript">
        function save(){
            ...
            jboxPost("<s:url value='/xjgl/paper/saveTitle'/>", $("#myForm").serialize(), function (resp) {
                ...
                <table class="basic" style="width: 100%;">
                    ...
                    <tr>
                        ...
                        <td colspan="2">刊物类型：<select class="select validate[required]" name="journalType" style="width:156px;" onchange="chooseJournal(this.value)" ${param.viewFlag eq 'view'?'disabled':''}>
                            <option/>
                            <option value="sci" <c:if test="${extInfo.journalType eq 'sci'}">selected</c:if>>SCI</option>
                            <option value="ei" <c:if test="${extInfo.journalType eq 'ei'}">selected</c:if>>EI</option>
                            <option value="zwhx" <c:if test="${extInfo.journalType eq 'zwhx'}">selected</c:if>>中文核心</option>
                            <option value="zwtjy" <c:if test="${extInfo.journalType eq 'zwtjy'}">selected</c:if>>中文统计源</option>
                            <option value="qt" <c:if test="${extInfo.journalType eq 'qt'}">selected</c:if>>其他</option>
                        </select></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="2">文章发表状态：<select class="select validate[required]" name="articleStatus" style="width:156px;" ${param.viewFlag eq 'view'?'disabled':''}>
                            <option/>
                            <option value="fb" <c:if test="${extInfo.articleStatus eq 'fb'}">selected</c:if>>发表</option>
                            <option value="ly" <c:if test="${extInfo.articleStatus eq 'ly'}">selected</c:if>>录用</option>
                            <option value="qt" <c:if test="${extInfo.articleStatus eq 'qt'}">selected</c:if>>其他</option>
                        </select></td>
