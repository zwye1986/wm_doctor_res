...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            loadMajorName($("input[name='eduUserExtInfoForm.underMajor']"));
            loadMajorName($("input[name='eduUserExtInfoForm.gotMasterCertSpe']"));
            loadSubjectName($("input[name='eduUserExtInfoForm.masterSubject']"));
            loadSubjectName($("input[name='eduUserExtInfoForm.gotBachelorCertSubject']"));
        });
        function loadOrgAndTeacher(){
            var orgArray = new Array();
            if(${not empty orgList}){
                <c:forEach items="${orgList}" var="org" varStatus="i">
                    var orgFlow='${org.orgFlow}';
                    if(${empty org.orgFlow}){
                        orgFlow="";
                    }
                    var orgName='${org.orgName}';
                    var courseName='${org.orgName}';
                    if(${empty org.orgName}){
                        orgName="";
                        courseName="";
                    }
                    orgArray[${i.index}]=new Array(orgFlow,orgName,courseName);
                    if($("#resDoctor_orgFlow").val() == orgFlow){
                        $("#param_Org").val(orgName);
                    }
                </c:forEach>
                jboxStartLoading();
                $("#param_Org").suggest(orgArray,{
                    attachObject:'#suggest_Org',
                    dataContainer:'#resDoctor_orgFlow',
                    courseName:'#resDoctor_orgName',
            ...
            var secondTeacherArray = new Array();
            if(${not empty tutorList}){
                <c:forEach items="${tutorList}" var="tutor" varStatus="i">
                var secondTeacherFlow='${tutor.doctorFlow}';
                if(${empty tutor.doctorFlow}){
                    secondTeacherFlow="";
                }
                var secondTeacher='${tutor.doctorName}';
                var courseName='${tutor.doctorName}';
                if(${empty tutor.doctorName}){
                    secondTeacher="";
                    courseName="";
                }
                secondTeacherArray[${i.index}]=new Array(secondTeacherFlow,secondTeacher,courseName);
                if($("#secondTeacherFlow").val() == secondTeacherFlow){
                    $("#param_secondTeacher").val(secondTeacher);
                }
                </c:forEach>
                jboxStartLoading();
                $("#param_secondTeacher").suggest(secondTeacherArray,{
                    attachObject:'#suggest_secondTeacher',
                    dataContainer:'#secondTeacherFlow',
                    courseName:'#secondTeacher',
                    triggerFunc:function(orgFlow){
                    },
                    enterFunc:function(orgFlow){
                    }
        ...
        function adjustResults() {
            $("#suggest_Org").css("left",$("#param_Org").offset().left);
            $("#suggest_Org").css("top",$("#param_Org").offset().top+$("#param_Org").outerHeight());
        }
        ...
        function adjustResults2() {
            $("#suggest_secondTeacher").css("left",$("#param_secondTeacher").offset().left);
            $("#suggest_secondTeacher").css("top",$("#param_secondTeacher").offset().top+$("#param_secondTeacher").outerHeight());
        }
        ...
        function save() {
            ...
            //学位分委会 名称 赋值
            var deptIdText = $("#deptFlow option:selected").val();
            $("#trainOrgId").val(deptIdText);
            var deptNameText = $("#deptFlow option:selected").text();
            $("#deptName").val(deptNameText);
            $("#trainOrgName").val(deptNameText);
            //未选择省/市/区 清空信息
            ...
            //清空培养单位，导师信息
            if($("#param_Org").val()==""){
                $("#resDoctor_orgFlow").val("");
                $("#resDoctor_orgName").val("");
            }
            if($("#param_firstTeacher").val()==""){
                $("#firstTeacherFlow").val("");
                $("#firstTeacher").val("");
            }
            if($("#param_secondTeacher").val()==""){
                $("#secondTeacherFlow").val("");
                $("#secondTeacher").val("");
            }
            //任意填写导师可以保存姓名
            if($("#firstTeacher").val()!=$("#param_firstTeacher").val()){
                $("#firstTeacherFlow").val("");
                $("#firstTeacher").val($("#param_firstTeacher").val());
            }
            if($("#secondTeacher").val()!=$("#param_secondTeacher").val()){
                $("#secondTeacherFlow").val("");
                $("#secondTeacher").val($("#param_secondTeacher").val());
            }
            var form = $("#addForm").serialize();
            jboxPost("<s:url value='/xjgl/user/saveEduUser'/>", form, function (obj) {
        ...
        $(document).ready(function () {
            <c:if test="${roleFlag eq 'global' && !isFeeMaster }">
            ...
            if ($('.politics') == "01") {
                $('.redStarPB').show();
                $('.needFlagPB').addClass("validate[required]");
            } else {
                $('.redStarPB').hide();
                $('.needFlagPB').removeClass("validate[required]");
            }
            //政治面貌change绑定
            ...
            <c:if test="${isSzkMaster}">
                selectTag(this,'needInfo');
            </c:if>
            <c:if test="${!isSzkMaster}">
                selectTag(this,'baseInfo');
            </c:if>
            bindStydyForm('${eduUser.studyFormId}');
        });
        function getOrgName() {
        ...
        function printTemp(userFlow){
            var url = '<s:url value="/xjgl/user/printUserCertificateInfo?userFlow="/>'+userFlow;
            jboxTip("打印中，请稍等...");
            setTimeout(function(){
        ...
        function showFeeDetail(sid){
            var url = '<s:url value="/xjgl/user/searchStuFeeInfo?sid="/>'+sid;
            jboxOpen(url,"金额明细",800,600,true);
        }
    </script>
</head>
<body>
            ...
            <table class="basic" style="width: 100%; background-color: white;" align="center">
                <tr>
                    <td>
                        ...
                        <c:if test="${roleFlag eq 'global' && !isFeeMaster && (empty eduUser.chooseCourseStatusId or (eduUser.chooseCourseStatusId eq xjChooseCourseStatusEnumUnChoose.id))}">
                            <c:set var="importModifyFlag" value="true"/>
                        </c:if>
                        <c:if test="${permissionCfg.cfgValue eq 'N' && roleFlag eq 'global' && !isFeeMaster}">
                            &#12288;是否允许修改学籍：
                        ...
                        <div style="float: right;margin-top: 3px;">
                            <c:if test="${roleFlag eq 'doctor' && (employmentFlag or paperFlag or degreeGrantFlag or requiredFlag or optionalFlag)}">
                            ...
                            <input type="button" class="search" onclick="printTemp('${eduUser.userFlow}');" value="打&#12288;印"/>
                        </div>
                    </td>
                </tr>
            </table>
                ...
                <div style="margin-top:10px;">
                        ...
                        <div class="spreadOneOne">
                            <table class="basic" style="width:100%; margin-top: 3px; margin-bottom: 30px;">
                                        ...
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <select style="width: 144px;" id="deptFlow" name="sysUser.deptFlow" class=" inputText">
                                                ...
                                                <c:forEach items="${deptList }" var="dept">
                                                    <option value="${dept.deptFlow}" <c:if test="${dept.deptFlow==sysUser.deptFlow }">selected</c:if>>${dept.deptName}</option>
                                                </c:forEach>
                                            </select>
                                            <input type="hidden" id="deptName" name="sysUser.deptName">
                                            <input type="hidden" id="trainOrgId" name="eduUser.trainOrgId">
                                            <input type="hidden" id="trainOrgName" name="eduUser.trainOrgName">
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <c:forEach items="${deptList }" var="dept">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input id="param_Org"  placeholder="输入单位名称" class="inputText"  style="width:141px;text-align: left;" onkeydown="adjustResults();" onfocus="adjustResults();"/>
                                            <div id="suggest_Org" class="ac_results" style="margin:0;position: fixed; z-index: 100;width:140px;"></div>
                                            <input type="hidden" id="resDoctor_orgFlow" name="resDoctor.orgFlow" value="${resDoctor.orgFlow}"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <label>${resDoctor.orgName }</label>
                                        </c:if>
                                        <input type="hidden" id="resDoctor_orgName" name="resDoctor.orgName" value="${resDoctor.orgName }"/>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input id="param_secondTeacher"  placeholder="输入导师姓名" class="inputText"  style="width:141px;text-align: left;" onkeydown="adjustResults2();" onfocus="adjustResults2();" value="${eduUser.secondTeacher}"/>
                                            <div id="suggest_secondTeacher" class="ac_results" style="margin:0;position: fixed; z-index: 100;width:140px;"></div>
                                            <input type="hidden" id="secondTeacherFlow" name="eduUser.secondTeacherFlow" value="${eduUser.secondTeacherFlow}"/>
                                            <input type="hidden" id="secondTeacher" name="eduUser.secondTeacher" value="${eduUser.secondTeacher}"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <select style="width: 144px;" name="eduUser.studyFormId" class="inputText" onchange="bindStydyForm(this.value)">
                                                <option/>
                                ...
                                <tr class="studyFormTr">
                                    <th><span >课程班入班时间：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input type="text" style="width: 140px;text-align: left;" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="inputText ctime validate[required]" name="eduUserExtInfoForm.inClassTime" value="${extInfoForm.inClassTime }">
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <label>${extInfoForm.inClassTime}</label>
                                        </c:if>
                                    </td>
                                    <th><span >课程班结业时间：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input type="text" style="width: 140px;text-align: left;" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="inputText ctime validate[required]" name="eduUserExtInfoForm.classEndTime" value="${extInfoForm.classEndTime }">
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <label>${extInfoForm.classEndTime}</label>
                                        </c:if>
                                    </td>
                                </tr>
                                <tr class="studyFormTr">
                                    <th><span >结业证书编号：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input type="text" class="inputText validate[required]" style="width: 140px;text-align: left;" name="eduUserExtInfoForm.classCertNo" value="${extInfoForm.classCertNo }"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <label>${extInfoForm.classCertNo}</label>
                                        </c:if>
                                    </td>
                                    <th><span >导师接收时间：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input type="text" style="width: 140px;text-align: left;" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="inputText ctime validate[required]" name="eduUserExtInfoForm.receiveTime" value="${extInfoForm.receiveTime }">
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <label>${extInfoForm.receiveTime}</label>
                                        </c:if>
                                    </td>
                                </tr>
                                <tr>
                                        ...
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <select style="width: 144px;" name="eduUserExtInfoForm.recruitType" class="inputText recruitType">
                                                ...
                                                <c:forEach items="${dictTypeEnumRecruitTypeList}" var="dict">
                                                    <option value="${dict.dictId}" ${extInfoForm.recruitType eq dict.dictId?'selected':''}>${dict.dictName}</option>
                                                </c:forEach>
                                            </select>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                        ...
                        <div class="spreadOneThree">
                            <form id="spreadOneThreeForm">
                                <table class="basic" style="width:100%; margin-top: 3px; margin-bottom: 30px;">
                                    ...
                                    <tr>
                                        <th style="width:20%;"><span class="redStarPB" style="color: red;">*</span>&#12288;<span >所属党支部：</span></th>
                                        <td colspan="3">
                                            <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                                <select style="width: 144px;" name="sysUser.partyBranchId" class="notBlank inputText validate[required] needFlagPB">
                                                    <option/>
                                                    <c:forEach items="${dictTypeEnumXjPartyBranchList }" var="dict">
                                                        <option value="${dict.dictId }" <c:if test="${sysUser.partyBranchId eq dict.dictId }">selected</c:if>>${dict.dictName }</option>
                                                    </c:forEach>
                                                </select><input type="hidden" name="sysUser.partyBranchName" value="${sysUser.partyBranchName}">
                                            </c:if>
                                            <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                                <c:if test="${requiredFlag}">
                                                    <select style="width: 144px;" name="sysUser.partyBranchId" class="notBlank inputText validate[required] needFlagPB">
                                                ...
                                                <c:if test="${!requiredFlag}">
                                                    <label>${sysUser.partyBranchName}</label>
                                                </c:if>
                                            </c:if>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <th><span style="color: red;">*</span>&#12288;<span >微信账号：</span></th>
                                        <td>
                                        ...
                                        <th><span style="color: red;">*</span>&#12288;<span >QQ号码：</span></th>
                                        <td>
                    ...
                    <div class="spreadOne" hidden="hidden" style="margin-bottom:10px;" id="feeInfo">
                        ...
                        <div class="spreadOneFive">
                            <table class="basic" style="width:100%; margin-top: 3px; margin-bottom: 30px;">
                                <tr>
                                    <th style="width:20%;"><span >应收总额：</span></th>
                                    <td style="width:30%;">
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR||isFeeMaster }">
                                            <input type="text" class="inputText" style="width: 140px;text-align: left;" name="eduUserExtInfoForm.tuitionFee" value="${stuAllFeeInfo.xysje }"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
                                            <label>${stuAllFeeInfo.xysje }</label>
                                            <input type="hidden" name="eduUserExtInfoForm.tuitionFee" value="${stuAllFeeInfo.xysje }">
                                        </c:if>
                                    </td>
                                    <th><span >实收总额：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR||isFeeMaster }">
                                            <input type="text" class="inputText" style="width: 140px;text-align: left;" name="eduUserExtInfoForm.paytuitionFee" value="${stuAllFeeInfo.sjje }"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR }">
                                            <label>${stuAllFeeInfo.sjje }</label>
                                            <input type="hidden" name="eduUserExtInfoForm.paytuitionFee" value="${stuAllFeeInfo.sjje }">
                                        </c:if>
                                            <%--&emsp;<a onclick="showFeeDetail('${eduUser.sid}');" style="cursor: pointer; color: blue;" >[金额明细]</a>--%>
                                    </td>
                                </tr>
                                <tr>
                                    <th><span >欠费总额：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <input type="text" class="inputText" style="width: 140px;text-align: left;" name="eduUserExtInfoForm.arrearageFee" value="${stuAllFeeInfo.qfje }"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR }">
                                            <label>${stuAllFeeInfo.qfje }</label>
                                            <input type="hidden" name="eduUserExtInfoForm.arrearageFee" value="${stuAllFeeInfo.qfje }">
                                        </c:if>
                                    </td>
                                    <th><span >学费是否已缴清：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR||isFeeMaster }">
                                            <select style="width: 144px;" name="eduUserExtInfoForm.isFeeComplete" class="inputText">
                                                <option/>
                                                <option value="Y" ${extInfoForm.isFeeComplete eq 'Y'?'selected':''}>是</option>
                                                <option value="N" ${extInfoForm.isFeeComplete eq 'N'?'selected':''}>否</option>
                                            </select>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR  }">
                                            <label>${extInfoForm.isFeeComplete eq 'Y'?'是':''}${extInfoForm.isFeeComplete eq 'N'?'否':''}</label>
                                            <input type="hidden" name="eduUserExtInfoForm.isFeeComplete" value="${extInfoForm.isFeeComplete}">
                                        </c:if>
                                    </td>
                                </tr>
                                <tr>
                                    <th><span >档案是否已到齐：</span></th>
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR||isFeeMaster }">
                                            <select style="width: 144px;" name="eduUserExtInfoForm.isArchivesComplete" class="inputText">
                                                <option/>
                                                <option value="Y" ${extInfoForm.isArchivesComplete eq 'Y'?'selected':''}>是</option>
                                                <option value="N" ${extInfoForm.isArchivesComplete eq 'N'?'selected':''}>否</option>
                                            </select>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR  }">
                                            <label>${extInfoForm.isArchivesComplete eq 'Y'?'是':''}${extInfoForm.isArchivesComplete eq 'N'?'否':''}</label>
                                            <input type="hidden" name="eduUserExtInfoForm.isArchivesComplete" value="${extInfoForm.isArchivesComplete}">
                                        </c:if>
                                    </td>
                                    <th></th>
                                    <td></td>
                        ...
                        <div class="spreadTwoOne">
                            <table class="basic" style="width:100%; margin-top: 3px; margin-bottom: 30px;">
                                ...
                                <tr class="xsgainDisplayContent">
                                    ...
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input name="eduUserExtInfoForm.underMajor" value="${extInfoForm.underMajor }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <c:if test="${degreeGrantFlag}">
                                                <input name="eduUserExtInfoForm.underMajor" value="${extInfoForm.underMajor }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                            ...
                                            <c:if test="${!degreeGrantFlag}">
                                                <c:forEach items="${userDegreeMajorEnumList}" var="degree">
                                                    ${extInfoForm.underMajor eq degree.id?degree.name:''}
                                                </c:forEach>
                                ...
                                <tr class="xsgainDisplayContent">
                                    ...
                                    <td>
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input name="eduUserExtInfoForm.gotBachelorCertSubject" value="${extInfoForm.gotBachelorCertSubject }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <c:if test="${degreeGrantFlag}">
                                                <input name="eduUserExtInfoForm.gotBachelorCertSubject" value="${extInfoForm.gotBachelorCertSubject }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                            ...
                                            <c:if test="${!degreeGrantFlag}">
                                                <c:forEach items="${userDegreeSubjectEnumList}" var="sub">
                                                    ${extInfoForm.gotBachelorCertSubject eq sub.id?sub.name:''}
                                                </c:forEach>
                                ...
                                <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                    ...
                                    <tr class="linkRecruitType ssxwgainDisplayContent">
                                        <th><span style="color: red;">*</span>&#12288;<span>获硕士学位单位名称：</span></th>
                                        <td>
                                        ...
                                        <td>
                                            <input name="eduUserExtInfoForm.masterSubject" value="${extInfoForm.masterSubject }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                        </td>
                                    </tr>
                                    <tr class="gotMasterCertSpeTr ssxwgainDisplayContent">
                                        <th><span style="color: red;">*</span>&#12288;<span>获硕士学位专业名称：</span></th>
                                        <td>
                                            <input name="eduUserExtInfoForm.gotMasterCertSpe" value="${extInfoForm.gotMasterCertSpe }" type="text" class="notBlank inputText validate[required]" style="width: 140px;text-align: left;" title="所填数据必须与学位证书上一致" placeholder="所填数据必须与学位证书上一致"/>
                                        </td>
                                        <th></th>
                                        <td></td>
                                    </tr>
                                    ...
                                    <c:if test="${ not empty eduUser.trainTypeId and eduUser.trainTypeId >'1' }">
                                        ...
                                        <tr class="linkRecruitType ssxwgainDisplayContent">
                                            ...
                                            <td>
                                                ...
                                                <c:if test="${!degreeGrantFlag}">
                                                    <c:forEach items="${userDegreeSubjectEnumList}" var="sub">
                                                        ${extInfoForm.masterSubject eq sub.id?sub.name:''}
                                                    </c:forEach>
                                        ...
                                        <tr class="gotMasterCertSpeTr ssxwgainDisplayContent">
                                            ...
                                            <td>
                                                ...
                                                <c:if test="${!degreeGrantFlag}">
                                                    <c:forEach items="${userDegreeMajorEnumList}" var="degree">
                                                        ${extInfoForm.gotMasterCertSpe eq degree.id?degree.name:''}
                                                    </c:forEach>
                        ...
                        <div class="spreadTwoTwo">
                            <table class="basic" style="width:100%; margin-top: 3px; margin-bottom: 30px;">
                                <tr>
                                    ...
                                    <td style="width: 30%">
                                        <c:if test="${roleFlag!=GlobalConstant.RES_ROLE_SCOPE_DOCTOR && !isFeeMaster }">
                                            <input type="text" class="inputText ctime" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width: 140px;text-align: left;" name="eduUser.preGraduateDate" value="${eduUser.preGraduateDate }"/>
                                        </c:if>
                                        <c:if test="${roleFlag==GlobalConstant.RES_ROLE_SCOPE_DOCTOR ||isFeeMaster }">
                                            <%--<c:if test="${degreeGrantFlag}">--%>
                                                <%--<input type="text" class="notBlank inputText ctime validate[required]" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width: 140px;text-align: left;" name="eduUser.preGraduateDate" value="${eduUser.preGraduateDate }"/>--%>
                                            <%--</c:if>--%>
                                            <%--<c:if test="${!degreeGrantFlag}">--%>
                                                ${eduUser.preGraduateDate }
                                                <%--<input type="hidden" name="eduUser.preGraduateDate" value="${eduUser.preGraduateDate}">--%>
                                            <%--</c:if>--%>
                                        </c:if>
                                    </td>
                                ...
                                <tbody id="graduateFlagTbody" style="${extInfoForm.graduateFlag eq '1'?'':'none'}">
                                    <tr>
