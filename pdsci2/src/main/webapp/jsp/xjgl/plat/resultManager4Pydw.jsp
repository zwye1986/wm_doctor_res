...
<head>
    ...
    <script type="text/javascript">
        function result(userFlow,period){
            jboxOpen("<s:url value='/xjgl/user/resultSun4Pydw'/>?userFlow="+userFlow+"&period="+period,"成绩管理",1000,500);
        }
        ...
        function printResultList(){
            ...
            var url = "<s:url value='/xjgl/user/print4AdminOL'/>?orgFlow="+orgFlow+"&sid="+sid+"&period="+period+"&userName="+encodeURI(encodeURI(userName))+"&printFlag="+printFlag;
            jboxTip("打印中，请稍等...");
            setTimeout(function(){
...
<div class="mainright">
    <div class="content">
        <form id="recSearchForm" method="post" action="<s:url value='/xjgl/user/selectStudentCourseSunEdit'/>">
                            ...
                            <tr>
                                <td>学&#12288;&#12288;号：<input type="text" id="sid" name="sid" value="${param.sid}" style="width: 137px;"/>
                                    ...
                                    <c:if test="${flag eq 'view'}">
                                        ...
                                        <br/>
                                        年&#12288;&#12288;级：<input style="width:137px;" value="${param.period}" name="period" onClick="WdatePicker({dateFmt:'yyyy'})" readonly="readonly" />
                                        &#12288;打印选项：<select  name="printFlag" style="width: 141px;">
        ...
        <table class="basicData" style="width: 100%;">
            ...
            <c:forEach items="${recordList}" var="record">
                <tr id="${record.recordFlow}">
                    ...
                    <c:if test="${flag eq 'edit' or !empty param.from}">
                        <td>
                            <c:if test="${pdfn:getCurrYear()-1 eq record.studentPeriod}">
                                <a href="javascript:void(0);" onclick="result('${record.userFlow}','${record.studentPeriod}')" style="color: blue;">成绩管理</a>
                            </c:if>
                            <c:if test="${pdfn:getCurrYear()-1 ne record.studentPeriod}">
                                <a style="color:grey;">成绩管理</a>
                            </c:if>
                        </td>
                    </c:if>
                </tr>
