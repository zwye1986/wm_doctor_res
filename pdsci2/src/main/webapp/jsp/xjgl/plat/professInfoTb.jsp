	...
	<div hidden="hidden" id="delFlowsDiv"></div>
				<tr>
					...
					<td rowspan="2" style="width: 8%;">
						<div style="padding:3px 0;"><a style="cursor: pointer;" onclick="toMove('Degree','right');"><img
								style="width: 30px;"
								src="<s:url value='/'/>css/skin/LightBlue/images/select_add.png"></a></div>
				    </td>
					<th style="width: 28%;">学位课程</th>
				</tr>
				<tr>
				    <td rowspan="9" id="majorTd"><c:if test="${not empty param.majorId}">(${param.majorId })${majorName }</c:if></td>
				    <td rowspan="9" id="notIncludeTd">
						...
						<td id="DegreeTd" class="sumTd">
					    <c:forEach items="${courseMap['degreeList'] }" var="course">
					    <c:set var="sumPeriod" value="${sumPeriod+course.coursePeriod }"/>
					     <c:set var="sumCredit" value="${sumCredit+course.courseCredit }"/>
				...
				<tr>
					<td colspan="4">合计：<c:set var="sumCourse" value="${fn:length(courseMap['degreeList'])+fn:length(courseMap['publicList'])+fn:length(courseMap['publicNeedList'])+fn:length(courseMap['optionalList'])+fn:length(courseMap['optionalNeedList']) }"/></td>
