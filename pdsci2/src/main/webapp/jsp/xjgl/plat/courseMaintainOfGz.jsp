...
<head>
    ...
    <script type="text/javascript">
        ...
        function calculate(){
            ...
            var degreeCourseCreditSum = 0;
            var degreeCoursePeriodSum = 0;
            var $courseFlow = $("input[name=courseFlow]:checked");
            var $degreeCourseFlag = $("input[name=degreeCourseFlag]:checked");
            $courseFlow.each(function(){
                ...
                //学位课程学时学分统计
                if ($(this).closest("tr").find("input[name='degreeCourseFlag']").is(':checked')){
                    degreeCourseCreditSum = parseFloat(degreeCourseCreditSum) + parseFloat(courseCredit);
                    degreeCoursePeriodSum = parseFloat(degreeCoursePeriodSum) + parseFloat(coursePeriod);
                }
            });
            ...
            $("#degreeCourseCount").text($degreeCourseFlag.length);
            $("#degreeCourseCreditSum").text(parseFloat(degreeCourseCreditSum).toFixed(1));
            $("#degreeCoursePeriodSum").text(parseFloat(degreeCoursePeriodSum));
        }
        ...
        function saveCourseMaintain(courseFlow, recordStatus,replenishFlag){
            ...
            var url = "<s:url value='/xjgl/student/course/saveCourseMaintain'/>";
            jboxPost(url, data, function(resp){
                if("${GlobalConstant.OPRE_SUCCESSED}" == resp){
                    if(!$("input[name='periodCheckbox']").is(':checked')){
                        strideOption($('#stride'));
                    }
                }
            }, null, true);
        ...
        function saveDegreeCourse(recordFlow,courseFlow,obj){
            if(!$(obj).closest("tr").find("input[name='courseFlow']").is(':checked')){
                jboxTip("请先在操作中勾选此门课程！")
                strideOption($('#stride'));
                return false;
            }
            var degreeCourseFlag = $(obj).closest("tr").find("input[name='degreeCourseFlag']").is(':checked')?"Y":"N";
            var url = "<s:url value='/xjgl/student/course/saveDegreeCourse'/>?courseFlow="+courseFlow+"&userFlow=${eduUser.userFlow}&degreeCourseFlag="+degreeCourseFlag+"&recordFlow="+recordFlow;
        ...
        function search(){
            ...
            var url = "<s:url value='/xjgl/student/course/editCourseMaintain'/>?period=${period}&majorId="+majorId+"&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}";
            jboxLoad("contentDiv", url, true);
        }
        function reOpen(){
            jboxConfirm("确认开放该学员重新选课?",function(){
                jboxGet("<s:url value='/xjgl/student/course/reOpenChooseCourse'/>?userFlow=${eduUser.userFlow}",null,function(resp){
                    if("${GlobalConstant.OPRE_SUCCESSED}" == resp){
                        $("#reOpenBtn").hide();
                    }
                },null,true);
        ...
        function strideOption(obj){
            var majorId = $("#result_Major").val();
            var url = "<s:url value='/xjgl/student/course/editCourseMaintain'/>?period=${period}&majorId="+majorId+"&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}&strideFlag="+$(obj).attr("checked")+"&courseName="+$('#courseName').val()+"&periodFlag="+$('#period').attr("checked")+"&periodYear="+$('#periodYear').val();
            jboxLoad("contentDiv", url, true);
        }
        function searchCouse(obj){
            var majorId = $("#result_Major").val();
            var url = "<s:url value='/xjgl/student/course/editCourseMaintain'/>?period=${period}&majorId="+majorId+"&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}&strideFlag="+$('#stride').attr("checked")+"&courseName="+$(obj).val()+"&periodFlag="+$('#period').attr("checked")+"&periodYear="+$('#periodYear').val();
            jboxLoad("contentDiv", url, true);
        }
        //跨年级选课
        function stridePeriod(obj){
            var majorId = $("#result_Major").val();
            var url = "<s:url value='/xjgl/student/course/editCourseMaintain'/>?period=${period}&majorId="+majorId+"&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}&strideFlag="+$('#stride').attr("checked")+"&courseName="+$('#courseName').val()+"&periodFlag="+$(obj).attr("checked")+"&periodYear="+$('#periodYear').val();
            jboxLoad("contentDiv", url, true);
        }
        function searchPeriod(obj){
            var majorId = $("#result_Major").val();
            var url = "<s:url value='/xjgl/student/course/editCourseMaintain'/>?period=${period}&majorId="+majorId+"&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}&strideFlag="+$('#stride').attr("checked")+"&courseName="+$('#courseName').val()+"&periodFlag="+$('#period').attr("checked")+"&periodYear="+$(obj).val();
            jboxLoad("contentDiv", url, true);
        }
        $(function(){
            loadMajorList();
        ...
        <table class="basic" style="width: 100%;margin: 10px 0px;">
            ...
            <tr>
                ...
                <td style="border-style:none;">
                    ...
                    <input placeholder="输入课程代码" class="inputText" id="courseName" onchange="searchCouse(this)" style="width: 95px;text-align: left;" />
            ...
            <tr>
                ...
                <td style="border-style:none;width:160px;">
                    <c:if test="${eduUser.chooseCourseStatusId!=xjChooseCourseStatusEnumChoose.id}">
                        <span style="color: red">选课状态：未提交</span>
                    </c:if>
                </td>
                <td style="border-style:none;">
                    <input type="checkbox" onclick="stridePeriod(this)" id="period" name="periodCheckbox"><label for="period" style="color: black;">跨年级选课</label>
