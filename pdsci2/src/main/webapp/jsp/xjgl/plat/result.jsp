	...
	<table class="basic" style="width: 100%;margin-bottom: 5px;margin-top: 0px;">
			<tr>
				<td>
					...
					<c:if test="${GlobalConstant.FLAG_Y != param.openType && false}">
						<span style="float: right;">
							<img title="新增" src="<s:url value="/css/skin/${skinPath}/images/add3.png" />" style="margin-right: 10px;cursor: pointer;" onclick="add();"/>
							<img title="删除" src="<s:url value="/css/skin/${skinPath}/images/del1.png" />" style="margin-right: 15px;cursor: pointer;" onclick="del();"/>
						</span>
					</c:if>
					<br/>
					<span style="float: right;">
						<%--<input class="search"  type="button" value="打&#12288;印" onclick="doPrint(1);"/>--%>
						<%--<input class="search"  type="button" value="无章打印" onclick="doPrint(0);"/>--%>
						<script>
							function showStmp(x){
								if(1==x) $("#stmp").show();
								if(0==x) {
						...
						<input class="search"  type="button" value="打印预览" onclick="printFlag();"/>
