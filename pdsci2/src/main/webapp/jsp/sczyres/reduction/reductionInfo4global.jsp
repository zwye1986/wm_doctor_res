...
<script type="text/javascript">
    ...
    function searchIn() {
        var url = "<s:url value='/sczy/reduction/reductionManage'/>";
        jboxPostLoad("div_table", url, $("#inForm").serialize(), true);
    }
    function showCheckReductionInfo(recordFlow) {
        var url = "<s:url value='/sczy/reduction/showCheckReductionInfo'/>?roleId=${roleId}&recordFlow=" + recordFlow;
        ...
        <form id="inForm">
            ...
            <c:if test="${operType eq 'isCheck'}">
                <table class="searchTable">
                    <tr>
                        ...
                        <td>
                            <select name="trainingTypeId" id="trainingTypeId" class="select" style="width:107px;">
                                ...
                                <c:forEach items="${speCatEnumList}" var="trainCategory">
                    ...
                    <tr>
                        <td class="td_left" style="line-height: 25px;">减免后的&#12288;<br/>培训年限：</td>
                        <td>
                            <select name="afterReduceTrainYear" id="afterReduceTrainYear" class="select"
                                ...
                                <option value="3"
                                        <c:if test="${param.afterReduceTrainYear eq '3'}">selected="selected"</c:if>>3年
                                </option>
                            </select>
                        </td>
                        <td colspan="6">
                            <input class="btn_blue" type="button" onclick="searchIn()" value="查&#12288;询"/>
                        </td>
                    </tr>
                </table>
            </c:if>
            ...
            <table border="0" cellpadding="0" cellspacing="0" class="grid">
                ...
                <c:forEach items="${resDoctorReductions}" var="reduction">
                    <tr>
                        ...
                        <td title="${pdfn:transDate(reduction.xtLocalAuditTime)}${reduction.xtLocalAuditOpinion}">${pdfn:cutString(reduction.xtLocalAuditOpinion,10,true,3)}</td>
                        <td title="${pdfn:transDate(reduction.localAuditTime)}${reduction.localAuditOpinion}">${pdfn:cutString(reduction.localAuditOpinion,10,true,3)}</td>
                        <c:if test="${operType eq 'isQuery'}">
                            <td title="${pdfn:transDate(reduction.globalAuditTime)}${reduction.globalAuditOpinion}">${pdfn:cutString(reduction.globalAuditOpinion,10,true,3)}</td>
