...
<head>
    ...
    <script type="text/javascript">
        ...
        function saveSelResult(selResultTd) {
            ...
            jboxPost("<s:url value='/schTwo/template/saveFreeRostering'/>", data, function (resp) {
                if (resp == "${GlobalConstant.OPRE_SUCCESSED_FLAG}") {
                    jboxTip("${GlobalConstant.OPRE_SUCCESSED}");
                    window.parent.frames["mainIframe"].window.refreshResult();
                    jboxClose();
                }
            }, null, false);
        }
...
<div id="selResultTd">
    <table class="basic" style="margin-left: 10px;width:98%;">
        <input type="hidden" id="doctorFlow" value="${doctorFlow}">
        <tr>
            ...
            <td style="text-align: center;">
                <select id="schDept">
                    ...
                    <c:forEach items="${deptList}" var="dept">
                        <option value="${dept.schDeptFlow}">${dept.schDeptName}</option>
                    </c:forEach>
                </select>
            </td>
        </tr>
