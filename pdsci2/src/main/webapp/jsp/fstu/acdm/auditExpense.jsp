...
<head>
    ...
    <script type="text/javascript">
        ...
        function audit(academicFlow,flag){
            var title = flag == "edit"?"报销编辑":"报销审核";
            var url = "<s:url value='/fstu/academic/auditExpenseOption?academicFlow='/>"+academicFlow;
...
<div class="mainright">
    <div class="content">
        <form id="searchForm" action="<s:url value='/fstu/academic/auditExpense'/>" method="post">
            <table class="basic" style="width: 100%;margin: 10px 0px;">
                ...
                <div class="searchDiv">
                    &#12288;&#12288;报销状态：
                    <input type="radio" name="expenseStatusId" value="ALL" <c:if test="${param.expenseStatusId eq 'ALL' || param.flag eq 'ALL'}">checked="checked"</c:if>>全部
                    <input type="radio" name="expenseStatusId" value="Y" <c:if test="${param.expenseStatusId eq 'Y'}">checked="checked"</c:if>>已审核
                    <input type="radio" name="expenseStatusId" value="N" <c:if test="${param.expenseStatusId eq 'N'}">checked="checked"</c:if>>未审核
                </div>
        ...
        <table class="xllist" style="width:100%;">
            ...
            <c:forEach items="${academicList}" var="activity">
                <tr>
                    ...
                    <td>
                        <c:if test="${activity.expenseStatusId eq 'Passing'}"><%-- 待审核 --%>
                            <a onclick="audit('${activity.academicFlow}');" style="cursor: pointer;color: blue;">审核</a>
                        </c:if>
                        <c:if test="${activity.expenseStatusId ne 'Passing'}">
                            <a onclick="audit('${activity.academicFlow}','edit');" style="cursor: pointer;color: blue;">编辑</a>
                        </c:if>
