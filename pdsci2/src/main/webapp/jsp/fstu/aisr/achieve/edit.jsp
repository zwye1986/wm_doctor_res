...
<head>
    ...
    <script type="text/javascript">
        function saveAchieveAndAuthor() {
            if (false == $("#achieveForm").validationEngine("validate")) {
                return false;
            }
            ...
            jboxConfirm("确认保存？", function () {
                var achieve = $("#achieveForm").serializeJson();
                ...
                var requestData = {'authorList': datas, 'achieve': achieve, 'pubFileList': fileDatas};
                $("#jsondata").val(JSON.stringify(requestData));
                //alert($("#jsondata").val());
                var url = "<s:url value='/fstu/aisr/achieve/save'/>";
                jboxStartLoading();
                jboxSubmit($("#achieveForm"), url, function (resp) {
        ...
        function delAuthor(authorFlow, obj) {
            var url = '<s:url value="/fstu/aisr/achieve/deleteAuthor?authorFlow="/>' + authorFlow;
            jboxConfirm("确认删除？", function () {
...
<div class="mainright">
    <div class="content">
        <div class="title1 clearfix">
            <div id="look">
                <form id="achieveForm" method="post" enctype="multipart/form-data" style="position: relative;">
                    ...
                    <table class="basic" style="width: 100%;">
                        ...
                        <tr>
                            <th style="width: 17%;">成果名称：</th>
                            <td colspan="5">
                                <input type="hidden" name="achieveFlow" value="${achieve.achieveFlow}"/>
                                <input class="validate[required,maxSize[100]] name xltext" style="width: 97%;"
                                       name="achieveName" type="text" value="${achieve.achieveName}"/>
                            </td>
                        </tr>
                        <tr>
                            <th>所属单位：</th>
                            <td>
                                <c:forEach var="dict" items="${dictTypeEnumSatOrgList}">
                                    <input type="radio" name="orgBelongId" value="${dict.dictId}"
                                           ...
                                           <c:if test="${achieve.orgBelongId==dict.dictId}">checked="checked"</c:if>/><label
                        ...
                        <tr>
                                ...
                                <select class="xlselect" name="achTypeId" style="width: 168px;margin-top: 6px">
                                    ...
                                    <c:forEach var="dict" items="${dictTypeEnumSatTypeList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${achieve.achTypeId==dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th>获奖日期：</th>
                            <td colspan="3">
                                <input style="width: 200px" readonly="readonly"
                                       ...
                                       type="text" value="${achieve.prizedDate}"/>
                            </td>
                        </tr>
                        <tr>
                            <th>获奖级别：</th>
                            <td>
                                <select class="xlselect" name=prizedGradeId style="width: 168px;">
                                    ...
                                    <c:forEach var="dict" items="${dictTypeEnumSatGradeList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${achieve.prizedGradeId==dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th>获奖等级：</th>
                            <td colspan="3">
                                <select class="xlselect" name="prizedLevelId" style="width: 208px;">
                                    ...
                                    <c:forEach var="dict" items="${dictTypeEnumSatLevelList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${achieve.prizedLevelId==dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th>学科门类：</th>
                            <td class="td_blue">
                                <c:forEach var="dict" items="${dictTypeEnumSubjectCategoriesList}">
                                    <input type="radio" name="categoryId" value="${dict.dictId}"
                                           id="categoryId_${dict.dictId}"
                                           <c:if test="${achieve.categoryId==dict.dictId}">checked="checked"</c:if>/>
                                    <label
                            ...
                            <td colspan="3">
                                <input type="text" class="xltext" style="text-align: left; width: 200px"
                                       name="approvalCode" value="${achieve.approvalCode}"/>
                            </td>
                        </tr>
                        <tr>
                            <th>项目来源：</th>
                            <td>
                                <select name="projSourceId" class="xlselect" style="width: 208px;">
                                    ...
                                    <c:forEach var="dict" items="${dictTypeEnumProSourceList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${dict.dictId eq achieve.projSourceId}">selected</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea class="xltxtarea" placeholder="50个字以内" style="margin-left: 0px;"
                                          name="remark" onblur="strChar(this,100)">${achieve.remark }</textarea>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${not empty file.fileFlow}">
                                            <a id="down"
                                               href='<s:url value="/fstu/aisr/achieve/fileDown?fileFlow=${file.fileFlow}"/>'>${file.fileName}</a>
                ...
                <form id="authorList">
                    <table class="basic" style="width: 100%;">
                        <tr class="bs_name" style="height: 26px">
                            <td style="text-align:left" colspan="8">成果作者
                        ...
                        <%-- 默认作者 --%>
                        <c:if test="${empty param.achieveFlow}">
                            <tr>
                                <td>
                        ...
                        <c:forEach items="${achieveAuthorList}" var="author">
                            <td>
                                <input class="xltext" style="width: 85%;" type="text" name="typeName"
                ...
                <p align="center">
                    <c:if test="${param.editFlag != GlobalConstant.FLAG_N}">
                        <input type="button" value="保&#12288;存" onclick="saveAchieveAndAuthor()" class="search"/>
                ...
                <table id="addAuthorTable" style="display: none" class="basic" style="width: 100%;">
                    <tr>
                        ...
                        <td style="text-align: center;">
                            ...
                            <input type="hidden" name="achieveFlow" value="${param.achieveFlow}"/>
                            [<a onclick="delAuthorTr(this)" style="cursor: pointer;">删除</a>]
                        </td>
                    </tr>
                </table>
