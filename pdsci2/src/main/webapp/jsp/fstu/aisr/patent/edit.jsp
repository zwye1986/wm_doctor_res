...
<head>
    ...
    <script type="text/javascript">
        function savePatentAndAuthor() {
            if (false == $("#patentForm").validationEngine("validate")) {
                return false;
            }
            if (false == $("#authorList").validationEngine("validate")) {
                return false;
            }
            if($("input[name='prizedStartDate']").val()>$("input[name='prizedEndDate']").val()){
                jboxTip("开始时间不能大于结束时间，请检查");
                return false;
            }
            jboxConfirm("确认保存？", function () {
                var patent = $("#patentForm").serializeJson();
                ...
                var requestData = {'authorList': datas, 'patent': patent, 'pubFileList': fileDatas};
                $("#jsondata").val(JSON.stringify(requestData));
                //alert($("#jsondata").val());
                var url = "<s:url value='/fstu/aisr/patent/save'/>";
                jboxStartLoading();
                jboxSubmit($("#patentForm"), url, function (resp) {
        ...
        function delAuthor(authorFlow, obj) {
            var url = '<s:url value="/fstu/aisr/patent/deleteAuthor?authorFlow="/>' + authorFlow;
            jboxConfirm("确认删除？", function () {
...
//            $('#look select').attr('disabled', "disabled");
        ...
        function backOpt(){
            var url = "<s:url value='/fstu/aisr/patent/saveAudit?patentFlow=${param.patentFlow}&agreeFlag=N'/>";
...
<div class="mainright">
    <div class="content">
        <div class="title1 clearfix">
            <div id="look">
                <form id="patentForm" method="post" enctype="multipart/form-data" style="position: relative;">
                    ...
                    <table class="basic" style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <td >
                                <input style="width: 200px" readonly="readonly"
                                       onClick="WdatePicker({dateFmt:'yyyy'})" class="ctime" name="prizedDate"
                                       type="text" value="${patent.prizedDate}"/>
                            </td>
                            <th>单&#12288;&#12288;位：</th>
                            <td colspan="3">
                                <input name="orgBelongName" class="xltext" type="text" style="width: 200px" value="${patent.orgBelongName}">
                            </td>
                        </tr>
                        <tr>
                            <th>项目来源：</th>
                                ...
                                <select name="projSourceId" class="xlselect" style="width: 208px;">
                                    ...
                                    <c:forEach var="dict" items="${dictTypeEnumProSourceList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${dict.dictId eq patent.projSourceId}">selected</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th>项目类别：</th>
                            <td colspan="3">
                                <select name="projTypeId" class="xlselect" style="width: 208px;">
                                    <option value="">请选择</option>
                                    <c:forEach var="dict" items="${dictTypeEnumProTypeList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${dict.dictId eq patent.projTypeId}">selected</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th style="width: 17%;"><font color="red">*</font>项目名称：</th>
                            <td>
                                <input type="hidden" name="patentFlow" value="${patent.patentFlow}"/>
                                <input class="validate[required,maxSize[100]] name xltext" style="width: 200px"
                                       name="patentName" type="text" value="${patent.patentName}"/>
                            </td>
                            <th>项目负责人：</th>
                            <td colspan="3">
                                <input class="xltext" style="width: 200px"
                                       name="patentChangeMan" type="text" value="${not empty changeMan?changeMan:patent.patentChangeMan}"/>
                            </td>
                        </tr>
                        <tr>
                            <th style="width: 17%;">承担科室：</th>
                            <td>
                                <input class="xltext" style="width: 200px"
                                       name="patentDeptName" type="text" value="${patent.patentDeptName}"/>
                            </td>
                            <th>项目级别：</th>
                            <td colspan="3">
                                <select class="xlselect" name=patentLevelId style="width: 208px;">
                                    <option value="">请选择</option>
                                    <c:forEach var="dict" items="${dictTypeEnumProLevelList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${patent.patentLevelId==dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input style="width: 200px" readonly="readonly"
                                       onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="ctime" name="prizedStartDate"
                                       type="text" value="${patent.prizedStartDate}"/>
                            </td>
                            <th>结束日期：</th>
                            <td colspan="3">
                                <input style="width: 200px" readonly="readonly"
                                       onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="ctime" name="prizedEndDate"
                                       type="text" value="${patent.prizedEndDate}"/>
                            </td>
                        </tr>
                        <tr>
                            <th>项目状态：</th>
                            <td>
                                <select class="xlselect" name="patentStatusId" style="width: 208px;">
                                    <option value="">请选择</option>
                                    <c:forEach var="dict" items="${dictTypeEnumFstuProStatusList}">
                                        <option value="${dict.dictId}"
                                                <c:if test="${patent.patentStatusId==dict.dictId}">selected="selected"</c:if>>${dict.dictName}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th>分&#12288;值：</th>
                            <td>
                                <input style="width: 200px" class="validate[custom[number]] xltext" name="scoreValue"
                                       type="text" value="${patent.scoreValue}"/>
                            </td>
                        </tr>
                    </table>
                    <table class="basic" style="width:100%;">
                        <thead>
                        <tr>
                            <th colspan="4" class="bs_name">项目简介</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <th width="17%">简介内容：</th>
                            <td>
                                <textarea class="xltxtarea validate[maxSize[500]]" placeholder="250个字以内" style="margin-left: 0px;"
                                          name="briefIntroduction" onblur="strChar(this,500)">${patent.briefIntroduction}</textarea>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea class="xltxtarea" placeholder="50个字以内" style="margin-left: 0px;"
                                          name="remark" onblur="strChar(this,100)">${patent.remark }</textarea>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                    ...
                    <table class="basic" style="width:100%;">
                        ...
                        <tr>
                            ...
                            <td width="20%" style="font-weight:bold;"  class="reCheck">附件操作</td>
                        </tr>
                        </thead>
                        <tbody id="projFileTb">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${not empty file.fileFlow}">
                                            <a id="down"
                                               href='<s:url value="/fstu/aisr/patent/fileDown?fileFlow=${file.fileFlow}"/>'>${file.fileName}</a>
                ...
                <%--<form id="authorList">--%>
                    <%--<table class="basic" style="width: 100%;">--%>
                        <%--<tr class="bs_name" style="height: 26px">--%>
                            <%--<td style="text-align:left" colspan="8">专利作者--%>
                        ...
                        <%--<c:if test="${empty param.patentFlow}">--%>
                            <%--<tr>--%>
                                ...
                                <%--</td>--%>
                            <%--</tr>--%>
                        <%--</c:if>&lt;%&ndash; 默认作者 End&ndash;%&gt;--%>


                        <%--<c:forEach items="${patentAuthorList}" var="author">--%>
                            <%--<td>--%>
                                <%--<input class="xltext" style="width: 85%;" type="text" name="typeName"--%>
                ...
                <p align="center">
                    <c:if test="${param.editFlag != GlobalConstant.FLAG_N}">
                        <input type="button" value="保&#12288;存" onclick="savePatentAndAuthor()" class="search"/>
                        <input type="button" value="关&#12288;闭" onclick="jboxClose();" class="search"/>
                    </c:if>
                    <c:if test="${patent.operStatusId eq 'Passed'}">
                ...
                <table id="addAuthorTable" style="display: none" class="basic" style="width: 100%;">
                    <tr>
                        ...
                        <td style="text-align: center;">
                            ...
                            <input type="hidden" name="patentFlow" value="${param.patentFlow}"/>
                            [<a onclick="delAuthorTr(this)" style="cursor: pointer;">删除</a>]
                        </td>
                    </tr>
                </table>
