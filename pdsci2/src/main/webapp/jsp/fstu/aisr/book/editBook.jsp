...
<script type="text/javascript">
    function saveBookAndAuthor() {
        ...
        jboxConfirm("确认保存？",function(){
            ...
            var t = {'authorList': datas, 'book': book, 'fileList': fileDatas};
            $("#jsondata").val(JSON.stringify(t));
            var url = "<s:url value='/fstu/book/save'/>";
            //$('#book').submit();
            jboxStartLoading();
    ...
    function delAuthor(authorFlow, obj) {
        var url = '<s:url value="/fstu/book/deleteAuthor?authorFlow="/>' + authorFlow;
...
//        $('#look select').attr('disabled', "disabled");
    ...
    function backOpt(){
        var url = "<s:url value='/fstu/book/saveAudit?bookFlow=${param.bookFlow}&agreeFlag=N'/>";
        ...
        <div class="title1 clearfix">
            <div id="look">
                <form id="book" style="position: relative;" enctype="multipart/form-data" method="post"
                      action="<s:url value='/fstu/book/save'/>" target="screct_frame">
                    ...
                    <table class="basic" style="width: 100%">
                        ...
                        <tr>
                            <th>出版地：</th>
                            <td>
                                <c:forEach var="dict" items="${dictTypeEnumFstuPlaceNameTypeList}">
                            ...
                            <th>著作类别：</th>
                            <td>
                                <c:forEach var="dict" items="${dictTypeEnumFstuBookTypeList}">
                        ...
                        <tr>
                            <th>学科门类：</th>
                            <td>
                                <c:forEach var="dict" items="${dictTypeEnumFstuSubjectTypeList}">
                        ...
                        <tr>
                            ...
                            <%--<td>--%>
                                <%--<select name="languageTypeId" class="xlselect" style="width: 168px;">--%>
                                    <%--<option value="">请选择</option>--%>
                                    <%--<c:forEach items="${dictTypeEnumFstuLanguageTypeList }" var="dict">--%>
                                        <%--<option value="${dict.dictId }"--%>
                            ...
                            <th>分值：</th>
                            <td>
                                <input class="xltext validate[custom[number]]" type="text" name="scoreValue" value="${book.scoreValue}" style="margin-right: 0px;">
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea class="xltxtarea" placeholder="50个字以内" style="margin-left: 0px;"
                                          name="remark" onkeyup="strChar(this,100)">${book.remark }</textarea>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="writeTypeId" class="xlselect" style="width: 85%;">
                                        <option value="">请选择</option>
                                        <c:forEach var="dict" items="${dictTypeEnumFstuWriteNameTypeList}">
                                            <option value="${dict.dictId }"
                ...
                <p align="center">
                    <c:if test="${param.editFlag != GlobalConstant.FLAG_N}">
                        <input type="button" id="saveBtn" value="保&#12288;存" onclick="saveBookAndAuthor();" class="search"/>
                        <input type="button" value="关&#12288;闭" onclick="jboxClose();" class="search"/>
                    </c:if>
                    <c:if test="${book.operStatusId eq 'Passed'}">
