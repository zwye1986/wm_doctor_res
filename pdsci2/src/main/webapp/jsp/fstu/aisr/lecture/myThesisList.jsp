...
<head>
    ...
    <script type="text/javascript">
        ...
        function add() {
            jboxStartLoading();
            jboxOpen("<s:url value='/fstu/thesis/edit'/>", "编辑论文信息", width, height);
        }

        function delThesis(thesisFlow) {
            url = "<s:url value='/fstu/thesis/delete?thesisFlow='/>" + thesisFlow;
        ...
        function edit(thesisFlow) {
            jboxOpen("<s:url value='/fstu/thesis/edit?thesisFlow='/>" + thesisFlow, "编辑论文信息", width, height);
        }

        function submitAudit(thesisFlow) {
            var url = "<s:url value='/fstu/thesis/submitAudit?thesisFlow='/>" + thesisFlow;
            jboxConfirm("确认送审,送审后无法修改?", function () {
                jboxGet(url, null, function (resp) {
                    if (resp == "1") {
                        jboxTip("送审成功");
                        window.location.href = "<s:url value='/fstu/thesis/list'/>";
                    }
                }, null, false);
            });
        }
        ...
        function auditProcess(flow) {
            jboxOpen("<s:url value='/fstu/thesis/auditProcess?flow='/>" + flow, "操作记录", width, height);
        }
...
<div class="mainright">
    <div class="content">
        <div class="title1 clearfix">
            <form id="searchForm" action="<s:url value="/fstu/thesis/list"/>" method="post">
        ...
        <table class="xllist">
            ...
            <c:forEach items="${thesisList}" var="thesis">
                <tr>
                    ...
                    <td width="20%">
                        <c:choose>
                            <c:when test="${thesis.operStatusId != proStatusEnumApply.id and thesis.operStatusId != proStatusEnumPassed.id}">
                                <c:if test="${thesis.operStatusId eq proStatusEnumDraft.id or thesis.operStatusId eq proStatusEnumUnPassed.id}">
