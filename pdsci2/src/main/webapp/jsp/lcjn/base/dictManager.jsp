...
function edit(dictFlow){
	jboxOpen("<s:url value='/lcjn/base/edit?dictFlow='/>"+dictFlow,"编辑", 500,350);
}

function add(){
	jboxOpen("<s:url value='/lcjn/base/edit?dictTypeId=${param.dictTypeId}'/>","新增", 500,350);
}
function imp(){
	jboxOpen("<s:url value='/jsp/lcjn/base/importDict.jsp'/>", "导入",600,200);
}
function search(){
	jboxStartLoading();
	$("#searchForm").submit();	
...
function refresh(){
	var url = "<s:url value='/lcjn/base/doRefresh'/>";
...
<div class="mainright">
	<div class="content">
		 ...
		 <form id="searchForm" action="<s:url value="/lcjn/base/dictList" />" method="post" >
			 <div class="choseDivNewStyle">
				 ...
				 <div class="newStyleSubDiv">
					 ...
					 <input type="button" class="search" onclick="imp();" value="导&#12288;入">
					 <input type="button" class="search" onclick="refresh();" value="刷&#12288;新">
					 <span style="color:red;">字典维护好后请刷新生效！</span>
				 </div>
			 </div>
		</form>
		</div>
