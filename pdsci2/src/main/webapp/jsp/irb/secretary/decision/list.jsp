...
<div class="mainright">
	<div class="content">
		<div class="title1 clearfix">
			<form id="searchForm" action="<s:url value="/irb/secretary/list?irbStageId=${param.irbStageId}" />"method="post">
				<p>
					...
					<select class="xlselect" name="irbTypeId">
						...
						<c:forEach items="${irbTypeEnumList}" var="irbType">
							<option value="${irbType.id}" <c:if test="${param.irbTypeId eq irbType.id}">selected="selected"</c:if> >${irbType.scName}</option>
						</c:forEach>
					</select>
			...
			<tbody>
				<c:forEach items="${irbFormList}" var="irbForm">
					<tr>
						...
						<td>${irbForm.irb.irbDecisionName}</td>
						<td>[<a href="<s:url value='/irb/secretary/decision'/>?irbFlow=${irbForm.irb.irbFlow}">进入</a>]</td>
					</tr>
				</c:forEach>
			</tbody>
			<c:if test="${empty irbFormList}"> 
				<tr> 
