...
<head>
	...
	<script type="text/javascript">
		...
		function toPage4(page){
			...
			jboxPostLoad("doctorScoreDiv","<s:url value="/osca/doctorScore/gradeManageList"/>",$("#doctorScoreForm").serialize(),true);
		}
...
//			var isLocal=$("select[name='isLocal']").val();
			var isLocal=$("input[name='isLocal']").val();
			var typeName=isLocal=="Y"?"院内考核":"结业考核";
			var msg="确认导出"+clinicalYear+"年"+typeName+"成绩吗?";
			if(clinicalYear==""){
				msg="确认导出"+typeName+"成绩吗?";
			}
			jboxConfirm(msg, function () {
				jboxStartLoading();
				var url = "<s:url value='/osca/doctorScore/exportAllScore'/>?clinicalYear="+clinicalYear+"&isLocal="+isLocal+"&flag=org";
				jboxTip("导出中…………");
		...
		function batchOscaSubmit(obj)
		{
			var url = "<s:url value='/jsres/temp/batchOscaSubmit'/>";
			jboxPost(url, null, function (resp) {
				jboxTip(resp);
			...
			<%--<div class="choseDivNewStyle" style="min-width: 1180px;">--%>
				...
				<div class="inputDiv">
					...
					<select id="trainingSpeId" name="speId" class="qselect" onchange="selectSpe1()">
						...
						<c:forEach items="${dictTypeEnumOscaTrainingTypeList}" var="dict">
							...
							<%--<span style="padding-left:5px;"></span>考核类型：--%>
							<%--<select id="isLocal" name="isLocal" style="width:134px;" class="select" onchange="initSpe()">--%>
								<%--<option value="Y" <c:if test="${osa.isLocal eq 'Y' or empty osa.isLocal}">selected="selected"</c:if>>院内考核</option>--%>
								<%--<option value="N" <c:if test="${osa.isLocal eq 'N'}">selected="selected"</c:if>>结业考核</option>--%>
							<%--</select>--%>

							<%--<span style="padding-left:5px;"></span>考核方案：--%>
							<%--<select id="actionTypeId" name="actionTypeId" style="width:134px;" class="select" onchange="selectSpe1()">--%>
								<%--<c:if test="${not empty assessmentProEnumList}">--%>
                ...
                <div class="inputDiv">
                    ...
                    <select id="clinicalFlow" name="clinicalFlow" class="qselect">
                        <%--<c:if test="${not empty param.clinicalFlow}"><option value="${osa.clinicalFlow}">${osa.clinicalName}</option></c:if>--%>
                ...
                <div class="inputDiv">
                    ...
                    <c:if test="${osa.isLocal eq 'Y'}">
                        ...
                        <input type="radio" name="isShow" value="Y" ${osa.isShow eq 'Y'?'checked':''} onchange="isShowOpt(this.value)">是
                            ...
                            <c:if test="${osa.isShow eq 'Y'}">
                        <div class="inputDiv">
                            学员能否查看评分表：
                            <input type="radio" name="isShowFroom" value="N" ${osa.isShowFroom eq 'N'?'checked':''} onchange="isShowForm(this.value)">否
                            <input type="radio" name="isShowFroom" value="Y" ${osa.isShowFroom eq 'Y'?'checked':''} onchange="isShowForm(this.value)">是
                        </div>
                        </c:if>
                    </c:if>
                ...
                <div class="funcDiv">
                    <input type="button" class="searchInput" value="成绩导入" onclick="gradeImport()"/>
                    <input type="button" class="searchInput" value="成绩导出" onclick="exportScoreExcel()"/>
                    <c:if test="${osa.isLocal eq 'Y'}">
                        <input type="button" class="searchInput" value="成绩发布" onclick="isShowOpt('grade')"/>
                    </c:if>
                    <c:if test="${osa.isLocal eq 'N'}">
                        <c:if test="${osa.isGradeReleased eq 'N'}">
                            <input type="button" class="searchInput" value="成绩上报" onclick="isShowOpt('school')"/>
                        </c:if>
                        <c:if test="${not empty osa.isGradeReleased and osa.isGradeReleased ne 'N'}">
                            <input type="button" class="searchInput" value="已上报" style="cursor: auto;"/>
                        </c:if>
                    </c:if>
                    <input type="button" class="searchInput" value="导出全部成绩" onclick="exportAllScore()"/>

					<input class="searchInput" type="button" value="提交已保存成绩" onclick="batchOscaSubmit();"/>
                </div>
                <div class="funcDiv">
                    <font style="color:red;">当前考核通过率<c:if test="${not empty gradeList}">${percent}</c:if>%（说明：查询结果通过率=考核通过人数/参加考核人数）
                        <c:if test="${osa.isLocal eq 'N'}">&#12288;
			...
			<c:if test="${not empty gradeList}">
				<c:forEach items="${gradeList}" var="info" varStatus="i">
					<tr style="${info.IS_PASS eq 'Miss'?'background-color:#D7D7D7':''}">
						...
						<td>
							<c:if test="${((info.IS_PASS eq 'Miss')||(osa.isGradeReleased ne 'N'))&&(osa.isLocal eq 'N')}"><a style="color:#4195c5;">编辑</a></c:if>
							<c:if test="${((info.IS_PASS ne 'Miss')&&(osa.isGradeReleased eq 'N'))||(osa.isLocal eq 'Y')}">
								<a onclick="editGradeOpt('${info.DOCTOR_FLOW}')" style="cursor:pointer;color:#4195c5;">编辑</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
