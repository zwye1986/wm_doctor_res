...
<c:set var="weekList" value="${weekNuMap['keyList']}"></c:set><!-- 有几周 -->
	...
	<c:forEach items="${classOrderEnumOfGzList}" var="classOrder">
		<tr>
			...
			<c:forEach items="${days}" var="day">
							...
							<tr style="border: 0px solid #e3e3e3;text-align: center;height: 100%;">
								<c:forEach items="${extList}" var="result" varStatus="status">
									...
									<td class="nameTitle2" id="${key}" recordFlow="${result.recordFlow}" term_flow="${termFlow}" flow="${key}" style=" border:1px solid #e3e3e3;text-align: center;width:50%;${param.roleFlag eq 'skz'?'cursor:pointer':''}"
										class_time="${day}" class_order="${fn:substring(classOrder.name,1,fn:length(classOrder.name)-1)}" course_flow="${result.courseFlow}" <c:if test="${param.roleFlag eq 'skz' || param.roleFlag eq 'xx'}">onclick="arrangePlan(this<c:if test="${param.roleFlag eq 'skz'}">,'<c:forEach items="${limitList}" var="limit"><c:if test="${day ge limit.beginTime && day le limit.endTime}">${limit.memo}期间不可排课！</c:if></c:forEach>'</c:if>)"</c:if>>
				...
				<c:if test="${empty extList}">
					<td style="width: 100px;max-width: 100px; ${(param.roleFlag eq 'skz' || param.roleFlag eq 'xx')&& term.termStartTime le day && day le term.termEndTime?'cursor:pointer':''}" recordFlow="" class_time="${day}" class_order="${fn:substring(classOrder.name,1,fn:length(classOrder.name)-1)}"
...
<script type="text/javascript">
	...
	function showCourseInfo(courseFlow){
		jboxOpen("<s:url value='/gyxjgl/term/manage/currCourse'/>?courseFlow="+courseFlow,"课程信息",850,450);
	}
</script>
