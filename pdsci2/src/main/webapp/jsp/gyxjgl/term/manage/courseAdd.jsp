...
<head>
	...
	<script type="text/javascript">
		function save(){
			...
			<%--if(${param.courseTypeId eq 'Public'}){--%>
				<%--jboxConfirm("该选修课将安排到所有班级课表中，确认保存？",  function(){--%>
					<%--var url="<s:url value='/gyxjgl/term/manage/saveOptionClass'/>";--%>
					<%--jboxPost(url,$("#myForm").serialize(),function(resp){--%>
						<%--if('${GlobalConstant.SAVE_SUCCESSED}'==resp){--%>
							<%--<c:forEach items="${terms}" var="term">--%>
								<%--var weekNumber = window.parent.frames['mainIframe'].$("#${term.recordFlow}_weekNumber").val();--%>
								<%--window.parent.frames['mainIframe'].loadDetail('${term.recordFlow}',weekNumber,${term.recordFlow eq schedule.termFlow?'1':'2'});--%>
							<%--</c:forEach>--%>
							<%--jboxClose();--%>
						<%--}--%>
					<%--},null,true);--%>
			...
			<%--}else{--%>
				jboxConfirm('确认保存?',function(){
					var url="<s:url value='/gyxjgl/term/manage/saveScheduleClass'/>";
					jboxPost(url,$("#myForm").serialize(),function(resp){
						if('${GlobalConstant.SAVE_SUCCESSED}'==resp){
