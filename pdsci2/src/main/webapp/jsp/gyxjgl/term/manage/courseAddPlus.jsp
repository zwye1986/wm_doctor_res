...
<head>
	...
	<link rel="stylesheet" href="<s:url value='/css/kalendae.css'/>" type="text/css">
	<script type="text/javascript" src="<s:url value='/js/kalendae.standalone.js'/>"></script>
	...
	<script type="text/javascript">
		$(function(){
			var datas = [];
			var selected = []
			<c:forEach items="${classOrderEnumOfGzList}" var="order">
			var arry = {"id":"${order.id}","text":"${order.name}"};
			datas.push(arry);
			</c:forEach>
			if(datas.length!=0){
				$.itemSelect("classOrder",datas,null,null,selected,0,138);
			}
			...
			$("#classOrder-reqHome").css("z-index", "999");
		});
		function save(){
			...
			jboxConfirm('确认保存?',function(){
				var url="<s:url value='/gyxjgl/term/manage/saveScheduleClassPlus'/>";
				jboxPost(url,$("#myForm").serialize(),function(resp){
					if('${GlobalConstant.SAVE_SUCCESSED}'==resp){
						...
						$("#returnData").show();
						$("#kuang").show();
						$("#hanghao").html(resp.msgTip);
						top.jboxOpenContent($("#returnData").html(),"提示",350,200,true);
						$("#returnData").hide();
					}
				},null,false);
			});
		...
		function bindStudentMaxmun() {
			var courseFlow=$("input[name='courseFlow']").val();
			var url="<s:url value='/gyxjgl/term/manage/searchEduCourse'/>?courseFlow="+courseFlow;
			jboxPost(url,null,function(resp){
				$("input[name='studentMaxmun']").val(resp.courseUserCount);
			},null,false);
		}
	</script>
</head>
...
<div class="mainright">
		...
		<form  id="myForm" method="post">
			...
			<table class="basic" style="width: 100%;">
				...
				<tr>
					<th>上课节次</th>
					<td>
						<input id="classOrder" name="classOrder" type="text" readonly="readonly" class="validate[required]"/>
					</td>
				</tr>
				<tr>
					<th>上课时间</th>
					<td>
						<input type="text" readonly="readonly" class="validate[required] auto-kal" data-kal="mode:'multiple'" name="classTime" style="text-align: left;width: 133px;"/>
					</td>
				</tr>
				<tr>
					<th>上课地点</th>
					<td><select name="classroomId" style="width:137px;" class="select">
						<option/>
						<c:forEach items="${dictTypeEnumGyTeachingPlaceList}" var="room">
				...
				<tr>
					...
					<td>
						<c:if test="${param.roleFlag eq 'skz'}">
							<input type="text" name="studentMaxmun" class="validate[custom[number]]" value="${empty schedule.studentMaxmun?course['COURSE_USER_COUNT']:schedule.studentMaxmun}" readonly="readonly"/>
						</c:if>
						<c:if test="${param.roleFlag eq 'xx'}">
							<input type="text" name="studentMaxmun" class="validate[custom[number]]" value="${schedule.studentMaxmun}" readonly="readonly"/>
						</c:if>
					</td>
				</tr>
				<tr>
	...
	<div id="returnData" hidden="hidden">
		<div style="top:8px;left:20px;position:absolute;">
			<div id="kuang" class="kuang" style="width:350px; height:130px; overflow:auto;margin-top: 5px;" hidden="hidden">
