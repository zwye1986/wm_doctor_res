...
<script type="text/javascript">
	function editApply(userFlow,applyTypeId,recordFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editApply'/>?editFlag=Y&recordFlow="+recordFlow+"&applyTypeId="+applyTypeId+"&userFlow="+userFlow;
	...
	function look(applyTypeId,recordFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editApply'/>?lookFlag=Y&viewFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
		jboxOpen(url ,"查看申请",1100,550);
	}
	function Apply(userFlow){
		var url = "<s:url value='/gyxjgl/change/apply/editApply'/>?userFlow="+userFlow;
		var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
		jboxMessager(iframe,"申请编辑",1100,550);
	}
	function lookAudit(applyTypeId,recordFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editApply'/>?audit=Y&lookFlag=Y&auditFlag=Y&viewFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
		jboxOpen(url ,"查看申请",1100,550);
	}
...
<div class="mainright">
	<div class="content">
		<div class="title1 clearfix">
			<form id="searchForm" method="post" action="<s:url value='/gyxjgl/change/apply/editApply'/>">
				...
				<c:if test="${sessionScope.currWsId ne GlobalConstant.CMIS_WS_ID}">
					<div class="queryDiv">
						...
						<!-- schoolRollStatusId状态：1，已注册学籍 -->
						<c:if test="${user.schoolRollStatusId eq '1'}"><div class="lastDiv"><input type="button" value="异动申请" class="searchInput" onclick="Apply('${sessionScope.currUser.userFlow}');"/></div></c:if>
						<c:if test="${user.schoolRollStatusId ne '1'}"><div style="color:red;margin-top:10px;">当前学籍未注册，不能使用异动申请！</div></c:if>
					</div>
				</c:if>
			</form>
			<div class="resultDiv">
