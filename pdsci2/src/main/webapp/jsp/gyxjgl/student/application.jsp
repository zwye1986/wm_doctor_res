...
$(document).ready(function(){
	...
	function save(userFlow,recordFlow){
	  ...
	  if($("#f").val()==''&&$("#chooseSelect").val()=="${userChangeApplyTypeEnumChangeMajor.id}"){
		...
		}else if($("#chooseSelect").val()=="${userChangeApplyTypeEnumChangeMajor.id}"){
		  $("#swichmajorName").val($("#f option:selected").text().split("]")[1])
	  }
...
function downloadTblFile(){
	...
	jboxTip("下载中…………");
	var url ="<s:url value='/gyxjgl/change/apply/downloadTblFile'/>?applyTypeId="+$("#chooseSelect").val();
	window.location.href = url;
}
function instructionShow(){
	var url = "<s:url value='/gyxjgl/change/apply/uploadInstruction'/>?viewFlag=Y";
			...
			<table class="basic" style="width: 100%;margin: 10px 0px;">
				<tr>
					<th colspan="4">
						...
						<input type="hidden" id="applyTypeName" name="apply.applyTypeName" value="${existApply.applyTypeName}"/>
					</th>
				</tr>
				<tr>
				...
				<tr style="display: none;" class="${userChangeApplyTypeEnumChangeTrainType.id} condition">
					...
					<td colspan="3">
						<select id="b" class="xlselect choos validate[required]" name="willTrainType"  <c:if test="${param.viewFlag eq GlobalConstant.FLAG_Y }">style="display: none;"</c:if>>
							<option value="">请选择</option>
							<c:forEach items="${dictTypeEnumGyTrainCategoryList}" var="tra">
				...
				<tr style="display: none;" class="${userChangeApplyTypeEnumChangeMajor.id} condition">
					...
					<td colspan="3">
						<select id="f" class="xlselect choos validate[required]"name="swichmajorId" <c:if test="${param.viewFlag eq GlobalConstant.FLAG_Y }">style="display: none;"</c:if>>
							<option value="">请选择</option>
								<c:forEach items="${dictTypeEnumGyMajorList}" var="mr">
									<option value="${mr.dictId}" <c:if test="${form.swichmajorId eq mr.dictId}">selected="selected"</c:if>
									<c:if test="${user.majorId eq mr.dictId}">style="display: none;"</c:if>
									>[${mr.dictId}]${mr.dictName}
									</option>
								</c:forEach>
						</select>
						<input type="hidden" id="swichmajorName" name="swichmajorName" value="${form.swichmajorName}">
						<c:if test="${param.viewFlag eq GlobalConstant.FLAG_Y }">
