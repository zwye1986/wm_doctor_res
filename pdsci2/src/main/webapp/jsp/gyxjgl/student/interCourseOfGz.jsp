...
<head>
    ...
    <script type="text/javascript">
        ...
        function LoadEditCourseList(){
            var url = "<s:url value='/gyxjgl/student/course/editCourseList'/>?period=${eduUser.period}&majorId=${eduUser.majorId}&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}";
            jboxLoad("contentDiv", url, true);
        }

        function LoadCourseList(){
            var url = "<s:url value='/gyxjgl/student/course/courseList'/>?studentPeriod=${eduUser.period}&majorId=${eduUser.majorId}&userFlow=${eduUser.userFlow}";
            jboxLoad("contentDiv", url, true);
        }

        function replenish(){
            var url = "<s:url value='/gyxjgl/student/course/replenish'/>?period=${eduUser.period}&majorId=${eduUser.majorId}&trainTypeId=${eduUser.trainTypeId}&userFlow=${eduUser.userFlow}";
            jboxOpen(url, "其他选课", 850, 500);
        }
        function scheduleCourse() {
            jboxStartLoading();
            var url ="<s:url value='/gyxjgl/student/course/choseCourse'/>?userFlow=${eduUser.userFlow}&classId=${eduUser.classId}&majorId=${eduUser.majorId}&trainTypeId=${eduUser.trainTypeId}";
            var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
            jboxMessager(iframe,'选课',1000,650);
            jboxEndLoading();
        }
        function print(userFlow,majorId,period){
            jboxOpen("<s:url value='/gyxjgl/user/resultSun'/>?openType=${GlobalConstant.FLAG_Y}&userFlow="+userFlow+"&majorId="+majorId+"&period="+period,"我的成绩单",1000,500);
        }
        function searchAllCourse(year){
            jboxStartLoading();
            var url ="<s:url value='/gyxjgl/course/manage/searchAllCourse?year='/>"+year;
            var iframe ="<iframe name='jbox-message-iframe' id='jbox-message-iframe' width='100%' height='100%' marginheight='0' marginwidth='0' frameborder='0' scrolling='auto' src='"+url+"'></iframe>";
        ...
        <table class="basic" style="width: 100%;margin: 10px 0px;">
            ...
            <tr>
                ...
                <td>
                    ...
                    <a style="cursor:pointer;color:blue;margin-left:15px;text-decoration:underline;" onclick="searchAllCourse('${curYear}');">${pdfn:getCurrYear()}年研究生课程（全部）</a>
                </td>
                <td>
                ...
                <td  colspan="2">
                    <div style="float: right;">
                        <c:if test="${inChooseTime}">
                            ...
                            <input type="button" id="scheduleCourse" class="search" value="课表选课" onclick="scheduleCourse();" style="background-color: #0b58a2;border: 1px solid #0b58a2;"/>
                        </c:if>
                        <c:if test="${!inChooseTime}">
