...
<head>
    ...
    <script type="text/javascript">
        function updateOpt(recordFlow,flag,column,obj){
            if ($(obj).parent("form") && !$(obj).parent("form").validationEngine("validate")) {
                return;
            }
        ...
        function expExcel(){
            var url = "<s:url value='/gyxjgl/paper/expResult'/>";
            jboxTip("导出中…………");
...
<div class="mainright">
    <div class="content">
        ...
        <c:if test="${param.role ne 'xs'}">
            <form id="searchForm" action="<s:url value="/gyxjgl/paper/defenceResult"/>" method="post">
                <div class="choseDivNewStyle">
                    <input id="currentPage" type="hidden" name="currentPage" value="1"/>
                    <input type="hidden" name="role" value="${param.role}">
                    <span style=""></span>学&#12288;&#12288;号：
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <th style="min-width:80px;">答辩起止时间</th>
                ...
                <th style="min-width:100px;">答辩结果详情</th>
            </tr>
            <c:forEach items="${defenceList}" var="defence" varStatus="i">
                <tr>
                    ...
                    <td>${fn:substring(defence.defenceBeginTime,0,10)}<br/>${fn:substring(defence.defenceEndTime,0,10)}</td>
                    ...
                    <td>
                        <form>优秀&#12288;（<input type="text" name="excellentResult${i.index}" class="validate[custom[number]]" value="${defence.excellentResult}" style="text-align:center;width:30px;" onchange="updateOpt('${defence.recordFlow}',this.value,'excellentResult',this)" ${param.role eq 'xs'?'':'disabled'}>）</form>
                        <form>良好&#12288;（<input type="text" name="goodResult${i.index}" class="validate[custom[number]]" value="${defence.goodResult}" style="text-align:center;width:30px;" onchange="updateOpt('${defence.recordFlow}',this.value,'goodResult',this)" ${param.role eq 'xs'?'':'disabled'}>）</form>
                        <form>及格&#12288;（<input type="text" name="passResult${i.index}" class="validate[custom[number]]" value="${defence.passResult}" style="text-align:center;width:30px;" onchange="updateOpt('${defence.recordFlow}',this.value,'passResult',this)" ${param.role eq 'xs'?'':'disabled'}>）</form>
                        <form>不及格（<input type="text" name="failResult${i.index}" class="validate[custom[number]]" value="${defence.failResult}" style="text-align:center;width:30px;" onchange="updateOpt('${defence.recordFlow}',this.value,'failResult',this)" ${param.role eq 'xs'?'':'disabled'}>）</form>
                    </td>
                </tr>
            </c:forEach>
