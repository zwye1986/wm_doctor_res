...
<head>
    ...
    <script type="text/javascript">
        ...
        function expExcel(){
            var url = "<s:url value='/gyxjgl/paper/expQualify?role=${param.role}'/>";
            jboxTip("导出中…………");
...
<div class="mainright">
    <div class="content">
        ...
        <c:if test="${param.role ne 'xs'}">
            <form id="searchForm" action="<s:url value="/gyxjgl/paper/qualifyApply?role=${param.role}"/>" method="post">
                <div class="choseDivNewStyle">
                    <input id="currentPage" type="hidden" name="currentPage" value="1"/>
                    <span style=""></span>学&#12288;&#12288;号：
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <th rowspan="2" style="min-width:60px;">二级机构</th>
            ...
            <c:forEach items="${defenceList}" var="defence" varStatus="i">
                <tr>
                    ...
                    <td>
                        <input type="radio" id="paperReadFlagY${i.index}" name="paperReadFlag${i.index}" ${defence.paperReadFlag eq 'Y'?'checked':''} onchange="updateOpt('${defence.recordFlow}','Y','paperReadFlag')" ${param.role eq 'ds' or param.role eq 'xx'?'':'disabled'}><label for="paperReadFlagY${i.index}">合格</label><br/>
                        <input type="radio" id="paperReadFlagN${i.index}" name="paperReadFlag${i.index}" ${defence.paperReadFlag eq 'N'?'checked':''} onchange="updateOpt('${defence.recordFlow}','N','paperReadFlag')" ${param.role eq 'ds' or param.role eq 'xx'?'':'disabled'}><label for="paperReadFlagN${i.index}">不合格</label>
                    </td>
                    <td>
