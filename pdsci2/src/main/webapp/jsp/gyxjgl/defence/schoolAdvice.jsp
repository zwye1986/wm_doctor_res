...
<head>
    ...
    <script type="text/javascript">
        ...
        function hsAdvice(recordFlow){
            jboxOpen("<s:url value='/gyxjgl/paper/hsAdvice?recordFlow='/>"+recordFlow, "缓授意见",600,300);
        }
...
<div class="mainright">
    <div class="content">
        <c:if test="${param.role eq 'xs'}">
            <form id="searchForm">
                <div class="choseDivNewStyle">
                    ...
                    <input type="hidden" name="funcFlag" value="xxAdvice"/>
        ...
        <c:if test="${param.role ne 'xs'}">
            <form id="searchForm" action="<s:url value="/gyxjgl/paper/schoolAdvice"/>" method="post">
                <div class="choseDivNewStyle">
                    <input id="currentPage" type="hidden" name="currentPage" value="1"/>
                    <input type="hidden" name="role" value="${param.role}"/>
                    <input type="hidden" name="funcFlag" value="xxAdvice"/>
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <th style="min-width:90px;">校学位会意见</th>
            </tr>
            <c:forEach items="${defenceList}" var="defence" varStatus="i">
                <tr>
                    ...
                    <%--</td>--%>
                    <td>
                        <input type="radio" id="xxwhAdviceFlagY${i.index}" name="xxwhAdviceFlag${i.index}" ${defence.xxwhAdviceFlag eq 'Y'?'checked':''} onchange="updateOpt('${defence.recordFlow}','Y','xxwhAdviceFlag')" ${(param.role eq 'xx' and readOnlyFlag ne 'Y')?'':'disabled'}><label for="xxwhAdviceFlagY${i.index}">建议授予</label><br/>
                        <input type="radio" id="xxwhAdviceFlagN${i.index}" name="xxwhAdviceFlag${i.index}" ${defence.xxwhAdviceFlag eq 'N'?'checked':''} onchange="updateOpt('${defence.recordFlow}','N','xxwhAdviceFlag');" onclick="hsAdvice('${defence.recordFlow}')" ${(param.role eq 'xx' and readOnlyFlag ne 'Y')?'':'disabled'}><label title="${defence.hsAdvice}" for="xxwhAdviceFlagN${i.index}">建议缓授</label>
                    </td>
                </tr>
            </c:forEach>
