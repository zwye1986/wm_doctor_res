...
<head>
    ...
    <script type="text/javascript">
        ...
        function editApply(recordFlow,defenceFlow,viewFlag){
            var url = "<s:url value='/gyxjgl/paper/editTitle?recordFlow='/>"+recordFlow+"&defenceFlow="+defenceFlow+"&viewFlag="+viewFlag+"&isReplenish=${isReplenish}";
            jboxOpen(url, viewFlag=="view"?"查看":"编辑",800,600);
        }
        function addApply(defenceFlow){
            var url = "<s:url value='/gyxjgl/paper/editTitle?defenceFlow='/>"+defenceFlow+"&isReplenish=${isReplenish}";
            jboxOpen(url, "新增",800,600);
        }
        function delInfo(recordFlow){
            jboxConfirm("确认删除论期刊论文题目？", function(){
                var url = "<s:url value='/gyxjgl/paper/delTitle?recordFlow='/>"+recordFlow;
        ...
        function expExcel(){
            var url = "<s:url value='/gyxjgl/paper/expTitleExcel'/>";
            jboxTip("导出中…………");
        ...
        function markDegreePaper(recordFlow){
            jboxConfirm("学位申请论文只允许存在一篇，是否确认？", function(){
                var url = "<s:url value='/gyxjgl/paper/degreePaperOption?recordFlow='/>"+recordFlow;
                jboxPost(url, null, function(resp){
...
<div class="mainright">
    <div class="content">
        ...
        <c:if test="${!empty param.role}">
            <form id="searchForm" action="<s:url value="/gyxjgl/paper/paperList"/>" method="post">
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <c:if test="${pdfn:contains(baseInfo.trainCategoryName,'学术')}">
                    <th style="min-width:40px;max-width:60px">是否作为学位论文申请</th>
                </c:if>
            ...
            <c:forEach items="${titleList}" var="title">
                <tr>
                    ...
                    <c:if test="${pdfn:contains(baseInfo.trainCategoryName,'学术')}">
                        <td>${title.degreeReqFlag eq 'Y'?'是':'否'}</td>
                    </c:if>
                    <td>
                        <c:if test="${empty param.role}">
                            ...
                            <c:if test="${!empty title.recordFlow}">
                                ...
                                <c:if test="${pdfn:contains(baseInfo.trainCategoryName,'学术')}">
                                    <c:if test='${title.degreeReqFlag ne "Y"}'>
                                        <a onclick="markDegreePaper('${title.recordFlow}');" style="cursor:pointer;color:blue;">学位申请论文</a>
                                    </c:if>
                                    <c:if test='${title.degreeReqFlag eq "Y"}'>
                                        <a style="color:grey;">学位申请论文</a>
                                    </c:if>
                                </c:if>
                            </c:if>
                        </c:if>
