...
<head>
    ...
    <script type="text/javascript">
        function daoRu(){
            ...
            jboxStartLoading();
            var url = "<s:url value='/gyxjgl/paper/importDefenceFromExcel'/>";
            jboxSubmit(
                    $('#excelForm'),
                    url,
                    function(resp){
...
<form id="excelForm" method="post" enctype="multipart/form-data">
    <table class="basic" style="width: 100%;">
        ...
        <tr>
            <th>模板文件</th>
            <td>
                <c:if test="${param.isReplenish ne 'Y'}">
                    <a href="<s:url value="/jsp/gyxjgl/defence/importDefenceTemp1.xlsx"/>">答辩申请信息模板.xls</a>
                </c:if>
                <c:if test="${param.isReplenish eq 'Y'}">
                    <a href="<s:url value="/jsp/gyxjgl/defence/importDefenceTemp.xlsx"/>">补授学位信息模板.xls</a>
                </c:if>
            </td>
        </tr>
    </table>
