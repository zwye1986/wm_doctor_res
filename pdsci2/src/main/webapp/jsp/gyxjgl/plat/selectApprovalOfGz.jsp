...
<head>
    ...
    <script type="text/javascript">
        ...
        function reOpen(userFlow){
            jboxConfirm("确认开放该学员重新选课?",function(){
                jboxGet("<s:url value='/gyxjgl/student/course/reOpenChooseCourse'/>?userFlow="+userFlow,null,function(resp){
        ...
        function courseMaintain(period, majorId, userFlow){
            var url = "<s:url value='/gyxjgl/student/course/courseMaintain'/>?period=" + period + "&majorId=" + majorId + "&userFlow=" + userFlow;
            jboxOpen(url,"选课修改", 850, 500, false);
        }
    ...
    <div class="content">
        <table style="width: 100%;margin: 10px 0px;border: none;">
            <tr>
                <td style="border: none;line-height: 260%;">
                    <form id="searchForm" method="post" action="<s:url value="/gyxjgl/student/course/selectApproval?from=${param.from }"/>">
        ...
        <table class="xllist" style="width:100%;">
            <tr>
                ...
                <th rowspan="2" width="160px;">[课程代码]选修课程</th>
                <th rowspan="2" width="160px;">[课程代码]必选课程</th>
                <th colspan="3">合计</th>
            ...
            <c:forEach items="${eduUserExtList}" var="eduUserExt" >
                ...
                <c:if test="${!empty periodMap }">
                    <c:forEach items="${periodMap}" var="period" varStatus="status">
                        ...
                        <tr>
                            ...
                            <td id="${eduUserExt.userFlow}PeriodSum${period.key}">
                                    <%--<fmt:formatNumber type="number" value="${coursePeriodSum }" maxFractionDigits="1"/>--%>
                                <script>
                                    var v = "${coursePeriodSum}"==""?"":Number('${coursePeriodSum}');
                                    $("#${eduUserExt.userFlow}PeriodSum${period.key}").html(v);
                                </script>
                            </td>
                            <td id="${eduUserExt.userFlow}CreditSum${period.key}">
                                ...
                                <script>
                                    ...
                                    $("#${eduUserExt.userFlow}CreditSum${period.key}").html(v);
                                </script>
                            </td>
                            <td id="${eduUserExt.userFlow}DegreeCreditSum${period.key}">
                                <script>
                                    var v = "${degreeCourseCreditSum}"==""?"":Number('${degreeCourseCreditSum}').toFixed(1);
                                    $("#${eduUserExt.userFlow}DegreeCreditSum${period.key}").html(v);
                                </script>
                            </td>
                            <c:if test="${empty param.from }">
                                <td style="line-height: 150%;">
                                    <%--<a style="cursor: pointer; color: blue;" onclick="reOpen('${eduUserExt.userFlow}');">开放选课</a><br/>--%>
                                    <a style="cursor: pointer; color: blue;" onclick="courseMaintain('${period.key}','${eduUserMap[eduUserExt.userFlow].majorId}','${eduUserExt.userFlow}');">选课修改</a>
                                </td>
                            </c:if>
                        </tr>
                    </c:forEach>
                ...
                <c:if test="${empty  periodMap}">
                    <tr>
                        ...
                        <td>--</td>
                        <c:if test="${empty param.from }">
                            <td><a style="cursor: pointer; color: blue;" onclick="courseMaintain('${period.key}','${eduUserMap[eduUserExt.userFlow].majorId}','${eduUserExt.userFlow}');">选课修改</a></td>
                        </c:if>
                    </tr>
                </c:if>
            </c:forEach>
