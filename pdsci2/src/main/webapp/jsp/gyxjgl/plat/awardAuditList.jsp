...
<script type="text/javascript">
	...
	function look(recordFlow,applyTypeId,userFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editAwardApply'/>?userFlow="+userFlow+"&audit=Y&lookFlag=Y&auditFlag=Y&viewFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
		jboxOpen(url ,"查看申请",1100,550);
	}
	function audit(recordFlow,applyTypeId,userFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editAwardApply'/>?userFlow="+userFlow+"&viewFlag=Y&auditFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
	...
	function print(recordFlow){
		jboxTip("打印中,请稍等...");
		var url = '<s:url value="/gyxjgl/change/apply/awardPrint"/>?recordFlow='+recordFlow;
	...
	function exportExcel(){
		...
		var url = "<s:url value='/gyxjgl/change/apply/exportExcel?awardFlag=Y'/>&trainOrgId="+trainOrgId+"&applyTypeId="+applyTypeId;
		jboxTip("导出中…………");
		window.location.href = url;	
		jboxEndLoading();
	}
	function uploadForm(){
		var url =  "<s:url value='/gyxjgl/change/apply/uploadForm?awardFlag=Y'/>";
		jboxOpen(url ,"上传表格",900,550);
	}
	function uploadInstruction(){
		var url = "<s:url value='/gyxjgl/change/apply/uploadInstruction?awardFlag=Y'/>";
...
<div class="mainright">
	<div class="content">
	<form id="searchForm" action="<s:url value="/gyxjgl/change/apply/auditAwardApply"/>" method="post">
			...
			<c:forEach items="${changeExtList}" var="change">
				<tr>
					...
					<td style="text-align: center;padding-left: 0px;">
						<c:set var="bool" value="${change.statusId eq userChangeApplyStatusEnumSubmit.id && (change.applyTypeId eq userAwardApplyTypeEnumPoorArchives.id || change.applyTypeId eq userAwardApplyTypeEnumTuitionDelay.id)}"/>
						<c:if test="${bool}">
