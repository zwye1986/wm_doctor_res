...
<head>
    ...
    <script type="text/javascript">
        ...
        function addInfo(userFlow){
            ...
            var url = "<s:url value='/gyxjgl/secondaryOrg/addSecondaryAdmin?userFlow='/>"+userFlow;
            jboxOpen(url, title,600,360,true);
        }
        function resetPwd(userFlow){
            jboxConfirm("重置密码之后为：gzykdx@yjs！", function(){
                var url = "<s:url value='/gyxjgl/secondaryOrg/resetPwd?userFlow='/>"+userFlow;
...
<div class="mainright">
    <div class="content">
        <form id="searchForm" action="<s:url value="/gyxjgl/secondaryOrg/secondaryOrgList"/>" method="post">
            <div class="choseDivNewStyle">
                ...
                <select class="select" style="width:137px;" name="orgFlow">
                    ...
                    <c:forEach var="org" items="${orgMap}">
                        <option value="${org.key}" <c:if test="${param.orgFlow eq org.key}">selected="selected"</c:if>>${org.value}</option>
                    </c:forEach>
                </select>
        ...
        <table class="xllist" style="width:100%;">
            ...
            <c:forEach items="${dataList}" var="info" varStatus="i">
                <tr>
                    ...
                    <td>${info.userCode}</td>
                    <td>
                        <a onclick="addInfo('${info.userFlow}');" style="cursor:pointer;color:blue;"><c:out value="编辑"/></a>
                        <a onclick="resetPwd('${info.userFlow}');" style="cursor:pointer;color:blue;"><c:out value="重置密码"/></a>
                    </td>
                </tr>
            </c:forEach>
        </table>
