...
<script type="text/javascript">
	function add(courseFlow){
		...
		jboxConfirm('确认保存?',function(){
			...
			if(courseFlow==""){
				...
				var url="<s:url value='/gyxjgl/course/manage/saveCourse'/>";
				...
				$('[name="courseMoudleName"]').val($('#courseMoudleId option:selected').text());
				$('[name="courseSpeaker"]').val($('#courseSpeakerFlow option:selected').text());
				var url="<s:url value='/gyxjgl/course/manage/saveCourse'/>?courseFlow="+courseFlow;
				var fromSerizalize=$("#form").serialize();
				 jboxPost(url,fromSerizalize,function(){
					setTimeout(function(){
						window.parent.frames['mainIframe'].window.toPage();
	...
	function courseCodeOk(){//课程代码+年份组合维度
		...
		var url="<s:url value='/gyxjgl/course/manage/courseCodeOk'/>?courseCode="+courseCode+"&courseSession="+courseSession;
	...
	<div class="content">
		...
		<form  id="form" method="post">
			<table class="basic" style="width: 100%;margin: 10px 0px;">
				...
				<tr>
					<th><span style="color: red">*</span>&nbsp;容纳人数：</th>
					<td style="text-align: left;padding-left: 10px" ><input style="width: 137px;" class="validate[required,custom[number]]"  type="text" name="courseUserCount" value="${course.courseUserCount}"/></td>
					<th>所属模块：</th>
					...
					<td>
						<select style="width: 140px;" name="courseTypeId"  id="courseTypeId" class="validate[required]">
							...
							<c:forEach items="${xjglCourseTypeEnumList}" var="courseType">
								<c:if test="${courseType.id eq 'Public' or courseType.id eq 'OptionalNeed'}">
									<option value="${courseType.id}" <c:if test="${course.courseTypeId==courseType.id}">selected="selected"</c:if>>${fn:substring(courseType.name,2,5)}</option>
								</c:if>
							</c:forEach>
						</select></td>
				</tr>
				...
				<tr>
					...
					<th><span style="color: red">*</span>&nbsp;理论课老师：</th>
					<td  colspan="3"  style="text-align: left;padding-left: 10px;">
						<input type="text" class="validate[required]"  style="width: 402px;"  name="teacherNameList" placeholder="多老师用“,”隔开" value="<c:forEach items="${teaList}" var="tea" varStatus="i">${tea.teacherName}<c:if test="${!i.last}">,</c:if></c:forEach>"/>
				<%--		<input type="hidden" name="oldTeacherNameList" value="<c:forEach items="${teaList}" var="tea" varStatus="i">${tea.teacherName}<c:if test="${!i.last}">,</c:if></c:forEach>"/>
					--%></td>

					<th><span style="color: red">*</span>&nbsp;实验课老师：</th>
					<td>
						<input type="text"  class="validate[required]" name="teacherPracticeList" placeholder="多老师用“,”隔开" value="<c:forEach items="${tepList}" var="tea" varStatus="i">${tea.teacherName}<c:if test="${!i.last}">,</c:if></c:forEach>"/>
					<%--	<input type="hidden" name="oldteacherPracticeList" value="<c:forEach items="${tepList}" var="tea" varStatus="i">${tea.classTeacherName}<c:if test="${!i.last}">,</c:if></c:forEach>"/>
					--%></td>
				</tr>
