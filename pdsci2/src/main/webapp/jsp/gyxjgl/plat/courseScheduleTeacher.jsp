...
<head>
    ...
    <script type="text/javascript">
        ...
        function details(recordFlow,courseMajorId,count){
            var url="<s:url value='/gyxjgl/teacher/details'/>?recordFlow="+recordFlow+"&courseMajorId="+courseMajorId+"&count="+count;
        ...
        <table style="width:100%;min-width: 1080px;margin: 10px 0px;border: none;">
            <tr>
                <td style="line-height: 260%;">
                    <form id="recSearchForm" method="post" action="<s:url value='/gyxjgl/teacher/teacherSchedule'/>">
                        年&#12288;&#12288;份：
                        <input type="text" name="courseSession" value="${empty param.courseSession?year:param.courseSession}" class="validate[required]" onclick="WdatePicker({dateFmt:'yyyy'})" readonly="readonly" style="width:137px;margin: 0">
                        &#12288;
                        学&#12288;&#12288;期：
                        <select name="gradeTermId" class="validate[required]" style="width: 141px;">
                            <c:forEach items="${dictTypeEnumGyTermSeasonList}" var="dict">
                                <option value="${dict.dictId}" <c:if test="${dict.dictId eq (empty param.gradeTermId?grade:param.gradeTermId)}">selected</c:if>>${dict.dictName}</option>
                            </c:forEach>
                        </select>
                        &#12288;
                        开始日期：
                        <input type="text" name="termStartTime" id="termStartTime" value="${empty param.termStartTime?startDate:param.termStartTime}" class="validate[required]" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width: 137px;">
                        &#12288;
                        结束日期：
                        <input type="text" name="termEndTime" id="termEndTime" value="${empty param.termEndTime?endDate:param.termEndTime}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width: 137px;">
                        <br/>班&#12288;&#12288;级：
                        <select style="width:141px;" class="validate[required]" name="classId">
