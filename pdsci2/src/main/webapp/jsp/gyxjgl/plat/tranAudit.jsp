...
<script type="text/javascript">
	...
	function look(recordFlow,applyTypeId,userFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editApply'/>?userFlow="+userFlow+"&audit=Y&lookFlag=Y&auditFlag=Y&viewFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
		jboxOpen(url ,"查看申请",1100,550);
	}
	function audit(recordFlow,applyTypeId,userFlow){
		var url =  "<s:url value='/gyxjgl/change/apply/editApply'/>?userFlow="+userFlow+"&viewFlag=Y&auditFlag=Y&applyTypeId="+applyTypeId+"&recordFlow="+recordFlow;
	...
	function exportExcel(){
		...
		var url = "<s:url value='/gyxjgl/change/apply/exportExcel'/>?trainOrgId="+trainOrgId+"&applyTypeId="+applyTypeId;
		jboxTip("导出中…………");
		window.location.href = url;	
		jboxEndLoading();
	}
	function uploadForm(){
		var url =  "<s:url value='/gyxjgl/change/apply/uploadForm'/>";
		jboxOpen(url ,"上传表格",900,550);
	}
	function uploadInstruction(){
		var url = "<s:url value='/gyxjgl/change/apply/uploadInstruction'/>";
...
<div class="mainright">
	<div class="content">
	<form id="searchForm" action="<s:url value="/gyxjgl/change/apply/findChangeInfo"/>" method="post">
