...
//                    loadCourseList($('#planYear').val());
        ...
        function result(courseCode,studentPeriod){
            jboxOpen("<s:url value='/gyxjgl/secondaryOrg/resultSun'/>?courseCode="+courseCode+"&studentPeriod="+studentPeriod,"成绩管理",960,600);
        }
        ...
        function passSelectOpt(){
            ...
            jboxConfirm("确认通过所选信息？", function(){
                var url = "<s:url value='/gyxjgl/teachingGroup/auditPartStu?recordList='/>"+recordList;
        ...
        function backSelectOpt(){
            ...
            jboxConfirm("确认退回所选信息？", function(){
                var url = "<s:url value='/gyxjgl/teachingGroup/auditPartStu?recordList='/>"+recordList+"&auditStatusId=UnPassed";
                jboxPost(url, null, function(resp){
                    if (resp == "${GlobalConstant.OPERATE_SUCCESSED}") {
                        window.parent.frames['mainIframe'].location.reload();
                    }
        ...
        function auditOption(recordFlow,statusId){
            ...
            jboxConfirm("确认该记录"+statusName+"？", function(){
                var url = "<s:url value='/gyxjgl/teachingGroup/auditPartStu?recordList='/>"+recordList+"&auditStatusId="+statusId;
                jboxPost(url, null, function(resp){
                    if (resp == "${GlobalConstant.OPERATE_SUCCESSED}") {
                        window.parent.frames['mainIframe'].location.reload();
                    }
        ...
        <table style="width:100%;min-width: 1080px;margin: 10px 0px;border: none;">
            <tr>
                <td style="line-height: 260%;">
                    <form id="recSearchForm" method="post" action="<s:url value='/gyxjgl/secondaryOrg/gradeResultAudit'/>">
                ...
                <c:forEach items="${dataList}" var="record">
                    <tr id="${record.RECORD_FLOW}">
                        ...
                        <td id="score${record.RECORD_FLOW}" style="text-align: center;padding: 0px;line-height: 120%;">
                            <c:choose>
                                <c:when test="${record.COURSE_GRADE eq 'Y' || record.COURSE_GRADE eq 'N' || record.COURSE_GRADE eq 'Excellent' || record.COURSE_GRADE eq 'Good'
								 || record.COURSE_GRADE eq 'Secondary' || record.COURSE_GRADE eq 'Pass' || record.COURSE_GRADE eq 'UnPass'}">
                                    <c:set var="gradeId" value="GyXjIsPassed.${record.COURSE_GRADE}" />
                                ...
                                <c:otherwise>
                                    ...
                                    <script>
                                        ...
                                        $("#score${record.COURSE_GRADE}").html(v);
                                    </script>
                                </c:otherwise>
                            </c:choose>
                        </td>
