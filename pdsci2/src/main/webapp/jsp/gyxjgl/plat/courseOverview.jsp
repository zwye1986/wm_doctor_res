...
function loadCourseList(planYear){
	...
	var url = "<s:url value='/gyxjgl/majorCourse/searchCourseJson'/>?assumeOrgFlow="+$("#assumeOrgName").attr("flow")+"&planYear="+planYear;
...
function loadStudentList(planYear, courseFlow, page){
	jboxStartLoading();
	var url = "<s:url value='/gyxjgl/course/manage/searchStudentList'/>?planYear=" + planYear + "&courseFlow=" + courseFlow + "&currentPage=" + page;
...
function exportExcel(courseFlow,studentCount){
	...
	var url = "<s:url value='/gyxjgl/course/manage/exportStudentList'/>";
	jboxTip("导出中…………");
...
function exportMultiExcle(){
	...
	var url = "<s:url value='/gyxjgl/course/manage/exportMultiExcle'/>?planYear="+ planYear;
	jboxTip("导出中…………");
	jboxSubmit($("#searchForm"), url, null, null, false);
	jboxEndLoading();
}
...
function editCourse(courseFlow,flag){
	var title = "开课申请";
	if(flag == '2'){
		title = "课程进度";
	}else if(flag == '3'){
	...
	jboxOpen("<s:url value='/gyxjgl/secondaryOrg/editCourse?viewFlag=Y'/>&courseFlow="+courseFlow+"&flag="+flag,title,460,260);
}
	function teachingTbl(courseFlow,courseSession){
		$("#courseFlow").val(courseFlow);
		$("#courseSession").val(courseSession);
		var url = "<s:url value='/gyxjgl/course/manage/teachingTbl'/>";
		jboxTip("导出中…………");
		jboxSubmit($("#myForm"), url, null, null, false);
		jboxEndLoading();
	}
	function printTeachingTbl(courseFlow,courseSession){
		$("#courseFlow").val(courseFlow);
		$("#courseSession").val(courseSession);
		var url = '<s:url value="/gyxjgl/course/manage/printTeachingTbl"/>';
		jboxStartLoading();
		jboxPost(url, $("#myForm").serialize(), function(data) {
			$("#printDiv").html(data);
			setTimeout(function(){
	...
	function signClass(courseFlow){
		jboxOpen("<s:url value='/jsp/gyxjgl/plat/signClass.jsp'/>?courseFlow="+courseFlow,"课程签到码",200,200);
	}
</script>
...
<div class="mainright">
	<div class="content">
		<form id="myForm">
			<input type="hidden" name="courseFlow" id="courseFlow">
		...
		<table class="basic" style="width: 100%;margin: 15px 0px;border: none;">
			<tr >
				<td style="border: none;height:auto;">
					<form id="searchForm" method="post" action="<s:url value='/gyxjgl/course/manage/courseMajor'/>">
				...
				<tr onclick="toogleStudent('${eduCourse.courseFlow}');">
					<td colspan="7" style="text-align: left;padding-left: 20px;cursor: pointer;">承担单位：${empty eduCourse.assumeOrgName?'--':eduCourse.assumeOrgName}&#12288;&#12288;课程：[${empty eduCourse.courseCode?'--':eduCourse.courseCode}]${empty eduCourse.courseName?'--':eduCourse.courseName}&#12288;&#12288;学分：${empty eduCourse.courseCredit?'--':eduCourse.courseCredit}&#12288;&#12288;总学时：${empty eduCourse.coursePeriod?'--':eduCourse.coursePeriod}&#12288;&#12288;上课人数：${empty studentList?'--':fn:length(studentList)}
						<span style="float: right;padding-right: 10px;">
							<c:if test="${eduCourse.classApplyFlag eq 'Submit'}"><a onclick="editCourse('${eduCourse.courseFlow}','1')" style="color:red;">开课申请</a>&#12288;</c:if>
							<c:if test="${eduCourse.courseScheduleFlag eq 'Submit'}"><a onclick="editCourse('${eduCourse.courseFlow}','2')" style="color:red;">课程进度</a>&#12288;</c:if>
							<c:if test="${eduCourse.courseEvaluateFlag eq 'Submit'}"><a onclick="editCourse('${eduCourse.courseFlow}','3')" style="color:red;">课程评价</a>&#12288;</c:if>
							<a href="javascript:signClass('${eduCourse.courseFlow}');" style="color:blue;">签到码</a>&ensp;
							<a href="javascript:teachingTbl('${eduCourse.courseFlow}','${eduCourse.courseSession}');" style="color:blue;">教学审批表下载</a>&ensp;
							<a href="javascript:printTeachingTbl('${eduCourse.courseFlow}','${eduCourse.courseSession}');" style="color:blue;">教学审批表打印</a>&ensp;
