...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#applyTypeId").bind("change",function(){
			var url = "<s:url value='/gyxjgl/change/apply/uploadForm?awardFlag=Y'/>&applyTypeId="+$(this).val();
	...
	function returnUrl(filePath){
		...
		$('#'+fileUrlId+'Del').show();
		$('#'+fileUrlId+'Span').show();
		$('#'+fileUrlId+'Span').find("input").attr("onclick","linkTable('"+filePath+"')");
	...
	function uploadFile(){
		...
		jboxStartLoading();
		var url = "<s:url value='/gyxjgl/change/apply/uploadTblFile'/>";
	...
	function save(){
		...
		jboxConfirm('确认保存?',function(){
			jboxPost("<s:url value='/gyxjgl/change/apply/saveTblFile?awardFlag=Y'/>", $("#submitForm").serialize() , function(resp){
					...
					<tr>
						<th style="width:40%;">
							...
							<select id="applyTypeId" class="xlselect validate[required]" name="applyTypeId">
								...
								<c:forEach items="${param.awardFlag eq 'Y'?userAwardApplyTypeEnumList:userChangeApplyTypeEnumList}" var="apply">
