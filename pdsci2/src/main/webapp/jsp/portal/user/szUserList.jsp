...
<table class="xllist" style="width:100%;margin-top: 10px;text-align: center">
		...
		<tr>
			...
			<td>
				<c:if test="${!empty applicationScope.sysCfgMap['portals_patient_role_flow']}">
					<label style="width: 35%;display: inline-block;">
						<input type="checkbox" onclick="selRole('${userExt.userFlow}',this.value)"
							   value="${applicationScope.sysCfgMap['portals_patient_role_flow'] }"
							   <c:if test="${pdfn:contain(applicationScope.sysCfgMap['portals_patient_role_flow'],sysUserRoleMap[userExt.userFlow]) }">checked='checked'</c:if>/>${!empty sysRoleMap[sysCfgMap['portals_patient_role_flow']]?sysRoleMap[sysCfgMap['portals_patient_role_flow']].roleName:'患者'}
					</label>
				</c:if>
				<c:if test="${!empty applicationScope.sysCfgMap['portals_doctor_role_flow']}">
					<label style="width: 35%;display: inline-block;">
						<input type="checkbox" onclick="selRole('${userExt.userFlow}',this.value)"
							   value="${applicationScope.sysCfgMap['portals_doctor_role_flow'] }"
							   <c:if test="${pdfn:contain(applicationScope.sysCfgMap['portals_doctor_role_flow'],sysUserRoleMap[userExt.userFlow]) }">checked='checked'</c:if>/>${!empty sysRoleMap[sysCfgMap['portals_doctor_role_flow']]?sysRoleMap[sysCfgMap['portals_doctor_role_flow']].roleName:'医生'}
					</label>
				</c:if>
			</td>
			<td>
