...
function edit(infoFlow){
	jboxOpen("<s:url value='/portal/manage/info/showEdit?infoFlow='/>"+infoFlow,"编辑资讯", 900, 620);
}
function add(){
	jboxOpen("<s:url value='/portal/manage/info/add'/>?role=${param.role}","新增资讯", 900, 620);
}
...
<div class="mainright" id="mainright">
	<div class="content">
	<form id="searchForm" action="<s:url value="/portal/manage/info/list" />" method="post" >
		 <div class="queryDiv">
		...
		<c:if test="${param.infoStatusId!= infoStatusEnumPassing.id && param.infoStatusId!= infoStatusEnumPassed.id && param.infoStatusId!= infoStatusEnumCityPassed.id}">
			<div class="funcDiv" style="color: red">
			...
			<c:forEach items="${infoList}" var="info">
				<tr>
					...
					<td>
                        ...
                        <c:if test="${info.infoStatusId!= infoStatusEnumPassed.id }">
							<c:if test="${(info.infoStatusId==infoStatusEnumPassing.id and param.role ne 'admin') ||
							(param.role eq 'admin' and info.createUserFlow eq currentUserFlow) }">
								[<a href="javascript:void(0)" onclick="updateOneStatus('${info.infoFlow}','${infoStatusEnumEdit.id}')" >撤回</a>]
							</c:if>
                            [<a href="javascript:void(0)" onclick="view('${info.infoFlow}','show')" >查看</a>]
                            <c:if test="${info.infoStatusId==infoStatusEnumEdit.id || info.infoStatusId==infoStatusEnumNoPassed.id and param.role != 'admin'}">
                                [<a href="javascript:void(0)" onclick="edit('${info.infoFlow}')" >修改</a>]
						...
						<c:if test="${ (info.infoStatusId== infoStatusEnumCityPassed.id ||
						info.infoStatusId== infoStatusEnumPassing.id ||
						info.infoStatusId== infoStatusEnumNoPassed.id
						)and (param.role == 'admin') and (info.createUserFlow ne currentUserFlow)}">
							[<a href="javascript:void(0)" onclick="view('${info.infoFlow}','pass')" >审核</a>]
						</c:if>
					</td>
				</tr>
