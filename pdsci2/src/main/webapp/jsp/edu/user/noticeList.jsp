...
<script type="text/javascript">
	function saveNotice(){
		...
		var url = "<s:url value='/edu/user/saveNotice'/>";
		jboxPost(url,$("#noticeForm").serialize(),
				function(resp){
					...
					var url ="<s:url value='/edu/user/notice?courseFlow=${param.courseFlow}'/>";
	...
	function delNotice(noticeFlow){
		jboxConfirm("确认删除此条通知?", function(){
			var url = "<s:url value='/edu/user/delNotice?noticeFlow='/>" + noticeFlow;
...
<div class="words clearfix" style="display: none;" id ="noticeDiv">
	...
	<input type="hidden" name="noticeFlow" id="noticeFlow"/>
 			<img class="headimages" src="${sysCfgMap['upload_base_url']}${sessionScope[GlobalConstant.CURR_EDU_USER].userImg}" onerror="this.src='<s:url value="/jsp/edu/css/images/17326850.jpg"/>'" height="24" width="24"/>
...
<div class="aquestion" id="noticeList">
	...
	<div class="noticeList">
		<span class="publishtime fr">发布于&nbsp;${pdfn:transDateTime(noticeExt.createTime)}</span>
			<img src="${sysCfgMap['upload_base_url']}${noticeExt.eduUser.userImg}" onerror="this.src='<s:url value="/jsp/edu/css/images/17326850.jpg"/>'" class="headimages" height="24" width="24">
