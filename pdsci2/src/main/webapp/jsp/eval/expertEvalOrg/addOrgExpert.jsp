...
<script type="text/javascript">
	function expertEvalOrg(obj,userFlow,orgFlow,evalYear,recordFlow,userName,userCode,orgName)
	{
		...
		if(recordStatus == "${GlobalConstant.RECORD_STATUS_N}")
		{
			jboxConfirm("确认取消此专家评估？取消的同时将删除相应的评估配置",function(){
				save(obj,userFlow,orgFlow,evalYear,recordFlow,userName,userCode,orgName);
			},function(){
	...
	function save(obj,userFlow,orgFlow,evalYear,recordFlow,userName,userCode,orgName)
	{
		var checkedVal = $(obj).attr("checked");
		var recordStatus = "${GlobalConstant.RECORD_STATUS_N}";
		if ("checked" == checkedVal) {
		...
		jboxStartLoading();
		jboxPost2("<s:url value='/eval/expertEvalOrg/saveEvalOrg'/>",evalOrg,function(resp){
			if('${GlobalConstant.SAVE_SUCCESSED}'!=resp){
				$(obj).attr("checked",false);
			}else{
				if(recordStatus == "${GlobalConstant.RECORD_STATUS_N}")
				{
					window.parent.frames['mainIframe'].window.$("#"+orgFlow+userFlow).remove();
				}else {
					var html=
						"<div style='width: 24%;float: left;' id='"+orgFlow+userFlow+"'>" +
								...
								"<tr>" +
									"<td style='width: 100%;border: 0px;'  cellpadding='0' cellspacing='0'>" +
										"<label>" +
											...
											" onclick='cfgOrgDetail(\""+userFlow+"\",\""+orgFlow+"\",\""+evalYear+"\",\""+userName+"\",\""+orgName+"\")'>"+
											userName+"["+userCode+"]"+"</a>"+
										"</label> " +
									"</td>" +
								"</tr>" +
							"</table>" +
						"</div>";
					window.parent.frames['mainIframe'].window.$("#"+orgFlow).append(html);
				}
			}
			setTimeout(function(){
				jboxEndLoading();
												...
												<td style="width: 100%;border: 0px;"  cellpadding="0" cellspacing="0">
													<label>
														<input type="checkbox" id="${user.userFlow}${org.orgFlow}Org"
															   onclick="expertEvalOrg(this,'${user.userFlow}','${param.orgFlow}','${param.evalYear}','${evalOrgMap[user.userFlow].recordFlow}','${user.userName}','${user.userCode}','${org.orgName}')"
