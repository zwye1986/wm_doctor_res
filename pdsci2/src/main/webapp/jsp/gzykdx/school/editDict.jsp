...
<script type="text/javascript">
	function doSave() {
		...
		var url = "<s:url value='/gzykdx/school/save'/>";
		...
		<div id="tagContent">
			<div class="tagContent selectTag" id="tagContent0">
				<table style="width: 400px" cellpadding="0" cellspacing="0" class="basic">
							...
							<select class="validate[required] xlselect" name="dictTypeId">
								...
								<c:forEach var="dictTypeEnum" items="${dictTypeEnumList}">
									<c:if test="${dictTypeEnum.wsid eq sessionScope.currWsId}">
										<option value="${dictTypeEnum.id}" <c:if test="${dict.dictTypeId eq dictTypeEnum.id}">selected="selected"</c:if>>${dictTypeEnum.name}</option>
									</c:if>
								</c:forEach>
							</select>
						</td>
					...
					<tr>
						<th>字典代码：</th>
						<td><input class="validate[required] xltext" name="dictId" type="text" value="${dict.dictId}" /></td>
					</tr>
					<tr>
						<th>字典名称：</th>
						<td><input class="xltext validate[maxSize[50]]" name="dictName" type="text" value="${dict.dictName }" /></td>
					</tr>
					<tr>
						<th>代码描述：</th>
						<td><input class="xltext validate[maxSize[200]]" name="dictDesc" type="text" value="${dict.dictDesc }" /></td>
					</tr>
					<tr>
						<th >排序码：</th>
