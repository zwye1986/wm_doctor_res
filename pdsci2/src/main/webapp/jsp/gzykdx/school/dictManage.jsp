...
function edit(dictFlow){
	jboxOpen("<s:url value='/gzykdx/school/edit?dictFlow='/>"+dictFlow,"编辑", 500,350);
}

function add(){
	jboxOpen("<s:url value='/gzykdx/school/edit?dictTypeId=${param.dictTypeId}'/>","新增", 500,350);
}
function search(){
	jboxStartLoading();
	$("#searchForm").submit();	
...
function importInfo(){
	jboxOpen("<s:url value='/jsp/gzykdx/school/importSpe.jsp'/>", "导入",600,200);
}
...
function refresh(){
	var url = "<s:url value='/gzykdx/school/doRefresh'/>";
...
<div class="mainright">
	<div class="content">
		 ...
		 <form id="searchForm" action="<s:url value="/gzykdx/school/dictManage" />" method="post" >
			 <div class="choseDivNewStyle" style="width: 1100px">
				 <span></span>字典类型：
				 ...
				 <span style="padding-left:20px;"></span>字典代码：
				 <input name="dictId" type="text" value="${param.dictId}"/>
				 <span style="padding-left:20px;"></span>字典名称：
				 ...
				 <input type="button" class="search" onclick="importInfo();" value="导入专业">
			 </div>
		</form>
		</div>
		<table class="xllist">
