...
function daoRu(){
	...
	jboxStartLoading();
	var url = "<s:url value='/hbres/singup/importSkillScoreFromExcel'/>";
	jboxSubmit(
		$('#excelForm'),
		url,
		function(resp){
			...
			if(resp.substring(0,5)=="${GlobalConstant.UPLOAD_SUCCESSED}"){
				window.parent.toPage(1,scoreYear);
				top.jboxClose();
			}
	...
	<table class="grid" style="width: 100%;">
		<tr>
			...
			<td style="text-align: left;">
				&ensp;<input type="text" id="scoreYear" name="scoreYear" value="${pdfn:getCurrYear()}" class="input validate[required]" readonly="readonly" style="width: 100px;margin-left: 0px"/>
			</td>
		</tr>
		<tr>
			<th style="text-align: right;">请选择导入：</th>
			<td style="text-align: left;">
				&ensp;<input id="file" type="file" name="file" onchange="checkFile(this);" class="validate[required]"/>
			</td>
		</tr>
		<tr>
			<th style="text-align: right;">模板文件：</th>
			<td style="text-align: left;">&ensp;<a href="<s:url value='/jsp/hbres/graduate/importSchoolRollSkill.xlsx'/>">学生技能成绩导入模板.xlsx</a></td>
		</tr>
	</table>
</form>
