...
<script type="text/javascript">
	...
	{
		var url = "<s:url value='/hbres/asse/showAllInfoMain?isShow=Y&doctorFlow='/>"+doctorFlow+"&applyYear="+applyYear;
	...
	{
		var url = "<s:url value='/hbres/asse/localAuditApply?applyFlow='/>"+applyFlow;
				...
				<c:forEach items="${list}" var="doctor">
					<tr class="fixTrTd">
						...
						<c:if test="${ f eq 'Y'}">
							...
							<c:if test="${ (roleFlag eq 'jointOrg')}">
								<c:if test="${ (empty doctor.cityAuditStatusId) and (not empty doctor.localAuditStatusId) and doctor.applyYear eq pdfn:getCurrYear() }">
									<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">重新审核</a>
								</c:if>
								<c:if test="${ (doctor.auditStatusId eq jsResAuditStatusEnumAuditing.id) }">
									<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">审核</a>
								</c:if>
							</c:if>
							<c:if test="${ (roleFlag ne 'jointOrg' and doctor.orgFlow eq currentUser.orgFlow)  and doctor.applyYear eq pdfn:getCurrYear() }">
								<c:if test="${ (empty doctor.cityAuditStatusId) and (not empty doctor.localAuditStatusId)}">
									<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">重新审核</a>
								</c:if>
								<c:if test="${ (doctor.auditStatusId eq jsResAuditStatusEnumAuditing.id) }">
									<a class="btn"  onclick="chargeAuditApply('${doctor.applyFlow}');" style="margin-top: 5px;margin-bottom: 5px;">审核</a>
