...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('select[name=mulDeptFlow]').UCFormSelect();
            $(".SelectVal").css("width","160px").css("margin","4px 10px");
            $(".SelectBox").css("width","160px").css("margin-left","10px");
        });
        function saveUser() {
            ...
            var url = "<s:url value='/res/nurse/save'/>";
...
<div>
    <form id="sysUserForm" style="position: relative;">
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="base_info" style="margin-top: 40px;">
            <colgroup>
                <col width="15%"/>
                <col width="30%"/>
                <col width="15%"/>
            ...
            <tr>
                ...
                <th> 科室：</th>
<%--                <td>--%>
<%--                    <select name="mulDeptFlow" class="select validate[required]" multiple="multiple">--%>
...
<%--                </td>--%>
                <td>
                    <select name="mulDeptFlow" class="select validate[required]" id="" multiple="multiple">
                        <c:forEach items="${depts}" var="dept">
                            <option value="${dept.deptFlow}"
                                    <c:if test="${sysUser.deptFlow eq dept.deptFlow}">selected</c:if>>${dept.deptName}</option>
                        </c:forEach>
