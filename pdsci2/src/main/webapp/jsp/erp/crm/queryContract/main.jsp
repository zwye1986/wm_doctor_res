	...
	<script type="text/javascript">
		...
		function moneyMask(){
			...
			$.each(moneyBox,function(i,n){
				var m=parseInt($(n).html()==""?"0":$(n).html());
				if(m>=0)
				{
...
//							$("#chargeUser2Flow").append('<option value="'+data[i].userFlow+'">'+data[i].userName+'</option>');
		...
		function search(){
			jboxPostLoad("payList" ,"<s:url value='/erp/crm/queryContract/list'/>" ,$("#searchForm").serialize() , true);
		}
		...
		function planAndBillInfo(contractFlow) {
			var url= "<s:url value='/erp/crm/queryContract/planAndBillInfo'/>?contractFlow=" + contractFlow;
			jboxOpen(url,"款项及开票详情", 800, 600);
		}
	</script>
</head>
<body>
		...
		<form  id="searchForm" method="post">
			...
			<table class="basic searchTable" style="width:100%;margin-top: 10px;">
					...
					<td>
						...
						<select class="xlselect" id="contractCategoryId" name="contractCategoryId" onchange="changeContractTypeOption(this);" style="width:128px;">
							...
							<c:forEach var="contractCategory" items="${contractCategoryEnumList}">
								<c:if test="${(type eq 'yszcx')and (type eq 'cwyscx')}">
									<c:if test="${contractCategoryEnumPurchase.id ne contractCategory.id }">
				...
				<tr>
					<td>
						<label>合同创建日：</label>
						<input type="text" name="createDateStart"  class="ctime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width:100px;margin: 0"/>&nbsp;~&nbsp;
						<input type="text" name="createDateEnd"  class="ctime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width:100px;"/>

						<c:if test="${type eq 'zxcx'}">
							<label>&#12288;到期脱保：</label>
							<select class="xlselect" name="dqtb" style="width:128px;">
								<option value="">请选择</option>
								<option value="zcht">正常合同</option>
								<option value="htdqwtb">合同到期未脱保</option>
								<option value="htdqytb">合同到期已脱保</option>
								<option value="htwdqwtb">合同未到期已脱保</option>
							</select>
							<label>&#12288;脱保时间：</label>
							<input type="checkbox" id="one" name="tbsj" checked="checked" value="one"><label for="one">&nbsp;一个月</label>
							<input type="checkbox" id="two" name="tbsj" checked="checked" value="two"><label for="two">&nbsp;一个月以上</label>
							<input type="checkbox" id="three" name="tbsj" checked="checked" value="three"><label for="three">&nbsp;三个月以上</label>
							<input type="checkbox" id="six" name="tbsj" checked="checked" value="six"><label for="six">&nbsp;六个月以上</label>
						</c:if>
					</td>
				</tr>

				<c:if test="${type eq 'dkcx'}">
				<tr>
					<td>
						<label>&#12288;到账逾期：</label>
						<select class="xlselect" name="yuqi" style="width:128px;">
							<option value="">请选择</option>
							<option value="two">2个月以下</option>
							<option value="four">2~4个月</option>
							<option value="six">4~6个月</option>
							<option value="twelve">6~12个月</option>
							<option value="twelveMore">12个月以上</option>
						</select>
						<label>&#12288;实际到帐日期：</label>
						<input type="text" name="payStartDate" class="ctime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width:120px;margin: 0"/>&nbsp;~&nbsp;
						<input type="text" name="payEndDate"  class="ctime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" style="width:120px;"/>
				...
				<tr>
					<td>
						...
						<c:if test="${type eq 'yszcx'}">
							<input type="checkbox" id="yqht" name="yqht" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="yqht">&nbsp;款项执行计划延期的合同</label>
						</c:if>
						<input type="checkbox" id="notCG" name="notCG" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="notCG">不含采购合同</label>
