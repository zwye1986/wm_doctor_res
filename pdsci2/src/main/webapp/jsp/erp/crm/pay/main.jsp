...
<head>
	...
	<script type="text/javascript">
		...
		function search(){
			jboxPostLoad("payList" ,"<s:url value='/erp/crm/payPlan/payList'/>" ,$("#searchForm").serialize() , true);
		}
				...
				<tr>
					<td>
						...
						<c:if test="${type eq 'plan'}">
							<input type="checkbox" id="waitJH" name="waitJH" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="waitJH">&nbsp;待款项计划合同</label>
						</c:if>
						<c:if test="${type eq 'action'}">
							<input type="checkbox" id="waitKP" name="waitKP" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="waitKP">&nbsp;待款项执行合同</label>&#12288;
							<input type="checkbox" id="KXBH" name="KXBH" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="KXBH">&nbsp;款项驳回合同</label>
						</c:if>
						<c:if test="${type eq 'audit'}">
							<input type="checkbox" id="waitAudit" name="waitAudit" checked="checked" value="${GlobalConstant.FLAG_Y }"><label for="waitAudit">&nbsp;待款项确认合同</label>
						</c:if>
