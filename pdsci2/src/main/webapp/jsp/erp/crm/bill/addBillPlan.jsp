...
function save(){
		...
		if(payPlanTr.length>0){
			$.each(payPlanTr , function(i , n){
				var billPlanFlow=$(n).find("input[name='billPlanFlow']").val();
			...
			console.log(allData);
			 ...
			 var url = "<s:url value='/erp/crm/billPlan/saveBillPlan'/>";
				jboxSubmit($('#planForm'),url,function(resp){
							jboxTip(resp);
							if(resp=='${GlobalConstant.SAVE_SUCCESSED}')
							{
								setTimeout(function(){
								window.parent.frames['mainIframe'].window.refreshBillPlan('${param.contractFlow }');
									jboxClose();
								},1000);
							}
				},
...
function  sumMoney(obj)
{
	...
	reMoneyMask();
	var sum=0;
	$("#planForm").find("input[name='billPayFund']").each(function(){
		var money=$(this).val();
	...
	<div class="title1 clearfix">
			<form id="planForm" >
				...
				<table cellpadding="0" cellspacing="0" class="xllist" style="width: 100%;">
					...
					<tr>
						<th colspan="4" style="text-align: left;padding-left: 10px">开票执行计划<span class="red" style="padding-left: 12px;">*</span>
							<c:if test="${empty billPlan}">
					...
					<tr>
						<th></th>
						<th>计划开票日期</th>
						<th colspan="2">计划开票金额</th>
					</tr>
					<tbody id="payPlanTb">
					  <c:forEach items="${billPlanList }" var="plan">
					   <tr>
		                 <td>
		                 ...
		                 <td>
		                    <c:choose>
								<c:when test="${billPlan.billPlanFlow eq plan.billPlanFlow}">
		                 ...
		                 <td colspan="2">
		                    <c:choose>
								<c:when test="${billPlan.billPlanFlow eq plan.billPlanFlow}">
								<input type="text" value="${plan.billPayFund }" class="inputText validate[custom[number],maxSize[9],min[1]] money" name="billPayFund" style="width: 150px;" onkeyup="timeMask(this);"/>元
								</c:when>
								<c:otherwise>
								  <label class="money">${plan.billPayFund }</label>
								  <input type="hidden" value="${plan.billPayFund }" class="inputText validate[custom[number],maxSize[9],min[1]] money" name="billPayFund" style="width: 150px;" onkeyup="timeMask(this);"/>元
								</c:otherwise>
		                    </c:choose>
		                 </td>
	                   </tr>
...
<table style="display: none;" id="payPlanClone">
	<tr>
		...
		<td>
			<input type="text" class="inputText ctime validate[required]" name="billPlanDate" style="width: 130px;" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
		</td>
		<td colspan="2">
			<input type="text" class="money inputText validate[required,custom[number],maxSize[9],min[1]]" name="billPayFund" style="width: 150px;" onkeyup="timeMask(this);"/>元
		</td>
	</tr>
</table>
</body>
