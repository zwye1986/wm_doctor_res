...
<script type="text/javascript">
    ...
    function toPage(page) {
        ...
        jboxPostLoad("content", "<s:url value='/zseyjxres/hospital/staffManage'/>", $('#searchForm').serialize(), true);
    }
    ...
    function edit(flag, userFlow) {
        ...
        jboxOpen("<s:url value='/zseyjxres/hospital/editStaff'/>?userFlow=" + userFlow + "&roleId=" + roleId, flag, 600, 350, true);
    }
    //导入
    function showImport() {
        var url = "<s:url value='/zseyjxres/hospital/showImport'/>";
        typeName = "导入医院人员";
    ...
    function del(userFlow) {
        jboxConfirm("确定刪除吗？", function () {
            jboxPost("<s:url value='/zseyjxres/hospital/delStaff'/>", {"userFlow": userFlow}, function (resp) {
                staffManage();
            }, null, true);
        }, null);
    }
    <%--function operRole(resp, userFlow) {--%>
        ...
        <%--jboxPost("<s:url value='/gzzyjxres/hospital/operRole'/>", {"role": role, "userFlow": userFlow}, null, null, true);--%>
    <%--}--%>
</script>
<div class="main_hd">
    <h2>本院人员维护</h2>
