...
<script type="text/javascript">
	...
	//新增/编辑
	function add(arrangeFlow) {
		var doctorName = '${doctor.doctorName}';
		var doctorFlow = '${doctor.doctorFlow}';
		jboxOpen("<s:url value='/zseyjxres/hospital/add?doctorFlow='/>"+doctorFlow+"&doctorName="+encodeURI(encodeURI(doctorName))+"&arrangeFlow="+arrangeFlow,"新增轮转科室信息", 900, 300);
	}
	...
	function del(){
		if($(".arrangeList:checked").length>0){
			...
			jboxConfirm("确认删除此条记录?", function(){
				var url = "<s:url value='/zseyjxres/hospital/del?arrangeFlows='/>"+flows;
		...
		<tbody>
			<c:forEach items="${arrangeList}" var="arrange">
				<tr>
					...
					<td>${arrange.memo}</td>
					<td><a style="cursor: pointer;color: #0a8cd2" onclick="add('${arrange.arrangeFlow}')">编辑</a></td>
				</tr>
			</c:forEach>
