...
<head>
    ...
    <script type="text/javascript">
        ...
        function saveFromTitle(titleId, index) {
            ...
            var url = "<s:url value='/zseyjxres/evaluation/saveEvalConfig'/>";
        ...
        function deleteTitle(id) {
            jboxConfirm("确认删除此评分项目？", function () {
                ...
                var url = "<s:url value='/zseyjxres/evaluation/deleteTitle'/>?configFlow=" + configFlow + "&id=" + id;
        ...
        function deleteItem(id) {
            jboxConfirm("确认删除此评分指标？", function () {
                var configFlow = $("input[name='configFlow']").val();
                var url = "<s:url value='/zseyjxres/evaluation/deleteItem'/>?configFlow=" + configFlow + "&id=" + id;
                jboxPost(url, null, function (resp) {
                    if ("${GlobalConstant.DELETE_SUCCESSED}" == resp) {
                        selectEvl();
                    }
        ...
        function modifyItem(obj, itemId) {
            ...
            jboxPost("<s:url value='/zseyjxres/evaluation/modifyItem'/>", data, function (resp) {
                if ("${GlobalConstant.SAVE_SUCCESSED}" == resp) {
                    selectEvl();
                }
            }, null, true);
        ...
        function save() {
            ...
            var url = "<s:url value='/zseyjxres/evaluation/saveEvalItemList'/>";
        ...
        function selectEvl(){
            var roleId = $("#evlRoleId").find("option:selected").val();
            evaluationMaintenance(roleId);
        }
    </script>
</head>
<body>
    ...
    <div class="div_search">
        <form id="fromTitleForm">
            ...
            <div style="margin:0px 10px 0px 0px;float:left;">
                <label>评价角色：</label>
                <select id="evlRoleId" class="select" style="width: 100px;" name="roleId" onchange="selectEvl();">
                    <c:forEach items="${stuRoleEnumList}" var="role">
                        <option value="${role.id}" <c:if test="${param.roleId eq role.id}">selected</c:if> >${role.name}</option>
                    </c:forEach>
                </select>
            </div>
            <input class="btn_green" type="button" style="margin:5px 10px 0px 20px;float:left;" id="addTitleButton"
                   value="添加考评项目" onclick="addFromTitle();"/>
            <div id="titleParam" style="display:none;float:left;">
                评分项目：<input type="text" name="name" id="name" class="input validate[required]"/>
                <input class="btn_green" type="button" id="saveTitleButton" value="保&#12288;存"
                       onclick="saveFromTitle('','');"/>
                <input class="btn_green" type="button" id="cancelSaveTitleButton" value="取&#12288;消"
                       onclick="cancelSaveFromTitle();"/>
            </div>
        </form>
    </div>
    <div style="margin-top:40px;padding:0px 40px 0px 40px;">
        <form id="cfgForm">
            <input type="hidden" name="configFlow" value="${evalConfig.configFlow}"/>
            <!-- 标题 -->
            <c:forEach items="${titleFormList}" var="title" varStatus="s">
                <table class="grid">
                    <input id="assessOrder_${title.id}" class="bigOrder_${s.index+1}" type="hidden"
                           value="${title.orderNum}"/>
                    <tr>
                        <th>评分项目：
								<span class="name" style="width: 150px;">
									<font>${title.name}</font>
									<input id="assessType_${title.id}" type="text" class="validate[required]"
                                           style="display:none;border:1px solid grey;"
                                           onblur="saveFromTitle('${title.id}','${s.index+1}');" value="${title.name}"/>
								</span>
                        </th>
                        <th>总分：<font id="scoreCount_${title.id}"></font></th>
                        <th colspan="2">
                            <a style="cursor: pointer;" onclick="addItem('${title.id}')"><img alt="新增" title="新增"
                                                                                              src="<s:url value="/css/skin/Blue/images/add.png"/>"/></a>&nbsp;&nbsp;
                            <a style="cursor: pointer;" onclick="editTitle('${title.id}');"><img alt="修改" title="修改"
                                                                                                 src="<s:url value="/css/skin/Blue/images/modify.png"/>"/></a>&nbsp;
                            <a style="cursor: pointer;" onclick="deleteTitle('${title.id}')"><img alt="删除" title="删除"
                                                                                                  src="<s:url value="/css/skin/Blue/images/del2.png"/>"></a>
                        </th>
                    </tr>
                    <tr>
                    ...
                    <c:forEach items="${title.itemList}" var="item" varStatus="status">
                        <tr>
                            <td colspan="2">
                                ...
                                <input type="text" name="name" class="validate[required]" title="${item.name}"
                                       oldvalue="${item.name}" value="${pdfn:cutString(item.name,45,true,6)}"
                                       style="text-align:center;height:20px;line-height:20px;width:100%;"
                                       readonly="readonly"/>
                            </td>
                            <td>
                                <input type="text" name="score" class="validate[required,custom[number]]"
                                       oldvalue="${item.score}" value="${item.score}"
                                       style="text-align:center;height:20px;line-height:20px;" readonly="readonly"/>
                            </td>
                            <td>
        ...
        <div style="margin-top:10px;text-align:center;">
            <input class="btn_green" type="button" id="saveButton" style="display:none;" value="保&#12288;存"
                   onclick="save();"/>
        </div>
    ...
    <tr class="addTr">
        <td colspan="2">
            ...
            <input type="text" name="name" class="validate[required]"
                   style="border:1px solid grey;width:50%;text-align:center;"/>
        </td>
        <td><input type="text" name="score" class="validate[required,custom[number]]"
                   style="border:1px solid grey;width:80px;text-align:center;"/></td>
        <td>[<a>删除</a>]</td>
    </tr>
</table>
</body>
