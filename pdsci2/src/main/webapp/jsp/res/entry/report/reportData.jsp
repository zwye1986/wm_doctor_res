...
<script>
	...
	function getReportData(){
		jboxPostLoad("reportData","<s:url value='/res/entry/report/reportData'/>",$("#searchForm").serialize(),true);
	}
</script>
<div>
	...
	<div class="" style="width: 100%;">
			...
			<div class="inputDiv">
				...
				<input type="text" class="qtext" name="reportDate" value="${param.reportDate}"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly"/>
			</div>
	...
	<th style="width: 100px;">二维码提供人</th>
		<c:forEach items="${list}" var="bean" varStatus="status">
			<tr>
				...
				<td>${bean.reportDate}</td>
				<td>${bean.schDeptName}</td>
				<td>${bean.schStartDate}~${bean.schEndDate}</td>
				<td>${bean.headUserName}</td>
			</tr>
		</c:forEach>
		<c:if test="${empty list}">
			<tr><td colspan="99">暂无报到信息</td></tr>
		</c:if>
</table>
<p>
