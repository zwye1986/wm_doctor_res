...
$(function(){
	<c:if test="${not empty deptFlowsStr}">
		...
		getReportData(1);
		setInterval("getReportData(1)",300000);
	</c:if>
});
function getReportData(pageIndex){
	jboxLoad("reportData","<s:url value='/res/entry/report/reportData'/>",false);
}
function getuuid() {
	var s = [];
...
function initCode(){
	$("#code").empty();
	var url = "${signUrl}"+ "&signTime="+moment().format('YYYYMMDDHHmmss')+ "&token="+token;
	console.log(url);
	$("#code").qrcode({
		render: "canvas",
		width: 250,
...
{
	$.ajax({
		url: "<s:url value='/res/entry/report/codeIsUse'/>",
...
<div class="mainright">
	...
	<c:if test="${not empty deptFlowsStr}">
		<div class="content" >
			<div id="reportData" style="width: 70%;float: left;padding-top: 10px;">
			</div>
			<div style="width: 30%;padding-top: 10px;float: right;text-align: center">
				<div id="code"></div>
				<br/><br/>
				<font style="color: red;">Tip:报到二维码每1分钟刷新一次</font>
			</div>
		</div>
	</c:if>
	<c:if test="${empty deptFlowsStr}">
		<div class="content" >
			你暂无所在科室信息，请联系基地管理员！！
		</div>
	</c:if>
