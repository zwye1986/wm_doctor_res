...
function importUsers(){
	...
	jboxStartLoading();
	var url = "<s:url value='/res/teacher/importThoerySkillScore'/>";
	jboxSubmit(
		$('#excelForm'),
		url,
		function(resp){
			...
			if(resp.substring(0,5)=="${GlobalConstant.UPLOAD_SUCCESSED}"){
				window.parent.frames['mainIframe'].window.showView();
				top.jboxClose();
			}
		},
		function(resp){
...
<form id="excelForm" method="post" enctype="multipart/form-data">
	<table width="100%" class="basic">
		...
		<tr>
			<th>模板文件</th>
			<td style="padding-left:10px;"><a href="<s:url value='/jsp/res/teacher/importTheorySkillTemp.xlsx'/>">成绩导入模板.xlsx</a></td>
		</tr>
	</table>
	<br/>
	<p style="text-align: center;">
		<input type="button" onclick="importUsers();" value="成绩导入" class="search"/>
	</p>
</form>
</div>
</div>
