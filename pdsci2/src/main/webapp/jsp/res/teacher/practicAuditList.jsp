...
<head>
	...
	<script type="text/javascript">
		...
		function recAuditList(doctorFlow,schDeptFlow,recTypeId,processFlow,rotationFlow,resultFlow){
			...
			var url="<s:url value='/res/teacher/recAuditList' />?typeId=BasicPractice&recTypeId="+recTypeId+"&roleFlag=${param.roleFlag}&doctorFlow="+doctorFlow+"&schDeptFlow="+schDeptFlow+"&processFlow="+processFlow+"&rotationFlow="+rotationFlow+"&resultFlow="+resultFlow;
...
<div class="mainright">
  <div class="content">
	  <form id="searchForm" action="<s:url value='/res/teacher/audit/teacher?typeId=BasicPractice'/>" method="post">
  			...
  			<tr>
  				...
  				<c:forEach items="${practicRegistryTypeEnumList}" var="registryType">
					...
					<c:if test="${sysCfgMap[viewCfgKey] eq GlobalConstant.FLAG_Y}">
						...
						class="toAudit" 
						onclick="recAuditList('${process.userFlow}','${process.schDeptFlow}','${registryType.id}','${process.processFlow}','${doctorMap[process.userFlow].rotationFlow}','${process.schResultFlow}');"
						>
							<img src="<s:url value='/css/skin/${skinPath}/images/unchecked.png'/>" style="visibility: ${(waitAuditCountMap[countKey]+waitAuditAppealCountMap[countKey])>0?'visible':'hidden'};margin-left:-10px;margin-top:-5px;"/>
