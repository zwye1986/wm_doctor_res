...
<script type="text/javascript">
    $(document).ready(function(){
		...
		}else if (${param.gradeRole eq 'admin'}){
            $(".selectTag").removeClass("selectTag");
            $('#admin').addClass('selectTag');
        }
	})
...
//		$("font",span).toggle();
	...
	function operDetail(name,keyCode,date){
		var startDate = $("#operStartDate").val();
		var endDate = $("#operEndDate").val();
		var url = "<s:url value='/res/head/gradeSearchDoc'/>?gradeRole=${param.gradeRole}&keyCode="+keyCode+"&operStartDate="+startDate+"&operEndDate="+endDate+"&date="+date;
    ...
    function exportEval() {
        ...
        var url = "<s:url value='/res/head/exportEvalNew?gradeRole='/>" + gradeRole;
        jboxTip("导出中...");
		...
		<div class="top-tab">
			<div class="clearfix" style="padding-top: 10px;">
				<ul id="tags">
                    ...
                    <li id="admin">
                        <a onclick="selTag('admin',this);" href="javascript:void(0)">继续教育科</a>
                    </li>
				</ul>
				<div id="tagContent">
					<div class="content">
						<div class="module-content" style="width: 100%;">
							<form id="gradeSearchForm" action="<s:url value="/res/head/gradeSearchNew"/>" method="post">
							...
							<table class="basic list" width="100%" style="margin-top: 10px;">
								<colgroup>
                                    ...
                                    <c:if test="${param.gradeRole eq 'admin'}">
								...
								<tr>
									...
									<c:if test="${empty param.sessionNumber}">
										<th style="display: none">${FirstTwoYearsDate}级总均分</th>
										<th style="display: none">${PreviouYearDate}级总均分</th>
										<th style="display: none">${currDate}级总均分</th>
									</c:if>
									<c:if test="${not empty param.sessionNumber}">
										<th style="display: none">${param.sessionNumber}级总均分</th>
									</c:if>
									<th >标准分</th>
									<th >总均分</th>
								</tr>
								...
								<c:forEach items="${datas}" var="data">
                                    ...
                                    <c:if test="${param.gradeRole eq 'admin'}">
                                        <c:set var="key" value="${data.orgFlow}"/>
                                        <c:set var="name" value="${data.orgName}"/>
                                    </c:if>
									...
									<tr>
										<th style="text-align: left;padding-left: 10px;">
											<span style="cursor: pointer;color: blue;font-size: 0.8em;line-height: 5px;" onclick="detailShow(this,'${key}');">${name}</span>
										</th>
										<c:if test="${param.gradeRole eq 'teacher'}">
											<th>${data.deptName}</th>
										</c:if>
