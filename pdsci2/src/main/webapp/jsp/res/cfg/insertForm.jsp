...
<head>
    ...
    <script type="text/javascript">
        ...
        function insertForm() {
            var url = "<s:url value='/res/cfg/saveFrom'/>";
            jboxSubmit($('#excelForm'), url, function (data) {
                if ('${GlobalConstant.SAVE_SUCCESSED}' == data) {
                    try {
                        window.parent.frames['mainIframe'].window.searchForm();
                    } catch (e) {
...
<%--        <form id="excelForm" method="post" enctype="multipart/form-data">--%>
        <form id="excelForm" method="post" action="/res/cfg/saveFrom">
            <table width="100%" class="basic">
                <tr>
                    <th width="30%">评价类型：</th>
                    <td style="padding-left:10px;">
                        <select name="cfgCodeId" class="qselect validate[required]">
                            <option value="">全部</option>
                            <c:forEach items="${resAssessTypeEnums}" var="resAssessTypeEnum">
                                <option value="${resAssessTypeEnum.id}"
                                >${resAssessTypeEnum.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
                <tr>
                    <th width="30%">表单名称</th>
                    <td style="padding-left:10px;">
                        <input type="text" name="formName" class="qtext validate[required]"/>
                    </td>
                </tr>
               <%-- <tr>
                    <th width="30%">上传附件：</th>
                    <td style="padding-left:10px;">
                        <input id="file" type="file" name="file" onchange="checkFile(this);"
                ...
                <tr>
                    ...
                    <td style="padding-left:10px;"><a
                            href="<s:url value='/jsp/sys/impactorFactor/template/insertFormTemplate.xlsx'/>">表单导入模板.xlsx</a>
                    </td>
                </tr>--%>
            </table>
            <br/>
            <p style="text-align: center;">
                <input type="button" onclick="insertForm();" value="保存" class="search"/>
            </p>
        </form>
    </div>
</div>
