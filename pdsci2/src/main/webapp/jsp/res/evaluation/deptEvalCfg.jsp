...
function saveAssessTitle(titleId){
	...
	var url = "<s:url value='/res/evaluation/saveEvaluationTitle'/>?cfgFlow=${evaluationCfg.cfgFlow}&cfgCodeId="+cfgCodeId;
	jboxPost(url, data,
			function(resp){
...
function search(){
	var cfgCodeId = $("select[name='cfgCodeId'] option:selected").val();
	var url = "<s:url value='/res/evaluation/deptEvalCfg'/>?cfgCodeId=" + cfgCodeId;
...
<div class="content">
	<div class="title1 clearfix">
		...
		<select name="cfgCodeId" onchange="search();" class="validate[required]">
			<option value="${evaluationTypeEnumDeptEval.id}">${evaluationTypeEnumDeptEval.name}</option>
		</select>
		</div>
		<div style="float: left;">
		<form id="assessTitleForm">
			<input type="hidden" name="id" id="titleId"/>
			<input type="hidden" name="assessTypeId"  value="${evaluationTypeEnumDeptEval.id}"/>
			<input type="hidden" name="assessTypeName" value="${evaluationTypeEnumDeptEval.name}"/>
			...
			<c:forEach items="${titleFormList}" var="title">
				<div class="cont_list" style="margin-top: 15px;">
					<div class="left" style="width: 910px;">
                        ...
                        <%--<c:if test="${evaluationCfg.assessTypeId eq resAssessScoreTypeEnumPercentile.id}">--%>
				...
				<div style="padding-top: 0px;">
					<table class="xllist"  cellpadding="0" cellspacing="0" style="width: 100%">
						<colgroup>
                            ...
                            <col width="10%"/>
                            <%--</c:if>--%>
                            <%--<c:if test="${evaluationCfg.assessTypeId ne resAssessScoreTypeEnumPercentile.id}">--%>
                                <col width="20%"/>
                            <%--</c:if>--%>
						...
						<c:forEach items="${title.itemList}" var="item" varStatus="status">
						  <tr>
						    ...
						    <td <%--style="display: <c:if test="${!(evaluationCfg.assessTypeId eq resAssessScoreTypeEnumPercentile.id)}">none;</c:if>"--%>>
