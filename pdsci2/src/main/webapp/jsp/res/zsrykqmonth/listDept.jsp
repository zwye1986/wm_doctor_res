...
<script type="text/javascript">
	...
	function search(){
		...
		var url = '<s:url value="/res/zsrykqmonth/list/dept/${role}"/>';
	...
	function exportDoc(){
		...
		var url = "<s:url value='/res/zsrykqmonth/exportExl2/${role}'/>";
		jboxTip("导出中…………");
		...
		<div class="content">
			<div class="title1 clearfix">
				<form id="searchForm" method="post">
					...
					<div class="queryDiv">
						<div class="inputDiv" style="max-width: 280px;min-width: 280px;padding-left: 0px;">
							考勤时间：
				...
				<table class="basic" width="100%">
					...
					<c:forEach items="${schArrangeResults}" var="result">
						<tr>
							...
							<td style="text-align: center;padding: 0px;">${result.sessionNumber}</td>
							<td style="text-align: center;padding: 0px;">
							<c:set var="key" value="${result.doctorFlow}"></c:set>
							<c:choose>
								<c:when test="${doctorMap[key].doctorTypeId eq 'Company'}">
									${doctorMap[key].departMentName}
								</c:when>
								<c:when test="${doctorMap[key].doctorTypeId eq 'Social'}">
									--
								</c:when>
								<c:otherwise>
									${doctorMap[key].workOrgName}
								</c:otherwise>
							</c:choose>
							</td>
							<td style="text-align: center;padding: 0px;">${doctorMap[key].trainingSpeName}</td>
							<td style="text-align: center;padding: 0px;">${doctorMap[key].trainingYears}</td>
				...
				<div>
				   	<c:set var="pageView" value="${pdfn:getPageView(schArrangeResults)}" scope="request"/>
