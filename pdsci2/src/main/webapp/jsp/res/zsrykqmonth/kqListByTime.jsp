...
<script type="text/javascript">
	...
	function search(){
		...
		var url = '<s:url value="/res/zsrykqmonth/kqList/time/${role}"/>';
	...
	function exportDoc(){
		...
		var url = "<s:url value='/res/zsrykqmonth/exportKqListTime/${role}'/>";
		jboxTip("导出中…………");
			...
			<div class="title1 clearfix">
				<form id="searchForm" method="post">
					...
					<div class="queryDiv">
						...
						<div class="lastDiv" style="min-width: 90%;max-width: 90%;text-align: left;margin-left: 20px;">
							...
							<input type="button" value="导&#12288;出" class="searchInput" onclick="exportDoc();"/>
						</div>
					</div>
				</form>
				<table class="basic" width="100%">
					...
					<c:forEach items="${resultMapList}" var="result">
						<tr>
							<c:set var="time1" value="${empty param.startTime?startTime:param.startTime}"></c:set>
							<c:set var="time2" value="${empty param.endTime?endTime:param.endTime}"></c:set>
							<c:set var="time3" value="${time1}~${time2}"></c:set>
							<c:set var="content" value="${time1 eq time2?time1:time3}"></c:set>
							<td class="kqny" style="text-align: center;padding: 0px;">${empty result["KQ_DATE"]?content:result["KQ_DATE"]}</td>
