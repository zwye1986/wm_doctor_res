...
<script type="text/javascript">
	...
	$(function(){
		$('.createResultByStandardDept_toEdit').on('click',function(){
			if(currResult){
				cancelEdit(currResult);
			}
			currResult = 'main';
		});
		
		$('.createResultByStandardDept_toCancel').on('click',function(){
			currResult = '';
		});
		
		if('${empty arrResultList}' == 'true'){
			slideTheFormToEdit();
		}
	});
</script>
...
<c:forEach items="${groupList}" var="group" varStatus="g" >
	<c:set var="deptList" value="${deptMap[group.groupFlow]}"/>
	<c:forEach items="${deptList}" var="dept" >
		<c:set var="resultKey" value="${dept.standardDeptId}${dept.groupFlow}"/>
		<c:set var="arrResultList" value="${resultMap[resultKey]}"/>
...
function add(standardGroupFlow,doctorFlow,standardDeptId){
	var url = '<s:url value="/res/doc/editResultByStandardDept"/>?standardGroupFlow='+standardGroupFlow+'&addFlag=Y&standardDeptId='+standardDeptId+'&doctorFlow='+doctorFlow;
	jboxOpen(url, "新增",900,350);
}
function editResult(standardDeptId,doctorFlow,resultFlow,standardGroupFlow){
	var url = '<s:url value="/res/doc/editResultByStandardDept"/>?addFlag=Y&standardGroupFlow='+standardGroupFlow+'&standardDeptId='+standardDeptId+'&resultFlow='+resultFlow+'&doctorFlow='+doctorFlow;
	jboxOpen(url, "编辑", 900,350);
}
function delResult(doctorFlow,resultFlow){
	jboxConfirm("确认删除？<font color='red'>该科室下关联的登记数据也将一并删除！</font>",function() {
		var url="<s:url value='/res/doc/delResult'/>?resultFlow="+resultFlow;
		jboxPost(url,null,function(resp){
			top.document.mainIframe.location.reload();
		},null,true);
	});
}
function searchRule(rotationFlow,recordFlow,typeId){
	var url ="<s:url value='/res/doc/rotationRequire'/>?roleFlag=${param.roleFlag}&rotationFlow="+rotationFlow+"&recordFlow="+recordFlow+"&typeId="+typeId;
	jboxOpen(url,'轮转要求',800,350);
}
function showDetail(recordFlow,doctorFlow,typeId){
	var url ="<s:url value='/res/doc/rotationFinishDetail'/>?roleFlag=${param.roleFlag}&doctorFlow="+doctorFlow+"&recordFlow="+recordFlow+"&typeId="+typeId;
	jboxOpen(url,'标准科室完成情况',800,350);
}
</script>
	    ...
	    <div class="pxxx">
			...
			<div id="userInfo" style="display:none;">
				...
				<table class="xllist">
                    ...
                    <c:if test="${doctor.trainingTypeId eq 'ChineseMedicine'}" >
		   ...
		   <table class="" style="border:0px;">
			   ...
			   <tr>
				   <td style="border:0px;">
					   ...
					   <c:if test="${(sysCfgMap['training_manual_download'] eq GlobalConstant.FLAG_Y)}">
...
<div  class="content" style="margin-top: 15px;" id="horz">
		<c:forEach items="${groupList}" var="group" varStatus="g" >
			...
			<c:forEach items="${deptList}" var="dept" varStatus="d">
				<c:set var="standardDeptKey" value="${dept.groupFlow}${dept.standardDeptId}"/>
				<c:set value="${finishPerMap[standardDeptKey]+0}" var="standardDeptPer"/>
				<c:set value="${pdfn:percentRoundHalfUp(standardDeptPer,2)}" var="standardDeptPer"/>
				<table class="reorder" >
					<tr>
						<td style="padding: 10px;">
							<table class="reorder tableSize" style="height: 40px;margin-bottom: 0px" >
								<tr style="background-color: #f4f4f4">
									<td width="20%">
										<label style="margin-left: 10px;
										<c:if test="${group.rotationFlow eq doctor.rotationFlow}">color:#008B8B;</c:if>
										<c:if test="${group.rotationFlow eq doctor.secondRotationFlow}">color:#0000CD;</c:if>
										">[${group.groupName }&nbsp;${group.schStageName }]</label>
									</td>
									<td width="20%">
										<label>${dept.standardDeptName}</label>
									</td>
									<td width="19%">
										<label>轮转时间：${dept.schMonth}${applicationScope[unitKey].name}</label>
									</td>
									<td width="6%">
										<a onclick="searchRule('${dept.rotationFlow}','${dept.recordFlow}','${dept.practicOrTheory}');" style="color: blue;font-weight: normal;">轮转要求</a>
									</td>
									<td width="10%" style="text-align: right;">
										完成总比例：
									</td>
									<td  width="15%" style="padding: 0 10px 0 10px;text-align: left;">
										<div style="width:100%; top:0px;position:relative;text-align: center;cursor:pointer;border: 1px solid #bbbbbb;height: 20px;" onclick="showDetail('${dept.recordFlow}','${doctor.doctorFlow}','${dept.practicOrTheory}')">
											<span style="width: ${standardDeptPer}%;background-color: #3891ef;display:block;float:left;padding-top: 1px;">&nbsp;</span>
											<span style="width: ${100-standardDeptPer}%;background-color: #EEEEEE;display:block;float:left;padding-top: 1px;">&nbsp;</span>
											<span style="display:block;float:left;padding-top: 1px;position:absolute;top:0px;">
											<fmt:formatNumber type="Number" value="${standardDeptPer}" maxFractionDigits="0"/>%
											</span>

										</div>
									</td>
									<td width="10%">
										<label style="float: right;"><img class="opBtn" title="新增" src="<s:url value="/css/skin/${skinPath}/images/add3.png" />" style="cursor: pointer;" onclick="add('${group.groupFlow}','${doctor.doctorFlow}','${dept.standardDeptId }');"></img>&#12288;</label>
									</td>
								</tr>
							</table>
						</td>
					...
					<tr>
						<td style="padding: 10px;">
							<c:set var="resultKey" value="${dept.standardDeptId}${dept.groupFlow}"/>
							<c:set var="arrResultListInfo" value="${resultMap[resultKey]}"/>
							<c:forEach items="${arrResultListInfo}" var="result" varStatus="status">
							...
							<table class="reorder">
								<tr>
									<td style="width: 47px;">
										<div class="sch_img_1 ${result.resultFlow}Td">
											<c:if test="${process.schFlag == GlobalConstant.FLAG_Y}">
												<img alt="" src="<s:url value='/css/skin/${skinPath}/images/yck_1.png'/>">
											</c:if>
											
											<c:if test="${process.isCurrentFlag == GlobalConstant.FLAG_Y}">
												<img alt="" src="<s:url value='/css/skin/${skinPath}/images/Rotary_1.png'/>">
											</c:if>
											
 											<c:if test="${!(process.isCurrentFlag eq GlobalConstant.FLAG_Y) && !(process.schFlag == GlobalConstant.FLAG_Y)}">
									...
									<td ${clickContent} style="text-align:left;width: 400px; padding-left:45px;" class="${result.resultFlow}Td">
										<h3>
											...
											<c:if test="${GlobalConstant.FLAG_Y != process.schFlag}">
												<c:if test="${GlobalConstant.FLAG_Y != process.isExternal}">
												&nbsp;
												<a 
													onclick="editResult('${result.standardDeptId }','${result.doctorFlow}','${result.resultFlow}','${dept.groupFlow}');"
													style="font-size: 10px;color: blue;"
												>编辑</a>
												<label style="color: black;font-size: 2px">|</label>
												<a 
													onclick="delResult('${result.doctorFlow}','${result.resultFlow}');"
													style="font-size: 10px;color: blue;"
												>删除</a>
												</c:if>
											</c:if>
							...
							<c:if test="${empty arrResultListInfo}">
								<table class="reorder" style="height:40px ">
									<tr>
										<td style="text-align: center;">暂无轮转计划，点击<a onclick="add('${group.groupFlow}','${doctor.doctorFlow}','${dept.standardDeptId }');"><font color="blue">这里</font></a>新增!</td>
									</tr>
								</table>
							</c:if>
						</td>
...
<jsp:include page="/jsp/res/doctor/createResultByStandardDept.jsp" flush="true">
	...
	<jsp:param value="reorder" name="topDivClass"/>
</jsp:include>
<script type="text/javascript">
	$(function(){
		var doctorFlow = $("#doctorFlow").val();
		var resultFlow = $("#resultFlow").val();
		getFormPage(doctorFlow,resultFlow,function(resp){
			$("#page").html(resp);
			$(".createResultByStandardDept_tipText").hide();
		}); 
	...
	function realod(){
		window.parent.frames['mainIframe'].window.reload();//top.document.mainIframe.reload();
	}
	function closePage(){
		_dialogClose('openDialog');
	}
</script> 
	<input type="hidden" id="doctorFlow" />
	<input type="hidden" id="resultFlow" />
	<div id="page"></div>
	<div style="margin-top: 10px">
		<label style="margin-left: 230px;">
			<a onclick="save();" style="cursor: pointer;color: blue">保存</a>&#12288;
			<a onclick="closePage()" style="cursor: pointer;color: blue">取消</a>
		</label>
	</div>
</div>
</body>
