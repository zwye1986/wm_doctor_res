...
<script type="text/javascript">
	function saveRec(cfgFlow){
		if($("#resRecGradeForm").validationEngine("validate")){
			jboxPost("<s:url value='/res/rec/saveGrade'/>?cfgFlow="+cfgFlow,$("#resRecGradeForm").serialize(),function(resp){
								...
								<tr>
                                    ...
                                    <c:if test="${assessCfg.assessTypeId eq resAssessScoreTypeEnumFive.id}">
                                            <c:if test="${!(isRead || rec.statusId eq recStatusEnumSubmit.id)}">
                                               好
                                                <label>
                                                    <input name="${item.id}score" type="radio"  class="validate[required] scoreCount" value="5" <c:if test="${gradeMap[item.id]['score'] == '1'}">checked</c:if> onchange="$('#${item.id}scoreSel').val(this.value);countScore();"/>
                                                </label>
                                                较好
                                                <label>
                                                    <input name="${item.id}score" type="radio" class="validate[required] scoreCount" value="4" <c:if test="${gradeMap[item.id]['score'] == '2'}">checked</c:if> onchange="$('#${item.id}scoreSel').val(this.value);countScore();"/>
                                                </label>
                                                一般
                                                <label>
                                                    <input name="${item.id}score" type="radio" class="validate[required] scoreCount" value="3" <c:if test="${gradeMap[item.id]['score'] == '3'}">checked</c:if> onchange="$('#${item.id}scoreSel').val(this.value);countScore();"/>
                                                ...
                                                较差
                                                <label>
                                                    <input name="${item.id}score" type="radio" class="validate[required] scoreCount" value="2" <c:if test="${gradeMap[item.id]['score'] == '4'}">checked</c:if> onchange="$('#${item.id}scoreSel').val(this.value);countScore();"/>
                                                </label>
                                                差
                                                <label>
                                                    <input name="${item.id}score" type="radio" class="validate[required] scoreCount" value="1" <c:if test="${gradeMap[item.id]['score'] == '5'}">checked</c:if> onchange="$('#${item.id}scoreSel').val(this.value);countScore();"/>
                                                </label>
                                            </c:if>
                                            <c:if test="${isRead || rec.statusId eq recStatusEnumSubmit.id}">
					...
					<div style="text-align: center;margin-top: 5px;">
						<c:if test="${empty rec.statusId}">
							<c:if test="${param.roleFlag == GlobalConstant.RES_ROLE_SCOPE_DOCTOR }">
								...
								<input type="button" value="保&#12288;存" class="search" onclick="saveRec('${assessCfg.cfgFlow}');"/>
							</c:if>
						</c:if>
