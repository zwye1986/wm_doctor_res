    ...
    <script type="text/javascript">
        ...
        function selTag(tabTag){
            if("SingnOut"==tabTag)
            {
                ...
                $("#SingnOut").removeClass("selectTag");
                $("#SingnOutDiv").hide();
                $("#SingnDiv").show();
            }
//            showSecondCode(tabTag);
        ...
        function showSecondCode(tabTag)
        {
            ...
            if(canvas!=null)
            {
                ...
                {
                    ctx.fillText("                                                                ", 0, 415);
                    ctx.fillText("【签退二维码】（此二维码在活动结束前${endTime}分钟至结束后${endTime}分钟内有效）", 0, 415);   // 文字
//                    $("#qrcode2").hide();
                }else{
                    ctx.fillText("                                                                ", 0, 415);
                    ctx.fillText("【签到二维码】（此二维码在活动开始前${startTime}分钟至开始后${startTime}分钟内有效）", 0, 415);   // 文字
//                    $("#qrcode").hide();
                }
//                $("#qrcode3").show();
            }
        ...
        function  down(name)
        {
            $('#qrcode canvas').downCanvasImg("${activity.activityName}【签到二维码】（此二维码在活动开始前${startTime}分钟至开始后${startTime}分钟内有效）.png");
        }
        function  down2(name)
        {
            $('#qrcode2 canvas').downCanvasImg("${activity.activityName}【签退二维码】（此二维码在活动结束前${endTime}分钟至结束后${endTime}分钟内有效）.png");
        }
        function  down3(name)
        {
            var fileName="${activity.activityName}[签退二维码].png";
            if(!$("#SingnOut").hasClass("selectTag"))
            {
...
<div id="SingnDiv" style="display: none;">
    ...
    <div>
        【签到二维码】（此二维码在活动开始前${startTime}分钟至开始后${startTime}分钟内有效）
    </div>
    <div id="downDiv"  style="text-align: center;margin-top: 5px;">
        ...
        <input  type="button" class="search" onclick="down();" style="display:${pdfn:resPowerCfgMap(key).cfgValue eq 'Y'? 'none':''}"  value="下&#12288;载"/>&nbsp;
    </div>
</div>
<div id="SingnOutDiv" style="display: none;">
    ...
    <div>
        【签退二维码】（此二维码在活动结束前${endTime}分钟至结束后${endTime}分钟内有效）
    </div>
    <div id="downDiv2"  style="text-align: center;margin-top: 5px;">
        ${pdfn:resPowerCfgMap(key).cfgValue eq 'Y'? '<font color="red">此二维码为动态二维码，每5秒钟刷新一次</font>':''}
        <input  type="button" class="search" onclick="down2();" style="display:${pdfn:resPowerCfgMap(key).cfgValue eq 'Y'? 'none':''}"  value="下&#12288;载"/>&nbsp;
    </div>
</div>
<div style="display: none">
    ...
    <div id="downDiv3"  style="text-align: center;margin-top: 5px;">
        <input  type="button" class="search" onclick="down3();" value="下&#12288;载"/>&nbsp;
    </div>
</div>
</body>
</html>
