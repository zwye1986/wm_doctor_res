...
<script type="text/javascript">
	...
	function editAbsence(absenceFlow,doctorFlow){
		...
		jboxOpen("<s:url value='/res/doc/editAbsence'/>?resRoleScope=${resRoleScope}&absenceFlow="+absenceFlow+"&isRegister=${GlobalConstant.FLAG_Y}"+"&doctorFlow="+doctorFlow,title,800,500);
	}
	...
	function exportDoc(){
		...
		var url = "<s:url value='/res/teacher/exportDoc/${resRoleScope}'/>";
		jboxTip("导出中…………");
		jboxExp($("#doctorSearchForm"),url);
	}
	function importInfo(){
		jboxOpen("<s:url value='/jsp/res/college/importSpe.jsp'/>", "导入",600,200);
	}
</script>
</head>
<body>
				...
				<form id="doctorSearchForm" method="post" action="<s:url value='/res/teacher/absentManage/${resRoleScope}'/>">
					...
					<c:if test="${GlobalConstant.RES_ROLE_SCOPE_MANAGER ne resRoleScope and 'global' ne resRoleScope}">
						<div class="queryDiv">
							...
							<div class="inputDiv" style="width: 280px;min-width: 280px;max-width: 280px;">
								<input type="button" class="searchInput" onclick="editAbsence('');" value="缺勤登记"/>
								<input type="button" class="searchInput" onclick="importInfo();" value="导&#12288;入"/>
								<input type="button" class="searchInput" onclick="exportDoc();" value="导&#12288;出"/>
							</div>
						</div>
					</c:if>
				</form>
			...
			<div class="resultDiv">
				<table class="xllist" >
						<tr>
							...
							<th style="text-align: center;width: 8%;" >联系方式</th>
							<th style="text-align: center;width: 5%;" >年级</th>
							<th style="text-align: center;">培训专业</th>
							<th style="text-align: center;width: 10%;" >轮转科室</th>
							<th style="text-align: center;width: 8%;" >开始时间</th>
							<th style="text-align: center;width: 8%;" >结束时间</th>
							<th style="text-align: center;width: 5%;" >缺勤天数</th>
							<th style="text-align: center;width: 5%;" >缺勤天数合计</th>
							<th style="text-align: center;width: 5%;" >缺勤类型</th>
										...
										<c:forEach items="${docAbsenceMap[doctor.doctorFlow]}" var="docAbsence" varStatus="status">
											...
											<tr>
												...
												<c:if test="${status.first}">
													...
													<td rowspan="${colspan}">${doctor.sysUser.sexName}</td>
													<td rowspan="${colspan}">${doctor.sysUser.userPhone}</td>
													<td rowspan="${colspan}">${doctor.sessionNumber}</td>
													<td rowspan="${colspan}">${doctor.trainingSpeName}</td>
												</c:if>
												<td>${docAbsence.schDeptName}</td>
