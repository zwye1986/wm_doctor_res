    ...
    <script type="text/javascript">
        ...
        function submitNotes(flag) {
            ...
            if ("${roleScope}" == "disciple") {
                var tent = $.trim($("#auditContent").val());
                if (!tent) {
                    jboxTip("审核意见未填写，无法审核！");
                    return false;
                }
            }
            if ("${roleScope}" == "doctor") {
                var tent = $.trim($("#experienceContent").val());
                if (!tent) {
                    jboxTip("跟师心得体会未填写，无法提交！");
                    return false;
                }
            }
        ...
        function agreeRecordBatch(exp){
            ...
            jboxConfirm("确认审核通过该学员所有待审核跟师心得？" , function() {
        ...
        <div style="margin-top: 10px;width: 100%;">
            <div style="width: 28%;margin-bottom: 40px; float: left">
                <div class="index_title" style="height: 50px;width: 100%;">
                    <p>
                        <span style="float: left;margin-top: 10px;">跟师心得:</span>
                        <c:if test="${roleScope eq GlobalConstant.RES_ROLE_SCOPE_DOCTOR}">
                            <input class="btn_blue" style="float: right" type="button" value="添加心得"
                                   onclick="editNotes()"/>
                        </c:if>
                    </p>
                </div>
                <div style="height: 800px;width: 100%;margin-bottom: 20px;border:1px solid #D0E3F2;overflow:auto;">
                    ...
                    <c:forEach items="${resDiscipleNoteInfoList}" var="noteInfo">
                        ...
                        <div class="noteItems" style="width: 100%;padding-bottom: 10px;padding-top: 10px;<c:if
                            ...
                            <p>跟师时间：<span name="studyStartDate">${noteInfo.studyStartDate}</span>&#12288;&#12288;<span
                            >${noteInfo.studyTimeNmae}</span></p>
                            <p>创建时间：<span name="createTime">${pdfn:transDateTime(noteInfo.createTime)}</span>
                                <br/>状态：
                                <span title="${noteInfo.auditStatusName}" style="color: ${statusColor}">${noteInfo.auditStatusName}</span></p>
                        </div>
                    </c:forEach>
                </div>
            </div>
                ...
                <div style="height: 50px;width: 100%;position: relative;">
                    ...
                    <div id="operateBtn"
                        ...
                        <c:if test="${(roleScope eq GlobalConstant.RES_ROLE_SCOPE_DOCTOR)}">
                            ...
                            <p id="editTip" style="padding-top: 3px;" class="red" style="<c:if test="${noteInfoWithBLOBs.auditStatusId eq 'Apply' or noteInfoWithBLOBs.auditStatusId eq 'UnQualified' }">display: none;</c:if>">
                                （每年撰写3篇${(empty sysCfgMap['zy_gsxd_length'])?'500':sysCfgMap['zy_gsxd_length']}字以上跟师心得）
                            </p>
                        </c:if>
                        ...
                        <div id="noteInfos"
                            ...
                            <table class="basic" width="90%" style="margin-left: 30px">
                                ...
                                <tr>
                                    <td colspan="4">
                                        跟师临证（实践）主要病种（内容）：
                                        <textarea class="xltxtarea validate[required]" style="height: 250px" name="studyContent"
                                                  <c:if test="${not ((noteInfoWithBLOBs.auditStatusId eq 'Apply' or noteInfoWithBLOBs.auditStatusId eq 'UnQualified' ) or (empty noteInfoWithBLOBs))}">readonly="readonly"</c:if>>${noteInfoWithBLOBs.studyContent}</textarea>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="4">
                                        跟师心得体会（要求理论联系实际，不少于${(empty sysCfgMap['zy_gsxd_length'])?'500':sysCfgMap['zy_gsxd_length']}字）
                                ...
                                <tr>
                                    ...
                                    <textarea id="auditContent"
                                            ...
                                            class="xltxtarea " style="height: 250px"
                                            name="auditContent">${noteInfoWithBLOBs.auditContent}</textarea>
                                        <p style="float: right;margin-right: 30px;margin-bottom: 15px;text-align: right">
                                            <span>师承签名：</span>
                                            <c:if test="${((noteInfoWithBLOBs.auditStatusId != 'Submit')  and(noteInfoWithBLOBs.auditStatusId != 'Apply'or noteInfoWithBLOBs.auditStatusId eq 'UnQualified' )and (not empty noteInfoWithBLOBs.auditStatusId )) or (roleScope eq GlobalConstant.RES_ROLE_SCOPE_DISCIPLE)}">
                                                <c:set var="teaFlow" value="${noteInfoWithBLOBs.teacherFlow}"/>
                                                ${pdfn:getSingnPhoto(teaFlow)}<br/>
                                            </c:if>
                                            <c:if test="${!(((noteInfoWithBLOBs.auditStatusId != 'Submit')  and(noteInfoWithBLOBs.auditStatusId != 'Apply'or noteInfoWithBLOBs.auditStatusId eq 'UnQualified' ) and (not empty noteInfoWithBLOBs.auditStatusId )) or (roleScope eq GlobalConstant.RES_ROLE_SCOPE_DISCIPLE))}">
                                                <input class="inputText"  readonly="readonly"/><br/>
                                            </c:if>
                                            <c:if test="${((noteInfoWithBLOBs.auditStatusId != 'Submit') and(noteInfoWithBLOBs.auditStatusId != 'Apply'or noteInfoWithBLOBs.auditStatusId eq 'UnQualified' ) and (not empty noteInfoWithBLOBs.auditStatusId )) or (roleScope eq GlobalConstant.RES_ROLE_SCOPE_DISCIPLE)}">
                                                <c:set var="teaFlow" value="${noteInfoWithBLOBs.teacherFlow}"/>
            ...
            <table class="basic" width="90%" style="margin-left: 30px">
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" name="teacherFlow"
                               value="${resDoctor.discipleTeacherFlow}"/>
                    </td>
                </tr>
                <tr>
                    <td colspan="4">
                        跟师临证（实践）主要病种（内容）：
                        <textarea class="xltxtarea validate[required]" style="height: 250px" name="studyContent"></textarea>
                    </td>
                </tr>
                <tr>
                    <td colspan="4">
                        跟师心得体会（要求理论联系实际，不少于${(empty sysCfgMap['zy_gsxd_length'])?'500':sysCfgMap['zy_gsxd_length']}字）
                                <textarea id="experienceContent" class="xltxtarea" style="height: 250px"
                                          name="experienceContent"></textarea>
                        <p style="float: right;margin-right: 30px;margin-bottom: 15px;text-align: right">
                            <span>跟师规培学员签名：</span><input class="inputText"
                                                         value="${resDoctor.doctorName}"/><br/>
                            <input type="text" name="studentSignTime" class="inputText"
                                   value="${pdfn:transDate(noteInfoWithBLOBs.studentSignTime)}"
                                   readonly="readonly"/>
                        </p>
                    ...
                    <td colspan="4">
                        师承指导老师批阅意见：（要有针对性和指导性，能体现指导老师的学术和水平）
                                    <textarea readonly="readonly"
                                              class="xltxtarea" style="height: 250px"
                                              name="auditContent"></textarea>
                        <p style="float: right;margin-right: 30px;margin-bottom: 15px;text-align: right">
                            <span>师承签名：</span><input class="inputText" readonly="readonly"/><br/>
                            <input type="text" class="inputText" name="auditTime"
                                                                   readonly="readonly"/>
                        </p>
                    </td>
                </tr>
            </table>
            <span style="color: #FF0000;margin-left: 30px;">注：本日期需与跟师 记录相对应</span>
        </div>
    </div>
</div>
<%--附件模板--%>
