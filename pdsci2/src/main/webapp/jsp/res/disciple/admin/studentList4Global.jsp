...
<head>
    ...
    <script type="text/javascript">
        ...
        function showInfo(infoFlow) {
            ...
            var url = "<s:url value='/res/disciple/discipleTeacherIndex?role=global'/>&infoFlow=" + infoFlow;
            jboxOpen(url, "师承指导老师简况表", width, height);
        }
        function showBook(docFlow) {
            var width = (window.screen.width) * 0.7;
            var height = (window.screen.height) * 0.6;
            var url = "<s:url value='/res/bookStudyRecord/list?role=global'/>&doctorFlow=" + docFlow;
            ...
            <table class="table" width="100%">
                ...
                <c:forEach items="${list}" var="bean" varStatus="status">
                    <tr>
                        ...
                        <td style="text-align: center;padding: 0px;">${bean.orgName}</td>
                        <td style="text-align: center;padding: 0px;">
                            <c:if test="${jszyFlag ne 'Y'}">${bean.doctorCategoryName}</c:if>
                            <c:if test="${jszyFlag eq 'Y'}">${bean.trainingTypeName}</c:if>
                        </td>
                        <td style="text-align: center;padding: 0px;line-height: 130%;" width="120">${bean.speName}</td>
                        <td style="text-align: center;padding: 0px;" width="40">${bean.sessionNumber}</td>
                        <td style="text-align: center;padding: 0px;">
                        ...
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showRecord('${bean.doctorFlow}','global');">${bean.recordCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showNoteInfo('${bean.doctorFlow}','global','${noteTypeEnumNote.id}');">${bean.noteCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showNoteInfo('${bean.doctorFlow}','global','${noteTypeEnumExperience.id}');">${bean.expCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showBook('${bean.doctorFlow}','global');">${bean.bookCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showNoteInfo('${bean.doctorFlow}','global','${noteTypeEnumBookExperience.id}');">${bean.bookExpCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showCaseInfo('${bean.doctorFlow}','global');">${bean.casesCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
                            <a style="color: blue" href="javascript:void(0)"
                               onclick="showYearAss('${bean.doctorFlow}','global');">${bean.yearCount}</a>
                        </td>
                        <td style="text-align: center;padding: 0px;">
