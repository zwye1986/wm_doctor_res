...
//				}, null, true);
		...
		<%--function back(){--%>
			...
			<%--if("teacher"==roleId){--%>
				<%--var currentPage = $("#currentPage").val();--%>
				<%--url="<s:url value='/res/typicalCases/teacherMain'/>?currentPage="+currentPage;--%>
			<%--}--%>
			<%--if("see"==roleId){--%>
				<%--var teacherCurrentPage= $("#teacherCurrentPage").val();--%>
			...
			<div style="width: 30%;margin-bottom: 40px; float: left">
				<div class="index_title" style="height: 50px;width: 100%;">
					...
					<p>
						...
						<input id="addCase" style="float: right" class="btn_blue" type="button" value="添加跟师医案" class="search" onclick="editTypicalCase();"/>
					</p>
				</div>
				...
				<div style="height: 50px;width: 100%;position: relative;">
					...
					<c:if test="${!empty resTypicalCasesList}">
						<div id="shenhe" style="float: left;">
							<input id="shenhe1" class="btn_blue" type="button" value="审核通过" class="search" onclick="checkTypicalCase('${resTypicalCases.recordFlow}',1);"/>
							&#12288;&#12288;<input id="shenheBack" class="btn_blue" type="button" value="审核不通过" class="search" onclick="checkTypicalCase('${resTypicalCases.recordFlow}','0');"/>
						</div>
					</c:if>
					<c:if test="${('adminSee' eq roleId or 'see' eq roleId) and not empty resTypicalCasesList}">
						<div  align="center">
					...
					<div style="float: left;min-width: 360px;">
						<p id="operateBtn" style="float: left;<c:if test='${empty resTypicalCasesList}'>display: none;</c:if>">
							<c:choose>
								<c:when test="${'Apply' eq seeFlag or 'UnQualified' eq seeFlag}"><input class="btn_blue" type="button" value="删除" class="search" onclick="deleteTypicalCase('${resTypicalCases.recordFlow}');"/></c:when>
								<c:when test="${'PendingAudit' eq seeFlag}"></c:when>
								<c:when test="${'Qualified' eq seeFlag}"></c:when>
								<c:otherwise></c:otherwise>
							</c:choose>
							&#12288;
							<c:choose>
								<c:when test="${'Apply' eq seeFlag or 'UnQualified' eq seeFlag}"><input class="btn_blue" type="button" value="保存并阅读" class="search" onclick="saveTypicalCase();"/></c:when>
								<c:when test="${'PendingAudit' eq seeFlag}"></c:when>
								<c:when test="${'Qualified' eq seeFlag}"><input class="btn_blue" type="button" value="保存并阅读" class="search" onclick="saveTypicalCase();"/></c:when>
								<c:otherwise></c:otherwise>
							</c:choose>
							&#12288;
							<c:choose>
								<c:when test="${'Apply' eq seeFlag or 'UnQualified' eq seeFlag}"><input class="btn_blue" type="button" value="提交" class="search" onclick="submitTypicalCase();"/>&#12288;（提交后指导老师才可审核）</c:when>
								<c:when test="${'PendingAudit' eq seeFlag}"></c:when>
								<c:when test="${'Qualified' eq seeFlag}"></c:when>
								<c:otherwise></c:otherwise>
							</c:choose>
							<%--<p style="float: right;margin-right: 30px;">--%>
							<%--<input id="back" class="btn_blue" style="float: right;" type="button" value="返回" class="search" onclick="back();"/>--%>
							<%--</p>--%>
								...
								<tr>
									...
									<td colspan="5">
										<c:if test="${resTypicalCases.auditStatusId eq 'Apply' or 'UnQualified' eq resTypicalCases.auditStatusId or 'Qualified' eq resTypicalCases.auditStatusId}">
											<table style="max-width: 480px;float: left;margin-left: 5px;margin-bottom: 10px;margin-top: 10px;" class="filesTable" id="filesTable">
												<tr>
