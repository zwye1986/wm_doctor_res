...
<head>
    ...
    <script type="text/javascript">
        ...
        function save() {
            ...
            var data = [];
            var resBaseinputFlag = true;
            $('.resBaseinputValue').each(function (){
                debugger
                var value = $(this).val();
                var id = $(this).attr('id');
                var rojectOfBasefundsName = $(this).attr('rojectOfBasefundsName');
                var re = (/^(?!0\d|[.]+$)\d{0,20}(\.\d{0,4})?$/).test(value) ;
                if(!re) {
                    resBaseinputFlag = false;
                    return;
                }
            ...
            var url = "<s:url value='/res/resFunds/saveBaseAllocationDetails'/>?role=${role}";
            var msg = "确认无误点击保存？";
            if(${ 'N' eq fundingRudit and role eq 'hospital'})
            {
                $("#auditStatusId").val('1');
                msg="该条申请省厅无需审核，数据提交后，无法修改，是否继续提交？";
            } else if(${ 'Y' eq fundingRudit and role eq 'hospital'} )
            {
                $("#auditStatusId").val('3');
                msg="该条数据提交后，无法修改，需要等待需要省厅审核，如未通过审核，可修改后继续提交，是否继续提交？";
            } else if(${role eq 'global' and  'N' eq fundingRudit})
            {
                msg="确认保存？添加数据后如需修改使用编辑功能！";
            }
            if(${role eq 'global'}){
                $("#auditStatusId").val('1');
            }
...
<body>
<div class="mainright" style="max-height: 800px; max-width: 900px; overflow: scroll; width: 800px; height: 700px;"  >
    <div class="basic" >
            ...
            <table class="grid" style="width:100%; margin-bottom: 10px; margin-top: 30px;">
                ...
                <tr>
                    <c:if test="${role eq 'global'}">
                        ...
                        <td>
                            <input style="width: 180px " id="trainOrg" oncontextmenu="return false" name="orgName" value="${resBaseFunds.orgName}"  class="toggleView input" type="text"
                        ...
                        <div id="pDiv" style="width: 0px;height: 0px;overflow: visible;float: left; position:relative; top:30px;">
                            <div class="boxHome trainOrg" id="trainOrgSel" style="left: 150px; max-height: 250px;overflow: auto; border: 1px #ccc solid;background-color: white;min-width: 200px;border-top: none;position: relative;display:none;">
                    ...
                    <td <c:if test="${role ne 'global'}"> colspan="3" </c:if>>
                        <input style="width: 180px" name="amountOfExpenses" id="amountOfExpenses" class="input validate[required,maxSize[200]]" value="${resBaseFunds.amountOfExpenses}" onblur="formatNumbers(this);" /><font >（万元）</font>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input style="width: 180px" type="text" id="inPlaceDate" name="inPlaceDate" value="${resBaseFunds.inPlaceDate}"  class="input validate[required]"  readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})">
                    </td>
                    <th style="text-align: right;"><font color="red" >*</font>上报时间：</th>
                    <td>
                        <input style="width: 180px" type="text" id="reportDate" name="reportDate" value="${resBaseFunds.reportDate}"  class="input validate[required]"  readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})">
                    </td>
                </tr>
                <tr>
                ...
                <c:forEach items="${dictTypeEnumBaseFundingUseList}" var="dict" >
                    <tr>
                        ...
                        <td colspan="2" style="text-align: left">
                            ...
                            <input name="hospitalTypeId" class="input resBaseinputValue" value="${detailMap[key]}" id="${dict.dictId}"  onblur="inputOnBlur();">
                        </td>
                    </tr>
                </c:forEach>
                <tr>
