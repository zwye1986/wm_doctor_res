		...
		<div class="infoAudit">
			...
			<c:set var="showMsg1" value="${param.roleFlag eq GlobalConstant.RES_ROLE_SCOPE_PROFESSIONALBASE && empty rec.headAuditStatusId &&  empty rec.managerAuditStatusId && not empty rec.auditStatusId}"/>
	 		...
	 		<form id="summaryForm" method="post">
				...
				<table class="orgTable" border="0" width="90%" cellspacing="0" cellpadding="0" style="margin-left: auto;margin-right: auto;">
					<caption class="orgCaption" style="color:#459ae9;">科室评价&#12288;</caption>
					<tr class="orgTr">
						<td class="orgTd" colspan="4" style="padding-top:10px;padding-left:0;">
							<c:if test="${headSub}">
				...
				<c:if test="${param.roleFlag eq GlobalConstant.RES_ROLE_SCOPE_PROFESSIONALBASE || !empty rec.managerAuditStatusId}">
					<table class="orgTable" border="0" width="90%" cellspacing="0" cellpadding="0" style="margin-left: auto;margin-right: auto;">
						<caption class="orgCaption" style="color:#459ae9;">
							基地秘书评价&#12288;
							<label style="font-weight: normal;color: black;">
								<c:if test="${baseSub && !showMsg1}">
									同意出科：
									<label>
										<input type="radio" name="isAgree" class="autoValue validate[required]"
											   <c:if test="${formDataMap['isAgree_id'] eq GlobalConstant.FLAG_Y}">checked</c:if>
											   value="${GlobalConstant.FLAG_Y}"
										/> 是
									</label>
									<label>
										<input type="radio" name="isAgree" class="autoValue validate[required]"
											   <c:if test="${formDataMap['isAgree_id'] ne GlobalConstant.FLAG_Y}">checked</c:if>
								...
								<c:if test="${!baseSub && !showMsg1}">
									同意出科：
									${(formDataMap['isAgree_id'] eq GlobalConstant.FLAG_Y)?'是':'否'}
									<input type="hidden" name="isAgree" value="${formDataMap['isAgree_id']}"/>
									<input type="hidden" name="isAgree_name" value="${formDataMap['isAgree']}"/>
								</c:if>
							</label>
						</caption>
						<tr class="orgTr">
							<td class="orgTd" colspan="4" style="padding-top:10px;padding-left:0;">
								<c:if test="${baseSub}">
									<textarea name="baseAutograth" class="validate[required]">${formDataMap['baseAutograth']}</textarea>
								</c:if>
								<c:if test="${!baseSub}">
									&#12288;${formDataMap['baseAutograth']}
									<input type="hidden" name="baseAutograth" value="${formDataMap['baseAutograth']}"/>
								</c:if>
							</td>
						</tr>
					</table>
					<div align="right" style="margin-top: 10px;width: 90%;margin-right: auto;margin-left: auto;">
						基地秘书：
						<c:if test="${baseSub}">
							<input type="text" class="toggleView3 validate[required] inputText" name="base" style="width: 100px;" value="${empty formDataMap['base']?sessionScope.currUser.userName:formDataMap['base']}"/>
							<label style="display: none;" class="toggleView3">${formDataMap['base']}</label>
						</c:if>
						<c:if test="${!baseSub}">
							<label>${formDataMap['base']}</label>
							<input type="hidden" name="head" value="${formDataMap['base']}"/>
						</c:if>
						&#12288;日期：
						<c:if test="${baseSub}">
							<input type="text" class="toggleView3 inputText validate[required] ${GlobalConstant.RES_ROLE_SCOPE_DOCTOR} ctrlInput" name="baseDate" value="${empty formDataMap['baseDate']?pdfn:getCurrDate():formDataMap['baseDate']}" style="width: 100px;" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
							<label style="display: none;" class="toggleView3">${formDataMap['baseDate']}</label>
						</c:if>
						<c:if test="${!baseSub}">
							<label>${formDataMap['baseDate']}</label>
							<input type="hidden" name="baseDate" value="${formDataMap['baseDate']}"/>
						</c:if>
					</div>
				</c:if>
	         </form>
			...
			<div style="text-align: center;margin-top: 5px;width: 90%;margin-left: auto;margin-right: auto;">
				...
				<c:if test="${docSub || teaSub || headSub || baseSub}">
