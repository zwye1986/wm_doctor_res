...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			...
        			<td>
        				<c:if test="${doctor}">
	        				<label><input type="radio" name="skill_result" <c:if test="${'是' eq formDataMap['skill_result']}">checked</c:if> value="是" onchange="check(this.value);"/>是</label>&#12288;
	        				<label><input type="radio" name="skill_result"  <c:if test="${'否' eq formDataMap['skill_result']}">checked</c:if> value="否" onchange="check(this.value);"/>否</label>
        				</c:if>
        				<c:if test="${!doctor}">
