      ...
      <div class="content">
        ...
        <form id="skillForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		<tr>
        			<td style="width: 12%;"><span style="color: red;">*</span>操作名称：</td>
        			<td colspan="5">
						<script>
	                	$(function(){
...
<%--       	 			</c:if> --%>
        		...
        		<tr>
        			<td><span style="color: red;">*</span>病人姓名：</td>
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 160px;" name="skill_pName" value="${formDataMap['skill_pName']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['skill_pName']}
        				</c:if>
        			</td>
        			<td style="width: 12%;"><span style="color: red;">*</span>操作日期：</td>
        			<td style="width: 21%;">
        				<c:if test="${doctor}">
       						<input type="text" class="validate[required]" style="width: 160px;" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" name="skill_operDate" value="${formDataMap['skill_operDate']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<tr>
        			<td style="width: 14%;"><span style="color: red;">*</span>病历号：</td>
        			<td style="width: 20%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 160px;" name="skill_mrNo" value="${formDataMap['skill_mrNo']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['skill_mrNo']}
        				</c:if>
        			</td>
        			<td><span style="color: red;">*</span>是否成功：</td>
        			<td>
        				<c:if test="${doctor}">
	        				<label><input class="autoValue validate[required]" type="radio" name="skill_result" <c:if test="${'1' eq formDataMap['skill_result_id'] or empty  formDataMap['skill_result_id']}">checked</c:if> value="1" onchange="check(this.value);"/>是</label>&#12288;
	        				<label><input class="autoValue validate[required]" type="radio" name="skill_result"  <c:if test="${'0' eq formDataMap['skill_result_id']}">checked</c:if> value="0" onchange="check(this.value);"/>否</label>
        				</c:if>
        				<c:if test="${!doctor}">
