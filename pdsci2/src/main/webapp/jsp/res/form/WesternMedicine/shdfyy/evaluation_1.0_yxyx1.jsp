<table class="basic" width="100%" style="margin-top: 10px;">
	...
	<tr>
		...
		<td colspan="2">报告书写</td>
		<td style="text-align: center">
			<c:if test="${(param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId) }">
				<input class="validate[custom[integer],min[0],max[100]]"type="text" name="cEleven" value="${formDataMap['cEleven']}" onchange="count(this)"
					   style="width: 50%;">
			</c:if>
			<c:if test="${!((param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_TEACHER && empty rec.auditStatusId) || (param.roleFlag==GlobalConstant.RES_ROLE_SCOPE_HEAD && empty rec.headAuditStatusId)) }">
				<c:if test="${!empty formDataMap['cEleven']}">
					${formDataMap['cEleven']}
				</c:if>
				<input type="hidden" name="cEleven" value="${formDataMap['cEleven']}"/>
			</c:if>
		</td>
		<td>10.0%</td>
		...
		<tr>
			<td rowspan="3" colspan="2">临床综合能力考核（60%）</td>
		...
		<tr>
			...
			<td>35.0%</td>
			<td class="sSum">${formDataMap['cEight']*0.35}</td>
		</tr>
		<tr>
