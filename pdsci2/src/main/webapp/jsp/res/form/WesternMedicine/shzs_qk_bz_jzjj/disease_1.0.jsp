        ...
        <form id="dblForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		...
        		<tr>
					...
					<td style="width: 30%;">
						<c:if test="${doctor}">
							<input type="text" class="validate[required]" style="width:140px;" name="disease_patientName" value="${formDataMap['disease_patientName']}"/>
						</c:if>
						<c:if test="${!doctor}">
							<label>${formDataMap['di class="validate[required]"sease_patientName']}</label>
						</c:if>
					</td>
        			<td style="width: 20%;"><font color="red">*</font>病历号:</td>
        			<td style="width: 30%;">
        				<c:if test="${doctor}">
							<input class="validate[required]" type="text" name="disease_anamnesisNumbers" style="width: 160px;" value="${formDataMap['disease_anamnesisNumbers']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['disease_anamnesisNumbers']}
							<input type="hidden" name="disease_anamnesisNumbers" value="${formDataMap['disease_anamnesisNumbers']}"/>
        				</c:if>
        			</td>
        		</tr>
        		<tr>
        			<td><font color="red">*</font>突发事件:</td>
        			<td>
        				<c:if test="${doctor}">
							<input class="validate[required]" type="text" name="disease_emergency" style="width: 160px;" value="${formDataMap['disease_emergency']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        					${formDataMap['disease_emergency']}
							<input type="hidden" name="disease_emergency" value="${formDataMap['disease_emergency']}"/>
        				</c:if>
        			</td>
					<td style="width: 20%;"><span style="color: red;">*</span>是否主管：</td>
					<td style="width: 30%;">
						<c:if test="${doctor}">
							<label><input type="radio" name="disease_director" <c:if test="${'是' eq formDataMap['disease_director']}">checked</c:if> value="是" class="autoValue validate[required]"/>是</label>&#12288;
							<label><input type="radio" name="disease_director"  <c:if test="${'否' eq formDataMap['disease_director']}">checked</c:if> value="否" class="autoValue validate[required]" />否</label>
						</c:if>
						<c:if test="${!doctor}">
							${formDataMap['disease_director']}
						</c:if>
					</td>
        			</tr>
				<tr>
					<td style="width: 20%;"><font color="red">*</font>上级医师姓名:</td>
					<td colspan="3">
						<c:if test="${doctor}">
							<input class="validate[required]" type="text" name="disease_superiorPhysicianName" style="width: 160px;" value="${formDataMap['disease_superiorPhysicianName']}"/>
						</c:if>
						<c:if test="${!doctor}">
