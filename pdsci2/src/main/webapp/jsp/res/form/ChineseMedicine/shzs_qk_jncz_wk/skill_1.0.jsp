						...
						<td colspan="5">
									...
									<div id="reqHome" style="height: 0px;width: 100%;position: relative;display: none;">
										<c:forEach items="${deptReqList}" var="req">
											<div class="itemDiv" style="width: 100%;height: 30px;border-bottom: 1px #bbb solid;border-left: 1px #bbb solid;border-right: 1px #bbb solid;padding-left: 2px;"
												...
												<input class="itemCheckbox" style="margin-left: 8px;" type="checkbox" name="itemId" value="${req.itemId}" onclick="loadOther(this);" <c:if test="${rec.itemId eq req.itemId}">checked</c:if>/>
							...
							<c:if test="${doctor}">
								<div id="otherName" style="float: left;display: none;margin-left: 20px;height: 0px;overflow: visible;margin-top: -9px;">
									...
									<input type="text" name="regItem" value="${formDataMap['skill_operName']}" style="width: 160px;" class="validate[required]"/>
								</div>
							</c:if>
						</td>
					</tr>
					<tr>
						<td><font color="red">*</font>病人姓名:</td>
						<td style="width: 21%;">
							<c:if test="${doctor}">
						...
						<td style="width: 12%;"><font color="red">*</font>日期:</td>
						<td style="width: 21%;">
							<c:if test="${doctor}">
								<input class="validate[required]" type="text" style="width: 160px;" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" name="skill_operDate" value="${formDataMap['skill_operDate']}"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['skill_operDate']}
								<input type="hidden" name="skill_operDate" value="${formDataMap['skill_operDate']}"/>
							</c:if>
						</td>
					</tr>
					<tr>
						<td style="width: 12%;"><font color="red">*</font>病历号:</td>
						<td style="width: 21%;">
							<c:if test="${doctor}">
						...
						<td style="width: 12%;"><font color="red">*</font>术者/助手:</td>
						<td style="width: 21%;">
							<c:if test="${doctor}">
								<label><input class="autoValue validate[required]" type="radio" name="skill_operatorOrHelper" <c:if test="${'术者' eq formDataMap['skill_operatorOrHelper']}">checked</c:if> value="术者" />术者</label>&#12288;
								<label><input class="autoValue validate[required]" type="radio" name="skill_operatorOrHelper"  <c:if test="${'助手' eq formDataMap['skill_operatorOrHelper']}">checked</c:if> value="助手" />助手</label>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['skill_operatorOrHelper']}
							</c:if>
						</td>
					</tr>
					<tr>
						<td><font color="red">*</font>上级医师姓名:</td>
						<td colspan="3">
							<c:if test="${doctor}">
								<input class="validate[required]" type="text" name="skill_superiorPhysicianName" style="width: 160px;" value="${formDataMap['skill_superiorPhysicianName']}"/>
							</c:if>
							<c:if test="${!doctor}">
								<label>${formDataMap['skill_superiorPhysicianName']}</label>
							</c:if>
						</td>
					</tr>
				</table>
