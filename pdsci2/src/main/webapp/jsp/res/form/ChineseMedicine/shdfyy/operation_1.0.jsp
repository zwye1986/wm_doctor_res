		...
		<div class="title1 clearfix">
			<form id="dblForm">
				...
				<table class="basic" width="100%" style="margin-top: 10px;">
					<tr>
						<td style="width: 12%;"><font color="red">*</font>手术名称:</td>
					...
					<tr>
						<td style="width: 20%;"><font color="red">*</font>术式:</td>
						<td style="width: 30%;">
							<c:if test="${doctor}">
								<input class="validate[required]" type="text" name="operMethod" style="width: 160px;" value="${formDataMap['operMethod']}"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['operMethod']}
								<input type="hidden" name="operMethod" value="${formDataMap['operMethod']}"/>
							</c:if>
						</td>
						<td style="width: 20%;"><font color="red">*</font>病人姓名:</td>
						<td colspan="3" style="width: 30%;">
							<c:if test="${doctor}">
								<input class="validate[required]" type="text" name="patientName" style="width: 160px;" value="${formDataMap['patientName']}"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['patientName']}
								<input type="hidden" name="patientName" value="${formDataMap['patientName']}"/>
							</c:if>
						</td>
					</tr>
					<tr>
						<td><font color="red">*</font>手术日期:</td>
						<td>
							<c:if test="${doctor}">
								<input style="width: 160px;" class="validate[required] " name="operDate" type="text" value="${formDataMap['operDate']}" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['operDate']}
								<input type="hidden" name="operDate" value="${formDataMap['operDate']}"/>
							</c:if>
						</td>
						<td><font color="red">*</font>病历号:</td>
						<td>
							<c:if test="${doctor}">
								<input class="validate[required]" type="text" name="cardNo" style="width: 160px;" value="${formDataMap['cardNo']}"/>
							</c:if>
							<c:if test="${!doctor}">
								${formDataMap['cardNo']}
								<input type="hidden" name="cardNo" value="${formDataMap['cardNo']}"/>
							</c:if>
						</td>
					</tr>
					<tr>
						<td style="width: 100px;"><font color="red">*</font>手术结果：</td>
						<td colspan="5" style="width: 90%;">
							<select class="autoValue validate[required]" name="operResult" style="width:164px;">
								<option value="">请选择</option>
								<option value="1"<c:if test="${'1'eq formDataMap['operResult_id']}">selected</c:if>>成功</option>
								<option value="0"<c:if test="${'0'eq formDataMap['operResult_id']}">selected</c:if>>失败</option>
							</select>
						</td>
					</tr>
				</table>
