        ...
        <form id="dblForm">
        	...
        	<table class="basic" width="100%" style="margin-top: 10px;">
        		<tr>
        			...
        			<td style="width: 30%;">
        			<c:if test="${doctor}">
        				<input type="text" class="validate[required]" style="width:140px;" name="mr_pName" value="${formDataMap['mr_pName']}"/>
        			</c:if>
        			<c:if test="${!doctor}">
        			...
        			<td style="width: 30%;">
        			<c:if test="${doctor}">
        				<input type="text" class="validate[required]" style="width: 140px;" name="mr_no" value="${formDataMap['mr_no']}"/>
        			</c:if>
        			<c:if test="${!doctor}">
        		...
        		<tr>
        			<td style="width:20%;"><span style="color: red;">*</span>疾病名称：</td>
        			<td style="width:30%;">
        				<c:if test="${doctor}">
        					<input type="text" class="validate[required]" style="width: 140px;" name="disease_pName" value="${formDataMap['disease_pName']}"/>
        				</c:if>
        				<c:if test="${!doctor}">
        			...
        			<td>
        				<c:if test="${doctor}">
       					<select name="mr_diagType" style="width: 144px;" class="autoValue validate[required]">
        					<option></option>
        					<option value="1" <c:if test="${'1' eq formDataMap['mr_diagType_id'] or empty formDataMap['mr_diagType_id']}">selected</c:if>>主要诊断</option>
