...
<%--function checkFile(file){--%>
	...
	<%--if($.trim(filePath)!="" && !regStr.test(filePath)){--%>
		<%--file.value = "";--%>
		<%--jboxTip("请上传&nbsp;${sysCfgMap['inx_image_support_suffix']}格式的文件");--%>
	<%--}--%>
<%--}--%>
...
function returnUrl(){
	...
	window.parent.frames['mainIframe'].$(operType+'Up').text("重新上传");
...
<body>
<form id="uploadFileForm" method="post" action="<s:url value='/res/ProfessionalBase/checkUploadFile'/>" enctype="multipart/form-data">
...
<div class="content">
	<div class="title1 clearfix">
		<div id="uploadFileDiv" style="display: ${empty result?'':'none'}">
			...
			<%--  onchange="checkFile(this);" accept="${sysCfgMap['inx_image_support_mime']}"--%>
			/>
			</div>
			<div class="button">
