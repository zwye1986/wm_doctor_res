...
<script type="text/javascript">
    ...
    //报名审核
    function search(){
        jboxPostLoad("content","<s:url value='/dwjxres/hospital/audit'/>?isQuery=Y",$('#auditForm').serialize(),true);
    }
    //打印进修申请表
    ...
    //退回操作
    function returnInfo(userFlow,resumeFlow,titleTypeName){
        console.log(titleTypeName);
        jboxConfirm("确认退回?", function(){
            jboxPost("<s:url value='/dwjxres/hospital/returnInfo'/>",{"resumeFlow":resumeFlow,"userFlow":userFlow,"titleTypeName":titleTypeName}, function(resp){
                jboxTip(resp);
                toPage($("#currentPage").val());
            } , null , true);
        });
    ...
    function  exportInfo()
    {
        var url = "<s:url value='/dwjxres/hospital/exportAuditPassed'/>";
        jboxTip("导出中…………");
...
<div class="main_hd">
    ...
    <div class="title_tab" id="toptab">
        <ul>
            ...
            <li class="${statusId=='Back'?'tab_select':'tab' }" statusId="Back"><a>已退回</a></li>
        </ul>
    </div>
</div>
<div class="main_bd" id="div_table_0" >
    ...
    <div class="div_table">
        <c:if test="${statusId ne stuStatusEnumUnPassed.id && statusId ne 'Back'}">
                ...
                <c:forEach items="${stuUserLst}" var="user">
                    <tr>
                       ...
                       <%-- <c:if test="${statusId eq regStatusEnumPassed.id}">
                        ...
                        <td>
                                <c:if test="${statusId==stuStatusEnumPassing.id }">
        ...
        <c:if test="${statusId eq regStatusEnumUnPassed.id or statusId eq 'Back'}">
            <table border="0" cellpadding="0" cellspacing="0" class="grid">
                <tr>
                    <th>姓名</th>
                    <th>身份证号</th>
                    ...
                    <tr>
                        ...
                        <c:if test="${statusId eq regStatusEnumUnPassed.id}">
                            <td>
                                ...
                                <a onclick="returnInfo('${user.sysUser.userFlow}','${user.resumeFlow}','${extInfoMap[user.resumeFlow].titleTypeName}')">[退回]</a>
                            </td>
                        </c:if>
                    </tr>
                </c:forEach>
