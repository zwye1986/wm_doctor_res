...
<div class="infoAudit2" style="padding:0;">
	...
	<div class="div_table" <c:if test="${param.isHide eq 'Y' or flag eq 'Y'}">style="display: none;" </c:if>>
		...
		<div align="center" style="margin-top: 20px; margin-bottom:20px;">
			<c:if test="${stuUser.stuStatusId eq stuStatusEnumPassing.id and stuUser.nursingStatusId eq stuStatusEnumPassing.id}">
...
<script type="text/javascript">
	...
	function audit(nursingStatusId,title){
		if("${stuStatusEnumUnPassed.id}"==nursingStatusId||nursingStatusId=="${stuStatusEnumUnRecruitted.id}"){
		...
		jboxConfirm("确认"+title+"?",function(){
			var data = {
                ...
                nursingStatusId:nursingStatusId,

			};
			jboxPost("<s:url value='/dwjxres/nursing/auditNurseOption'/>",data,
				function(resp){
	...
	function returnInfo(resumeFlow){
		...
		jboxConfirm("确认退回?", function(){
			jboxPost("<s:url value='/dwjxres/nursing/returnInfo'/>",{"resumeFlow":resumeFlow,"reason":reason,"userFlow":userFlow}, function(resp){
				jboxTip(resp);
				window.parent.search();
				jboxClose();
			} , null , true);
