<script type="text/javascript">
    ...
    function toPage(page) {
        ...
        jboxPostLoad("content","<s:url value='/dwjxres/hospital/speMaintenance'/>",{"currentPage":$('#currentPage').val()},true);
    }
    //刷新
    function refresh(){
        jboxPost('<s:url value="/dwjxres/hospital/refresh"/>', null, function (resp) {
            if (resp == "1") {
                jboxTip("刷新成功");
                speMaintenance();
            }
    ...
    function edit(flag,dictFlow){
        jboxOpen("<s:url value='/dwjxres/hospital/editSpe'/>?dictFlow="+dictFlow, flag,800,500,true);
    }
    //删除
    function del(dictFlow){
        jboxConfirm("确定刪除吗？",function(){
            jboxPost("<s:url value='/dwjxres/hospital/delSpe'/>",{"dictFlow":dictFlow},function(resp){
                speMaintenance();
            },null,true);
        },null);
    }
