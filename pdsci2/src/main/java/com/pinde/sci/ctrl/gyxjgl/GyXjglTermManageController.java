...
@Controller
@RequestMapping("/gyxjgl/term/manage")
public class GyXjglTermManageController extends GeneralController {
    @Autowired
    ...
    public String list(String roleFlag, String weekNumber, String assumeOrgFlow, String courseFlow, EduTerm term, String classTeacherName, Model model) {
        ...
        return "gyxjgl/term/manage/list";
    }
...
//            maps = eduTermBiz.getFirstClassOfGzByFlow(recordFlow);
...
//            model.addAttribute("firstClassMap",firstClassMap);
        ...
        return "gyxjgl/term/manage/loadDetail";
    }
    ...
    public String currCourse(Model model,String courseFlow,String year,String termSeason,String classId,String trainTypeId,String userFlow,String recordFlow,String studMaxNum,String isChosedCourseName,String flag,String lastWeek){
        ...
        return "gyxjgl/term/manage/courseInfo";
    }
    ...
    public Map<String, Object> getWeekMap2(Map<String, Object> param) {
     ...
     * @return
     */
    public String getWeekFirstDate(String date) {
     ...
     * @return
     */
    public String getWeekLastDate(String date) {
     ...
     * @return
     */
    private int getStartWeek(String startTime, String string) {
     ...
     * @param param
     */
    public void setSearchTime(EduTerm temp, Map<String, Object> param) {
    ...
    public String saveTerm(EduTerm term, Model model) {
        if (StringUtil.isNotBlank(term.getClassId())) {
            term.setClassName(DictTypeEnum.GyXjClass.getDictNameById(term.getClassId()));
        }
        if (StringUtil.isNotBlank(term.getGradationId())) {
            term.setGradationName(DictTypeEnum.GyTrainType.getDictNameById(term.getGradationId()));
        }
        if (StringUtil.isNotBlank(term.getGradeTermId())) {
            term.setGradeTermName(DictTypeEnum.GyTermSeason.getDictNameById(term.getGradeTermId()));
        }
    ...
    public String courseAdd(EduScheduleClass schedule,String courseTypeId,Model model) throws Exception{
        ...
        //广医暂去除选修排课模式（现与必修排课模式同）
//        if(XjglCourseTypeEnum.Public.getId().equals(courseTypeId)){
//            //排选修课后，同步刷新所有班级课表
//            EduTerm term = new EduTerm();
//            term.setSessionNumber(schedule.getSessionNumber());
//            term.setRecordStatus(GlobalConstant.RECORD_STATUS_Y);
//            List<EduTerm> terms = eduTermBiz.seachlistByTerm(term);
//            model.addAttribute("terms",terms);
//        }
        model.addAttribute("schedule",schedule);
        return "gyxjgl/term/manage/courseAdd";
    }

    @RequestMapping(value = "/courseAddPlus")
    public String courseAddPlus(EduScheduleClass schedule,String courseTypeId,Model model) throws Exception{
        EduTerm term = eduTermBiz.getEduTermByFlow(schedule.getTermFlow());
        model.addAttribute("term",term);
        List<Map<String,Object>> list = eduCourseBiz.queryOrgOrCourse(GlobalContext.getCurrentUser().getUserFlow());
        model.addAttribute("course",null != list && list.size()>0 ? list.get(0):list);
        //广医暂去除选修排课模式（现与必修排课模式同）
        model.addAttribute("schedule",schedule);
        return "gyxjgl/term/manage/courseAddPlus";
    }
    /**
     * 必修课 排课针对班级
     */
    ...
    /**
     * 必修课 排课针对班级 （通过节次和时间批量排课）
     */
    @RequestMapping(value = "/saveScheduleClassPlus")
    @ResponseBody
    public Object saveScheduleClassPlus(EduScheduleClass schedule,String teacherNameList,String searchCourseFlag) {
        String limitError="";
        List<EduScheduleLimit> limitList = new ArrayList<>();
        if(!GlobalConstant.FLAG_Y.equals(searchCourseFlag)&&StringUtil.isNotBlank(schedule.getSessionNumber())){
            limitList = eduCourseBiz.queryScheduleLimit(schedule.getSessionNumber());
        }
        Map<String,Object> resultMap = new HashMap<>();
        List<EduScheduleClass> dataList=new ArrayList<>();
        if(GlobalConstant.FLAG_Y.equals(searchCourseFlag)){
            EduCourse course = eduCourseBiz.readCourse(schedule.getCourseFlow());
            schedule.setCourseCode(course.getCourseCode());
            schedule.setClassCourseName(course.getCourseName());
        ...
        //批量日期、批量节次处理
        if(StringUtil.isNotBlank(schedule.getClassOrder())&&StringUtil.isNotBlank(schedule.getClassTime())){
            List<String> classOrderList = Arrays.asList(schedule.getClassOrder().split(","));
            List<String> classTimeList = Arrays.asList(schedule.getClassTime().split(","));
            if(classOrderList!=null&&classOrderList.size()>0&&classTimeList!=null&&classTimeList.size()>0){
                for (String order:classOrderList){
                    String classOrder=order.substring(1,order.length()-1);
                    for (String time:classTimeList) {
                        boolean needCourseFlag=true;
                        if(limitList.size()>0){
                            for (EduScheduleLimit limit:limitList) {
                                if(time.trim().compareTo(limit.getBeginTime())>=0&&time.trim().compareTo(limit.getEndTime())<=0){
                                    limitError=limitError+(time+"不可排课！<br/>");
                                    needCourseFlag=false;
                                }
                            }
                        }
                        EduScheduleClass temp = new EduScheduleClass();
                        temp.setAssumeOrgFlow(schedule.getAssumeOrgFlow());
                        temp.setAssumeOrgName(schedule.getAssumeOrgName());
                        temp.setClassCourseName(schedule.getClassCourseName());
                        temp.setClassId(schedule.getClassId());
                        temp.setClassName(schedule.getClassName());
                        temp.setClassPeriod(schedule.getClassPeriod());
                        temp.setClassroomId(schedule.getClassroomId());
                        temp.setClassroomName(schedule.getClassroomName());
                        temp.setCourseFlow(schedule.getCourseFlow());
                        temp.setCourseCode(schedule.getCourseCode());
                        temp.setTermFlow(schedule.getTermFlow());
                        temp.setGradationId(schedule.getGradationId());
                        temp.setGradationName(schedule.getGradationName());
                        temp.setGradeTermId(schedule.getGradeTermId());
                        temp.setGradeTermName(schedule.getGradeTermName());
                        temp.setStudentMaxmun(schedule.getStudentMaxmun());
                        temp.setSessionNumber(schedule.getSessionNumber());
                        temp.setMemo(schedule.getMemo());
                        temp.setClassOrder(classOrder);
                        temp.setClassTime(time.trim());
                        if (needCourseFlag){
                            Map<String,Object> param = new HashMap<>();
                            param.put("termFlow",temp.getTermFlow());
                            param.put("classTime",temp.getClassTime());
                            param.put("classOrder",temp.getClassOrder());
                            List<Map<String,Object>> dataMap = eduTermExtMapper.checkNeedCourse(param);
                            //同一时间是否已经有必选课
                            if(null != dataMap && dataMap.size()>0){
                                limitError=limitError+(time+"，"+order+"已有必选课!无法排课！<br/>");
                            }
                        }
                        dataList.add(temp);
                    }
        ...
        if(StringUtil.isNotBlank(limitError)){
            resultMap.put("msgTip",limitError);
            return resultMap;
        }
        try{
            resultMap = (Map<String,Object>) scheduleBiz.saveScheduleClassPlus(dataList,teacherNameList);
        }catch (Exception e){
            resultMap.put("msgTip",e.getMessage());
        }
        if(GlobalConstant.FLAG_Y.equals(resultMap.get("seccess"))){
            return GlobalConstant.SAVE_SUCCESSED;
        }
        return resultMap;
    }
     ...
     * 选修课 排课针对所有班级
     */
    @RequestMapping(value = "/saveOptionClass")
    @ResponseBody
    public String saveOptionClass(EduScheduleClass schedule,String teacherNameList,String recordMap) {
        EduTerm term = new EduTerm();
        term.setSessionNumber(schedule.getSessionNumber());
        term.setRecordStatus(GlobalConstant.RECORD_STATUS_Y);
        List<EduTerm> termList = eduTermBiz.seachlistByTerm(term);
        Map<String, Object> map = new HashMap<>();
        if(StringUtil.isNotBlank(recordMap)){
            map = JSON.parseObject(recordMap, Map.class);
        }
        int num = 0;
        if(null != termList && !termList.isEmpty()){
            for(int i=0;i<termList.size();i++){
                schedule.setTermFlow(termList.get(i).getRecordFlow());
                schedule.setRecordFlow((String) map.get(schedule.getTermFlow()));
                schedule.setSessionNumber(termList.get(i).getSessionNumber());
                schedule.setGradeTermId(termList.get(i).getGradeTermId());
                schedule.setGradeTermName(termList.get(i).getGradeTermName());
                schedule.setClassId(termList.get(i).getClassId());
                schedule.setClassName(termList.get(i).getClassName());
                schedule.setGradationId(termList.get(i).getGradationId());
                schedule.setGradationName(termList.get(i).getGradationName());
                num += scheduleBiz.saveOptionClass(schedule,teacherNameList);
            }
        }
        if(num > 0){
            return GlobalConstant.SAVE_SUCCESSED;
    ...
    public String scheduleSetting(String sessionNumber, Integer currentPage, HttpServletRequest request,Model model) {
        ...
        return "gyxjgl/term/manage/scheduleSetting";
    }
    ...
    @RequestMapping(value = "/searchEduCourse")
    @ResponseBody
    public EduCourse searchEduCourse(String courseFlow) {
        EduCourse course=new EduCourse();
        if(StringUtil.isNotBlank(courseFlow)){
            course=courseMapper.selectByPrimaryKey(courseFlow);
        }
        return course;
    }
}
