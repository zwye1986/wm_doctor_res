...
import com.pinde.sci.enums.jszy.JszyResAuditStatusEnum;
...
@Controller
@RequestMapping("/jszy/asseGlobal")
public class JszyDoctorGlobalAsseController extends GeneralController {
    @Autowired
    ...
    public String waitAuditMain(Model model,String roleFlag){
        ...
        return "jszy/asse/global/waitAuditMain";
    }
    /**
     * 省厅或基地待审核列表
     * @param model
    ...
    public String waitAuditList(Model model,String roleFlag,Integer currentPage ,HttpServletRequest request,String applyYear,
                                ...
                                String isTheroy,String isSkill,String isFive,String isTen,String makeUp
                                ){
        //查询条件
        ...
        param.put("isTheroy",isTheroy);
        param.put("isSkill",isSkill);
        param.put("isFive",isFive);
        param.put("isTen",isTen);
        param.put("makeUp",makeUp);
        ...
        return "jszy/asse/global/waitAuditList";
    }
    /**
     * 省厅或基地已审核列表
     * @param model
    ...
    public String AuditList(Model model,String roleFlag,Integer currentPage ,HttpServletRequest request,String applyYear,
        ...
        return "jszy/asse/global/AuditList";
    }
                        ...
                        if (userResumeExt != null) {
                            ...
                            if(StringUtil.isNotBlank(userResumeExt.getQualificationMaterialCode()))
                            {
                                ...
                                if(doctor!=null&&"Y".equals(doctor.getDoctorLicenseFlag()))
                                {
                                    ...
                                    temp.put("qualificationMaterialCode", "");
                                }
                            }
...
//                            practicingScopeName
    ...
    public String chargeAuditApply(Model model,String applyFlow,String roleFlag){
        ...
        return "jszy/asse/global/chargeAuditApply";
    }
    ...
    public String batchAuditApply(Model model,String []applyFlow,String roleFlag){
        ...
        return "jszy/asse/global/batchAuditApply";
    }
    //TODO====
    ...
    public String chargeSaveAudit(Model model,String applyFlow,String auditStatusId,String auditReason,String roleFlag){
        ...
        {
            if(GlobalConstant.USER_LIST_GLOBAL.equals(roleFlag)) {
                if (JszyResAuditStatusEnum.ChargeNotPassed.getId().equals(apply.getLocalAuditStatusId())) {
                ...
                apply.setCityAuditStatusName(JszyResAuditStatusEnum.getNameById(auditStatusId));
                apply.setCityReason(auditReason);
                if (JszyResAuditStatusEnum.NotPassed.getId().equals(auditStatusId))//省厅或基地审核不通过 直接不通过
                {
                    apply.setAuditStatusId(JszyResAsseAuditListEnum.ChargeNotPassed.getId());
                    apply.setAuditStatusName(JszyResAsseAuditListEnum.ChargeNotPassed.getName());
                } else {//省厅或基地审核通过
                    apply.setAuditStatusId(JszyResAsseAuditListEnum.WaitGlobalPass.getId());
                    apply.setAuditStatusName(JszyResAsseAuditListEnum.WaitGlobalPass.getName());
                }
                String nowTime = DateUtil.transDateTime(DateUtil.getCurrentTime());
                JsresGraduationApplyLog log = new JsresGraduationApplyLog();
                log.setApplyFlow(applyFlow);
                ...
                if(JszyResAuditStatusEnum.NotPassed.getId().equals(auditStatusId))//市局审核不通过 直接不通过
                {
                    ...
                    apply.setAuditStatusId(JszyResAuditStatusEnum.WaitChargePass.getId());
                    apply.setAuditStatusName(JszyResAuditStatusEnum.WaitChargePass.getName());
                }
                String nowTime=DateUtil.transDateTime(DateUtil.getCurrentTime());
                String startDate=InitConfig.getSysCfg("jszy_local_submit_start_time");
                String endDate=InitConfig.getSysCfg("jszy_local_submit_end_time");
    ...
    public String batchSaveAudit(Model model,String []applyFlows,String auditStatusId,String auditReason,String roleFlag){
        if(applyFlows.length>0) {
            ...
            for(String applyFlow:applyFlows) {
                JsresGraduationApply apply = graduationApplyBiz.readByFlow(applyFlow);
                if (apply != null) {
                    if (JszyResAuditStatusEnum.ChargeNotPassed.getId().equals(apply.getCityAuditStatusId())) {
    ...
    public String AuditListMain(Model model,String roleFlag){
        ...
        model.addAttribute("orgFlowList", orgFlowList);
        model.addAttribute("roleFlag", roleFlag);
        return "jszy/asse/global/AuditListMain";
    }
