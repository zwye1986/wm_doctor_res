...
@Controller
@RequestMapping("/inx/czyyjxres")
public class InxCzyyjxResController extends GeneralController{
	...
	public String login(String userCode, String userPasswd,String verifyCode , Model model , HttpServletRequest request){
		...
		try{
			...
			setSessionAttribute(GlobalConstant.CURRENT_ORG, CZYY_ORG_FLOW);
			//设置当前用户部门列表
			setSessionAttribute(GlobalConstant.CURRENT_DEPT_LIST, deptBiz.searchDeptByOrg(CZYY_ORG_FLOW));
...
//            System.out.println(s);
			...
			if(sur!=null){
				...
				doctor.setOrgFlow(CZYY_ORG_FLOW);
                SysOrg so = orgBiz.readSysOrg(CZYY_ORG_FLOW);
                if(null!=so){
...
//		model.addAttribute("userEmail",userEmail);
		return "redirect:/inx/czyyjxres/sendEmail";
	}
...
//		model.addAttribute("userEmail" , user.getUserEmail());
		return "inx/czyyjxres/sendEmail";
	}
	...
	public String uploadFile(String operType,Model model){
		...
		return "inx/czyyjxres/uploadFile";
	}
