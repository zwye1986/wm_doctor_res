...
@Controller
@RequestMapping("/inx/gzzyyy")
public class InxGzzyyyController extends GeneralController{
	...
	public String regist(SysUser registerUser , String verifyCode, Model model){
		...
		return "redirect:/inx/gzzyyy/sendEmail";
	}
...
//        if(flagId == null || flagId.equals("Y")) {
        infos = this.inxInfoExtMapper.searchInfoByOrgNoRoleFlow2(null, typeId, sysId);
//        }else if(flagId.equals("N")){
//            infos = this.noticeBiz.searchInfoByOrg("N", typeId, sysId);
//        }
        model.addAttribute("infos",infos);
		return "inx/gzzyyy/loadNotices";
	}
...
//        }
		model.addAttribute("infos",infos);
		return "inx/gzzyyy/morenotice";
	}
	
	@RequestMapping("/forgetpasswd")
	public String forgetpasswd(){
		return "inx/gzzyyy/forgetpasswd";
	}
		...
		if(fileCheckInfo!=null){
			...
			if(uploadFile!=null){
				if(!uploadFile.isEmpty()){
					resultPath = this.resDoctorBiz.saveImg("",uploadFile, "gzzyyyImages");
				}
			}
