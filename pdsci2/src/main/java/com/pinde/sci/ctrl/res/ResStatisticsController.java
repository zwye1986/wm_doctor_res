...
public class ResStatisticsController extends GeneralController {
    ...
    @RequestMapping(value = {"/jszyStudentStatistics"})
	public String jszyStudentStatistics(Integer currentPage, HttpServletRequest request, Model model) {
		...
		int month = cal.get(Calendar.MONTH) + 1;
		if (month < 9) {
			statisticYear = cal.get(Calendar.YEAR) - 1;
		} else {
			statisticYear = cal.get(Calendar.YEAR);
		}
		for (int i = 0; i < 4; i++) {
			sessionNumbers.add(statisticYear + "");
			statisticYear -= 1;
		}
		Collections.reverse(sessionNumbers);
		...
		//人员类型培训专业
		List<String> doctorCategoryIdList = new ArrayList<String>();
		for (JszyTrainCategoryEnum e : JszyTrainCategoryEnum.values()) {
			if(GlobalConstant.FLAG_Y.equals(e.getIsView())){
				doctorCategoryIdList.add(e.getId());
			}
		}
		param.put("doctorCategoryIdList", doctorCategoryIdList);
		List<Map<String, Object>> list = resStatisticBiz.jszyStudentStatistics(param);
		if (list != null && list.size() > 0) {
			Map<String, String> countInfoMap = new HashMap<>();
			for (Map<String, Object> tempMap : list) {
				String orgFlow = tempMap.get("jointOrgFlow").toString();
				String sessionNumber = tempMap.get("sessionNumber").toString();
				String count = tempMap.get("num").toString();
				countInfoMap.put(orgFlow + sessionNumber, count);
			}
		...
		return "res/manager/jszy/jszyStudentStatistics";
	}

	@RequestMapping(value = {"/showOrgDoctorType"})
	public String showOrgDoctorType(String orgFlow, Model model) {
		Map<String, Object> param = new HashMap<>();
		...
		param.put("doctorCategoryIdList", doctorCategoryIdList);
		param.put("docTypeList", docTypeList);
		List<Map<String, Object>> list = resStatisticBiz.showOrgDoctorType(param);
		List<String> sessionNumbers = new ArrayList<>();
		if (list != null && list.size() > 0) {
			Map<String, String> countInfoMap = new HashMap<>();
			for (Map<String, Object> tempMap : list) {
				String sessionNumber = tempMap.get("sessionNumber").toString();
				String doctorTypeId = tempMap.get("doctorTypeId").toString();
				String count = tempMap.get("num").toString();
				countInfoMap.put(sessionNumber + doctorTypeId, count);
				if (!sessionNumbers.contains(sessionNumber)) {
		...
		model.addAttribute("sessionNumbers", sessionNumbers);
		return "res/manager/jszy/jszyOrgDoctorTypeDetails";
	}

	@RequestMapping(value = {"/jszySpeStatistics"})
	public String jszySpeStatistics(Integer currentPage, HttpServletRequest request, Model model) {
		...
		param.put("doctorCategoryIdList", doctorCategoryIdList);
		List<Map<String, Object>> list = resStatisticBiz.jszySpeStatistics(param);
		if (list != null && list.size() > 0) {
			Map<String, String> countInfoMap = new HashMap<>();
			for (Map<String, Object> tempMap : list) {
				String orgFlow = tempMap.get("jointOrgFlow").toString();
				String trainingTypeId = tempMap.get("doctorCategoryId").toString();
				String sessionNumber = tempMap.get("sessionNumber").toString();
				String count = tempMap.get("num").toString();
				countInfoMap.put(orgFlow + trainingTypeId + sessionNumber, count);
			}
			model.addAttribute("countInfoMap", countInfoMap);
		}
		//查询基地
		...
		return "res/manager/jszy/jszySpeStatistics";
	}

    @RequestMapping(value = {"/exportJszySpeStatistics"})
    public void exportJszySpeStatistics(HttpServletResponse response) throws Exception {
        ...
        param.put("doctorCategoryIdList", doctorCategoryIdList);
        List<Map<String, Object>> list = resStatisticBiz.jszySpeStatistics(param);
        Map<String, String> countInfoMap = null;
        if (list != null && list.size() > 0) {
            countInfoMap = new HashMap<>();
            for (Map<String, Object> tempMap : list) {
                String orgFlow = tempMap.get("jointOrgFlow").toString();
                String trainingTypeId = tempMap.get("doctorCategoryId").toString();
                String sessionNumber = tempMap.get("sessionNumber").toString();
...
//        styleTwo.setFont(fontTwo);
        ...
        cell.setCellValue("统计查询-- 在培学员专业统计");
        ...
        cell.setCellValue("中医");
        cell.setCellStyle(style);
        sheet.addMergedRegion(new CellRangeAddress(1, 1, 1, 4));
        cell = row.createCell(5);
        cell.setCellValue("中医全科");
        cell.setCellStyle(style);
        ...
        for(int i = 0 ; i< orgs.size();i++){
            ...
            cell = row.createCell(0);
            cell.setCellValue(orgs.get(i).getOrgName());
            cell.setCellStyle(styleTwo);
            sheet.setColumnWidth(0, orgs.get(i).getOrgName().length()*700);
            for(int k = 0;k<8;k++){
                cell = row.createCell(k+1);
                String key = "";
                if(k<4){
                    key = orgs.get(i).getOrgFlow()+"ChineseMedicine"+(2014+k%4);
                }else{
                    key = orgs.get(i).getOrgFlow()+"TCMGeneral"+(2014+k%4);
                }
                count = StringUtil.isEmpty(countInfoMap.get(key))?0:Integer.parseInt(countInfoMap.get(key));
                totalSum += count;
                if(k<4){
                    chineseMedicineSum += count;
                }else{
                    tCMGeneralSum += count;
                }
        ...
        row = sheet.createRow( orgs.size()+3);
        cell = row.createCell(0);
        cell.setCellValue("合计");
        cell.setCellStyle(styleTwo);
        cell = row.createCell(1);
        cell.setCellValue(chineseMedicineSum);
        cell.setCellStyle(styleTwo);
        sheet.addMergedRegion(new CellRangeAddress(orgs.size()+3, orgs.size()+3, 1, 4));
        cell = row.createCell(5);
        cell.setCellValue(tCMGeneralSum);
        cell.setCellStyle(styleTwo);
        sheet.addMergedRegion(new CellRangeAddress(orgs.size()+3, orgs.size()+3, 5, 8));
        cell = row.createCell(9);
        cell.setCellValue(chineseMedicineSum+tCMGeneralSum);
        cell.setCellStyle(styleTwo);

        String fileName = "在培学员专业统计.xls";
        fileName = URLEncoder.encode(fileName, "UTF-8");
        response.setHeader("Content-Disposition", "attachment; filename=\"" + fileName + "\"");
        response.setContentType("application/octet-stream;charset=UTF-8");
        wb.write(response.getOutputStream());
    ...
    @RequestMapping(value = {"/exportJszyStudentStatistics"})
    public void exportJszyStudentStatistics(HttpServletResponse response) throws Exception {
        String[] headLines ={"统计查询--在培学员统计"};
        String[] titles = {
                ":序号",
                "orgName:基地名称",
                "sum2014:2014",
...
//        PageHelper.startPage(currentPage, getPageSize(request));
        ...
        for (SysOrg org:orgs) {
            ...
            sum2014 = StringUtil.isEmpty(countInfoMap.get(org.getOrgFlow()+"2014"))?0:Integer.parseInt(countInfoMap.get(org.getOrgFlow()+"2014"));
            sum2015 = StringUtil.isEmpty(countInfoMap.get(org.getOrgFlow()+"2015"))?0:Integer.parseInt(countInfoMap.get(org.getOrgFlow()+"2015"));
            sum2016 = StringUtil.isEmpty(countInfoMap.get(org.getOrgFlow()+"2016"))?0:Integer.parseInt(countInfoMap.get(org.getOrgFlow()+"2016"));
            sum2017 = StringUtil.isEmpty(countInfoMap.get(org.getOrgFlow()+"2017"))?0:Integer.parseInt(countInfoMap.get(org.getOrgFlow()+"2017"));
            totalSum = sum2014+sum2015+sum2016+sum2017;
            totalSum2014 += sum2014;
            totalSum2015 += sum2015;
            totalSum2016 += sum2016;
            totalSum2017 += sum2017;
            totalSumAll  +=totalSum;
            data.put("orgName", org.getOrgName());//基地名称
            data.put("sum2014",sum2014);
            data.put("sum2015",sum2015);
            data.put("sum2016",sum2016);
            data.put("sum2017",sum2017);
            data.put("totalSum",totalSum);
            dataList.add(data);
        }
        data = new HashMap<String, Object>();
        data.put("orgName", "合计");//基地名称
        data.put("sum2014",totalSum2014);
        data.put("sum2015",totalSum2015);
        data.put("sum2016",totalSum2016);
        data.put("sum2017",totalSum2017);
        data.put("totalSum",totalSumAll);
        dataList.add(data);
        ExcleUtile.createSimpleSheetWithHeadline(headLines,titles,dataList,wb,"统计查询-在培学员统计");
