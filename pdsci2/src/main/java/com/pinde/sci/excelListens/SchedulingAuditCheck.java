...
@Slf4j
@Data
public class SchedulingAuditCheck {
     ...
     * ~~~~~~~~~溺水的鱼~~~~~~~~
     * @Author: 吴强
     * @Date: 2024/10/30 11:42
     * @Description: 两行表头需要单独set
     */
    public SchedulingAuditCheck(Map<String, List<SchRotationDept>> bzMap,
                                List<SysDept> lzDept,
                                List<SchedulingDataModel> data) {
        this.bzMap = bzMap;
        this.lzDept = lzDept;
        this.data = data;
        if (CollectionUtil.isNotEmpty(data)) {
            Set<String> collect = data.stream().map(e -> e.getCellData().get(0).getName()).collect(Collectors.toSet());
            this.excelUserName = collect;
            Set<String> collect2 = data.stream().map(e -> e.getCellData().get(1).getName()).collect(Collectors.toSet());
            this.excelIdNo = collect2;
        }
    }
