...
import com.pinde.sci.dao.edu.EduCourseChapterExtMapper;
...
public class EduCourseChapterBizImpl implements IEduCourseChapterBiz {
	@Autowired
	private EduCourseChapterMapper eduCourseChapterMapper;
	@Autowired
	private EduCourseChapterExtMapper eduCourseChapterExtMapper;
	@Autowired
	...
	@Override
	public EduCourseChapter searchOneChapter(String courseFlow){
		...
		List<EduCourseChapter> chapterList=eduCourseChapterMapper.selectByExample(example);
	...
	public List<EduCourseChapter> getAllChapter(String courseFlow) {
		EduCourseChapterExample example = new EduCourseChapterExample();
		example.createCriteria().andCourseFlowEqualTo(courseFlow).andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y);
		example.setOrderByClause("to_number(CHAPTER_NO),CREATE_TIME");
	...
	public String saveChapterReturnFlow(EduCourseChapter chapter) {
		return saveChapter(chapter)+":"+chapter.getChapterFlow()+":"+getAllChapterCredit(chapter.getCourseFlow()).doubleValue();
	}
	...
	@Override
	public String saveResEduChapter(EduCourseChapter chapter,String courseFlow) {
		SysUser currUser=GlobalContext.getCurrentUser();
		if(chapter!=null){
			EduCourseChapter readChapter=seachOne(chapter.getChapterFlow());
			if(readChapter==null){
			  chapter.setTeacherId(currUser.getUserFlow());
			}
			EduCourse course=this.courseBiz.readCourse(courseFlow);
			if(course!=null){
				chapter.setChapterName(course.getCourseName());
	...
	@Override
	public BigDecimal getAllChapterCredit(String courseFlow) {
		return eduCourseChapterExtMapper.sumAllChapterCredit(courseFlow);
	}
