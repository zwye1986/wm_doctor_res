...
import com.pinde.sci.dao.gyxjgl.GyXjEduCourseMajorExtMapper;
...
public class GyXjEduCourseMajorBizImpl implements IGyXjEduCourseMajorBiz {

	@Autowired
	private GyXjEduCourseMajorExtMapper courseMajorExtMapper;
	@Autowired
	private EduCourseMajorMapper courseMajorMapper;
	@Autowired
	private IGyXjEduStudentCourseBiz studentCourseBiz;
	@Autowired
	private GyXjEduCourseBizImpl eduCourseBiz;
	@Autowired
			...
			return ExcelUtile.importDataExcel(HashMap.class, 1, wb, keys, new IExcelUtil<HashMap>(){
				...
				public String checkExcelData(HashMap eduCourseMajor,ExcelUtile eu) {
					...
					if(StringUtil.isBlank((String)eduCourseMajor.get("courseTypeName"))){
 					...
 					//执行保存
					String majorName= DictTypeEnum.GyMajor.getDictNameById((String)eduCourseMajor.get("majorId"));
