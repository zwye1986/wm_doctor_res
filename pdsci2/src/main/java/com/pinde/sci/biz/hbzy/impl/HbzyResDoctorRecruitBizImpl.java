...
import com.pinde.sci.dao.hbzy.HbzyResDoctorRecruitExtMapper;
...
public class HbzyResDoctorRecruitBizImpl implements IJszyResDoctorRecruitBiz {
	@Autowired
	private IResDoctorBiz resDoctorBiz;
	@Autowired
	private ResDoctorRecruitMapper doctorRecruitMapper;
    @Autowired
    private HbzyResRecruitDoctorInfoExtMapper jszyResDoctorRecruitInfoExtMapper;
    @Autowired
    private JsresRecruitDocInfoMapper jsResDoctorRecruitMapper;
    @Autowired
    private JsresRecruitInfoMapper recruitInfoMapper;
    @Autowired
	private HbzyResDoctorRecruitExtMapper jszyResDoctorRecruitExtMapper;
	@Autowired
			...
			return ExcelUtile.importDataExcel2(ResDoctorGraduationInfo.class, 1, wb, keys, new IExcelUtil<ResDoctorGraduationInfo>() {
				...
				public String checkExcelData(ResDoctorGraduationInfo data, ExcelUtile eu) {
					...
					if(StringUtil.isBlank(data.getTrainingTypeName()))
					{
						...
						if(!"中医全科".equals(data.getTrainingTypeName())&&!"中医".equals(data.getTrainingTypeName()))
						{
							return errorMsg(eu,"导入文件第" + (rowNum + 1) + "行培训专业只能是【中医】或【中医全科】，请确认后提交！！");
						}
