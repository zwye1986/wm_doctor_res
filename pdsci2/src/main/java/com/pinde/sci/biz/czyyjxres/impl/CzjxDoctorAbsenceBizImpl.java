...
import com.pinde.sci.biz.czyyjxres.ICzjxDoctorAbsenceBiz;
...
import com.pinde.sci.enums.czyyjxres.LeaveTypeEnum;
...
public class CzjxDoctorAbsenceBizImpl implements ICzjxDoctorAbsenceBiz {
    @Autowired
    ...
    @Override
    public int editSchDoctorAbsence(SchDoctorAbsence doctorAbsence, MultipartFile multipartFile) {
        ...
        if(StringUtil.isNotBlank(doctorAbsence.getAbsenceTypeId())){
            doctorAbsence.setAbsenceTypeName(LeaveTypeEnum.getNameById(doctorAbsence.getAbsenceTypeId()));
        }else{
        ...
        if (StringUtil.isBlank(doctorAbsence.getAbsenceFlow())) {//新增
            if (StringUtil.isBlank(doctorAbsence.getDoctorFlow())) {//医师新增请假
                ...
                List<SysUser> userList = userBiz.searchUserListByOrgFlow(paramMap);
                if(userList!=null && userList.size()>0){
                    doctorAbsence.setManagerFlow(userList.get(0).getUserFlow());
                    doctorAbsence.setManagerName(userList.get(0).getUserName());
                }
            }
