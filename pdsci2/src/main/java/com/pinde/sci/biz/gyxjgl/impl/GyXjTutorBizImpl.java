...
public class GyXjTutorBizImpl implements IGyXjTutorBiz {
	@Autowired
	...
	public List<NydsOfficialDoctor> queryDoctorList(NydsOfficialDoctor officialDoctor) {
		...
		if(StringUtil.isNotBlank(officialDoctor.getRecruitSpeId())){//专业
		...
		if(StringUtil.isNotBlank(officialDoctor.getXwkAuditStatusId())){//学位科审核状态
	...
	public int backAuditOption(String tutorFlow) {
		NydsOfficialDoctor record = new NydsOfficialDoctor();
		record.setDoctorFlow(tutorFlow);
		record.setApplyFlag(GlobalConstant.FLAG_N);
		record.setApplyTime("");
		//清空培养单位审核信息
	...
	@Override
	public List<NydsOfficialDoctor> queryDoctorList(NydsOfficialDoctor officialDoctor, String flag) {
		NydsOfficialDoctorExample example=new NydsOfficialDoctorExample();
		NydsOfficialDoctorExample.Criteria criteria=example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y);
		if(StringUtil.isNotBlank(officialDoctor.getDoctorName())){//姓名
			criteria.andDoctorNameLike("%"+officialDoctor.getDoctorName()+"%");
		...
		if("doctorAudit".equals(flag)){
			if(StringUtil.isNotBlank(officialDoctor.getCreateTime())){//导师申请开始时间
				criteria.andApplyTimeGreaterThanOrEqualTo(officialDoctor.getCreateTime());
			}
			if(StringUtil.isNotBlank(officialDoctor.getModifyTime())){//导师申请结束时间
				criteria.andApplyTimeLessThanOrEqualTo(officialDoctor.getModifyTime());
			}
		}else if("passedDoctor".equals(flag)){
			if(StringUtil.isNotBlank(officialDoctor.getCreateTime())){//学位科审核通过开始时间
				criteria.andXwkAuditTimeGreaterThanOrEqualTo(officialDoctor.getCreateTime());
			}
			if(StringUtil.isNotBlank(officialDoctor.getModifyTime())){//学位科审核通过结束时间
				criteria.andXwkAuditTimeLessThanOrEqualTo(officialDoctor.getModifyTime());
			}
			if(StringUtil.isNotBlank(officialDoctor.getCreateUserFlow())){//导师资格从审核通过后默认4年失效
				criteria.andXwkAuditTimeLike(Integer.valueOf(officialDoctor.getCreateUserFlow())-4+"%");
			}
		}
		example.setOrderByClause("APPLY_FLAG DESC,CREATE_TIME DESC");
		return doctorMapper.selectByExample(example);
	}
			...
			for(int i = 1;i < row_num; i++) {
				...
				for(int j = 0; j < colnames.size(); j++){
					...
					if("导师类型".equals(currTitle)){
						...
						if("学术型硕导".equals(value)){
							tutor.setDoctorTypeId("xsxsd");
						}
						if("专业型博导".equals(value)){
							tutor.setDoctorTypeId("zyxbd");
						}
						if("专业型硕导".equals(value)){
							tutor.setDoctorTypeId("zyxsd");
						}
					}
					if("二级机构".equals(currTitle)){
				...
				if(StringUtil.isBlank(tutor.getPydwOrgName())){
					throw new Exception("导入失败！第"+ (count+2) +"行二级机构不能为空！");
				}
				...
				orgExample.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y).andOrgNameEqualTo(tutor.getPydwOrgName());
				List<SysOrg> orgList = orgMapper.selectByExample(orgExample);
				if(null != orgList && orgList.size()>0){
					tutor.setPydwOrgFlow(orgList.get(0).getOrgFlow());
				}else{
					throw new Exception("导入失败！第"+ (count+2) +"行二级机构不存在！");
				}
	...
	@Override
	public int dockingTutorDate() {
		int num = 0;
		ApplicationContext contextgzykdx = new ClassPathXmlApplicationContext("spring-context-gzykdx.xml");
		GyXjEduUserExtMapper eduUserExtMapper = contextgzykdx.getBean(GyXjEduUserExtMapper.class);
		List<Map<String,Object>> dataList = eduUserExtMapper.queryOaTutorList();
		...
		if(null != dataList && !dataList.isEmpty()){
			for(int i=0;i<dataList.size();i++){
				for(SysUser user : userList){
					if(dataList.get(i).get("DOCTOR_FLOW").equals(user.getUserCode())){
						objList.add(dataList.get(i));
						break;
					}
				}
			}
			dataList.removeAll(objList);
			for(Map<String,Object> map : dataList){
				SysUser user = new SysUser();
				user.setUserFlow(PkUtil.getUUID());
				user.setUserCode((String)map.get("DOCTOR_FLOW"));
				user.setUserName((String)map.get("DOCTOR_NAME"));
				...
				record.setUserFlow(user.getUserFlow());
				record.setWsId("gycmis");
				record.setRoleFlow(InitConfig.getSysCfg("xjgl_tutor_role_flow"));
				GeneralMethod.setRecordInfo(record,true);
				userRoleMapper.insertSelective(record);
				NydsOfficialDoctor tutor = new NydsOfficialDoctor();
				tutor.setDoctorFlow(user.getUserFlow());
				...
				tutor.setApplyTime(DateUtil.getCurrDate());
				tutor.setPydwAuditStatusId("Passed");
				tutor.setPydwAuditStatusName("通过");
				tutor.setXwkAuditStatusId("Passed");
				tutor.setXwkAuditStatusName("通过");
				GeneralMethod.setRecordInfo(tutor,true);
				doctorMapper.insertSelective(tutor);
			}
		}
