...
import com.pinde.sci.dao.hbzy.HbzyResDoctorExtMapper;
import com.pinde.sci.dao.hbzy.HbzyResUserBalckListExtMapper;
import com.pinde.sci.enums.hbzy.JszyResDegreeCategoryEnum;
...
import com.pinde.sci.form.hbzy.JszyBackTrainForm;
...
public class HbzyResDoctorBizImpl implements IJszyResDoctorBiz {
	...
	@Autowired
	private HbzyResDoctorExtMapper jsresDoctorExtMapper;

	@Autowired
	private JsresUserBalcklistMapper balcklistMapper;
	@Autowired
	private HbzyResUserBalckListExtMapper balcklistExtMapper;
...
//	}
    ...
    @Override
    public void exportForDetail(List<JszyDoctorInfoExt> doctorInfoExts, HttpServletResponse response,String sessionNumber) throws Exception {
        //创建工作簿
        HSSFWorkbook wb = new HSSFWorkbook();
...
//				}
                ...
                if (ResDocTypeEnum.Company.getId().equals(doctor.getDoctorTypeId())) {
					...
					ResBaseExample.Criteria criteria = example.createCriteria().andRecordStatusEqualTo("Y");
					criteria.andOrgFlowEqualTo(doctor.getOrgFlow()).andSessionNumberEqualTo(sessionNumber);
					List<ResBase> list = resBaseMapper.selectByExample(example);
