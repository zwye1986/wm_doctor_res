...
import com.pinde.sci.dao.gyxjgl.GyXjDefenceExtMapper;
...
public class GyXjDefenceBizImpl implements IGyXjDefenceBiz {
	@Autowired
	...
	@Autowired
	private GyXjDefenceExtMapper defenceExtMapper;
	@Autowired
	...
	@Override
	public int degreePaperOption(String recordFlow) {
		NydbPaperTitle title = titleMapper.selectByPrimaryKey(recordFlow);
		title.setDegreeReqFlag(GlobalConstant.FLAG_Y);
		NydbPaperTitleExample example = new NydbPaperTitleExample();
		example.createCriteria().andRecordStatusEqualTo(GlobalConstant.RECORD_STATUS_Y).
				andDefenceFlowEqualTo(title.getDefenceFlow()).andDegreeReqFlagEqualTo(GlobalConstant.FLAG_Y);
		List<NydbPaperTitle> titleList = titleMapper.selectByExample(example);
		if(null != titleList && !titleList.isEmpty()){
			for(NydbPaperTitle t : titleList){
				t.setDegreeReqFlag("");
				titleMapper.updateByPrimaryKey(t);
			}
		}
			...
			for (int i = 1; i <= row_num; i++) {
				...
				for (int j = 0; j < colnames.size(); j++) {
					...
					if ("学号".equals(colnames.get(j))) {
						...
						List<SysDict> dictList = DictTypeEnum.sysListDictMap.get("GyDefenceTime");
						...
						List<SysDict> dictList = DictTypeEnum.sysListDictMap.get("GyReplenishTime");
						for (SysDict dict : dictList) {
							if (dict.getDictName().equals(value)) {
					...
					} else if ("二级机构审核".equals(colnames.get(j))) {
	...
	@Override
	public Map<String,Object> searchMajorCredit(String userFlow) {
		List<Map<String,Object>> creditList = defenceExtMapper.searchMajorCredit(userFlow);
